<script lang="ts">
  ...
  $: locked = busy || (runtime && $playerRuntimeState.busy);
  $: importLocked = locked || (runtime && $playerRuntimeState.running);

  async function refresh(prefer?: AokanaSaveEntry): Promise<void> {
  ...
  function importFile(): void {
    ...
    void action(async () => {
      ...
      if (targetGame === 'aokana') {
        const destination =
          targetSave && aokanaRegistryFold(targetSave.name) === aokanaRegistryFold(file.name)
            ? targetSave
            : undefined;
        const entry = await transfer.import(file.name, bytes, destination);
        await refresh(entry);
  ...
  function exportFile(): void {
    ...
    void action(async () => {
      if (targetGame === 'aokana') {
        ...
        downloadBytes(targetSave.name, await transfer.read(targetSave));
...
<section id="save-files" class="save-controls">
  ...
  <div class="file-actions">
    ...
    <button
      ...
      type="button"
      onclick={exportFile}
      disabled={locked || (game === 'aokana' && !selected)}>Export</button
    >
  </div>
  <input
    id="save-import-file"
    bind:this={input}
    type="file"
    accept={game === 'aokana' ? '.gdb,.cad' : '.dat'}
    onchange={importFile}
    hidden
  />
