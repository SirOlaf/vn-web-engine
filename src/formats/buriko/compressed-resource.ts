...
export class SdcIntegrityError extends Error {}
/** 0x1400f42d0 / 0x1400f4440. Checksums cover the stored (obfuscated) bytes. */
export function decodeSdc(bytes: Uint8Array, maximumSize: number | null = 0x10000000): Uint8Array {
  checkRange(bytes.length, 0, 32);
  if (!signature(bytes, 'SDC FORMAT 1.00\0')) throw new SdcIntegrityError('Not SDC 1.00');
  ...
  if (maximumSize !== null && size > maximumSize)
    throw new Error('SDC exceeds inspector memory limit');
  ...
  if (sum !== data.getUint16(28, true) || xor !== data.getUint16(30, true))
    throw new SdcIntegrityError('SDC checksum mismatch');
  ...
  if (q !== size) throw new SdcIntegrityError('SDC decoded size mismatch');
  return output;
}
