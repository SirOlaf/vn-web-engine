...
import type {MoviePcm} from '../formats/cri/movie.js';
...
worker.onmessage = async (event) => {
  ...
  try {
    if (event.data.type === 'open') {
      ...
      movie = new CriMovie(openWorkerSource(event.data.source));
    ...
    while (frames.length < 4 && !done) {
      ...
      if (b.info) {
        const targetFrame = Math.floor(seek * b.info.frameRate),
          targetSample = Math.floor(seek * b.info.sampleRate);
        ...
        for (const a of b.audio) {
          const trim = Math.max(0, targetSample - a.start);
          if (trim < a.channels[0]!.length)
            audio.push({start: a.start + trim, channels: a.channels.map((c) => c.slice(trim))});
        }
      }
