...
export function refinedReciprocal(denominator: number): number {
  const seed = burikoRosettaSseReciprocal(denominator);
...
function writeVector(p: BurikoBpPointer, values: readonly number[]): void {
  const v = view(p, 16);
  for (let i = 0; i < 4; i++) v.setInt32(i * 4, values[i]!, true);
}

function integer64(h: BurikoBpOpcodeContext, operation: number): 0 {
  ...
  if (operation === 0) result = a + b;
    ...
    if (b === 0n || (a === -(1n << 63n) && b === -1n))
      throw new Error('Buriko ._bp native 64-bit division fault');
...
export const fixedOpcodes: Readonly<Record<number, BurikoBpOpcodeHandler>> = {
  0x46: (h) => {
