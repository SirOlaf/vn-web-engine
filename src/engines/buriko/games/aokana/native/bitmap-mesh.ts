...
import {runAokanaBitmapMeshOperation} from './bitmap-operation-jobs.js';
...
export interface AokanaMeshVertex {
  readonly x: number;
  readonly y: number;
  readonly q: number;
  readonly uq: number;
...
export interface AokanaMeshScanline {
...
export interface AokanaMeshScanlineResult {
  readonly records: readonly AokanaMeshScanline[];
  readonly firstRow: number;
  readonly bounds: AokanaBitmapRectangle;
}
...
function resetEdge(edge: MeshEdge, vertex: AokanaMeshVertex): void {
  edge.x = vertex.x;
  edge.q = vertex.q;
  edge.uq = vertex.uq;
  edge.vq = vertex.vq;
...
function advanceEdgeSegment(
  edge: MeshEdge,
  direction: -1 | 1,
  vertices: readonly AokanaMeshVertex[],
  row: number,
  top: number,
  bottom: number,
): void {
...
function meshReciprocal(value: number): number {
  const seed = aokanaRosettaSseReciprocal(value),
...
function clearMeshSpan(bitmap: AokanaBitmap, row: number, first: number, end: number): void {
...
function rasterizeAokanaMesh(
  ...
  for (let row = start; row < end; row++) {
    ...
    if (record === undefined)
      throw new RangeError('Aokana mesh rasterizer is missing a native scanline record');
    ...
    for (let column = first; column < last; column++) {
      const sample = sampleAokanaMesh(source, q, uq, vq),
...
export function buildAokanaMeshVertices(geometry: AokanaMeshGeometry): readonly AokanaMeshVertex[] {
...
export function buildAokanaMeshScanlines(
  vertices: readonly AokanaMeshVertex[],
  clippingHeight: number,
): AokanaMeshScanlineResult | null {
  if (vertices.length !== 4)
    throw new RangeError('Aokana mesh scanline construction requires exactly four vertices');
  ...
  if (count <= 0)
    throw new RangeError('Aokana mesh clipped count has no safe browser allocation profile');
  ...
  const records: AokanaMeshScanline[] = [];
  ...
  for (let row = firstRow; row <= bottom && row < (clippingHeight | 0); row++) {
    ...
    if (width <= 0) {
      ...
      if (horizontalDq === undefined || horizontalDuq === undefined || horizontalDvq === undefined)
        throw new Error('Aokana mesh reaches a narrow row before initializing native slopes');
  ...
  if (records.length !== count)
    throw new Error('Aokana mesh scanline count diverges from its native clipped allocation');
...
export function copyAokanaBitmapMesh32(
  ...
  rasterizeAokanaMesh(destination, source, records, firstRow, xOffset, 0, 'copy');
}
...
export function blendAokanaBitmapMeshRgb32(
  ...
  rasterizeAokanaMesh(destination, source, records, firstRow, xOffset, transparency, 'rgb');
}
...
export function blendAokanaBitmapMeshAlphaIntoRgb32(
  ...
  rasterizeAokanaMesh(destination, source, records, firstRow, xOffset, transparency, 'alpha');
}
...
export function drawAokanaBitmapMesh(
  ...
  if (
    ...
    copyAokanaBitmapMesh32(destination, source, records, firstRow, xOffset);
  else if (mode === 1 && destination.format === 1) {
    if (source.format === 1)
      blendAokanaBitmapMeshRgb32(destination, source, records, firstRow, xOffset, transparency);
