...
export function createGroup90SpriteConfiguration(
  ...
  const validatePriority = (context: AokanaBpOpcodeContext, value: number): Validation =>
    value >= 0x10000 ? fatal(context, `無効なプライオリティ [ ${value | 0} ] が指定されました`) : 0;
  const validateEffectLevel = (context: AokanaBpOpcodeContext, value: number): Validation =>
    value > 0x100
      ? fatal(
          context,
      ...
      : 0;
  const validateEffectMode = (context: AokanaBpOpcodeContext, value: number): Validation =>
    value <= 9 ||
  ...
  const validateBitmapNumber = (context: AokanaBpOpcodeContext, value: number): Validation =>
    value >= 0x4000
      ? fatal(context, `無効なビットマップ番号 [ ${value | 0} ] が指定されました`)
      : 0;
  const validateMixRatio = (context: AokanaBpOpcodeContext, value: number): Validation =>
    value > 0x100 ? fatal(context, `無効な混合比 [ ${value | 0} ] が指定されました`) : 0;
  const validateTransparency = (context: AokanaBpOpcodeContext, value: number): Validation =>
    value > 0x100 ? fatal(context, `無効な透明度 [ ${value | 0} ] が指定されました`) : 0;
  const invalidHandle = (context: AokanaBpOpcodeContext): Promise<never> =>
    ...
    },
    {
      ...
      execute: (context) => {
        const layer = pop32(context.thread),
          ...
          revealProgress = pop32(context.thread),
