...
import {AokanaNativeText, copyText, textByte} from './text.js';

const pointer = (text: string): AokanaBpPointer => ({
...
const lowerOpen = pointer('<l>'),
  ...
  open = pointer('<'),
  close = pointer('>');
const at = (source: AokanaBpPointer, offset: number): AokanaBpPointer => ({
  bytes: source.bytes,
  offset,
});
function move(destination: AokanaBpPointer, source: AokanaBpPointer, length: number): void {
  if (length === 0) return;
...
export function collectAokanaRawLinks(
  text: AokanaNativeText,
  output: AokanaBpPointer | null,
  source: AokanaBpPointer | null,
): number {
  if (source === null) throw new Error('Aokana raw link encoding detection reads null source');
  const mode = text.detectEncoding(source.bytes, source.offset, false);
  let cursor = source.offset,
    count = 0;
  const find = (start: number, needle: AokanaBpPointer): number | null => {
...
export function stripAokanaTextTags(
  text: AokanaNativeText,
  output: AokanaBpPointer | null,
  source: AokanaBpPointer | null,
): void {
  if (source === null) throw new Error('Aokana tag-strip encoding detection reads null source');
  const mode = text.detectEncoding(source.bytes, source.offset, false);
  let cursor = source.offset,
    destination = output?.offset ?? 0;
  const target = (): AokanaBpPointer => {
    if (output === null) throw new Error('Aokana tag stripping writes through null');
