...
export function aokanaDisplayViewport(display: AokanaNativeDisplayState): AokanaNativeRectangle {
  ...
  } else throw new Error('Aokana display viewport reads unwritten native rectangle edges');
  return [x | 0, y | 0, (width - x - 1) | 0, (height - y - 1) | 0];
}

/** 1400b2740 scales each DWORD after a wrapping multiply, with unsigned DIV. */
export function aokanaDisplayScaleSize(
  ...
  if (width === 0 || height === 0) throw new Error('Aokana display size scaling DIV by zero');
  return [
...
export function aokanaDisplayTransformRectangle(
  display: AokanaNativeDisplayState,
  rectangle: AokanaNativeRectangle,
): AokanaNativeRectangle {
