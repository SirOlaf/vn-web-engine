...
export class AokanaDisplayMouseTrails {
  ...
  async read(): Promise<number> {
    ...
    if (result.result === 0 && result.value!.type === 1 && result.value!.data.length <= 512) {
      ...
      if (!terminated) throw new Error('Aokana mouse-trails registry string has no terminator');
      value = parseAokanaCrtWideInteger(text, 10, true).value | 0;
    }
