...
import type {AokanaSaveSlots} from './save-slots.js';
import {textBytes} from './text.js';
import type {AokanaNativeSlotDefinition} from './types.js';

export function createGroup80SaveSlots(service: AokanaSaveSlots): AokanaNativeSlotDefinition[] {
  return [
    {
      primary: 0x80,
      secondary: 0x74,
    ...
    },
    {
      ...
      execute: async (h): Promise<0> => {
        const comment = h.memory.resolve(h.thread, pop32(h.thread)),
          slot = pop32(h.thread);
        if (comment === null) throw new RangeError('Aokana save wrapper consumed a null comment');
    ...
    },
    {
      ...
      execute: async (h): Promise<0> => {
        ...
        if (globals.length < 0x400)
          throw new RangeError('Aokana save wrapper reads beyond its global arena');
