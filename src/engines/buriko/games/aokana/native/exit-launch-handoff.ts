...
export interface AokanaExitLaunchRequest {
...
export class AokanaExitLaunchHandoff {
  private pending: AokanaExitLaunchRequest | null = null;
  private tornDown = false;

  constructor(readonly messages: AokanaWindowMessages) {}

  get hasPendingLaunch(): boolean { return this.pending !== null; }
  ...
  request(
    ...
    if (this.tornDown) throw new Error('Aokana exit-and-launch handoff already passed teardown');
    this.pending = {
  ...
  markEngineTornDown(): void { this.tornDown = true; }

  /** Read copied launch bytes while graph text and media owners are still available. */
  snapshotPendingForTeardown(): AokanaExitLaunchRequest | null {
    if (this.tornDown)
      throw new Error('Aokana exit-and-launch preparation must precede engine teardown');
  ...
  takeAfterTeardown(): AokanaExitLaunchRequest | null {
    if (!this.tornDown)
      throw new Error('Aokana exit-and-launch command cannot run before engine teardown');
