...
function divide32(numerator: number, denominator: number): number {
  ...
  if (denominator === 0 || (numerator === -0x80000000 && denominator === -1))
    throw new Error('Aokana display control native signed division fault');
...
export class AokanaDisplayControlProcess extends AokanaProcedure {
  protected object: AokanaDisplayObject;
  protected current = 0;
  protected total = 1;
  ...
  constructor(
    ...
    if (object === null)
      throw new Error('Aokana display control constructor dereferences an absent object');
    this.object = object;
  }
...
export class AokanaSplineDisplayControlProcess extends AokanaDisplayControlProcess {
  ...
  protected override update(forceFinish: boolean): boolean {
    ...
    if (!finished) {
      ...
      if (index < 0 || index >= this.path.length / 2)
        throw new Error('Aokana spline display control reads outside its native sampled path');
      x = this.path[index * 2]!;
