import {intersectAokanaBitmapRectangle, type AokanaBitmapRectangle} from './bitmap.js';
...
const area = (r: AokanaBitmapRectangle): number =>
  Math.imul((r.right - r.left + 1) | 0, (r.bottom - r.top + 1) | 0) >>> 0;
const union = (a: AokanaBitmapRectangle, b: AokanaBitmapRectangle): AokanaBitmapRectangle => ({
  left: Math.min(a.left, b.left),
  top: Math.min(a.top, b.top),
  right: Math.max(a.right, b.right),
  bottom: Math.max(a.bottom, b.bottom),
});
export const aokanaRectangleContained = (
  a: AokanaBitmapRectangle,
  b: AokanaBitmapRectangle,
): boolean => b.left <= a.left && b.top <= a.top && a.right <= b.right && a.bottom <= b.bottom;
const overlap = (a: AokanaBitmapRectangle, b: AokanaBitmapRectangle): boolean =>
  a.left <= b.right && b.left <= a.right && a.top <= b.bottom && b.top <= a.bottom;

export class AokanaDisplayDamageLifetimeError extends Error {
  constructor(readonly nativeAddress: string) {
    super(`Aokana damage recorder reads a released node at ${nativeAddress}`);
  }
}
const alive = (node: DamageNode, address: string): void => {
  if (node.released) throw new AokanaDisplayDamageLifetimeError(address);
};

/** CObjectManager +50..+7C. RecordDamage is the complete 06f3e0 recursive rectangle algorithm. */
export class AokanaDisplayDamage {
  private head: DamageNode | null = null;
  private criticalSection: AokanaDisplayCriticalSection | null = null;
  count = 0;
  fullRedraw = 0;
  constructor(
    public capacity: number,
    public clip: AokanaBitmapRectangle,
  ) {}

  /** The display manager binds the same +B8 section used by lists and draw traversals. */
  bindCriticalSection(section: AokanaDisplayCriticalSection): void {
    if (this.criticalSection !== null && this.criticalSection !== section)
      throw new Error('Aokana damage belongs to a different object manager');
  ...
  snapshot(): {rectangle: AokanaBitmapRectangle; key: number}[] {
    const output = [];
  ...
  record(key: number, rectangle: AokanaBitmapRectangle): void {
    this.locked(() => this.recordLocked(key, rectangle));
  }
  private recordLocked(key: number, rectangle: AokanaBitmapRectangle): void {
    key >>>= 0;
    if ((this.count | 0) >= (this.capacity | 0)) {
      this.force();
      return;
    }
    if (!intersectAokanaBitmapRectangle(rectangle, this.clip)) return;
    ...
    while (node !== null) {
      ...
      if (!overlap(current, old)) {
        ...
        if (aokanaRectangleContained(current, old)) return;
