...
function divide(value: number, divisor: number): number {
  ...
  if (divisor === 0 || (value === -2147483648 && divisor === -1))
    throw new RangeError('Aokana animated text native signed quotient is undefined');
...
export class AokanaTextDisplayProcess extends AokanaProcedure {
  protected readonly settings;
  protected readonly manager;
  protected captureToken: number;
  protected source: AokanaBpPointer | null = null;
  ...
  constructor(
    ...
    else throw new Error('Aokana animated text capture token is indeterminate');
  ...
  protected damage(area: AokanaBitmapRectangle | null): void {
  ...
  protected advance(): boolean {
    ...
    do {
      if (this.scrolling) this.scroll();
        ...
        if (this.source === null)
          throw new Error('Aokana animated text reads a null borrowed source');
  ...
  private glyph(): number {
    ...
    if (font.raster === null)
      throw new Error('Aokana animated text reads undefined font raster state');
    const scratch = allocateAokanaBitmap(Math.imul(font.size, 2), font.size, 1);
    try {
      const wide = nativeCp932CharacterToWide(encoded);
      if (wide === 0) throw new Error('Aokana animated text glyph metrics are indeterminate');
      const glyph = rasterAokanaGlyph(scratch, font.raster, wide, this.color);
      const view = {...scratch};
      ...
      if (this.window.characterSpacing === 0) {
        ...
        if (selected === undefined)
          throw new Error('Aokana animated glyph bearing reads outside native table');
        bearing = selected;
      ...
      if (effect.mode !== 0) {
        const shadow = allocateAokanaBitmap(view.width, view.height, 1);
        try {
      ...
      if (!damagePublished)
        throw new Error('Aokana animated text consumes an indeterminate glyph damage rectangle');
      if (effect.mode !== 0) {
  ...
  private page(): number {
    if (!this.pageInitialized) {
      ...
      if (this.window.writingDirection === 0 || this.window.writingDirection === 1)
        throw new Error(
          'Aokana native page-control loop does not terminate for this writing direction',
        );
  ...
  private waitOverlay(): boolean {
    ...
    if (this.phase === 0) {
      ...
      for (let index = 0; index < count; index++) {
        ...
        if (frame === undefined)
          throw new Error('Aokana animated overlay reads undefined frame storage');
    ...
    if (done) {
      ...
      if (count > 0) {
        ...
        if (frame.storage === null) this.window.setOverlayEnabled(0, 0);
          ...
          if (mode !== 1) {
            if (mode < 0 || mode > 4)
              throw new Error('Aokana overlay coordinates are indeterminate');
            const cursor = this.window.getTextCursor();
