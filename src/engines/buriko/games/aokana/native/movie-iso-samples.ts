...
export class AokanaIsoSampleError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'AokanaIsoSampleError';
  }
}
export interface AokanaIsoBox {
  readonly type: string;
  readonly start: number;
  readonly body: number;
  readonly end: number;
}
export interface AokanaIsoDescription {
  readonly type: string;
...
export interface AokanaIsoTrack {
  readonly id: number;
  ...
  readonly descriptions: readonly AokanaIsoDescription[];
  readonly dataReferences: readonly AokanaIsoDataReference[];
  readonly edits: readonly AokanaIsoEdit[];
  readonly samples: AokanaIsoSample[];
}
export interface AokanaIsoMovie {
  readonly bytes: Uint8Array;
  readonly timescale: number;
  readonly duration: bigint;
  readonly tracks: readonly AokanaIsoTrack[];
}
...
class Reader {
  ...
  require(at: number, length: number, end = this.bytes.length): void {
    if (
      ...
      throw new AokanaIsoSampleError('ISO sample metadata reads beyond its encoded box');
  }
  ...
  boxes(start: number, end: number): AokanaIsoBox[] {
    const result: AokanaIsoBox[] = [];
    for (let at = start; at < end;) {
      ...
      if (size < BigInt(header) || size > BigInt(end - at))
        throw new AokanaIsoSampleError('Invalid ISO box size');
  ...
  children(box: AokanaIsoBox): AokanaIsoBox[] {
    return this.boxes(box.body, box.end);
  }
  child(box: AokanaIsoBox, type: string): AokanaIsoBox | undefined {
    return this.children(box).find((entry) => entry.type === type);
  }
  required(box: AokanaIsoBox, type: string): AokanaIsoBox {
    const found = this.child(box, type);
    if (found === undefined) throw new AokanaIsoSampleError(`Missing ISO ${type} box`);
    return found;
  }
  full(box: AokanaIsoBox, maxVersion = 0): number {
    const flags = this.u32(box.body, box.end);
    if (flags >>> 24 > maxVersion)
      throw new AokanaIsoSampleError(`Unknown ISO ${box.type} version`);
    return flags;
  }
  rows(
    box: AokanaIsoBox,
...
class Runs {
  ...
  take(): number {
    ...
    if (run === undefined)
      throw new AokanaIsoSampleError('ISO timing table contains fewer samples than its size table');
    this.used++;
    return run.value;
  }
  finish(): void {
    ...
    if (this.index !== this.runs.length)
      throw new AokanaIsoSampleError('ISO timing table contains extra samples');
  }
}
function readRuns(reader: Reader, box: AokanaIsoBox, composition = false): Runs {
...
function readEdits(reader: Reader, track: AokanaIsoBox): AokanaIsoEdit[] {
...
function readDescriptions(reader: Reader, table: AokanaIsoBox): AokanaIsoDescription[] {
  ...
  if (descriptions.length !== count)
    throw new AokanaIsoSampleError('ISO sample-description count differs from its entries');
...
function readDataReferences(reader: Reader, information: AokanaIsoBox): AokanaIsoDataReference[] {
  ...
  if (entries.length !== count)
    throw new AokanaIsoSampleError('ISO data-reference count differs from its entries');
...
function readSizes(reader: Reader, table: AokanaIsoBox): number[] {
  const normal = reader.child(table, 'stsz'),
    compact = reader.child(table, 'stz2');
  if (normal !== undefined && compact !== undefined)
    throw new AokanaIsoSampleError('ISO track has two sample-size tables');
  const box = normal ?? compact;
  if (box === undefined) throw new AokanaIsoSampleError('ISO track lacks a sample-size table');
  reader.full(box);
  ...
  if (normal !== undefined) {
    ...
    if (bits !== 4 && bits !== 8 && bits !== 16)
      throw new AokanaIsoSampleError('Invalid ISO compact sample-size width');
...
function readOrdinarySamples(reader: Reader, table: AokanaIsoBox, track: AokanaIsoTrack): void {
  ...
  for (let index = 0; index < mapping.count; index++) {
    ...
    if (
      ...
      throw new AokanaIsoSampleError('Invalid ISO sample-to-chunk mapping');
  ...
  if (narrow !== undefined && wide !== undefined)
    throw new AokanaIsoSampleError('ISO track has two chunk-offset tables');
  const offsets = narrow ?? wide;
  if (offsets === undefined) throw new AokanaIsoSampleError('ISO track lacks chunk offsets');
  ...
  if (syncBox !== undefined) {
    ...
    for (let index = 0; index < rows.count; index++) {
      ...
      if (sample === 0 || sample > sizes.length)
        throw new AokanaIsoSampleError('Invalid ISO sync-sample number');
      sync!.add(sample);
    }
  }
  ...
  for (let chunk = 1; chunk <= offsetRows.count; chunk++) {
    ...
    if (entry === undefined) throw new AokanaIsoSampleError('ISO chunk has no sample mapping');
    ...
    for (let index = 0; index < entry.count; index++) {
      ...
      if (size === undefined)
        throw new AokanaIsoSampleError('ISO chunk map contains extra samples');
  ...
  if (sample !== sizes.length)
    throw new AokanaIsoSampleError('ISO chunk map contains fewer samples than its size table');
  if (chunks.length !== 0 && chunks[chunks.length - 1]!.first > offsetRows.count)
    throw new AokanaIsoSampleError('ISO sample mapping refers to a missing chunk');
  duration.finish();
  composition?.finish();
}
...
function readFragments(
  reader: Reader,
  movie: AokanaIsoBox,
  top: readonly AokanaIsoBox[],
  tracks: readonly AokanaIsoTrack[],
): void {
  ...
  if (extensions !== undefined)
    for (const box of reader.children(extensions))
      if (box.type === 'trex') {
        ...
        if (defaults.has(id))
          throw new AokanaIsoSampleError('ISO movie repeats track fragment defaults');
        defaults.set(id, {
  ...
  for (const fragment of top.filter((box) => box.type === 'moof')) {
    ...
    for (const traf of reader.children(fragment).filter((box) => box.type === 'traf')) {
      ...
      if (track === undefined || values === undefined)
        throw new AokanaIsoSampleError('ISO fragment references an undeclared track');
      let p = header.body + 8;
      const take = (): number => {
      ...
      if (description === 0 || description > track.descriptions.length)
        throw new AokanaIsoSampleError('ISO fragment has an invalid sample description');
      ...
      for (const run of reader.children(traf).filter((box) => box.type === 'trun')) {
        ...
        if ((flags & 0x10000) !== 0 && count !== 0)
          throw new AokanaIsoSampleError('ISO empty fragment contains samples');
        if ((runFlags & 0x404) === 0x404)
          throw new AokanaIsoSampleError('ISO run has both first-sample and per-sample flags');
        p = run.body + 8;
        const read = (): number => {
        ...
        for (let index = 0; index < count; index++) {
          ...
          if (offset < 0n)
            throw new AokanaIsoSampleError('ISO fragment sample has a negative byte offset');
          track.samples.push({
            offset,
            size,
            description,
...
export function readAokanaIsoMovie(bytes: Uint8Array): AokanaIsoMovie {
  const reader = new Reader(bytes),
    top = reader.boxes(0, bytes.length),
    movie = top.find((box) => box.type === 'moov');
  if (movie === undefined) throw new AokanaIsoSampleError('Movie has no ISO header');
  ...
  if (timescale === 0) throw new AokanaIsoSampleError('ISO movie has a zero timescale');
  const tracks: AokanaIsoTrack[] = [];
  for (const box of reader.children(movie).filter((entry) => entry.type === 'trak')) {
    ...
    if (id === 0 || tracks.some((track) => track.id === id))
      throw new AokanaIsoSampleError('ISO track ID is zero or repeated');
    ...
    if (mediaScale === 0) throw new AokanaIsoSampleError('ISO track has a zero timescale');
...
export function aokanaIsoSampleBytes(
  movie: AokanaIsoMovie,
  track: AokanaIsoTrack,
  sample: AokanaIsoSample,
): Uint8Array {
  const description = track.descriptions[sample.description - 1];
  if (description === undefined) throw new AokanaIsoSampleError('ISO sample has no description');
  const reference = track.dataReferences[description.dataReference - 1];
  if (reference === undefined || (reference.flags & 1) === 0)
    throw new AokanaIsoSampleError('ISO sample belongs to an external or missing data reference');
  if (sample.offset < 0n || sample.offset + BigInt(sample.size) > BigInt(movie.bytes.length))
    throw new AokanaIsoSampleError('ISO sample reads beyond the encoded file');
