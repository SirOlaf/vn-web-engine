...
export interface AokanaInstallerShortcutCall {
...
export class AokanaInstallerShortcutTransaction {
  constructor(readonly shortcuts: AokanaShellShortcuts) {
    if (shortcuts.files.specialFolders !== shortcuts.folders || shortcuts.files.metadata === null)
      throw new Error('Aokana installer shortcuts require mounted folder and file owners');
  }
  ...
  async create(call: AokanaInstallerShortcutCall): Promise<0 | 1> {
    ...
    const tree = new AokanaDirectoryTree(this.shortcuts.files);
    try {
