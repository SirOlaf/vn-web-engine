...
export class AokanaDisplayCriticalSection {
  ...
  enter(): void {
    ...
    if (this.ownerValue !== null && this.ownerValue !== actor)
      throw new Error('Aokana object-manager lock requires its owning actor to resume');
    this.ownerValue = actor;
    this.depthValue++;
  }
  leave(): void {
    if (this.depthValue === 0 || this.ownerValue !== this.currentActor())
      throw new Error('Aokana object-manager lock is not owned by its current actor');
