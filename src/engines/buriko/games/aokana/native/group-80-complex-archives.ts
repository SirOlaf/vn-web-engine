...
import type {AokanaProgramArchives} from './program-archives.js';
...
export function createGroup80ComplexArchives(
  archives: AokanaProgramArchives,
  ...
  return [
    {
      ...
      execute: async ({thread, memory}): Promise<0> => {
        ...
        if (addresses === null)
          throw new RangeError('Aokana complex archive consumed a null address array');
        ...
        const components = resolveAokanaAddressArray(memory, thread, addresses, count + 1);
