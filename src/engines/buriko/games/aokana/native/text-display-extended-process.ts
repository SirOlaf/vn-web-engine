...
export class AokanaExtendedTextDisplayProcess extends AokanaTextDisplayProcess {
  protected nodes: AokanaHorizontalTextLayoutNode[] = [];
  protected effect: AokanaHorizontalTextEffect;
  ...
  async initializeExtended(
    ...
    otherBits: number,
    readingColor = color,
    explicitEffect?: AokanaHorizontalTextEffect,
  ): Promise<void> {
    ...
    releaseAokanaHorizontalTextLayout(this.nodes);
    this.nodes = [];
    if (source === null) throw new Error('Aokana extended text preparation reads a null source');
  ...
  protected async prepare(
    ...
    const built = await runAsActor(() =>
      buildAokanaHorizontalTextLayout(this.settings, {
        source,
  ...
  protected override overlayOffset(): {x: number; y: number} {
    ...
    if (this.readingEnabled === undefined)
      throw new Error('Aokana extended text reading flag is indeterminate');
    return {
      x: 0,
  ...
  private emit(): void {
    ...
    const area: AokanaBitmapRectangle = {left: 0, top: 0, right: 0, bottom: 0};
    let published = false;
    const draw = (node: AokanaHorizontalTextLayoutNode, mode: number, value: number): void => {
      if (this.window.drawTextBitmap(area, node.x, node.y, node.bitmap, mode, value) === 0)
        published = true;
    };
    const damage = (): void => {
      if (!published) throw new Error('Aokana extended text consumes indeterminate draw damage');
      this.damage(area);
      this.markDirty();
    };
    const transparency = (counter: number, duration: number): number => {
      if (duration >>> 0 === 0)
        throw new RangeError('Aokana extended text divides by zero duration');
