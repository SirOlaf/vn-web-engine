...
function segment(
  ...
  const seed = square === 0 ? Infinity : aokanaRosettaSseReciprocal(square);
...
function write(output: AokanaBpPointer, index: number, value: number): void {
...
function scratchValue(values: (number | undefined)[], index: number): number {
  ...
  if (value === undefined)
    throw new Error('Aokana native collision reads uninitialized projected-distance scratch');
  return value;
}

/** 1400a1ea0 retains the native two projected-plane checks and its null-count result bug. */
export class AokanaLogicalSpatialCollision {
