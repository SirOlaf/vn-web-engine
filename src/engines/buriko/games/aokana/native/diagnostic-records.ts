...
export interface AokanaDiagnosticCounterBank {
  readonly bank: number;
  readonly count: number;
  readonly counts: AokanaBpPointer;
  readonly flags: AokanaBpPointer | null;
}
function word(pointer: AokanaBpPointer | null, offset: number): DataView {
  if (pointer === null)
    throw new Error('Aokana diagnostic counter dereferences a null flags array');
  const bytes = pointerBytes(pointer, 4, offset);
  return new DataView(bytes.buffer, bytes.byteOffset, 4);
}
function zero(pointer: AokanaBpPointer, length: number): void {
  for (let offset = 0; offset < length; offset++) {
    const at = pointer.offset + offset;
    if (at < 0 || at >= pointer.bytes.length)
      throw new Error('Aokana diagnostic clear writes beyond native counter storage');
    pointer.bytes[at] = 0;
  }
}

/** 1400aa8d0 initializes these arrays; this shipping executable has no incrementing writer. */
export class AokanaDiagnosticCounts {
  readonly banks: AokanaDiagnosticCounterBank[] = [];
  constructor() {
  ...
  register(
    bank: number,
    count: number,
    counts: AokanaBpPointer | null,
    flags: AokanaBpPointer | null,
  ): 0 | 1 {
    bank |= 0;
    count >>>= 0;
  ...
  enumerate(output: AokanaBpPointer | null): number {
    let total = 0;
...
export interface AokanaPooledAllocationRecord {
  readonly address: number;
  readonly text: Uint8Array;
}
/** 1400aac60/1400aabf0: retained native byte strings, newest first, independent logging flag. */
export class AokanaPooledAllocationDiagnostics {
  enabled = 0;
  readonly records: AokanaPooledAllocationRecord[] = [];

  record(address: number, size: number, thread: AokanaBpThread): 0 | 1 {
    ...
    if (!module)
      throw new Error('Aokana pooled-allocation diagnostic reads beyond its module list');
    ...
    if (text.length >= 256)
      throw new Error(
        'Aokana pooled-allocation diagnostic overwrites its native sprintf stack buffer',
      );
    text.set(prefix);
