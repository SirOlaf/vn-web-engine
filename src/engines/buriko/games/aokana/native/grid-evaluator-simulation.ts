...
import type {
  AokanaGridEvaluatorRecord,
  AokanaGridEvaluatorRecords,
...
export function simulateGridEvaluation(
  state: AokanaGridEvaluatorRecords,
  grid: AokanaLogicalGridManager,
  ...
  if (actor === undefined)
    throw new Error('Aokana grid simulation selected actor outside allocation');
  const record = (index: number): AokanaGridEvaluatorRecord => {
    const result = records[index];
    if (result === undefined) throw new Error('Aokana grid simulation actor outside allocation');
    return result;
  };
  const j = (word: number): number => job.getInt32(word * 4, true),
    ...
    visibility = new AokanaLogicalGridVisibility(grid),
    order = new Int32Array(128),
    orderAt = (index: number): number => {
      if (index < 0 || index >= order.length)
        throw new Error('Aokana grid simulation turn list exceeds native stack allocation');
      return order[index]!;
    },
    find = (x: number, y: number): AokanaGridEvaluatorRecord => {
      const result = state.findPosition(x, y, records);
      if (result === null) throw new Error('Aokana grid simulation dereferences missing target');
      return result;
    };
  ...
  const modifiers = (
    source: AokanaGridEvaluatorRecord,
    target: AokanaGridEvaluatorRecord | null,
    type: number,
  ): void => {
    gridEvaluatorModifiers(pointer(), type, source.pointer(), target?.pointer() ?? null);
  };
  const facing = (target: AokanaGridEvaluatorRecord, x: number, y: number): number => {
    if (grid.facingAngle(pointer(8), x, y, target.id) !== 0)
      throw new Error('Aokana grid simulation reads uninitialized facing angle');
    return gridEvaluatorFacing(values.getInt32(8, true), state.facingMultiplier);
  };
  const effect = (
    source: AokanaGridEvaluatorRecord,
    target: AokanaGridEvaluatorRecord,
    type: number,
    additions: number,
    orientation: number,
  ): number => {
  ...
  const mapValue = (
    entry: AokanaGridEvaluatorRecord,
    ability: number,
    extended: boolean,
    cell: number,
    lane = 0,
    ...
    if (map === null) throw new Error('Aokana grid simulation dereferences null precomputed map');
    return pointerView({bytes: map, offset: cell * 28 + lane * 4}, 4).getInt32(0, true);
  };
  const recompute = (entry: AokanaGridEvaluatorRecord, ability: number): void => {
  ...
  const adjacentFacing = (target: AokanaGridEvaluatorRecord, direction: number): number => {
    if (grid.copyAdjacentPosition(pointer(64), target.id, direction, true) !== 0)
      throw new Error('Aokana grid simulation reads uninitialized adjacent position');
      ...
      if (mapValue(current, 0, false, targetCell) !== 0) {
        ...
        if (
          ...
          if (direction < 0 || direction >= attackDirections.length)
            throw new Error('Aokana grid simulation cached direction outside stack array');
          let damage = effect(
            current,
            opponent,
            current.word(0x98),
  ...
  if (followAttack) {
    if (opponent === null)
      throw new Error('Aokana grid simulation follow-up attack dereferences null opponent');
...
function retimeActor(
  ...
  actor: number,
  time: number,
  records: AokanaGridEvaluatorRecord[],
): void {
  let shift = 0,
    start = -1,
    end = -1,
  ...
  for (; cursor < order.length && order[cursor]! >= 0; cursor++) {
    ...
    if (current === actor) {
      ...
      if (record === undefined)
        throw new Error('Aokana grid simulation retiming actor outside allocation');
  ...
  if (cursor >= order.length)
    throw new Error('Aokana grid simulation retiming reads beyond native stack list');
  if (start === end) return;
  ...
  if (insertion < 0 || insertion >= copy.length || shift + start < 0 || count < 0)
    throw new Error('Aokana grid simulation retiming uses invalid native copy range');
...
function insertActorLater(
  ...
  if (actor === undefined) throw new Error('Aokana grid simulation reads beyond native turn list');
  let insertion = 0,
    end = cursor + 1;
  for (; end < order.length && order[end]! >= 0; end++) {
    const entry = records[order[end]!];
    if (entry === undefined) throw new Error('Aokana grid simulation actor outside allocation');
    if (insertion === 0 && time < entry.word(0x4b)) insertion = end;
  }
  if (end + 1 >= order.length)
    throw new Error('Aokana grid simulation reinsertion exceeds native turn list');
