...
import type {AokanaLandscapeDisplays} from './landscape-displays.js';
...
export function createGroup91Landscapes(
  landscapes: AokanaLandscapeDisplays,
  ...
  const invalid = (h: AokanaBpOpcodeContext): Promise<never> =>
  ...
  const words =
    ...
    (index: number): number => {
      if (pointer === null) throw new Error('Aokana Landscape dereferences a null input pointer');
      const bytes = pointerBytes(pointer, 4, index * 4);
      return new DataView(bytes.buffer, bytes.byteOffset, 4).getUint32(0, true);
    };
  const write = (pointer: AokanaBpPointer | null, index: number, value: number): void => {
    if (pointer === null) throw new Error('Aokana Landscape dereferences a null output pointer');
    const bytes = pointerBytes(pointer, 4, index * 4);
