...
export type AokanaSurfaceCoefficientStatus = 0 | 0x10 | 0x11 | 0x12;

export interface AokanaSurfaceCoefficientQuery {
  readonly status: 0 | 0x10 | 0x11;
  readonly available: 0 | 1 | null;
}

export interface AokanaSurfaceCoefficientRecord {
  readonly active: 0 | 1;
...
export class AokanaSurfaceCoefficientTables {
  readonly capacity = 8;
  ...
  snapshot(index: number): AokanaSurfaceCoefficientRecord | null {
  ...
  query(index: number, offset: number, count: number): AokanaSurfaceCoefficientQuery {
    const record = this.record(index);
  ...
  expand(
    ...
    amplitude: number,
    count: number,
  ): AokanaSurfaceCoefficientStatus {
    ...
    if (destination === null)
      throw new Error('Aokana surface coefficient expansion writes through a null output');
    const record = this.records[index >>> 0]!,
      coefficients = record.coefficients;
    if (coefficients === null)
      throw new Error('Aokana active surface coefficient record has no native backing');
    const length = Math.imul(count, 4) >>> 0;
    if (destination.length < length)
      throw new RangeError('Aokana surface coefficient output is smaller than its native count');
    const start = this.start(record, offset);
    for (let output = 0; output < length; output++) {
      const coefficient = coefficients[(start + output) >>> 0];
      if (coefficient === undefined)
        throw new RangeError('Aokana surface coefficient expansion reads outside native backing');
  ...
  private start(record: MutableCoefficientRecord, offset: number): number {
    ...
    if (start < (record.lower | 0)) {
      ...
      if (record.span === 0)
        throw new RangeError('Aokana surface coefficient wrapping divides by a zero span');
  ...
  private writeSine(
    ...
    for (let phase = 0; phase < period; phase++) {
      if (output >= coefficients.length)
        throw new RangeError('Aokana ripple producer writes outside its native backing');
  ...
  private skip(coefficients: Uint32Array, output: number, count: number): number {
    ...
    if (!Number.isSafeInteger(next) || next > coefficients.length)
      throw new RangeError('Aokana ripple padding writes outside its native backing');
    return next;
  }
}
