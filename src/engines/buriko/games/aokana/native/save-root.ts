...
export class AokanaSaveRoot {
  ...
  async set(path: AokanaBpPointer | null): Promise<0 | 1> {
    if (path === null) throw new RangeError('Aokana save root consumed a null path');
    const wide = this.files.text.decodeAuto(path);
    if (wide.length >= 784) throw new RangeError('Aokana save root exceeds native wide scratch');
    assertAokanaPathDomain(wide);
    if (!(await this.files.isDirectoryWide(wide))) return 0;
    const source = textBytes(path);
    if (source.length === 0)
      throw new RangeError('Aokana save root reads before an empty native path');
    const append = source[source.length - 1] !== 92;
    const result = new Uint8Array(source.length + (append ? 2 : 1));
    if (result.length > 784) throw new RangeError('Aokana save root exceeds native byte buffer');
    result.set(source);
  ...
  path(name: Uint8Array): Uint8Array {
    ...
    if (result.length > 784) throw new RangeError('Aokana save filename exceeds native scratch');
