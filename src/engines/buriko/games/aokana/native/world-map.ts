...
export function worldMapPosition(source: AokanaBpPointer | null): Float32Array {
  if (source === null) throw new Error('Aokana world-map null position input');
...
function outputWord(pointer: AokanaBpPointer | null, value: number): void {
  if (pointer === null) throw new Error('Aokana world-map null DWORD output');
...
export class AokanaNativeWorldMaps {
  private nextId = 0;
  ...
  private dimensions(map: WorldMap): readonly [number, number] {
    if (map.nodeCount === undefined || map.typeCount === undefined)
      throw new Error('Aokana world-map reads uninitialized dimensions after failed creation');
  ...
  create(output: AokanaBpPointer | null, nodeCount: number, typeCount: number): number {
  ...
  setEdge(
    ...
    packedOverrides: AokanaBpPointer | null,
  ): number {
    const map = this.maps.get(id >>> 0);
    if (map === undefined) return 0x80000007;
    ...
    for (let i = 0; i < (overrideCount | 0); i++) {
      if (packedOverrides === null) throw new Error('Aokana world-map null edge overrides');
  ...
  findPath(
    ...
    pathOutput: AokanaBpPointer | null,
    id: number,
    source: number,
    destination: number,
    type: number,
    ...
    for (let node = destination; node !== source; node = parents[node]! >>> 0) {
      if (node >= nodeCount) throw new Error('Aokana world-map invalid native parent pointer');
      reversed.push(node);
    }
    reversed.reverse();
    outputWord(countOutput, reversed.length);
    if (reversed.length !== 0) {
      if (pathOutput === null) throw new Error('Aokana world-map null path output');
