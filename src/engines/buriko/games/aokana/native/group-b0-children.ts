...
export function createGroupB0Children(
  ...
  const checkSurface = async (h: AokanaBpOpcodeContext, slot: number): Promise<void> => {
    if (slot >= 0x4000) await fatal(h, `無効なビットマップ番号 [ ${slot | 0} ] が指定されました`);
  };
  const create = async (h: AokanaBpOpcodeContext, flags: number): Promise<0> => {
  ...
  add(0x1a, 0x1400d5690, 'DrawTextIntoChildWindow', async (h): Promise<0> => {
    ...
    const output = {
      get value() {
        if (local === undefined)
          throw new Error('Aokana child text opcode reads its unwritten native output DWORD');
        return local;
      },
