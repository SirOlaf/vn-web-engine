...
function byte(bytes: Uint8Array, offset: number): number {
  ...
  if (value === undefined)
    throw new RangeError('Aokana bitmap image reads outside its native byte buffer');
  return value;
}

/** 037730 identifies the engine's sixteen-byte bitmap header, independently of compression. */
export function aokanaPackedBitmapFormat(bytes: Uint8Array, initialized?: Uint8Array): number {
    ...
    for (let x = 0; x < width; x++) {
      ...
      for (let channel = 0; channel < channels; channel++) {
        ...
        if (predictor === undefined)
          throw new Error('Aokana bitmap delta reads an unwritten predictor byte');
        const value =
...
export function aokanaPackedBitmapDescriptor(bytes: Uint8Array): {
  ...
  const decoded = decodeAokanaPackedBitmap(bytes),
    data = view(decoded);
  const width = data.getUint16(0, true),
    height = data.getUint16(2, true);
  const format = aokanaPackedBitmapFormat(decoded),
    bytesPerPixel = aokanaBitmapPixelSize(format);
  ...
  return {
    bitmap: {
      storage: new AokanaBitmapStorage(decoded, true),
      offset: 16,
      width,
      height,
      format,
...
export function importAokanaPackedBitmap(
  surfaces: AokanaSurfaces,
  index: number,
  bytes: Uint8Array,
  initialized?: Uint8Array,
): 0 | 1 | 2 {
  const format = aokanaPackedBitmapFormat(bytes, initialized);
...
export function importAokanaWindowsBitmap(
  surfaces: AokanaSurfaces,
  index: number,
  bytes: Uint8Array,
): number {
    ...
    for (let x = 0; x < width >>> 0; x++) {
      ...
      for (let channel = 0; channel < inputBpp; channel++) {
        ...
        if (at + channel >= output.length)
          throw new RangeError('Aokana BMP conversion writes outside its native byte buffer');
