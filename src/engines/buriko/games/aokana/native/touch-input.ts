...
export interface AokanaNativeTouchSample {
  readonly id: number;
  readonly x: number;
  readonly y: number;
...
export interface AokanaNativeTouchWindow {
...
export class AokanaNativeTouch {
  ...
  private apply(sample: AokanaNativeTouchSample): void {
  ...
  receive(samples: readonly AokanaNativeTouchSample[] | null): number {
  ...
  copyContacts(destination: AokanaBpPointer | null): number {
  ...
  copyHistory(
    positions: AokanaBpPointer | null,
    angles: AokanaBpPointer | null,
    index: number,
    count: number,
  ): number {
    index >>>= 0;
    ...
    for (
      ...
      if (positions === null) throw new Error('Aokana native touch-history null position output');
      ...
      if (angles === null) throw new Error('Aokana native touch-history null angle output');
