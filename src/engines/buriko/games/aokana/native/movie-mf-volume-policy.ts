import {
  ...
  type AokanaFullscreenMovieControllerState,
...
export interface AokanaMfMovieVolumeController extends AokanaFullscreenMovieControllerState {
...
export class AokanaMfMovieVolumePolicy {
  private borrowedController: AokanaMfMovieVolumeController | null = null;
  private volume = 0; // 27427C is an initialized DWORD, independent of controller +90.

  constructor(readonly fullscreen: AokanaFullscreenMovieState) {}
  ...
  bindBorrowedController(controller: AokanaMfMovieVolumeController): void {
    if (this.borrowedController !== null)
      throw new Error('Aokana MF movie volume controller is already bound');
    if (this.fullscreen.controller !== controller)
      throw new Error('Aokana MF volume service requires the shared fullscreen controller');
    this.borrowedController = controller;
  }

  unbindBorrowedController(controller: AokanaMfMovieVolumeController): void {
    if (this.borrowedController !== controller)
      throw new Error('Aokana MF movie volume controller owner mismatch');
  ...
  initializeForNewController(rawVolume: number): void {
    ...
    const controller = this.currentController();
    if (controller === null)
      throw new Error('Aokana MF initial volume requires the actual fullscreen controller');
    this.volume = value;
    void this.callControllerVolume(controller, value);
  }

  private currentController(): AokanaMfMovieVolumeController | null {
    const current = this.fullscreen.controller;
    if (current === null) return null;
    if (this.borrowedController === null || this.borrowedController !== current)
      throw new Error('Aokana MF fullscreen controller has no matching volume service');
    return this.borrowedController;
  }

  private nativeDword(value: number): number {
    if (!Number.isSafeInteger(value))
      throw new RangeError('Aokana MF movie volume requires an exact DWORD');
    return value | 0;
  }

  private callControllerVolume(controller: AokanaMfMovieVolumeController, value: number): number {
    const result = controller.applyVolume(value);
    if (!Number.isSafeInteger(result))
      throw new Error('Aokana MF volume service requires a synchronous HRESULT');
    return result | 0;
  }
}
