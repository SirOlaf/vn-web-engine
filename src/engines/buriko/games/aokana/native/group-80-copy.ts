...
export function createGroup80Copy(files: AokanaProgramFiles): AokanaNativeSlotDefinition[] {
  const metadata = files.metadata;
  if (metadata === null) throw new Error('Aokana copy requires the shared metadata owner');
  return [
    {
      primary: 0x80,
      secondary: 0x2f,
      ...
      execute: async (h): Promise<0> => {
        ...
        if (sourcePointer === null || destinationPointer === null)
          throw new RangeError('Aokana CopyFile consumed a null native path');
        ...
        try {
          ...
          if (attributes === null)
            throw new Error('Aokana copy lacks imported destination attribute metadata');
          try {
