...
export class AokanaSelectionProcess extends AokanaProcedure {
  protected selection = 0;
  protected priorHover = -1;
  protected items: AokanaBpPointer[] = [];
  protected rectangles: AokanaBitmapRectangle[] = [];
  protected columns = 0;
  ...
  constructor(
    ...
    protected readonly waits: AokanaWindowMessages,
    protected readonly notifications: AokanaNativeNotifications,
    protected readonly cursor: AokanaNativeCursorMotion,
    protected readonly settings: AokanaSelectionState,
  ) {
  ...
  initialize(
    ...
    this.items = items.map((source) => {
      if (source === null) throw new Error('Aokana selection copies a null item');
    ...
    this.color = color >>> 0;
    this.rectangles = drawAokanaSelectionText(
      this.window,
      this.items,
      this.columns,
      centered,
  ...
  protected drawHighlight(color: number): void {
    ...
    if (rectangle === undefined || source === undefined)
      throw new Error('Aokana selection highlight consumes an undefined item');
    const bitmap = allocateAokanaBitmap(
    ...
    new AokanaBitmapText(surfaces.fonts, surfaces.compositor).draw(
      bitmap,
      {value: 0},
      0,
      0,
  ...
  protected drawSelection(): void {
    ...
    const rectangle = this.rectangles[this.selection];
    if (rectangle === undefined) throw new Error('Aokana selection positions an undefined item');
  ...
  protected moveCursor(index: number): void {
    ...
    if (r === undefined) throw new Error('Aokana selection cursor reads an undefined rectangle');
  ...
  protected handleInput(): 0 | 1 | 2 {
    ...
    if (this.columns === 0) throw new RangeError('Aokana selection navigation divides by zero');
  ...
  poll(): number {
    ...
    if (message === null)
      throw new Error('Aokana selection consumes undefined window-message scratch');
