export interface AokanaChildScrollbar {
  maximum: number;
  page: number;
  position: number;
  enabled: boolean;
...
export class AokanaChildScroll {
  ...
  wheelStep = 1;
  readonly bars: readonly [AokanaChildScrollbar, AokanaChildScrollbar] = [
  ...
  wheel(wParam: number): void {
    ...
    if (step === 0 || (numerator === -2147483648 && step === -1))
      throw new RangeError('Aokana child wheel reaches native IDIV overflow or zero divisor');
    ...
    throw new Error(
      `Aokana child wheel passes uninitialized native SCROLLINFO.cbSize for position ${position}`,
    );
  }
}
