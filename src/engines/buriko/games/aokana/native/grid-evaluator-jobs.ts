...
export class AokanaGridEvaluatorJobs {
...
export type AokanaGridEvaluationCandidates = {
  jobs: AokanaGridEvaluatorJobs;
...
export function prepareGridEvaluation(
...
): AokanaGridEvaluationCandidates {
  ...
  if (actor === undefined)
    throw new Error('Aokana grid evaluator selected actor address outside allocation');
  const visibility = new AokanaLogicalGridVisibility(grid),
    area = new Uint8Array(680);
  let route: Uint8Array | null = null,
    opponent: AokanaGridEvaluatorRecord | null = null;
  if (actor.word(3) >= 0) {
    ...
    if (opponent === null)
      throw new Error('Aokana grid evaluator opponent address outside allocation');
  ...
  if (grid.copyReachable({bytes: positions, offset: 0}, pointer(), actor.id, true) !== 0)
    throw new Error('Aokana grid evaluator reads uninitialized candidate-position count');
  const length = values.getUint32(0, true);
  grid.copyPosition({bytes: positions, offset: length * 8}, actor.id);
  const jobs = new AokanaGridEvaluatorJobs();
  let direction = 6;
  for (let positionIndex = 0; positionIndex <= length; positionIndex++) {
    ...
    if (grid.copyMetric(pointer(), actor.id, x, y) !== 0)
      throw new Error('Aokana grid evaluator reads uninitialized candidate movement cost');
    ...
    for (let ability = 0; ability < 16; ability++) {
      ...
      for (let hit = 0; hit < targets; hit++) {
        ...
        if (areaRange === 0) {
          ...
          if (target === null)
            throw new Error('Aokana grid evaluator dereferences missing ability target');
          ...
          for (let i = 0; i < areaCount; i++) {
            ...
            if (target === null)
              throw new Error('Aokana grid evaluator dereferences missing area target');
