...
export interface AokanaMovieTimedSample extends AokanaMovieSample {
  ...
  readonly mediaType?: AokanaMovieMediaType;
}
 ...
 */
export class AokanaMovieReceivePin {
  readonly events = new AokanaMovieRenderEvents();
  readonly timing: AokanaMovieRenderTiming;
  state: 0 | 1 | 2 = 0;
  streaming = false;
  connected = false;
  flushing = false;
  ...
  lastQuality: AokanaMovieQuality | null = null;
  private graphStart = 0n;
  private sampleEnd = 0n;
  private inputSampleEnd = 0n;
  private pending: AokanaMovieTimedSample | null = null;
  ...
  connect(type: AokanaMovieMediaType): number {
    if (this.disposed) throw new Error('Aokana movie connects a released renderer');
  ...
  get pendingSample(): AokanaMovieTimedSample | null {
    return this.pending;
  }
  ...
  receive(sample: AokanaMovieTimedSample | null): number | Promise<number> {
    return this.withRenderLock(() => {
  ...
  private prepare(sample: AokanaMovieTimedSample | null): number {
  ...
  private schedule(sample: AokanaMovieTimedSample): boolean {
    if (sample.time !== null) {
