...
export function buildGridAbilityMaps(
  ...
  if (manager.copyReachable({bytes: positions, offset: 8}, countOutput, agentId) !== 0)
    throw new Error('Aokana logical-grid ability map reads uninitialized reachable count');
  ...
  const visibility = new AokanaLogicalGridVisibility(manager);
  for (let requestIndex = 0; requestIndex < requestCount >>> 0; requestIndex++) {
    ...
    const parameter = (word: number): number => {
      if (request === null) throw new Error('Aokana logical-grid ability map null request');
    ...
    for (let positionIndex = 0; positionIndex < length; positionIndex++) {
      ...
      if (manager.copyMetric(costOutput, agentId, x, y) !== 0)
        throw new Error('Aokana logical-grid ability map reads uninitialized movement cost');
      ...
      if (status !== 0)
        throw new Error('Aokana logical-grid ability map reads uninitialized visible count');
