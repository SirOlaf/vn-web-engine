...
function keyList(pointer: AokanaBpPointer | null): number[] {
  if (pointer === null) throw new Error('Aokana native key-list null source');
  const keys: number[] = [];
...
export function createGroup80Input(input: AokanaNativeInput): AokanaNativeSlotDefinition[] {
  return [
    {
      primary: 0x80,
      secondary: 0x0e,
    ...
    },
    {
      ...
      execute: (h) => {
        ...
        if (pointer === null) throw new Error('Aokana native key-count null source');
        let sum = 0;
        for (let offset = pointer.offset; ; offset += 4) {
          const key = pointerView({bytes: pointer.bytes, offset}, 4).getUint32(0, true);
          if (key === 0) break;
    ...
    },
    {
      ...
      execute: (h) => {
        ...
        if (result === 0x80000001)
          throw new Error(`Aokana native invalid key-group mask 0x${mask.toString(16)}`);
        if (result === 0x80000002) throw new Error('Aokana native key-group exceeds 15 keys');
        return 0;
      },
    },
    {
...
export function createGroup81Input(input: AokanaNativeInput): AokanaNativeSlotDefinition[] {
  return [
    {
      primary: 0x81,
      secondary: 0x0f,
    ...
    },
    {
      ...
      execute: (h) => {
        ...
        if (point !== null) {
          if (destination === null)
            throw new Error('Aokana native click-position null destination');
    ...
    },
    {
      ...
      execute: (h) => {
        ...
        if (destination === null) throw new Error('Aokana native keyboard-state null destination');
