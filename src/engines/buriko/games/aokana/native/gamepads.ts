...
export const AOKANA_GAMEPAD_INPUT_LOST = 0x8007001e;
export const AOKANA_GAMEPAD_FAILURE = 0x80004005;
...
export interface AokanaNativeGamepadCapabilities {
...
export interface AokanaNativeGamepadState {
...
export interface AokanaNativeGamepadEvent {
  readonly offset: number;
  readonly value: number;
}

export interface AokanaNativeGamepadStateResult {
  readonly status: number;
  readonly state?: AokanaNativeGamepadState;
}

export interface AokanaNativeGamepadEventResult {
  readonly status: number;
  readonly event: AokanaNativeGamepadEvent | null;
}
 ...
 */
export interface AokanaNativeGamepadDevice {
...
export interface AokanaNativeGamepadHost {
  open(): boolean;
  enumerateAttached(): {
    readonly status: number;
...
export interface AokanaNativeGamepadQuery {
  readonly x: number;
  readonly y: number;
  readonly z: number;
  readonly rz: number;
...
interface GamepadNode {
  readonly id: number;
  readonly instanceGuid: string;
  readonly device: AokanaNativeGamepadDevice;
  readonly capabilities: AokanaNativeGamepadCapabilities;
  needsPoll: number;
  configured: number;
}
...
const copyCapabilities = (
  value: AokanaNativeGamepadCapabilities,
): AokanaNativeGamepadCapabilities => ({
  size: value.size >>> 0,
  flags: value.flags >>> 0,
 ...
 */
export class AokanaNativeGamepads {
  enabled = 0;
  private counter = 0;
  ...
  capabilities(id: number): AokanaNativeGamepadCapabilities | null {
  ...
  query(selector: number): AokanaNativeGamepadQuery | null {
    selector >>>= 0;
    let x = 0,
      y = 0,
      z = 0,
  ...
  private applyEvent(node: GamepadNode, event: AokanaNativeGamepadEvent, changed: boolean[]): void {
  ...
  poll(): number {
    ...
    for (let index = 0; index < this.nodes.length;) {
      ...
      for (;;) {
        ...
        if (result.status >>> 0 === AOKANA_GAMEPAD_INPUT_LOST) {
          node.device.acquire();
          continue;
        }
