...
export interface AokanaSpriteEffect {
  selector: number;
  color: number;
  opacity: number;
}
...
export class AokanaSpriteEffects {
  private readonly records: AokanaSpriteEffect[] = Array.from({length: 16}, () => ({
    selector: 0,
    color: 0,
    opacity: 0,
  }));
  constructor(readonly compositor: AokanaBitmapCompositor) {}
  ...
  apply(destination: AokanaBitmap, source: AokanaBitmap | null = null): 0 | 1 {
