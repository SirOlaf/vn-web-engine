...
function requirePointer(pointer: AokanaBpPointer | null, what: string): AokanaBpPointer {
  if (pointer === null) throw new Error(`Aokana installation dereferences a null ${what}`);
  return pointer;
}

function copyString(pointer: AokanaBpPointer | null, what: string): Uint8Array {
...
export function copyStringArray(
  ...
  if (pointer === null) {
    if (nullable) return [];
    throw new Error(`Aokana installation dereferences a null ${what} array`);
  }
...
function scheduled(scheduler: AokanaBpScheduler, thread: AokanaBpThread): AokanaBpScheduledThread {
  const node = thread === scheduler.root.state ? scheduler.root : scheduler.findById(thread.id);
  if (node === null || node.state !== thread)
    throw new Error('Aokana installation thread is not linked to its scheduler');
  return node;
}
...
export function createGroup81Installation(
  service: AokanaInstallationService,
