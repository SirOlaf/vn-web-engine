...
import type {AokanaNativeRegistry} from './windows-registry.js';
...
export interface AokanaInstallationCall {
...
export interface AokanaInstallationProgressReceiver {
  update(
    archives: AokanaProgramResources['archives'],
    completed: number,
    total: number,
  ): void | Promise<void>;
}
...
export class AokanaInstallationService {
  private singleton: AokanaInstallationProcess | null = null;
  ...
  constructor(
    ...
    readonly procedures: AokanaProcedureState,
    readonly clock: AokanaNativeClock,
    readonly notifications: AokanaNativeNotifications,
    readonly localized: Pick<AokanaLocalizedMessages, 'lookup' | 'language'>,
    readonly registry: AokanaNativeRegistry,
    readonly progress: AokanaInstallationProgressReceiver | null = null,
  ...
  release(process: AokanaInstallationProcess): void {
  ...
  enqueueWorkerMessage(process: AokanaInstallationProcess, code: number, value: number): void {
  ...
  startFileWorker(process: AokanaInstallationProcess, record: InstallationWorkerRecord): void {
  ...
  private async copyAttempt(
    process: AokanaInstallationProcess,
  ...
  private async fileWorker(
    ...
    if (sourceTimes === null)
      throw new Error('Aokana installation source has no selected mounted FILETIME metadata');
  ...
  async writeUninstallList(call: AokanaInstallationCall): Promise<boolean> {
  ...
  async writeRegistry(call: AokanaInstallationCall): Promise<true> {
    const product = this.resources.files.path(call.product),
      publisher = this.resources.files.path(call.publisher),
  ...
  async rollback(tree: AokanaDirectoryTree | null): Promise<void> {
    const metadata = this.resources.files.metadata;
  ...
  clearOperation(tree: AokanaDirectoryTree | null): void {
    this.hvl = [];
    this.installed.length = 0;
    tree?.dispose();
  }
...
export class AokanaInstallationProcess extends AokanaProcedure {
  private readonly ownsSingleton: boolean;
  private readonly workerMessages: InstallationWorkerMessage[] = [];
  private call: AokanaInstallationCall | null = null;
  ...
  private snapshot(call: AokanaInstallationCall): AokanaInstallationCall {
    return {
  ...
  async initialize(
    ...
    const owned = this.snapshot(call),
      tree = new AokanaDirectoryTree(this.service.resources.files);
