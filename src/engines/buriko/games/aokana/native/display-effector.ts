...
import {
  ...
  clearAokanaEffectorBuffer,
} from './bitmap-display-filters.js';
import {displaceAokanaBitmap} from './bitmap-displacement.js';
import type {AokanaDisplayEffectorRegistry} from './display-effector-registry.js';
...
export type AokanaDisplayEffectorStatus =
  | 0
  | 0x80000001
  | 0x80000002
  | 0x80000003
...
export class AokanaDisplayEffector extends AokanaDisplayObject {
  effectorMode = -1;
  primaryMap = -1;
  secondaryMap = -1;
  vectorSampling = 0;
  ...
  constructor(
    ...
    private readonly registry: AokanaDisplayEffectorRegistry,
  ...
  resizeToDisplay(): 0 | 1 {
    ...
    this.retained = allocateAokanaBitmap(
      display.width,
      display.height,
  ...
  configureBlur(selector: number, blendValue: number, layer: number): AokanaDisplayEffectorStatus {
    this.check();
    selector >>>= 0;
  ...
  configureTransform(
    ...
    AokanaDisplayObject.prototype.setValueD8.call(this, 0, 0);
    this.setLayer(layer);
    return 0;
  }
  ...
  configureFeedback(blendValue: number, layer: number): void {
    ...
    this.setBlendValue(blendValue);
    this.setLayer(layer);
    clearAokanaEffectorBuffer(this.retained);
  }
  ...
  override draw(destination: AokanaBitmap, rectangle: AokanaBitmapRectangle, _key: number): void {
    ...
    switch (this.effectorMode) {
      ...
      case 4: {
        ...
        cropAokanaBitmap(retained, rectangle);
