import {AokanaDisplayLandscape, type LandscapeWords} from './display-landscape.js';
...
export class AokanaLandscapeDisplays {
  constructor(
    readonly manager: AokanaDisplayManager,
    readonly input: AokanaNativeInput,
  ) {}
  private landscape(handle: number): AokanaDisplayLandscape | null {
    const object = this.manager.find('landscape', handle);
    if (object === null) return null;
    if (!(object instanceof AokanaDisplayLandscape))
      throw new Error('Aokana Landscape pool contains another display class');
    return object;
  }
  create(
    width: number,
  ...
  detail(handle: number): number {
    ...
    if (object === null || object.value124 === undefined)
      throw new Error('Aokana Landscape detail is not initialized');
