...
export class AokanaDisplayLandscape extends AokanaDisplayObject {
  ...
  private terrain(index: number): Terrain {
    ...
    if (terrain === undefined)
      throw new RangeError('Aokana Landscape terrain access outside owned array');
    return terrain;
  }
  private chip(index: number): Chip {
    const chip = this.chips[index >>> 0];
    if (chip === undefined)
      throw new RangeError('Aokana Landscape chip access outside owned array');
    return chip;
  }
  private copyRect(
    source: AokanaBitmap,
    ...
    for (let i = 0; i < terrainCount; i++) {
      ...
      for (let j = 0; j < parts.length; j++) {
        ...
        if (j + 1 === top && j + 1 < count) {
          mask = allocateAokanaBitmap(width, height, 3);
  ...
  replaceChip(destination: number, source: number): number {
    ...
    if (
      ...
      return 9;
    copyAokanaBitmapRows(target.bitmap, input);
    if (target.pending) {
    ...
    for (const index of target.dependents) {
      const terrain = this.terrain(index);
      clearAokanaBitmap(terrain.bitmap);
  ...
  copyCellTop(surface: number, row: number, column: number): number {
    ...
    copyAokanaBitmapRows(this.surfaces.snapshot(surface)!, source);
    return 0;
  }
    ...
    for (const row of this.rows!) {
      ...
      for (const cell of row.cells) {
        ...
        if (cell.colorMode === 0)
          ...
          const tint = allocateAokanaBitmap(
              pair.source.width,
              pair.source.height,
              pair.source.format,
            ),
            mixed = allocateAokanaBitmap(pair.source.width, pair.source.height, pair.source.format);
          ...
          copyAokanaBitmapRows(mixed, pair.source);
          this.environment.compositor.composite(
            mixed,
            tint,
        ...
        for (let i = 0; i < 4; i++) {
          ...
          if (overlay === undefined)
            throw new RangeError('Aokana Landscape overlay access outside owned array');
