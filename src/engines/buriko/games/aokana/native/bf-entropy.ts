...
export function requireAokanaResourceRange(size: number, offset: number, length: number): void {
  if (
    ...
    throw new AokanaUndefinedResourceRead(
      'Aokana resource access crosses native allocation storage',
    );
  }
}
...
export class AokanaBfBits {
  ...
  read(count: number): number {
    ...
    if (this.initialized !== undefined) {
      requireAokanaResourceRange(this.initialized.length, first, last - first + 1);
      for (let index = first; index <= last; index++)
        if (this.initialized[index] === 0)
          throw new AokanaUndefinedResourceRead(
            'Aokana BF bit reader consumes unwritten native storage',
          );
    }
    let value = 0;
...
export function aokanaBfSymbol(bits: AokanaBfBits, tree: AokanaBfTree): number {
  ...
  if (entry === undefined)
    throw new AokanaUndefinedResourceRead(
      'Aokana BF Huffman lookup reads unwritten native table entries',
    );
  if (entry.length !== 0) {
