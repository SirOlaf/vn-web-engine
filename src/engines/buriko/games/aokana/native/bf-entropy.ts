...
export function requireAokanaResourceRange(size: number, offset: number, length: number): void {
  if (!Number.isSafeInteger(offset) || !Number.isSafeInteger(length) || offset < 0 || length < 0 || offset + length > size) {
    throw new AokanaUndefinedResourceRead('Aokana resource access crosses native allocation storage');
  }
}

/** 1401042a0: continuation length is unrestricted and x86 shifts wrap modulo32. */
export function aokanaBfVarint(bytes: Uint8Array, cursor: {position: number}): number {
  let value = 0, shift = 0;
  for (;;) {
    requireAokanaResourceRange(bytes.length, cursor.position, 1);
...
export class AokanaBfBits {
  ...
  peekByte(): number {
    const position = this.position, value = this.read(8);
    this.position = position;
    return value;
  }
}

export interface AokanaBfTree {
...
export function aokanaBfTree(weights: readonly number[]): AokanaBfTree {
  const leaves = weights.length, capacity = leaves * 2 - 1;
  const active = Array.from({length: capacity}, (_, index) => index < leaves && weights[index] !== 0);
  ...
  for (; root < capacity; root++) {
    ...
    for (let branch = 0; branch < 2; branch++) {
      let best = -1;
      for (let index = 0; index < root; index++) if (active[index]) {best = index; break;}
  ...
  requireAokanaResourceRange(capacity, root, 1);
  ...
  for (let prefix = 0; prefix < 256; prefix++) {
    ...
    for (let depth = 1; depth <= 8; depth++) {
      ...
      if (node < leaves) {lookup[prefix] = {length: depth, node}; break;}
      if (depth === 8) lookup[prefix] = {length: 0, node};
    }
  }
  return {leaves, children, lookup};
}

/** 140103b40 always performs the full eight-bit lookahead before selecting a short code. */
export function aokanaBfSymbol(bits: AokanaBfBits, tree: AokanaBfTree): number {
  const entry = tree.lookup[bits.peekByte()];
  if (entry === undefined) throw new AokanaUndefinedResourceRead('Aokana BF Huffman lookup reads unwritten native table entries');
  if (entry.length !== 0) {bits.position += entry.length; return entry.node;}
  bits.position += 8;
  let node = entry.node;
  while (node >= tree.leaves) {
    requireAokanaResourceRange(tree.children.length, node, 1);
    node = tree.children[node]![bits.read(1)]!;
  }
  return node;
}

export function aokanaBfSignedBits(bits: AokanaBfBits, count: number): number {
