...
import type {AokanaParticleAir} from './particle-air.js';
import type {AokanaParticleDisplays} from './particle-displays.js';
import type {
...
function popArguments(h: AokanaBpOpcodeContext, count: number): number[] {
  const values = new Array<number>(count);
  for (let i = count - 1; i >= 0; i--) values[i] = pop32(h.thread);
  return values;
}
function readDword(pointer: AokanaBpPointer | null, index = 0): number {
  if (pointer === null) throw new Error('Aokana particle parameter pointer is null');
...
export function createGroupC0Particle(
  particles: AokanaParticleDisplays,
  ...
  const checkHandle = (h: AokanaBpOpcodeContext, result: number): 0 | Promise<never> =>
    result === 0xffffffff ? invalidHandle(h) : 0;
  const invalidPattern = (h: AokanaBpOpcodeContext, variant: number): Promise<never> =>
    fatal(h, `無効なパターン番号 [ ${variant | 0} ] が指定されました`);
  const missingBitmap = (h: AokanaBpOpcodeContext, surface: number): Promise<never> =>
  ...
  handlers[0x10] = (h) => {
    const values = popArguments(h, 11),
      handle = pop32(h.thread);
    return particles.apply(handle, (object) =>
      object.controller.air.configure(...(values as Parameters<AokanaParticleAir['configure']>)),
    )
      ? 0
      : invalidHandle(h);
  };
