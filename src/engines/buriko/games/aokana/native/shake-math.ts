...
export function aokanaShakeRandom(random: AokanaCrtRandom, amplitude: number): number {
  ...
  for (let index = 0; index < 8; index++) {
    ...
    if (divisor === 0 || (numerator === -2147483648 && divisor === -1))
      throw new Error('Aokana shake random native IDIV fault');
...
export function aokanaShakeTarget(
  ...
  const x = aokanaShakeRandom(random, amplitude),
    y = aokanaShakeRandom(random, amplitude);
  switch (quadrant | 0) {
    case 0:
      return [x, -y | 0];
    case 1:
