...
export type AokanaRecordPredicateFormat =
...
export function decodeAokanaRecordPredicateFormat(format: number): AokanaRecordPredicateFormat {
...
function required(pointer: AokanaBpPointer | null): AokanaBpPointer {
  if (pointer === null)
    throw new RangeError('Aokana record predicate consumed a null native pointer');
  return pointer;
}
function at(pointer: AokanaBpPointer | null, bytes: number): AokanaBpPointer | null {
  return pointer === null ? null : {bytes: pointer.bytes, offset: pointer.offset + bytes};
}
function copiedScalar(pointer: AokanaBpPointer | null, size: number): bigint {
...
export function matchesAokanaRecordPredicate(
  record: AokanaBpPointer | null,
  comparison: AokanaBpPointer | null,
  format: number,
  operation: number,
  flags: number,
): boolean {
  const decoded = decodeAokanaRecordPredicateFormat(format);
...
export function findAokanaMatchingRecord(
  ...
  for (let index = 0; index < count; index = (index + 1) >>> 0) {
    if (matchesAokanaRecordPredicate(record, comparison, format, options & 0xffff, options >>> 16))
      return index;
...
export function writeAokanaMatchingRecordIndices(
  ...
  for (let index = 0; index < count; index = (index + 1) >>> 0) {
    if (
      matchesAokanaRecordPredicate(record, comparison, format, options & 0xffff, options >>> 16)
    ) {
