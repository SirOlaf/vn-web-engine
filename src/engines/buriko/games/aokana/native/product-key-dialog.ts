...
import {AokanaNativeText, textBytes, writeText} from './text.js';
...
export class AokanaProductKeyDialog {
  ...
  show(
    ...
    return this.dialogs.withNativeModal(async () => {
      ...
      if (prompt === null) prompts = ['プロダクトキーを入力してください'];
        ...
        if (raw.length >= 1024)
          throw new RangeError('Aokana product-key prompt overwrites native stack scratch storage');
      ...
      return new Promise<0 | 1>((resolve, reject) => {
        ...
        const finish = (accepted: boolean): void => {
          try {
            if (accepted) {
              if (output === null)
                throw new Error('Aokana product-key dialog dereferences a null output');
