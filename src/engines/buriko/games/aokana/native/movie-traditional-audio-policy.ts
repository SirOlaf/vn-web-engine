...
export interface AokanaTraditionalMovieGraphAudio {
...
export class AokanaTraditionalMovieAudioPolicy {
  private audio: AokanaTraditionalMovieGraphAudio | null = null;
  ...
  bindBorrowedGraphAudio(audio: AokanaTraditionalMovieGraphAudio): void {
    if (this.audio !== null)
      throw new Error('Aokana traditional movie graph audio is already bound');
    this.audio = audio;
  }

  unbindBorrowedGraphAudio(audio: AokanaTraditionalMovieGraphAudio): void {
    if (this.audio !== audio)
      throw new Error('Aokana traditional movie graph audio owner mismatch');
    this.audio = null;
  }
  ...
  setVolume(rawVolume: number): boolean {
    if (!Number.isSafeInteger(rawVolume))
      throw new RangeError('Aokana traditional movie volume requires an exact DWORD');
  ...
  setSuppression(rawValue: number): number {
    if (!Number.isSafeInteger(rawValue))
      throw new RangeError('Aokana traditional movie suppression requires an exact DWORD');
