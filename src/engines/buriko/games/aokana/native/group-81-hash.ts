...
function requirePointer(pointer: AokanaBpPointer | null): AokanaBpPointer {
  if (pointer === null) throw new Error('Aokana native checksum null pointer');
  return pointer;
}
...
export function nativeMd5(
  ...
  if (paddedSize < count) throw new RangeError('Aokana native MD5 allocation size wraps DWORD');
...
export const group81Hash: readonly AokanaNativeSlotDefinition[] = [
  {
    primary: 0x81,
    secondary: 0xe9,
