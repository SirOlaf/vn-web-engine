...
export interface AokanaBitmapAffineTransform {
  x: number;
  y: number;
  pivotX: number;
  pivotY: number;
...
export interface AokanaBitmapAffineCoordinates {
  startX: number;
  startY: number;
  columnX: number;
  columnY: number;
...
export function aokanaBitmapAffineCoordinates(
...
export function aokanaAlignedAffineSource(
  ...
  if (
    ...
    return null;
  const target = aokanaBitmapRectangle(destination),
    input = aokanaBitmapRectangle(source);
  translateAokanaBitmapRectangle(target, -(transform.x >> 16), -(transform.y >> 16));
  translateAokanaBitmapRectangle(input, -(transform.pivotX >> 16), -(transform.pivotY >> 16));
  if (
    ...
    return null;
  intersectAokanaBitmapRectangle(input, target);
  translateAokanaBitmapRectangle(input, transform.pivotX >> 16, transform.pivotY >> 16);
  const clipped = {...source};
  cropAokanaBitmap(clipped, input);
  return clipped;
}
...
function copyDimmed(destination: AokanaBitmap, source: AokanaBitmap, transparency: number): void {
...
function affinePixels(
  ...
  const coordinates = aokanaBitmapAffineCoordinates(transform);
...
function affineBitmap(
  ...
  if (
    ...
    runAokanaBitmapOperation(
      ...
      0,
      (bitmaps, point) => {
        if (point === null) throw new Error('Aokana affine strip is missing its native point');
        affineBitmap(
          compositor,
          bitmaps[0]!,
          source,
