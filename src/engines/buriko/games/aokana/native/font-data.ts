...
export interface AokanaFontName {
...
class FontReader {
  ...
  range(offset: number, length: number): Uint8Array {
    if (
      ...
      throw new RangeError('Aokana font table exceeds supplied resource');
...
function standalone(signature: number, tables: readonly Table[]): Uint8Array {
  ...
  sorted.forEach((table, index) => {
    ...
    if (table.tag === tags.head) {
      if (table.bytes.length < 12) throw new RangeError('Aokana font head table is truncated');
      headOffset = offset;
...
function names(table: FontReader): AokanaFontName[] {
  const version = table.u16(0);
  if (version > 1) throw new RangeError('Aokana font naming table version is invalid');
  ...
  for (let index = 0; index < count; index++) {
    ...
    if (
      ...
      if (bytes.length & 1) throw new RangeError('Aokana font UTF-16 name has odd length');
      unicode = '';
...
function face(reader: FontReader, directoryOffset: number, collection: boolean): AokanaFontData {
  ...
  if (
    ...
    throw new RangeError('Aokana font sfnt signature is invalid');
  const count = reader.u16(directoryOffset + 4);
  if (count === 0) throw new RangeError('Aokana font has no tables');
  ...
  for (let index = 0; index < count; index++) {
    ...
    if (map.has(tag)) throw new RangeError('Aokana font contains duplicate tables');
  ...
  if (!head || !hhea || !name)
    throw new RangeError('Aokana font lacks required metric/name tables');
  if (head.u32(0) !== 0x00010000 || hhea.u32(0) !== 0x00010000 || head.u32(12) !== 0x5f0f3cf5)
    throw new RangeError('Aokana font metric table version/signature is invalid');
  const unitsPerEm = head.u16(18);
  if (unitsPerEm < 16 || unitsPerEm > 16384)
    throw new RangeError('Aokana font units per em is invalid');
...
export function readAokanaFontData(bytes: Uint8Array): readonly AokanaFontData[] {
  const reader = new FontReader(bytes);
  if (reader.u32(0) !== 0x74746366) return [face(reader, 0, false)];
  const version = reader.u32(4);
  if (version !== 0x00010000 && version !== 0x00020000)
    throw new RangeError('Aokana font collection version is invalid');
  const count = reader.u32(8);
  if (count === 0) throw new RangeError('Aokana font collection is empty');
