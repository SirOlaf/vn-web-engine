...
export const aokanaFixedToFloat = (value: number): number => f32(f32(value | 0) / 65536);
...
export class AokanaLogicalSpatialRecord {
...
function outputWord(output: AokanaBpPointer | null, value: number): void {
  if (output === null) throw new Error('Aokana logical-space null DWORD output');
...
export function writeAokanaSpatialVector(
  output: AokanaBpPointer | null,
  vector: Float32Array,
): void {
  const values = Array.from(vector, fixedResult);
  if (output === null) throw new Error('Aokana logical-space null vector output');
...
export class AokanaLogicalSpatialManager {
  capacity = 64;
  readonly records = new Map<number, AokanaLogicalSpatialRecord>();

  record(index: number): AokanaLogicalSpatialRecord | undefined {
  ...
  createRecord(index: number, values: readonly number[], mask: number, flags: number): number {
    ...
    const record = new AokanaLogicalSpatialRecord(),
      view = record.view;
  ...
  setProperty(index: number, property: number, value: number): number {
    ...
    else record.view.setFloat32(4 + property * 4, aokanaFixedToFloat(value), true);
    return 0;
  }

  getProperty(output: AokanaBpPointer | null, index: number, property: number): number {
    const record = this.record(index);
    if (record === undefined) return 0xa0000001;
    property >>>= 0;
    if (property > 6) return 0xa0000002;
  ...
  getParent(output: AokanaBpPointer | null, index: number, group: number): number {
    const record = this.record(index);
    if (record === undefined) return 0xa0000001;
...
interface SpatialHandle {
  id: number;
  references: number;
  manager: AokanaLogicalSpatialManager;
}

/** Native 1400fa650/620 lifetime references and newest-first handle lookup. */
export class AokanaLogicalSpatialManagers {
  private nextId = 0;
  ...
  create(output: AokanaBpPointer | null): number {
    ...
    this.handles.unshift({
      id: this.nextId,
      references: 0,
      manager: new AokanaLogicalSpatialManager(),
    });
