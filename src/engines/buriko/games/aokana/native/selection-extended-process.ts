...
export class AokanaSelectionExtendedProcess extends AokanaSelectionProcess {
  ...
  protected override drawSelection(): void {
    super.drawSelection();
    let published: AokanaBitmapRectangle | null = null;
    for (const marker of [1, 2]) {
      ...
      if (this.selection === -1) this.window.setOverlayEnabled(marker, 0);
        ...
        if (rectangle === undefined)
          throw new Error('Aokana extended selection reads an undefined item');
        const x =
            marker === 1
        ...
        if (published === null)
          throw new Error('Aokana extended selection consumes undefined overlay damage');
...
export class AokanaSelectionBlinkProcess extends AokanaSelectionExtendedProcess {
