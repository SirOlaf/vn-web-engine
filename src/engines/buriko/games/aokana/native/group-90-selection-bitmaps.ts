...
import {drawAokanaSelectionBitmaps} from './selection-bitmaps.js';
...
export function createGroup90SelectionBitmaps(
  ...
  const execute = (h: AokanaBpOpcodeContext, extended: boolean): 0 | Promise<never> => {
    ...
    if (extended) {
      ...
      for (let index = 0; index < count; index++) {
        if (records === null)
          throw new Error('Aokana extended bitmap selection reads null records');
    ...
    if (!(window instanceof AokanaWindowDisplayObject))
      throw new Error('Aokana bitmap selection requires the actual Window owner');
    drawAokanaSelectionBitmaps(window, count, records);
