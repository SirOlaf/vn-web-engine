...
export function aokanaBfWriteByte(
...
export function aokanaBfWriteBytes(
  storage: AokanaBitmapStorage,
  offset: number,
  bytes: Uint8Array,
): void {
...
export function aokanaBfEncodedVarint(value: number): Uint8Array {
  const bytes: number[] = [];
  value >>>= 0;
  do {
    const next = value >>> 7;
...
export function aokanaBfEncoderTrees(rows: readonly Int16Array[]): {
  dc: AokanaBfTree;
  ac: AokanaBfTree;
  ...
  return {dc: aokanaBfTree(dc), ac: aokanaBfTree(ac), frequencies: [...dc, ...ac]};
}
function paths(tree: AokanaBfTree): number[][] {
...
function writeTokens(
  ...
  aokanaBfWriteByte(storage, start, 0);
  const put = (value: number) => {
    ...
    if (++bit === 8) {
      position++;
      bit = 0;
      if (position < capacity) aokanaBfWriteByte(storage, start + position, 0);
    }
  };
...
export function encodeAokanaBfCoefficientRow(
  coefficients: Int16Array,
  dc: AokanaBfTree,
  ac: AokanaBfTree,
): {storage: AokanaBitmapStorage; length: number} {
  const capacity = coefficients.length * 4,
    storage = new AokanaBitmapStorage(new Uint8Array(capacity), false);
...
export function encodeAokanaBfAlphaLz(
  input: Uint8Array,
  width: number,
  stride: number,
  capacity: number,
): {storage: AokanaBitmapStorage; length: number} {
  const storage = new AokanaBitmapStorage(new Uint8Array(capacity), false);
  let output = 1,
    flag = 0,
    bit = 0,
    position = 3;
  aokanaBfWriteByte(storage, 0, 0);
  ...
  while (position < input.length && output < capacity) {
    ...
    if (length >= 3) {
      ...
      aokanaBfWriteByte(storage, flag, storage.bytes[flag]! | (1 << bit));
      ...
      aokanaBfWriteByte(storage, output++, read(position));
      position += 4;
    }
    if (++bit === 8 && position < input.length && output < capacity) {
      flag = output++;
      bit = 0;
      aokanaBfWriteByte(storage, flag, 0);
    }
  }
