...
import type {AokanaProgramResourceConfiguration} from './program-resources.js';
import {terminatedNativeBytes} from './program-files.js';
import {AokanaNativeText, copyText, textBytes, writeText} from './text.js';
import {AokanaNativeRegistry} from './windows-registry.js';

export interface AokanaUserFolderProfile {
  desktop: string | null;
...
export interface AokanaSpecialFolderProfile {
  shellAllocatorAvailable: boolean;
  windows: string | null;
  programFiles: string | null;
  currentUser: AokanaUserFolderProfile;
  shellUser: AokanaUserFolderProfile;
  elevated: boolean;
...
function wideRegistryString(bytes: Uint8Array): string {
  ...
  throw new Error('Aokana special folder reads an unterminated native registry string');
}
...
export class AokanaSpecialFolders {
  constructor(
    readonly text: AokanaNativeText,
    readonly registry: AokanaNativeRegistry,
    readonly roots: Pick<AokanaProgramResourceConfiguration, 'primaryRoot' | 'secondaryRoot'>,
    readonly profile: AokanaSpecialFolderProfile,
  ) {}

  resourceRoot(output: AokanaBpPointer | null, selector: number): 0 | 1 {
    const source =
    ...
    if (output === null) throw new Error('Aokana resource root dereferences a null output');
  ...
  private encode(path: string): Uint8Array {
    if (path.length > 783)
      throw new RangeError('Aokana special folder overwrites native wide stack scratch');
  ...
  async query(output: AokanaBpPointer | null, selector: number): Promise<0 | 1> {
    ...
    if (selector === 5) encoded = await this.programFilesRegistry();
      ...
      if (path === null)
        throw new Error('Aokana special folder reads unwritten native wide stack scratch');
      ...
      encoded = this.encode(path);
      if (output === null) throw new Error('Aokana special folder dereferences a null output');
      ...
      if (this.profile.elevated && !impersonated) {
        ...
        if (prefix !== null) {
          ...
          if (prefixBytes.length > 784)
            throw new RangeError('Aokana special folder overwrites native encoded profile scratch');
          ...
          if (
            ...
            if (account === null)
              throw new Error('Aokana special folder cannot resolve its native shell account');
            const accountBytes = this.encode(account);
            if (accountBytes.length > 784 || encoded.length > 784)
              throw new RangeError('Aokana special folder overwrites native fallback scratch');
            ...
            if (this.encode(parent).length > 780)
              throw new Error('Aokana special folder reads incomplete native split-path conversion');
  ...
  combine(
    ...
    first: AokanaBpPointer,
    separator: number,
    second: AokanaBpPointer,
  ): void {
