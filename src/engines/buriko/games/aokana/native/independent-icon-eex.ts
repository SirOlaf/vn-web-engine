...
import type {AokanaVirtualDisplayObject} from './display-virtual.js';
import type {AokanaWindowDisplayObject} from './display-window.js';
import {AokanaIconMotionTree} from './icon-motion-tree.js';
...
function read(points: AokanaBpPointer | null, offset: number): number {
  if (points === null) throw new Error('Aokana IconEEx dereferences null motion points');
...
function fraction(elapsed: number, duration: number): number {
  const total = f32(duration >>> 0),
    seed = aokanaRosettaSseReciprocal(total),
...
export class AokanaIndependentIconEEx extends AokanaIndependentIconEx {
  override readonly type: number = 2;
  private readonly motions = new AokanaIconMotionTree<Motion>();
  constructor(
    ...
    private readonly splines: AokanaNativeSplines,
  ) {
  ...
  private motionChild(row: number, column: number): AokanaVirtualDisplayObject | null {
    for (const entry of this.flat)
      if (entry.present && entry.row === row && entry.column === column) return entry.child;
    return null;
  }
  private requireChild(child: AokanaVirtualDisplayObject | null): AokanaVirtualDisplayObject {
    if (child === null) throw new Error('Aokana IconEEx dereferences absent motion child');
    return child;
  }
