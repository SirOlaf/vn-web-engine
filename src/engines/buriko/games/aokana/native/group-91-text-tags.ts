...
import {collectAokanaRawLinks, stripAokanaTextTags} from './text-tags.js';
import type {AokanaNativeSlotDefinition} from './types.js';

/**DE7F0/DE7B0 use native encoded substring search and actual overlapping byte pointers. */
export function createGroup91TextTags(text: AokanaNativeText): AokanaNativeSlotDefinition[] {
  return [
    {
      primary: 0x91,
      secondary: 0x9e,
      ...
      execute: (context) => {
        ...
        push32(context.thread, collectAokanaRawLinks(text, output, source));
        return 0;
      },
    },
    {
      ...
      execute: (context) => {
        const source = context.memory.resolve(context.thread, pop32(context.thread)),
          output = context.memory.resolve(context.thread, pop32(context.thread));
        stripAokanaTextTags(text, output, source);
        return 0;
      },
    },
  ];
