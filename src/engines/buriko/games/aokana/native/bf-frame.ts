...
import {requireAokanaResourceRange as checkRange, AokanaBfBits as Bits, aokanaBfVarint as unsignedVarint, aokanaBfTree as movieFrequencyTree, aokanaBfSymbol, aokanaBfSignedBits as movieSignedBits} from './bf-entropy.js';
...
for (let index = 0; index < 256; index++) {
  ...
  colorTable[index] = f(f(chroma * f(1.402)) + 0.5);
  colorTable[256 + index] = f(chroma * f(-0.34414));
  colorTable[512 + index] = f(0.5 - f(chroma * f(0.71414)));
  colorTable[768 + index] = f(f(chroma * f(1.772)) + 0.5);
}
function colorLookup(base: number, index: number): number {
  const value = colorTable[base + index];
  if (value === undefined) throw new AokanaUndefinedResourceRead('Aokana BF color lookup reads outside initialized native lookup storage');
  return value;
}
...
export function decodeAokanaBfFrame(
  ...
  const alignedWidth = (width + 7) & ~7, alignedHeight = (height + 7) & ~7;
  const columns = alignedWidth >>> 3, rows = alignedHeight >>> 3, maskSize = (columns + 7) >>> 3;
  const frameExtent = Math.imul(Math.imul(width, height), 4) >>> 0;
  const surface = destination ?? {bytes: new Uint8Array(width * height * 4), initialized: new Uint8Array(width * height * 4)};
  ...
  const offsets = Array.from({length: rows + 1}, (_, index) => data.getUint32(cursor.position + index * 4, true));
  ...
  const requireCoefficients = (start: number, length: number): void => {
    ...
    for (let index = start; index < start + length; index++) {
      if (defined[index] === 0) throw new AokanaUndefinedResourceRead('Aokana BF reconstruction reads unwritten coefficient storage');
    }
  };
  const descriptors = Array.from({length: rows}, (_, row) => {
    const start = offsets[row]!, rowCursor = {position: start + maskSize};
    const count = unsignedVarint(frame, rowCursor);
    return {start, count, dataStart: rowCursor.position, limit: (offsets[row + 1]! - start - maskSize) >>> 0};
  });
  // 105f30 selects/validates the alpha decoder before starting any work callback.
  let alphaMode = 0, alphaStart = 0;
  if (depth === 32) {
    alphaStart = offsets[rows]!;
    checkRange(frame.length, alphaStart, 4);
    alphaMode = data.getUint32(alphaStart, true);
    if (alphaMode !== 1 && alphaMode !== 2) throw new AokanaResourceCodecException(9, `Aokana native BF alpha codec exception: ${alphaMode}`);
  }
  const decodeRow = (row: number): void => {
    ...
    const dcBits = new Bits(frame.subarray(rowCursor.position));
    let dc = 0;
    ...
    const acBits = new Bits(frame.subarray(rowCursor.position + Math.ceil(dcBits.position / 8)));
    for (let index = 0; index < count && Math.floor(acBits.position / 8) < limit; index += 64) {
      for (let order = 1; order < 64;) {
        ...
        if (symbol === 0) break;
        if (symbol === 15) { order += 16; continue; }
        const targetOrder = order + (symbol & 15), target = movieZigzag[targetOrder];
        if (target === undefined) throw new AokanaUndefinedResourceRead('Aokana BF AC run reads beyond native zigzag table');
    ...
    const channels = depth >>> 3, components = channels === 4 ? 3 : channels;
    if (components === 0) throw new AokanaUndefinedResourceRead('Aokana BF reconstruction divides by zero components');
    ...
    for (let column = 0; column < columns; column++) {
      ...
      for (let component = 0; component < components; component++) {
        ...
        const transformed = movieIdct(coefficients.subarray(at, at + 64), quantization.subarray(component === 0 ? 0 : 64, component === 0 ? 64 : 128));
        // Native PACKUSWB saturates every IDCT value before writing it back as a word.
        for (let index = 0; index < 64; index++) coefficients[at + index] = clip(transformed[index]!);
      }
      const first = base + block * 64;
      for (let yy = 0; yy < 8 && row * 8 + yy < height; yy++) {
        for (let xx = 0; xx < 8 && column * 8 + xx < width; xx++) {
          const sample = yy * 8 + xx, at = ((row * 8 + yy) * width + column * 8 + xx) * 4;
          if ((xx & 3) === 0) {
            // Native reads each SIMD group of four words even for a cropped1..3-pixel tail.
            for (let plane = 0; plane < 3; plane++) requireCoefficients(first + plane * stride + sample, 4);
            if (components !== 1) for (let tail = 0; tail < 4; tail++) {
              const cb = coefficients[first + stride + sample + tail]!, cr = coefficients[first + stride * 2 + sample + tail]!;
              colorLookup(768, cb); colorLookup(256, cb); colorLookup(512, cr); colorLookup(0, cr);
            }
          }
          const y = coefficients[first + sample]!, cb = coefficients[first + stride + sample]!, cr = coefficients[first + stride * 2 + sample]!;
          surface.bytes[at] = components === 1 ? y : clip(f(y + colorLookup(768, cb)));
          surface.bytes[at + 1] = components === 1 ? y : clip(f(f(colorLookup(256, cb) + y) + colorLookup(512, cr)));
  ...
  processing.setCallback(() => {
    ...
    if (descriptor === 0) {
      if (depth !== 32) {
        for (let at = 3; at < frameExtent; at += 4) {surface.bytes[at] = 0; surface.initialized[at] = 1;}
      } else if (alphaMode === 1) decodeAlphaLz(frame.subarray(alphaStart + 4), surface, width, frameExtent);
      else decodeAlphaBlocks(frame.subarray(alphaStart + 4), surface, width, height, columns, rows);
...
function decodeAlphaLz(bytes: Uint8Array, surface: AokanaBfSurface, width: number, frameExtent: number): void {
  const data = view(bytes);
  let cursor = 0, output = 3;
  while (output < frameExtent) {
    ...
    for (let bit = 0; bit < 8 && output < frameExtent; bit++) {
      if (control & (1 << bit)) {
        ...
        const code = data.getUint16(cursor, true); cursor += 2;
        let dx = code & 63, dy = (code >>> 6) & 7;
        if (dx > 31) dx -= 64;
        if (dy !== 0) dy -= 8;
        const count = (code >>> 9) + 3, distance = dy * width * 4 + dx * 4;
        for (let index = 0; index < count; index++, output += 4) {
          ...
          if (surface.initialized[source] === 0) throw new AokanaUndefinedResourceRead('Aokana BF alpha LZ reads an unwritten destination byte');
...
function decodeAlphaBlocks(bytes: Uint8Array, surface: AokanaBfSurface, width: number, height: number, columns: number, rows: number): void {
  checkRange(bytes.length, 0, 4);
  const size = view(bytes).getUint32(0, true), cursor = {position: 4};
  ...
  for (let row = 0; row < rows; row++) for (let column = 0; column < columns; column++) {
    ...
    for (let yy = row * 8; yy < Math.min(row * 8 + 8, height); yy++) for (let xx = column * 8; xx < Math.min(column * 8 + 8, width); xx++) {
      if (read >= size) throw new AokanaUndefinedResourceRead('Aokana BF alpha mask reads unwritten Huffman output');
