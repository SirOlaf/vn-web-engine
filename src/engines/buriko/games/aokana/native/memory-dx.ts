...
export class AokanaMemoryDx {
  private allocated = 0;
  private storage: AokanaBitmapStorage | null = null;
  private length = 0;
  private disposed = false;

  private check(): void {
    if (this.disposed) throw new Error('Aokana accesses a deleted CMemoryDX');
  }
  get active(): number {
    this.check();
  ...
  get allocation(): AokanaBitmapStorage | null {
    this.check();
    return this.storage;
  }

  /** 06e0a0 marks the owner allocated even when the platform allocation returns null. */
  allocate(size: number): AokanaBitmapStorage | null {
    this.check();
    if (this.allocated !== 0) return null;
    size >>>= 0;
    let storage: AokanaBitmapStorage | null;
    try {
      storage = new AokanaBitmapStorage(new Uint8Array(size), false);
    } catch (error) {
