...
export function copyAokanaPathComponentPrefix(
...
export class AokanaDirectoryTree {
  private readonly created: string[] = [];

  constructor(private readonly files: AokanaProgramFiles) {}
  ...
  async ensure(path: Uint8Array): Promise<0 | 1> {
    ...
    for (let index = 0; ; index++) {
      const prefix = copyAokanaPathComponentPrefix(this.files, path, index);
