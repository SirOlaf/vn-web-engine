import {aokanaBfVarint, requireAokanaResourceRange} from './bf-entropy.js';
import {AokanaBitmapStorage, bitmapStorage, type AokanaBitmap} from './bitmap.js';
import {bitmapRead8, bitmapWrite8, bitmapWrite32} from './bitmap-scalar.js';

function read32(bytes: Uint8Array, offset: number): number {
  requireAokanaResourceRange(bytes.length, offset, 4);
...
function entropy(frame: Uint8Array): Uint8Array {
  const cursor = {position: 0},
    length = aokanaBfVarint(frame, cursor);
  ...
  for (let symbol = 0; symbol < 256; symbol++) {
    const frequency = aokanaBfVarint(frame, cursor);
  ...
  for (let index = 0; index < length; index++) {
    ...
    while (children[node]![0] !== -1 && children[node]![1] !== -1) {
      if (remaining === 0) {
        requireAokanaResourceRange(frame.length, cursor.position, 1);
...
function expand(
  ...
  const storage = new AokanaBitmapStorage(
  ...
  while (cursor.position < encoded.length && output < length) {
    const fill = aokanaBfVarint(encoded, cursor),
      literal = aokanaBfVarint(encoded, cursor);
    storage.range(output, fill, false);
    storage.bytes.fill(0x80, output, output + fill);
    storage.written(output, fill);
    output = (output + fill) >>> 0;
    requireAokanaResourceRange(encoded.length, cursor.position, literal);
...
function read8(storage: AokanaBitmapStorage, offset: number): number {
...
export function decodeAokanaLegacyBfFrameData(
  frame: Uint8Array,
  width: number,
  height: number,
  depth: number,
...
export function decodeAokanaLegacyBfFrame(
  movie: Uint8Array,
  frameIndex: number,
  destination: AokanaBitmap,
): boolean {
