...
export function fillAokanaLinearVectorMap(bitmap: AokanaBitmap, direction: number): number {
...
export function fillAokanaRadialVectorMap(
  bitmap: AokanaBitmap,
  mode: number,
  centerX: number,
  centerY: number,
  ...
  for (let y = 0; y < bitmap.height >>> 0; y++, dy = (dy + 1) | 0) {
    ...
    for (let x = 0; x < bitmap.width >>> 0; x++, dx = (dx + 1) | 0) {
      ...
      const seed = radius32 === 0 ? Infinity : aokanaRosettaSseReciprocal(radius32);
      ...
      if (divisor === 0) throw new RangeError('Aokana radial vector map divides by zero');
