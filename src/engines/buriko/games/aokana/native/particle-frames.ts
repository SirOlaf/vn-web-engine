...
import {AokanaParticleDisplays, type AokanaParticleRefreshNode} from './particle-displays.js';
  ...
  pollRefresh(): void {
    ...
    while (node !== null) {
      if (node.nextTick >>> 0 <= now) {
        ...
        if (interval === 0) throw new RangeError('Aokana particle refresh interval is zero');
