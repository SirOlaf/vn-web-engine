...
import {
  decodeAokanaBmvFrameData,
  decodeAokanaBmvIndexedFrame,
  validateAokanaBmvHeader,
} from './bmv-frame.js';
import {decodeAokanaLegacyBfFrame} from './bf-legacy-frame.js';
import type {AokanaBmvEntry, AokanaBmvRegistry} from './bmv-registry.js';
...
export interface AokanaBmvWorker {
  readonly actor: object;
  readonly surface: number;
  readonly entry: AokanaBmvEntry;
...
export class AokanaBmvService {
  ...
  constructor(
    ...
    readonly synchronous: AokanaDistributedProcessing,
    readonly asynchronous: AokanaDistributedProcessing | null,
  ) {
    if (
  ...
  start(
    ...
    const initial = this.registry.lock.run(() => {
      ...
      const modern = validateAokanaBmvHeader(entry.resource.bytes) === 0;
      return {status: modern && !this.admit(movie) ? 0x80000009 : 0, modern};
    });
    if (initial.status !== 0) return {status: initial.status, worker: null};
    const actor = {};
    const worker = this.asActor(actor, (): AokanaBmvWorker => {
      if (!this.surfaces.lock(surface))
        throw new Error('Aokana BMV worker cannot signal startup without a live surface');
      this.registry.lock.run(() => {
        const entry = this.registry.find(movie);
        if (entry === null)
          throw new Error('Aokana BMV worker cannot signal startup without its movie');
        entry.lock.enter();
      });
      const destination = this.surfaces.snapshot(surface);
      if (destination === null)
        throw new Error('Aokana BMV worker cannot signal startup without its descriptor');
      const entry = this.registry.find(movie);
      if (entry === null)
        throw new Error('Aokana BMV worker cannot signal startup without its locked movie');
      return {
        actor,
        surface,
        entry,
  ...
  private async decode(
    ...
    const bytes = entry.resource.bytes;
    if (validateAokanaBmvHeader(bytes) !== 0) {
      if (word(bytes, 0x10) !== 0) return 0x8000000a;
      this.asActor(actor, () => decodeAokanaLegacyBfFrame(bytes, frame, destination));
      return 0;
    }
    const provenance = entry.resource.provenance;
    if (provenance === null) {
      this.asActor(actor, () => decodeAokanaBmvIndexedFrame(bytes, frame, destination, processing));
      return 0;
    }
    ...
    if (read.result === 0)
      this.asActor(actor, () => decodeAokanaBmvFrameData(bytes, encoded, destination, processing));
    return 0;
  }
