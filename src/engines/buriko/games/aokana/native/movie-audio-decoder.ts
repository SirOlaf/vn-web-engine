import {aokanaIsoAacConfiguration, type AokanaAacDecoderConfiguration} from './movie-iso-codecs.js';
import {
...
interface BrowserAudioDecoder {
  ...
  ondequeue: (() => void) | null;
  configure(config: AokanaAacDecoderConfiguration): void;
...
interface BrowserAudioDecoderConstructor {
  ...
  isConfigSupported(config: AokanaAacDecoderConfiguration): Promise<{supported: boolean}>;
}
...
export interface AokanaDecodedMovieAudio {
 ...
 */
export class AokanaMovieAudioDecoder {
  private decoder: BrowserAudioDecoder;
  private readonly output: AokanaDecodedMovieAudio[] = [];
  ...
  static async create(
    ...
    const configurations = new Map<number, AokanaAacDecoderConfiguration>();
    for (const sample of track.samples) {
      if (configurations.has(sample.description)) continue;
      const description = track.descriptions[sample.description - 1];
      if (description === undefined)
        throw new AokanaIsoSampleError('AAC sample has no description');
      const config = aokanaIsoAacConfiguration(description),
  ...
  async next(): Promise<AokanaDecodedMovieAudio | null> {
    const generation = this.generation;
    this.check(generation);
    if (this.reading === generation) throw new Error('Concurrent Aokana AAC reads');
    ...
    try {
      for (;;) {
        ...
        if (sample.description !== this.description) {
          ...
          if (config === undefined)
            throw new AokanaIsoSampleError('AAC description was not negotiated');
