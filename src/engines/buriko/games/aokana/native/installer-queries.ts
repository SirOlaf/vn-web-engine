  ...
  string(): string {
    ...
    for (let offset = 0; ; offset += 2) {
      ...
      if (!this.initialized[offset] || !this.initialized[offset + 1])
        throw new Error('Aokana installer query reads unwritten wide stack scratch');
...
export class AokanaInstallerQueries {
  constructor(
    ...
    if (folders.registry !== registry || folders.text !== files.text)
      throw new Error('Aokana installer queries require shared registry/text owners');
  }
  private key(publisher: AokanaBpPointer, product: AokanaBpPointer): RegistryScratch {
  ...
  async installedFolder(
    ...
    publisher: AokanaBpPointer | null,
    product: AokanaBpPointer | null,
  ): Promise<0 | 1> {
    ...
    try {
      ...
      if (output === null) throw new Error('Aokana installed folder writes through null');
  ...
  async legacyFolder(
    ...
    if ((await this.folders.query(base, 0)) === 0)
      throw new Error('Aokana legacy folder consumes unwritten special-folder scratch');
    ...
    if (bytes.length !== 0) {
      if (output === null) throw new Error('Aokana legacy folder reads a file into null');
    ...
    const at = (relative: number): DataView => {
      if (output === null) throw new Error('Aokana legacy folder reads through null');
