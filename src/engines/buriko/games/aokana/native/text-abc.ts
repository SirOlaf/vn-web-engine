...
export async function collectAokanaRegisteredTextAbc(
  ...
  if (selected.result !== 0)
    throw new Error('Aokana registered ABC query returns an unwritten native font status');
  if (source === null) throw new Error('Aokana registered ABC query reads null text');
  const wide = state.text.decodeAuto(source),
    font = fonts.find(selected.id);
  if (font === null) throw new Error('Aokana registered ABC query has no published font');
  let written = 0;
  for (let index = 0; index < wide.length; index++) {
    ...
    if (output !== null) {
      if (font.raster === null)
        throw new Error('Aokana ABC query reads undefined native glyph cache');
