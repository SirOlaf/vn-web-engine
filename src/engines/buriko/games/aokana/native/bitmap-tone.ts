...
import type {AokanaSurfaceToneCurve} from './surface-tone-curves.js';
...
function tonePixel(
  ...
  for (let channel = 0; channel < 3; channel++) {
    ...
    if (entry === undefined)
      throw new Error('Aokana tone kernel reads outside its initialized table');
    mapped |= entry;
  }
  let output = 0;
...
function applyAokanaBitmapTonePixels(
  ...
  if (
    parallel &&
    runAokanaBitmapOperation(compositor.processing, [destination, source], source, 0, (bitmaps) => {
      applyAokanaBitmapTone(
        compositor,
        bitmaps[0]!,
        bitmaps[1]!,
        monochrome,
