...
export interface AokanaFrameMetricRaster {
...
export class AokanaFrameMetrics {
  enabled = 0;
  ...
  private good = 0;
  constructor(
    readonly counter: AokanaNativePerformanceCounter,
    readonly clock: AokanaNativeClock,
    readonly raster: AokanaFrameMetricRaster,
  ) {}
  ...
  enable(value: number): void {
    ...
    if (refresh === 0) throw new RangeError('Aokana frame metric refresh period is zero');
  ...
  /** 031c90 commits after the object draw even when that draw could not lock the display. */
    ...
    if (this.startScanline !== 0xffffffff && output.scanline === undefined)
      throw new Error('Aokana frame metric raster query supplied no end scanline');
    if (
