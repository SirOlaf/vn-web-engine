...
export class AokanaResourceRanges {
  ...
  private async loose(
    root: Uint8Array, name: Uint8Array, destination: AokanaBpPointer | null, offset: number, length: number,
  ): Promise<RawRead> {
    const terminated = terminatedNativeBytes(name);
    if (terminated[0] === 92 || terminated[1] === 58)
      return this.file(terminated, destination, offset, length);
    let result = await this.file(this.resources.loosePath(root, terminated), destination, offset, length);
    if (this.resources.configuration.searchDirectoriesEnabled !== 0) {
      for (const directory of this.resources.configuration.searchDirectories) {
        if (result.result !== 1) break;
        result = await this.file(this.resources.loosePath(
          this.resources.loosePath(root, directory), terminated, true), destination, offset, length);
      }
    }
    return result;
  }
  ...
  private async archive(
    root: Uint8Array, archive: Uint8Array, name: Uint8Array,
    destination: AokanaBpPointer | null, offset: number, length: number,
  ): Promise<number> {
    if (textLength({bytes: terminatedNativeBytes(name), offset: 0}) >= 96) {
      return this.resources.errors.fatal(this.resources.files.text.encodeWide(
        `指定されたファイル名 [ ${this.resources.files.path(name)} ] は95文字を超えています`, 1));
    }
  ...
  async size(archive: Uint8Array | null, name: Uint8Array): Promise<number> {
    ...
    if (archive === null) {
      if (result.result !== 0) result = await this.loose(configuration.secondaryRoot, name, null, 0, 0);
    ...
    if (size === 0x80000010 && this.resources.files.media.isAvailable(configuration.secondaryMediaPath))
  ...
  async read(
    destination: AokanaBpPointer | null, archive: Uint8Array | null, name: Uint8Array,
    ...
    let count = await this.archive(configuration.primaryRoot, archive, name, destination, offset, length);
    if ((count === 0x80000010 || count === 0x80000020) &&
        this.resources.files.media.isAvailable(configuration.secondaryMediaPath))
      count = await this.archive(configuration.secondaryRoot, archive, name, destination, offset, length);
  ...
  async metadata(archive: Uint8Array | null, name: Uint8Array, output: {value: bigint}): Promise<number> {
