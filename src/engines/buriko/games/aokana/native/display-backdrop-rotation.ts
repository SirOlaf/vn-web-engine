...
export class AokanaRotationBackdrop extends AokanaBackdrop {
  private source = -1;
  ...
  setDelta(scale: number, angle: number): number {
    ...
    if (this.configuration === undefined)
      throw new Error('Aokana rotation backdrop reads unwritten base scale');
  ...
  override drawContent(destination: AokanaBitmap, _rectangle: AokanaBitmapRectangle): 0 | 1 {
    ...
    if (configuration === undefined)
      throw new Error('Aokana rotation backdrop draws before base configuration');
    ...
    const angle =
      (Number(BigInt.asIntN(32, (BigInt(configuration.angleDelta) * BigInt(blend)) >> 8n)) +
        configuration.angle) |
      0;
    rotateCenteredAokanaBitmap(this.environment.compositor, destination, source, scale, angle);
    return 1;
  }
}
