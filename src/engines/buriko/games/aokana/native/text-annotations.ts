...
import {AokanaNativeText, textByte, textBytes, textLength, writeText} from './text.js';
...
export interface AokanaRubyAnnotation {
...
export function aokanaTextCodes(
  ...
  if (output !== null) {
    if (output.length < count)
      throw new RangeError('Aokana ruby code output is smaller than its native count');
    let offset = 0;
...
export class AokanaRubyAnnotations {
  private head: AokanaRubyAnnotation | null = null;
  constructor(readonly text: AokanaNativeText) {}

  get first(): AokanaRubyAnnotation | null {
    return this.head;
  }
  ...
  private reading(
    ...
    const readingLength = aokanaTextCodes(this.text, pointer(reading)),
      codes = new Uint32Array(readingLength);
    aokanaTextCodes(this.text, pointer(reading), codes);
    return {readingWide, reading, codes, readingLength};
  }

  /** 0785a0 updates the first matching entry only for a persistent insertion. */
  add(key: AokanaBpPointer, reading: AokanaBpPointer, oneUse = 0): void {
    ...
    const entry: AokanaRubyAnnotation = {
      key: normalized.slice(),
      keyByteLength,
      keyWideLength: aokanaTextCodes(this.text, pointer(normalized)),
  ...
  matchEncodedPrefix(source: AokanaBpPointer): Uint8Array | null {
  ...
  query(key: AokanaBpPointer): AokanaRubyAnnotation | null {
    this.text.convertEncoding(key, 1);
    for (let entry = this.head; entry !== null; entry = entry.next)
      if (aokanaCompareNamedBytes(key, pointer(entry.key)) === 0) return {...entry, next: null};
    return null;
  }

  /** 078410 removes the first matching normalized key allowed by the inline-only flag. */
  remove(key: AokanaBpPointer, inlineOnly = 0): 0 | 1 {
    ...
    while (entry !== null) {
      if (
        ...
        aokanaCompareNamedBytes(normalized, pointer(entry.key)) === 0
      ) {
  ...
  import(source: AokanaBpPointer | null): 0 | 1 {
    ...
    while (textByte(source.bytes, offset) !== 0) {
      ...
      if (keyLength !== null && keyLength > 0) {
        if (keyLength >= 256)
          throw new RangeError('Aokana ruby key exceeds its native scratch range');
        ...
        if (readingLength > 0) {
          if (readingLength >= 256)
            throw new RangeError('Aokana ruby reading exceeds its native scratch range');
      ...
      if (offset === before)
        throw new Error('Aokana ruby import does not advance its native input');
    }
    return 1;
  }
    ...
    while (textByte(normalized, offset) !== 0) {
      ...
      if (skip < 1) {
        ...
        if (matched !== null) {
          ...
          if (annotation === null)
            throw new Error('Aokana matched ruby key is absent from its registry');
