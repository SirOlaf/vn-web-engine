...
export interface AokanaMonochromeGlyph {
...
export class AokanaMonochromeFont {
  private active = false;
  private sizeValue: number | undefined;
  private packed = new AokanaBitmapStorage(new Uint8Array(1), false);
  ...
  constructor(readonly fonts: AokanaNativeFonts) {}
  ...
  async initialize(
    ...
    this.packed.release();
    this.packed = new AokanaBitmapStorage(new Uint8Array(blockSize * 512), false);
    ...
    this.dib = new AokanaBitmapStorage(new Uint8Array(this.dibStride * size), false);
    this.fixedCount = 0;
    this.active = true;
    return 0;
  }
  ...
  get size(): number {
    if (this.sizeValue === undefined) throw new Error('Aokana mono font reads unwritten size');
  ...
  glyph(character: number): AokanaMonochromeGlyph {
    character >>>= 0;
    if (this.fixedCount === undefined)
      throw new Error('Aokana mono font reads unwritten fixed-cache count');
    for (let index = 0; index < this.fixedCount; index++) {
      const record = this.fixedRecords[index];
      if (record === undefined)
        throw new Error('Aokana mono font dereferences an unwritten fixed record');
    ...
    for (let index = 0; index < this.used; index++) {
      if (cursor === undefined)
        throw new Error('Aokana mono font dereferences an unwritten cache link');
    ...
    if (selected === undefined) {
      ...
      if (selected === undefined)
        throw new Error('Aokana mono font dereferences an unwritten cache record');
      selected.character = character;
      selected.wide = aokanaWideCharacter(character);
  ...
  private raster(offset: number, character: number): readonly [number, number, number, number] {
    ...
    if (
      ...
      throw new Error('Aokana mono font consumes unwritten raster state');
    ...
    if (text !== undefined) {
      ...
      for (let y = 0; y < size; y++) {
        ...
        if (input + stride > raster.bytes.length)
          throw new RangeError('Aokana mono font platform DIB is truncated');
