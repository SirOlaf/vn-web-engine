...
function snapshot(pointer: AokanaBpPointer): Uint8Array {
...
export class AokanaFileAssociations {
  constructor(
  ...
  private async create(
    ...
    if (result.handle === undefined)
      throw new Error('Aokana file association host returned success without a registry handle');
  ...
  async register(
    extension: AokanaBpPointer,
    className: AokanaBpPointer,
    description: AokanaBpPointer,
    icon: AokanaBpPointer,
    command: AokanaBpPointer,
  ): Promise<0 | 1> {
