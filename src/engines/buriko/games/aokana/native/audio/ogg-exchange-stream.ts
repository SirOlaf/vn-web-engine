...
import {AokanaWaveBoxOggDecoder} from './wavebox-ogg.js';
...
export class AokanaOggExchangeDecoder extends AokanaWaveBoxOggDecoder {
  private selected = 0;
  private position = 0;
  ...
  constructor(
    readonly first: AokanaWaveBoxOggDecoder,
    readonly second: AokanaWaveBoxOggDecoder,
    private readonly firstStorage: AokanaLiveAudioStorage,
    private readonly secondStorage: AokanaLiveAudioStorage,
    rawLoopDword: number,
  ) {
  ...
  get activeHeader(): AokanaWaveBoxHeader {
...
async function selectorCodec(input: AokanaLiveAudioStorage, actor: object): Promise<number> {
  ...
  if (count < 64)
    throw new AokanaWaveBoxError(14, 'Aokana Ogg exchange selector read a short header');
  requireAokanaWaveHeaderBytes(initialized, 48, 4);
...
export async function createAokanaLiveOggExchangeWaveStream(
  first: AokanaLiveAudioStorage,
  second: AokanaLiveAudioStorage,
  rawLoopDword: number,
  ...
  let exchange: AokanaOggExchangeDecoder | undefined, stream: AokanaWaveStream | undefined;
  try {
    ...
    if (firstCodec !== secondCodec || firstCodec !== 3)
      throw new AokanaWaveBoxError(14, 'Aokana Ogg exchange requires matching codec3 inputs');
    await first.seek(0, actor);
    await second.seek(0, actor);
    // 119B10 opens the second Vorbis chain before the first.
    const secondDecoder = await materializeAokanaLiveOgg(second, options, actor, Context),
      firstDecoder = await materializeAokanaLiveOgg(first, options, actor, Context);
    exchange = new AokanaOggExchangeDecoder(
      firstDecoder,
      secondDecoder,
      first,
      second,
    ...
    stream = new AokanaWaveStream(exchange, rawMilliseconds, actors);
    ...
    if (error instanceof AokanaWaveBoxError && error.nativeCode !== 14)
      throw new AokanaWaveBoxError(14, 'Aokana Ogg exchange model rejected its input');
    throw error;
  }
}
