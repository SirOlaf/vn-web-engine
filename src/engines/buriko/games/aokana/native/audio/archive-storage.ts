...
import type {AokanaDcArchive} from './dc-archive.js';

/** 115DE0/115E70/115D10: member cursor owns its name, not the shared archive. */
export class AokanaArchiveFileStorage {
  private archive: AokanaDcArchive | undefined;
  private name = '';
  private sizeValue = 0;
  ...
  get position(): number {
    if (this.positionValue === undefined)
      throw new Error('Aokana archive storage reads unwritten cursor');
    return this.positionValue;
  }
  private check(): void {
    if (this.disposed) throw new Error('Aokana archive storage accesses released owner');
  }
  async open(archive: AokanaDcArchive, name: string, actor: object): Promise<boolean> {
    this.check();
  ...
  async readInto(
    ...
    if (count !== 0) {
      if (this.archive === undefined)
        throw new Error('Aokana archive storage reads unwritten archive');
