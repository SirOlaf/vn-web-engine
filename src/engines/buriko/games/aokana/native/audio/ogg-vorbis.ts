...
export function aokanaOggChecksum(page: Uint8Array): number {
  let crc = 0;
  for (let index = 0; index < page.length; index++) {
    crc ^= (index >= 22 && index < 26 ? 0 : page[index]!) << 24;
...
function identification(packet: Uint8Array): AokanaVorbisIdentification {
  if (packet.length < 30 || packet[0] !== 1 ||
  ...
  const channels = packet[11]!, sampleRate = view.getUint32(12, true);
  const shortExponent = packet[28]! & 15, longExponent = packet[28]! >>> 4;
  if (view.getUint32(7, true) !== 0 || channels === 0 || sampleRate === 0 ||
      shortExponent < 6 || longExponent < shortExponent || longExponent > 13 || (packet[29]! & 1) === 0) {
...
export function parseAokanaOggVorbisLinks(bytes: Uint8Array): readonly AokanaOggVorbisLink[] {
  ...
  let current: PendingLink | null = null, cursor = 0;
  while (cursor + 27 <= bytes.length) {
    if (bytes[cursor] !== 79 || bytes[cursor + 1] !== 103 || bytes[cursor + 2] !== 103 ||
        bytes[cursor + 3] !== 83 || bytes[cursor + 4] !== 0) {cursor++; continue;}
    const segments = bytes[cursor + 26]!, headerSize = 27 + segments;
    ...
    const page = bytes.subarray(cursor, cursor + size), view = new DataView(page.buffer, page.byteOffset, page.byteLength);
    if (aokanaOggChecksum(page) !== view.getUint32(22, true)) {cursor++; continue;}
    cursor += size;
    const serial = view.getUint32(14, true), flags = page[5]!;
    if (current === null || (current.ended && (flags & 2) !== 0)) {
      current = {serial, pages: [], packet: [], identification: null, ended: false, finalGranule: null};
      links.push(current);
    }
    ...
    if (current.identification === null) {
      ...
      for (let index = 0; index < segments; index++) {
        ...
        read += length;
        if (length !== 255) {current.identification = identification(Uint8Array.from(current.packet)); break;}
      }
    }
  }
  if (links.length === 0) throw new AokanaWaveBoxError(0x10000000, 'Aokana source has no valid Ogg page');
  return links.map((link) => {
    if (link.identification === null) throw new AokanaWaveBoxError(0x10000000, 'Aokana source has an incomplete Vorbis identification packet');
    const bytes = new Uint8Array(link.pages.reduce((sum, page) => sum + page.length, 0));
    let at = 0;
    for (const page of link.pages) {bytes.set(page, at); at += page.length;}
...
export async function decodeAokanaOggVorbis(
  ...
  for (const link of parseAokanaOggVorbisLinks(bytes)) {
    const context = new Context({numberOfChannels: link.channels, length: 1, sampleRate: link.sampleRate});
    let decoded: AudioBuffer;
    try {decoded = await context.decodeAudioData(link.bytes.slice().buffer);}
    catch (error) {
    ...
    const order = browserPlaneOrder[link.channels] ?? Array.from({length: link.channels}, (_, channel) => channel);
