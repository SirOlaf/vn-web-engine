...
export class AokanaFileStorage implements AokanaLiveAudioStorage {
  private readonly file: AokanaNativeFile;
  private opened = false;
  private disposed = false;
  private positionValue: number | undefined;
  private flagsValue: number | undefined;
  constructor(files: AokanaProgramFiles) {
    this.file = new AokanaNativeFile(files);
  }
  private check(): void {
    if (this.disposed) throw new Error('Aokana file storage accesses released owner');
  }
  get flags(): number {
    this.check();
    if (this.flagsValue === undefined) throw new Error('Aokana file storage reads unwritten flags');
    return this.flagsValue;
  }
  get position(): number {
    this.check();
    if (this.positionValue === undefined)
      throw new Error('Aokana file storage reads unwritten cursor');
