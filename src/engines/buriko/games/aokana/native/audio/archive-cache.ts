...
import {AokanaAudioArchiveTree} from './archive-cache-tree.js';
import {AokanaDcArchive, aokanaAudioWideLower} from './dc-archive.js';

import {audioPathAbsolute, audioPathTerminated, appendAudioWidePath} from './wide-path.js';

export interface AokanaAudioArchiveResult {
  readonly status: number;
  readonly archive?: AokanaDcArchive;
}

/** Actual27CCD0 cache; caller owns the same27CC70 section across the full raw operation. */
export class AokanaAudioArchiveCache {
  ...
  private readonly tree = new AokanaAudioArchiveTree<AokanaDcArchive>();
  private disposed = false;
  constructor(channels: AokanaAudioChannels, files: AokanaProgramFiles) {
    this.channels = channels;
    this.files = files;
  }
  private requireAdmission(actor: object): void {
    if (this.disposed) throw new Error('Aokana audio archive cache accesses released owner');
    if (this.channels.section.owner !== actor)
      throw new Error('Aokana audio archive cache requires actual shared27CC70 admission');
  }
  ...
  async find(path: string, member: string, actor: object): Promise<AokanaAudioArchiveResult> {
    this.requireAdmission(actor);
    path = audioPathTerminated(path);
    let archive = this.fastLookup !== 0 ? this.tree.find(path) : undefined;
    if (archive === undefined) {
      const relative = !audioPathAbsolute(path);
      const key = aokanaAudioWideLower(relative ? (await this.readRoot(actor)) + path : path);
      archive = this.tree.find(key);
      if (archive === undefined) {
        let candidate: AokanaDcArchive | null = new AokanaDcArchive(this.files);
        try {
