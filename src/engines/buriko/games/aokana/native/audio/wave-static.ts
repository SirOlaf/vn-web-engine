...
import {createAokanaCustomWaveBoxDecoder} from './wavebox-codecs.js';
...
export function fadeInAokanaStaticPcm(
  pcm: AokanaPcmStorage, frameCount: number, sampleRate: number, channels: number,
  bits: 16 | 24, milliseconds: number,
): number {
  milliseconds >>>= 0; sampleRate >>>= 0; channels >>>= 0;
  const frames = (Math.floor((Math.imul(milliseconds % 1000, sampleRate) >>> 0) / 1000) +
    Math.imul(sampleRate, Math.floor(milliseconds / 1000))) >>> 0;
  ...
  const width = bits >>> 3, data = new DataView(pcm.bytes.buffer, pcm.bytes.byteOffset, pcm.bytes.byteLength);
  let offset = 0;
  for (let frame = 0; frame < frames; frame++) for (let channel = 0; channel < channels; channel++, offset += width) {
    requirePcm(pcm, offset, width);
    const sample = bits === 16 ? data.getInt16(offset, true) :
      (pcm.bytes[offset]! | (pcm.bytes[offset + 1]! << 8) | (pcm.bytes[offset + 2]! << 16)) << 8 >> 8;
    let value = f32(f32(Math.imul(sample, frame)) * reciprocal);
    const floor = f32(Math.floor(value)), fraction = f32(value - floor), epsilon = 2 ** -14;
    ...
    const integer = !Number.isFinite(rounded) || rounded < -2147483648 || rounded > 2147483647 ? -2147483648 : rounded;
    for (let byte = 0; byte < width; byte++) pcm.bytes[offset + byte] = (integer >> (byte * 8)) & 255;
  }
  return 0;
}
...
export class AokanaWaveStatic {
  ...
  constructor(readonly header: AokanaWaveBoxHeader, readonly outputBits: 16 | 24, decoded: Uint8Array) {
    ...
    const capacity = header.codec === 0 ? (header.field08 << 2) >>> 0 : header.codec === 1 ? header.field08 :
      Math.imul(this.bytesPerFrame, header.sourceFrameCount) >>> 0;
    const bytes = new Uint8Array(capacity), initialized = new Uint8Array(capacity);
    const written = Math.min(capacity, decoded.length);
    bytes.set(decoded.subarray(0, written)); initialized.fill(1, 0, written);
  ...
  get loopEnabled(): number {return this.header.loopEnabled;}
  get visibleLoopCount(): number {return 0;}
  ...
  reset(): void {this.assertLive(); this.consumerPosition = 0; this.seek(0);}
  private restartLoop(): void {
    ...
    const byteOffset = this.header.codec === 1 ?
      (Math.imul(this.header.sourceFrameCount, this.header.loopStartFrame) >>> 0) >>> 2 :
      Math.imul(this.header.codec === 2 ? this.header.sourceFrameCount : this.bytesPerFrame, this.header.loopStartFrame);
    this.seek(byteOffset);
  }
  fadeIn(milliseconds: number): number {
    this.assertLive();
    return fadeInAokanaStaticPcm(this.pcm, this.sourceFrameCount, this.sampleRate, this.channels, this.outputBits, milliseconds);
  }
  /** 11c060 preserves the CMemoryStorage EOF sentinel in its unsigned frame-count arithmetic. */
  readInto(destination: Uint8Array, offset: number, count: number): number {
    this.assertLive(); count >>>= 0;
    let consumed = 0;
    while (consumed < count && this.consumerPosition < this.sourceFrameCount) {
      const requested = Math.min(count - consumed, (this.sourceFrameCount - this.consumerPosition) >>> 0);
      ...
      if (this.sourcePosition < this.pcm.bytes.length) {
        ...
        if (returned !== 0) destination.set(this.pcm.bytes.subarray(this.sourcePosition, this.sourcePosition + returned), at);
        this.sourcePosition += returned;
      }
      if (this.bytesPerFrame === 0) throw new RangeError('Aokana static WaveBox divides by zero frame size');
...
export async function createAokanaWaveStatic(
  bytes: Uint8Array, options: {gain: number; prefer24Bit: boolean},
  Context?: typeof OfflineAudioContext,
): Promise<AokanaWaveStatic> {
  const header = parseAokanaWaveBoxHeader(bytes);
  const decoder: AokanaWaveDecoder = header.codec === 3 ?
    await createAokanaWaveBoxOggDecoder(bytes, options, Context) : createAokanaCustomWaveBoxDecoder(bytes, options);
  const decoded = decoder.readFrameBytes(header.sourceFrameCount);
