...
export interface AokanaSurfaceTextArguments {
  surface: number;
  x: number;
  y: number;
...
export async function drawAokanaSurfaceText(
  surfaces: AokanaSurfaces,
  arguments_: AokanaSurfaceTextArguments,
  ...
  if (font.result !== 0)
    throw new Error('Aokana surface text returns an unwritten native font status');
  let metric: number | undefined;
  const output = {
    get value(): number {
      if (metric === undefined) throw new Error('Aokana surface text reads an unwritten metric');
      return metric;
    },
  ...
  if (metric === undefined)
    throw new Error('Aokana surface text consumes an unpublished native metric');
