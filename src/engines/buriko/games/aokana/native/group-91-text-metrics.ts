...
import {measureAokanaRegisteredText} from './text-metrics.js';
  ...
  return [
    {
      ...
      execute: (context) => {
        ...
        if (source === null) throw new Error('Aokana annotation collection reads null text');
    ...
    },
    {
      ...
      execute: async (context): Promise<0> => {
        const proportional = pop32(context.thread),
          ...
          output = address(context);
        const status = await measureAokanaRegisteredText(
          state,
          output,
          source,
          font,
