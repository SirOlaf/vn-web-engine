...
import type {AokanaNativeTouchSample, AokanaNativeTouchWindow} from './touch-input.js';
...
export class AokanaBrowserTouchWindow implements AokanaNativeTouchWindow {
  registered = false;
  registrationEpoch = 0;
...
export class AokanaMainTouchInput {
  ...
  constructor(
    ...
    readonly touch: AokanaNativeTouch,
    readonly touchWindow: AokanaBrowserTouchWindow,
  ) {
    if (
      ...
      throw new Error('Aokana main touch requires the shared live main-window owners');
  ...
  private sample(
    ...
    sample: AokanaNativeTouchSample;
    clientX: number;
    clientY: number;
    screenX: number;
    screenY: number;
