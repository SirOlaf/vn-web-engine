...
export function createGroup90DisplayBase(
  ...
  const invalidBitmap = (context: AokanaBpOpcodeContext, index: number): Promise<never> =>
    fatal(context, `無効なビットマップ番号 [ ${index | 0} ] が指定されました`);
  const invalidPriority = (context: AokanaBpOpcodeContext, priority: number): Promise<never> =>
    fatal(context, `無効なプライオリティ [ ${priority | 0} ] が指定されました`);
  const invalidEffect = (context: AokanaBpOpcodeContext, value: number): Promise<never> =>
    fatal(
      context,
  ...
  add(0x0e, 0x1400ddc40, 'SetFontCacheCapacity', (context) => {
    ...
    switch (result) {
      ...
      default:
        throw new Error('Aokana font cache returned an unknown native configuration status');
    }
  });
