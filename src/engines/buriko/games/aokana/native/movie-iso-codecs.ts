import {AokanaIsoSampleError, type AokanaIsoDescription} from './movie-iso-samples.js';
...
function requireBytes(bytes: Uint8Array, at: number, count: number, end = bytes.length): void {
  if (
    ...
    throw new AokanaIsoSampleError('ISO codec configuration exceeds its encoded entry');
}
function boxes(bytes: Uint8Array, start: number, end: number): Box[] {
  ...
  for (let at = start; at < end;) {
    ...
    if (size < BigInt(header) || size > BigInt(end - at))
      throw new AokanaIsoSampleError('Invalid ISO codec child-box size');
...
export function aokanaIsoAvcConfiguration(entry: AokanaIsoDescription): VideoDecoderConfig {
  if (entry.type !== 'avc1' && entry.type !== 'avc3')
    throw new AokanaIsoSampleError('Sample entry is not AVC');
  ...
  if (config === undefined)
    throw new AokanaIsoSampleError('AVC sample entry has no configuration record');
  requireBytes(bytes, config.body, 7, config.end);
  if (bytes[config.body] !== 1) throw new AokanaIsoSampleError('Unknown AVC configuration version');
  ...
  if (aspect !== undefined) {
    ...
    if (horizontal === 0 || vertical === 0)
      throw new AokanaIsoSampleError('AVC sample entry has a zero pixel-aspect dimension');
    ...
    } else throw new AokanaIsoSampleError('AVC display aspect cannot be represented by WebCodecs');
  }
  return output;
}
...
function descriptors(bytes: Uint8Array, start: number, end: number): Descriptor[] {
  ...
  for (let at = start; at < end;) {
    ...
    if (!terminated) throw new AokanaIsoSampleError('ISO descriptor length exceeds four bytes');
...
export interface AokanaAacDecoderConfiguration {
...
export function aokanaIsoAacConfiguration(
  entry: AokanaIsoDescription,
): AokanaAacDecoderConfiguration {
  if (entry.type !== 'mp4a') throw new AokanaIsoSampleError('Sample entry is not MPEG-4 audio');
  const bytes = entry.bytes,
    base = entry.headerSize,
    view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  requireBytes(bytes, base, 28);
  const version = view.getUint16(base + 8);
  if (version > 2) throw new AokanaIsoSampleError('Unknown QuickTime sound sample-entry version');
  ...
  if (esds === undefined)
    throw new AokanaIsoSampleError('MPEG-4 audio has no elementary-stream descriptors');
  requireBytes(bytes, esds.body, 4, esds.end);
  if (bytes[esds.body] !== 0)
    throw new AokanaIsoSampleError('Unknown elementary-stream descriptor version');
  const stream = descriptors(bytes, esds.body + 4, esds.end).find(
    (descriptor) => descriptor.tag === 3,
  );
  if (stream === undefined) throw new AokanaIsoSampleError('MPEG-4 audio has no ES descriptor');
  ...
  if (decoder === undefined)
    throw new AokanaIsoSampleError('MPEG-4 audio has no DecoderConfig descriptor');
  ...
  if (specific === undefined)
    throw new AokanaIsoSampleError('MPEG-4 audio has no AudioSpecificConfig');
