...
export type AokanaGridAbilityMap = {
  request: Uint8Array;
  ordinary: Uint8Array;
...
export function sameGridAbilityRequest(
  request: AokanaBpPointer | null,
  bytes: Uint8Array,
): boolean {
  if (request === null) throw new Error('Aokana logical-grid null ability request');
...
export class AokanaLogicalGridAgent {
  x = -1;
  y = -1;
  direction = 0;
  height = 3;
  ...
  costPlane = 0;
  occupancyHeight = 1;
  path: AokanaLogicalGridPath | null = null;
  readonly abilityMaps: AokanaGridAbilityMap[] = [];
...
export class AokanaLogicalGridManager {
  private disposed = false;
  width: number | undefined;
  height: number | undefined;
  cells: Uint8Array | null = null;
  corners: Uint8Array | null = null;
  readonly agents: AokanaLogicalGridAgent[] = [];
  ...
  dimensions(): {width: number; height: number} {
    if (this.width === undefined || this.height === undefined)
      throw new Error('Aokana logical-grid uninitialized dimensions');
  ...
  cell(x: number, y: number, cells = this.cells): DataView {
    if (cells === null) throw new Error('Aokana logical-grid null cells');
    return pointerView({bytes: cells, offset: this.index(x, y) * 16}, 16);
  }

  agent(id: number): AokanaLogicalGridAgent | undefined {
    return this.agents.find((agent) => agent.id === id >>> 0);
  }
  agentAt(x: number, y: number): AokanaLogicalGridAgent | undefined {
  ...
  clone(): AokanaLogicalGridManager | null {
    if (this.disposed)
      throw new Error('Aokana logical-grid clone dereferences a destroyed manager');
    if (this.cells === null) return null;
    const result = new AokanaLogicalGridManager(this.verticalDivisor);
    const {width, height} = this.dimensions();
    ...
    for (const agent of this.agents) {
      const copy = new AokanaLogicalGridAgent(agent.id);
      copy.x = agent.x;
      copy.y = agent.y;
  ...
  copyPosition(output: AokanaBpPointer | null, id: number): number {
  ...
  copyDirection(output: AokanaBpPointer | null, id: number): number {
    const agent = this.agent(id);
    if (agent === undefined) return 0x80000003;
  ...
  setCells(width: number, height: number, input: AokanaBpPointer | null): number {
    width |= 0;
    height |= 0;
  ...
  addCostPlane(output: AokanaBpPointer | null, input: AokanaBpPointer | null): number {
  ...
  createAgent(output: AokanaBpPointer | null): number {
    if (this.cells === null) return 0x80000002;
    const agent = new AokanaLogicalGridAgent(this.nextAgent);
  ...
  setMasks(id: number, input: AokanaBpPointer | null): number {
    const agent = this.agent(id);
    if (agent === undefined) return 0x80000003;
    if (input === null) throw new Error('Aokana logical-grid null agent mask input');
  ...
  initializePath(path: AokanaLogicalGridPath, planeId: number, stampAgents: boolean): number {
    if (this.cells === null) return 0x80000002;
    const {width, height} = this.dimensions(),
  ...
  private stampOccupancy(cells: Uint8Array, agent: AokanaLogicalGridAgent): void {
  ...
  search(
    ...
    if (agent === undefined) return 0x80000003;
    agent.path?.dispose();
    agent.path = new AokanaLogicalGridPath(this.verticalDivisor);
  ...
  copyResults(output: AokanaBpPointer | null, id: number): number {
    const agent = this.agent(id);
    if (agent === undefined) return 0x80000003;
    if (agent.path === null) return 0x80000006;
  ...
  copyRouteCoordinates(
    ...
    if (count === null) throw new Error('Aokana logical-grid null route count');
  ...
  directionOrder(output: AokanaBpPointer | null, direction: number, zeroBased: boolean): number {
  ...
  facingAngle(output: AokanaBpPointer | null, x: number, y: number, id: number): number {
    const agent = this.agent(id);
    if (agent === undefined) return 0x80000003;
    let angle = 0,
      status = 0;
...
export class AokanaLogicalGridManagers {
  private nextId = 0;
  private readonly entries: {id: number; manager: AokanaLogicalGridManager}[] = [];
  get(id: number): AokanaLogicalGridManager | undefined {
    return this.entries.find((entry) => entry.id === id >>> 0)?.manager;
  }
  create(output: AokanaBpPointer | null, kind: number, divisor: number): number {
    if ((kind | 0) !== 0) return 0;
    const id = this.nextId,
      manager = new AokanaLogicalGridManager(divisor | 0);
    this.nextId = (id + 1) >>> 0;
