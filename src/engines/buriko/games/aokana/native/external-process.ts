...
export type AokanaExternalProcessHandle = WindowsProcessHandle;
export type AokanaExternalProcessInformation = WindowsProcessInformation;
/** The native STARTUPINFOW is zeroed before these three nonzero fields are written. */
export interface AokanaExternalProcessStartup extends WindowsProcessStartup {
  readonly cb: 0x68;
...
export interface AokanaExternalProcessRequest extends WindowsProcessRequest {
...
export type AokanaExternalProcessHost = WindowsProcessHost;
export type AokanaShellExecuteHost = WindowsShellExecuteHost;
...
export interface AokanaExternalProcessWindowHost {
  ...
  setShowState(value: number): AokanaHostResult<void>;
  pumpMessages(): AokanaHostResult<number>;
}
...
export interface AokanaExitLaunchProcessPlan {
  readonly host: WindowsProcessHost;
  readonly request: AokanaExternalProcessRequest;
...
export class AokanaExternalProcesses {
  ...
  constructor(
    ...
    readonly shellHost: AokanaShellExecuteHost | null = null,
  ) {}
  ...
  prepareExitLaunch(request: AokanaExitLaunchRequest): AokanaExitLaunchProcessPlan {
    const flags = {
      childShow: 1,
      toggleMainWindow: 0,
      waitForCompletion: 0,
    ...
    return {
      ...
      failureDialog: request.failureMessage === null ? null : {
        title: text.decodeAuto({bytes: terminatedNativeBytes(titleBytes), offset: 0}),
        text: text.decodeMixed({
          bytes: terminatedNativeBytes(request.failureMessage), offset: 0,
        }).replace(/\\\\n/g, '\n'),
      },
      flags,
    };
  ...
  run(call: AokanaExternalProcessCall): Promise<0 | 1> {
    if (this.closed) throw new Error('Aokana external-process admission is closed');
  ...
  openShellPath(pathBytes: Uint8Array): Promise<0 | 1> {
    if (this.closed) throw new Error('Aokana external-process admission is closed');
    if (this.shellHost === null) throw new Error('Aokana shell-execute host is not selected');
    const path = this.resources.files.text.decodeAuto({bytes: pathBytes, offset: 0});
    if (path.length >= 784)
      throw new RangeError('Aokana ShellExecute path exceeds native wide local');
    return this.track(this.openShellPathAccepted(path, this.shellHost));
  }

  private async openShellPathAccepted(path: string, shell: AokanaShellExecuteHost): Promise<0 | 1> {
    ...
    try {
      ...
      if (token !== null) {
        ...
        if (typeof result !== 'boolean')
          throw new TypeError('Aokana shell impersonation must complete synchronously');
      ...
      if (typeof result !== 'number' && typeof result !== 'bigint')
        throw new TypeError('Aokana ShellExecuteW must complete synchronously');
      ...
      try {
        if (impersonated) {
          ...
          if (typeof result !== 'boolean')
            throw new TypeError('Aokana shell revert must complete synchronously');
        }
      } finally {
  ...
  private async runAccepted(call: AokanaExternalProcessCall): Promise<0 | 1> {
