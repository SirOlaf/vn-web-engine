...
export function readAokanaDisplayAdapter(
  display: AokanaNativeDisplayState,
  description: AokanaBpPointer,
  version: AokanaBpPointer,
): void {
  const bytes = normalizeAokanaAsciiSpaces({bytes: display.adapterIdentifier, offset: 0x200});
...
export async function setAokanaRequestedClientSize(
