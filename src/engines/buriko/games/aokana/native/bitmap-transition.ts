  ...
  for (let y = 0; y < bounds.height >>> 0; y++) {
    for (let x = 0; x < bounds.width >>> 0; x++) {
      ...
      for (let shift = 0; shift < 24; shift += 8) {
        ...
        const delta =
          small && extra === 0
            ? aokanaSignedProduct16(difference, coefficient) >> 7
            : Math.floor(((difference << 4) * coefficient) / 65536);
        result |= saturateAokanaByte(previous + delta) << shift;
      }
...
function transitionAokanaBitmapPixels(
  ...
  const area = aokanaBitmapRectangle(input),
    outputArea = aokanaBitmapRectangle(output);
  translateAokanaBitmapRectangle(outputArea, -x, -y);
  if (!intersectAokanaBitmapRectangle(area, outputArea)) return 4;
  if (!maskAtDestination) {
    cropAokanaBitmap(input, area);
    cropAokanaBitmap(matte, area);
    translateAokanaBitmapRectangle(area, x, y);
  } else {
    const maskArea = aokanaBitmapRectangle(matte);
    translateAokanaBitmapRectangle(maskArea, -x, -y);
    if (!intersectAokanaBitmapRectangle(area, maskArea)) return 4;
    cropAokanaBitmap(input, area);
    translateAokanaBitmapRectangle(area, x, y);
    intersectAokanaBitmapRectangle(area, aokanaBitmapRectangle(matte));
    cropAokanaBitmap(matte, area);
  }
