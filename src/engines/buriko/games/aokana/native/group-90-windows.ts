...
export function createGroup90Windows(
  ...
  const manager = state.manager,
    slots: AokanaNativeSlotDefinition[] = [];
  const add = (
    secondary: number,
  ...
  const activeDamage = (object: AokanaWindowDisplayObject): void => {
  ...
  add(0x83, 0x1400d91c0, 'CaptureWindowBitmap', (h) => {
    ...
    if (
      ...
      if (destination === null)
        throw new Error('Aokana window capture has no allocated surface descriptor');
  ...
  add(0x89, 0x1400d8d90, 'ReadWindowTextRectangle', (h) => {
    const handle = pop32(h.thread),
      ...
      object = find(handle);
    if (object === null) return missing(h);
    if (output === null) throw new Error('Aokana window text rectangle writes through null');
