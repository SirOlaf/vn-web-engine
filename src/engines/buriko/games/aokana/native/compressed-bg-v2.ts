...
import {type AokanaBfSurface, decodeAokanaBfFrame} from './bf-frame.js';
...
export async function decodeAokanaCompressedBgV2(
  ...
  if (width === 0 || height === 0 || copiedChannels > 4) {
    throw new AokanaUndefinedResourceRead(
      'Aokana CompressedBG geometry would access undefined native storage',
    );
  }
  ...
  if (sum !== input[44] || xor !== input[45])
    throw new AokanaResourceCodecException(
      3,
      'Aokana CompressedBG version2 table checksum mismatch',
    );
  ...
  const pixels = decodeAokanaBfFrame(
    frame,
    paddedWidth,
    paddedHeight,
    depth,
