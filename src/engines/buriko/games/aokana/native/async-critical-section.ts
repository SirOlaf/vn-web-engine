...
export class AokanaAsyncCriticalSection {
  private live = false;
  ...
  initialize(): void {
    if (this.live) throw new Error('Aokana async section is already initialized');
    this.live = true;
  }
  async enter(actor: object): Promise<void> {
    if (!this.live) throw new Error('Aokana async section is not initialized');
  ...
  leave(actor: object): void {
    if (!this.live || this.ownerValue !== actor || this.depthValue === 0)
      throw new Error('Aokana async section is not owned by the captured actor');
  ...
  dispose(): void {
    if (this.ownerValue !== null || this.waiters.length !== 0)
      throw new Error('Aokana async section still has an active owner');
    this.live = false;
  }
}
