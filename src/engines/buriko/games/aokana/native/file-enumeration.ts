...
export function aokanaDosMatch(
  pattern: string,
  name: string,
...
export class AokanaFileEnumeration {
  ...
  async enumerate(
    ...
    const append = (name: string): boolean => {
      ...
      if (bytes.length > 784)
        throw new RangeError('Aokana enumeration name exceeds native byte stack buffer');
      if (output !== null) {
    ...
    const visit = async (native: Uint8Array, prefix: string, limit: number): Promise<boolean> => {
      ...
      if (native.length > 784)
        throw new RangeError('Aokana recursive pattern exceeds native byte stack buffer');
      ...
      for (const entry of await this.find(this.files.text.decodeAuto({bytes: search, offset: 0}))) {
        ...
        if (slash < 0)
          throw new Error('Aokana recursive child path dereferences a null native suffix');
        ...
        if (child.length > 784 || head.length + child.length > 784)
          throw new RangeError('Aokana recursive child exceeds native byte stack buffer');
        ...
        if (this.files.text.encodeWide(nextPrefix, 1).length > 784)
          throw new RangeError('Aokana recursive prefix exceeds native byte stack buffer');
        if (
