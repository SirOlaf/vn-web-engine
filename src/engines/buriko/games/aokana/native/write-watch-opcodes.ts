...
import type {AokanaBpOpcodeHandler} from './types.js';

export const writeWatchOpcodes: Readonly<Record<number, AokanaBpOpcodeHandler>> = {
  ...
  0x75: ({thread, memory, diagnostics}) => {
    ...
    if (namePointer === null) throw new Error('Aokana write-watch name is null');
    const end = namePointer.bytes.indexOf(0, namePointer.offset);
    if (end < 0) throw new RangeError('Unterminated Aokana write-watch name');
    push32(
      thread,
      Number(
