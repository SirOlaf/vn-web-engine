import {AokanaDisplayKnob} from './display-knob.js';
import {AokanaDisplayManager, type AokanaDisplayCreateResult} from './display-manager.js';
import {AokanaNativeInput} from './input.js';
import {AokanaNativeNotifications} from './notification-queue.js';

export interface AokanaKnobPointerReceiver {
  readonly id: number;
  readonly object: AokanaDisplayKnob;
}

interface PointerReceiverNode extends AokanaKnobPointerReceiver {
...
export class AokanaKnobDisplays {
  ...
  create(targetHandle: number): AokanaDisplayCreateResult<1 | 2 | 3> {
    const result = this.manager.createKnob(
      targetHandle,
      (creationOrder, target) =>
        new AokanaDisplayKnob(this.manager.environment, creationOrder, target),
    );
  ...
  findPointerReceiver(): AokanaKnobPointerReceiver | null {
  ...
  latchRightClick(receiver: AokanaKnobPointerReceiver): void {
  ...
  beginPointerInteraction(receiver: AokanaKnobPointerReceiver | null, x: number, y: number): void {
  ...
  private find(handle: number): AokanaDisplayKnob | null {
    const object = this.manager.find('knob', handle);
    if (object === null) return null;
    if (!(object instanceof AokanaDisplayKnob))
      throw new Error('Aokana Knob pool contains a different native display class');
    return object;
  }
  ...
  private registerPointer(handle: number): void {
    const object = this.find(handle);
    if (object === null)
      throw new Error('Aokana Knob pointer registration has no current pool object');
  ...
  private pointerNode(receiver: AokanaKnobPointerReceiver): PointerReceiverNode | null {
    for (let node = this.pointerHead; node !== null; node = node.next)
