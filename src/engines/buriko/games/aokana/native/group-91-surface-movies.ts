...
import type {AokanaBrowserSurfaceMovieFactory} from './movie-browser-surface.js';
import type {AokanaMovieFramePosition} from './movie-frame-position.js';
...
export function createGroup91SurfaceMovies(
  factory: AokanaBrowserSurfaceMovieFactory,
  surfaces: AokanaSurfaces,
  movies: AokanaMovieRegistry,
  framePosition: AokanaMovieFramePosition,
  ...
  if (
    ...
    throw new Error('Aokana surface movie callbacks require one graph and registry');
  ...
  return [
    {
      ...
      execute: async (h): Promise<0> => {
        const volume = pop32(h.thread),
          ...
          name = pointer(h),
          slot = pop32(h.thread);
        if (name === null) throw new Error('Aokana surface movie consumes a null filename');
