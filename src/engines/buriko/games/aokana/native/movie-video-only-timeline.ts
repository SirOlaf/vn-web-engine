...
export class AokanaMovieVideoOnlyTimeline {
  ...
  constructor(
    ...
    if (
      !(tracks instanceof AokanaMovieSourceTracks) ||
      tracks.audio !== null ||
      !(clock instanceof AokanaMovieReferenceClock)
    )
      throw new TypeError(
    ...
    this.stopTime = aokanaIsoReferenceTime(this.stop);
  }
  ...
  get position(): AokanaIsoRational {
    this.check();
  ...
  get currentTime(): bigint {
    this.check();
    return this.start === null
      ? aokanaIsoReferenceTime(this.pausedPosition)
      : this.raw() - this.start;
  }
  ...
  run(): bigint {
    ...
    this.start = this.raw() - aokanaIsoReferenceTime(this.pausedPosition);
    return this.start;
  }
  ...
  pause(): bigint {
    ...
    return aokanaIsoReferenceTime(this.pausedPosition);
  }

  /** Future transport must quiesce its old Receive epoch before calling seek. */
  seek(position: AokanaIsoRational): bigint {
    this.check();
    ...
    this.seekGeneration++;
    return aokanaIsoReferenceTime(selected);
  }
