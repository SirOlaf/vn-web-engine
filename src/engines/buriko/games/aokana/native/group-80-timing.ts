...
import {AokanaCrtRandom, AokanaVmFrameHistory, nativeNanoseconds} from './system-timing.js';
import type {AokanaNativePerformanceCounter} from './system-timing.js';

export function createGroup80Timing(
  random: AokanaCrtRandom,
  history: AokanaVmFrameHistory,
  ...
  return [
    ...
    },
    {
      ...
      execute: (h) => {
        ...
        if (destination === null) throw new Error('Aokana native timer null destination');
...
export function createGroup80LocalTime(readLocalTime: () => Date): AokanaNativeSlotDefinition[] {
  return [
    {
      primary: 0x80,
      secondary: 0x0c,
      ...
      execute: (h) => {
        ...
        if (!Number.isFinite(value.getTime()))
          throw new Error('Aokana native local-time host returned an invalid date');
        if (destination === null) throw new Error('Aokana native local-time null destination');
        const words = [
          value.getFullYear(),
          value.getMonth() + 1,
          value.getDay(),
