...
function requirePointer(value: AokanaBpPointer | null): AokanaBpPointer {
  if (value === null) throw new Error('Aokana ._bp null memory dereference');
  return value;
}

function cString(value: AokanaBpPointer | null): Uint8Array | null {
  if (value === null) return null;
  const end = value.bytes.indexOf(0, value.offset);
  if (value.offset < 0 || end < 0) throw new Error('Aokana ._bp unterminated byte string');
...
export function createGroup7f(text: AokanaNativeText): AokanaNativeSlotDefinition[] {
