...
import type {AokanaInputCaptureView, AokanaNativeInput} from './input.js';
...
const sentinel: AokanaInputCaptureView = {token: 0xffffffff, object: null};
...
export function formatAokanaInputCaptures(input: AokanaNativeInput): Uint8Array {
  ...
  while (consumed < count) {
    if (key === undefined || pointer === undefined)
      throw new Error('Aokana capture diagnostic dereferences an absent initial list head');
    ...
    if (pointerToken < key.token >>> 0) {
      ...
      if (pointer.object === null) append(hex(pointerToken));
        ...
        if (activeObject === null)
          throw new Error('Aokana capture diagnostic dereferences a missing active-state object');
        const active = activeObject.inputActive();
        const categoryObject = pointer.object;
        if (!(categoryObject instanceof AokanaDisplayObject))
          throw new Error('Aokana capture diagnostic requires the native display category owner');
        const label = labels[categoryObject.category >>> 0];
        if (label === undefined)
          throw new Error('Aokana capture diagnostic indexes outside its native category table');
  ...
  return [
    {
      ...
      execute: async (): Promise<0> => {
        const list = {bytes: formatAokanaInputCaptures(input), offset: 0};
