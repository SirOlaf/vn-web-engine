...
import type {AokanaBitmapCacheServices} from './bitmap-cache-services.js';
import {aokanaBitmapMessage} from './bitmap-load-process.js';
...
export function createGroup90BitmapCacheServices(
  service: AokanaBitmapCacheServices,
): AokanaNativeSlotDefinition[] {
  const resources = service.loading.resources.resources,
    text = resources.files.text,
    pointer = (h: AokanaBpOpcodeContext) => h.memory.resolve(h.thread, pop32(h.thread)),
    invalid = (h: AokanaBpOpcodeContext, index: number) =>
  ...
  return [
    {
      ...
      execute: async (h): Promise<0> => {
        ...
        if (namePointer === null) throw new Error('Aokana header resource consumes null name');
        ...
        if (result === 0x8000000d || result === 0xffffffff) {
          const message = aokanaBitmapMessage(
            text,
            result === 0x8000000d
          ...
          if (message.length > 256)
            throw new RangeError('Aokana header bitmap diagnostic exceeds native scratch');
    ...
    },
    {
      ...
      execute: (h) => {
        ...
        if (index >>> 0 >= 0x4000) return invalid(h, index);
        const archive = archivePointer === null ? null : textBytes(archivePointer).slice();
        if (namePointer === null) throw new Error('Aokana cached bitmap lookup consumes null name');
        push32(
          h.thread,
          Number(
