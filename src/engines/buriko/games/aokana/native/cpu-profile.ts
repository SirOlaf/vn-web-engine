...
import {normalizeAokanaAsciiSpaces} from './byte-string-spaces.js';

/** CPUID register order is EAX, EBX, ECX, EDX, as stored by 0c65c0. */
export type AokanaCpuRegisters = readonly [number, number, number, number];
export interface AokanaLogicalProcessorInformation {
  relationship: number;
 ...
 */
export interface AokanaCpuHost {
  cpuid(leaf: number, subleaf: number): AokanaCpuRegisters;
  readTimestampCounter(): bigint;
  setCurrentThreadAffinity(mask: bigint): bigint;
  logicalProcessorCount(): number;
  logicalProcessorInformation(): Iterable<AokanaLogicalProcessorInformation>;
}
...
export class AokanaCpuProfile {
  ...
  query(leaf: number, subleaf: number = 0): AokanaCpuRegisters | null {
    leaf >>>= 0;
    subleaf >>>= 0;
  ...
  read(index: number): number {
    index >>>= 0;
    if (index >= 10)
      throw new RangeError('Aokana CPU record index is outside the indexed-read contract');
  ...
  brand(): Uint8Array | null {
    ...
    if (end < 0) throw new Error('Aokana CPU brand has no terminator in its native record');
    return normalizeAokanaAsciiSpaces({bytes, offset: 0});
  }
}
