...
export function createGroup91ObjectCoordinates(
  ...
  const coordinates =
    (apply: (handle: number, x: number, y: number, z: number) => boolean): AokanaBpOpcodeHandler =>
    (h) => {
  ...
  add(0x3d, 0x1400e1510, 'ReadObjectEffectivePosition', (h) => {
    ...
    if (output === null) throw new Error('Aokana effective-position query writes through null');
    const point = AokanaDisplayObject.prototype.effectivePosition.call(object),
      view = pointerView(output);
    view.setInt32(0, point.x, true);
    view.setInt32(4, point.y, true);
    return 0;
  });
  const childStatus = (h: AokanaBpOpcodeContext, status: number): 0 | Promise<never> => {
