...
function popPointer(context: AokanaBpOpcodeContext): AokanaBpPointer | null {
  return context.memory.resolve(context.thread, pop32(context.thread));
}
function name(pointer: AokanaBpPointer | null): Uint8Array {
  if (pointer === null) throw new Error('Aokana resource service dereferences a null name');
