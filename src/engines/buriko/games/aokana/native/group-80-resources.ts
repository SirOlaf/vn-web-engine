...
export function aokanaProgramLoadDiagnostic(
  ...
  return [
      ...
      execute: async (context): Promise<0> => {
        ...
        if (bytes === null)
          return resources.errors.threadFatal(
            thread,
            context.diagnostics,
            aokanaProgramLoadDiagnostic(resources, 'missing', archive, resource),
          );
        ...
        if (base === AOKANA_BP_MODULE_NO_SPACE)
          return resources.errors.threadFatal(
            thread,
            context.diagnostics,
            aokanaProgramLoadDiagnostic(resources, 'space', archive, resource),
          );
        push32(thread, base);
        return 0;
      },
    },
    {
      ...
      execute: async (context): Promise<0> => {
        ...
        if (bytes === null) {
          const diagnostic = aokanaProgramLoadDiagnostic(resources, 'missing', archive, resource);
        ...
        if (attachModule(child, resource, bytes) === AOKANA_BP_MODULE_NO_SPACE) {
          ...
          return resources.errors.threadFatal(
            thread,
            context.diagnostics,
            aokanaProgramLoadDiagnostic(resources, 'thread-space', archive, resource),
          );
        }
