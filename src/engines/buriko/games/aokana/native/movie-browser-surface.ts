...
import {AokanaMovieMediaGraph, AokanaMovieRenderer} from './movie-renderer.js';
...
class AokanaBrowserSurfaceMovieGraph extends AokanaMovieMediaGraph {
  ...
  constructor(
    ...
    if (context === null) throw new Error('Aokana browser movie has no pixel readback context');
  ...
  startFrames(): void {
    ...
    const receive = (): void => {
      ...
      try {
        ...
        this.renderer.deliver({storage: new AokanaBitmapStorage(bgra, true), offset: 0});
      } catch {
...
export class AokanaBrowserSurfaceMovieFactory {
  ...
  constructor(
    ...
    if (!Number.isSafeInteger(maxDocumentBytes) || maxDocumentBytes < 1)
      throw new RangeError('Invalid Aokana surface movie document byte budget');
  }
  ...
  async create(
    ...
    repeat: number,
    volume: number,
    archive: AokanaBpPointer | null = null,
  ): Promise<number> {
    if (this.resetting || this.closed)
      throw new Error('Aokana surface movie source admission is closed');
  ...
  async beginReset(): Promise<() => void> {
    if (this.resetting || this.closed)
      throw new Error('Aokana surface movie factory reset admission is unavailable');
    this.resetting = true;
    this.generation++;
  ...
  private async createSelected(
    ...
    if (selectedSource === null)
      throw new Error('Aokana native movie source status is unwritten for a missing file');
    const {surfaces, movies} = this.graph;
    surfaces.release(slot);
    await movies.joinSlotRetirements(slot);
    this.check(generation, signal);
    const selected = await AokanaMovieSourceDocument.openSelected(
      selectedSource,
    ...
    const renderer = new AokanaMovieRenderer(surfaces, slot, image, this.graph.notifications);
    const id = movies.append(renderer);
    let media: AokanaBrowserSurfaceMovieGraph | null = null;
    ...
    try {
      ...
      try {
        ...
        media = new AokanaBrowserSurfaceMovieGraph(video, url, this.document, renderer);
        ...
        if (record === null) throw new Error('Aokana movie lost its selected surface record');
        record.movieId = id;
        return 0;
      } finally {
        if (media === null) {
