...
export class AokanaGuiMessagePump {
  constructor(
    ...
    if (dispatcher.messages !== messages || typeof translation.translate !== 'function')
      throw new Error('Aokana GUI pump requires its selected queue, dispatcher and character host');
  }
      ...
      if (message.message === 0x100 || message.message === 0x104) {
        ...
        for (const character of characters) {
          if (
            ...
            throw new Error('Aokana character host returned an invalid Win32 character');
          this.messages.post({
