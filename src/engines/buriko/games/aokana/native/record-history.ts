...
function bytesAt(pointer: AokanaBpPointer | null, offset: number, length: number): Uint8Array {
  if (pointer === null)
    throw new RangeError('Aokana record history consumed a null native pointer');
...
export function encodeAokanaRecord(input: Uint8Array): Uint8Array {
  const output = new Uint8Array((input.length + 11) >>> 0);
  let target = 0,
    source = 0;
  const write = (value: number): void => {
    if (target >= output.length)
      throw new RangeError('Aokana record encoder exceeds native scratch');
    output[target++] = value;
  };
...
export function decodeAokanaRecord(destination: AokanaBpPointer | null, input: Uint8Array): number {
  let source = 0,
    produced = 0;
...
export class AokanaRecordHistories {
  private identifier = 0;
  private histories: History[] = [];
  private find(id: number): History | undefined {
    return this.histories.find((h) => h.id === id >>> 0);
  }
  create(output: AokanaBpPointer | null, capacity: number, size: number): number {
    capacity >>>= 0;
    size >>>= 0;
  ...
  count(output: AokanaBpPointer | null, id: number): number {
  ...
  append(id: number, source: AokanaBpPointer | null): number {
    ...
    const input = bytesAt(source, 0, history.size),
      bytes = history.mode === 1 ? encodeAokanaRecord(input) : input.slice();
  ...
  read(output: AokanaBpPointer | null, id: number, index: number): number {
    const history = this.find(id);
    if (history === undefined) return 1;
    ...
    if (history.mode === 1) decodeAokanaRecord(output, entry.bytes);
    else
