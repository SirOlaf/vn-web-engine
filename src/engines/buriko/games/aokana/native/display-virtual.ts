...
export class AokanaVirtualDisplayObject extends AokanaDisplayObject {
  value170 = 1;

  constructor(
    environment: AokanaDisplayObjectEnvironment,
    depthOrder: number,
    parent: AokanaDisplayObject | null,
  ) {
  ...
  override inputHitTest(x: number, y: number, checkBounds: number): number {
    ...
    if (width === 0 || height === 0)
      throw new Error('Aokana virtual display hit test divides by zero native geometry');
    const maskX = Math.floor((Math.imul(this.hitWidth, x) >>> 0) / width),
      maskY = Math.floor((Math.imul(this.hitHeight, y) >>> 0) / height);
    if (AokanaDisplayObject.prototype.inputHitTest.call(this, maskX, maskY, checkBounds) === 0)
      return 0;
