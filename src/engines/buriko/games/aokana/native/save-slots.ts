...
export class AokanaSaveSlots {
  ...
  private require(record: SaveRecord, offset: number, length: number): Uint8Array {
    if (offset + length > record.bytes.length)
      throw new RangeError('Aokana save record exceeds native allocation');
    if (record.initialized.subarray(offset, offset + length).some((v) => v === 0))
      throw new AokanaUndefinedResourceRead('Aokana save record consumes unwritten decoded bytes');
  ...
  async header(destination: AokanaBpPointer | null, slot: number): Promise<number> {
    const record = await this.record(slot, false);
    if (record.status === 0) {
      if (destination === null)
        throw new RangeError('Aokana save header consumed a null destination');
  ...
  async save(slot: number, comment: AokanaBpPointer): Promise<number> {
    const size = this.memory.globalMemory.length,
      bytes = new Uint8Array((size + 64) >>> 0),
      view = new DataView(bytes.buffer),
      value = this.readLocalTime();
    if (!Number.isFinite(value.getTime()))
      throw new Error('Aokana save local-time host returned an invalid date');
    const words = [
      value.getFullYear(),
      value.getMonth() + 1,
      value.getDay(),
    ...
    if (name.length > bytes.length - 16)
      throw new RangeError('Aokana save comment exceeds native record');
    bytes.set(name, 16);
