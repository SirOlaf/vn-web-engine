...
export type AokanaMovieFramePositionResult =
...
export class AokanaMovieFramePosition {
  constructor(
    readonly surfaces: AokanaSurfaces,
    readonly movies: AokanaMovieRegistry,
  ) {
    if (!surfaces.usesMovieRegistry(movies))
      throw new Error('Aokana frame query requires the attached surface movie registry');
  }

  read(slot: number): AokanaMovieFramePositionResult {
    const index = slot | 0;
    ...
    if (renderer.surfaces !== this.surfaces || renderer.slot !== index)
      throw new Error('Aokana frame query requires the surface movie registry identity');
