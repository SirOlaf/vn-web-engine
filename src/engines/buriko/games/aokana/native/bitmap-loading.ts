...
export class AokanaBitmapLoading {
  ...
  fromCache(index: number, archive: Uint8Array | null, name: Uint8Array, consume = 1): number {
    ...
    if (bytes === null) { bytes = this.resources.cache.read(archive, name); consume = 0; }
  ...
  async synchronous(index: number, archive: Uint8Array | null, name: Uint8Array): Promise<number> {
    ...
    if (bytes === null) {
      const loaded = await this.resources.resources.load(archive, name, false);
      if (loaded.result === 0) return 0x80000019;
      bytes = loaded.bytes;
      if (bytes === null) throw new Error('Aokana synchronous bitmap load has no successful resource bytes');
      this.resources.cache.insert(archive, name, bytes.subarray(0, loaded.result >>> 0));
    }
    const bitmap = importAokanaWindowsBitmap(this.surfaces, index, bytes);
