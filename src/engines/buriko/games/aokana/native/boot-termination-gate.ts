import {AokanaBpScheduler, type AokanaBpSchedulerResult} from '../bp/scheduler.js';
 ...
 */
export class AokanaBootTerminationGate {
  ...
  constructor(
    ...
    if (
      !(scheduler instanceof AokanaBpScheduler) ||
      !(loading instanceof AokanaResourceLoadingState)
    )
      throw new TypeError(
        'Aokana boot gate requires the actual scheduler and resource loading owners',
      );
  }
  ...
  beginOuterReset(): boolean {
    ...
    if (
      ...
      throw new Error('Aokana outer reset requires a retired root child chain');
    this.restart = false;
  ...
  beginSuccessfulBoot(childId: number): void {
    ...
    if (
      ...
      throw new Error('Aokana successful boot requires the linked ED170 child');
  ...
  observeSchedulerResult(result: AokanaBpSchedulerResult): void {
    if (!this.booted || this.awaitingBoot || (result !== 0 && result !== 1 && result !== 2))
      throw new Error('Aokana boot gate requires a live normal scheduler result');
  ...
  observePumpResult(result: number): void {
    if (!this.booted || this.awaitingBoot || !Number.isSafeInteger(result))
      throw new Error('Aokana boot gate requires a live GUI pump result');
