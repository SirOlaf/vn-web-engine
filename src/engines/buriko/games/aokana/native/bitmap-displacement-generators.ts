import {allocateAokanaBitmap, type AokanaBitmap, AokanaBitmapStorage} from './bitmap.js';
...
function valid(bitmap: AokanaBitmap): boolean {
...
export function deriveAokanaHeightDisplacement(
  ...
  const component = (first: number, second: number): number => {
    ...
    if (divisor === 0n)
      throw new Error('Aokana height displacement reaches native zero IDIV divisor');
...
export function fillAokanaRippleDisplacement(
  bitmap: AokanaBitmap,
  centerX: number,
  centerY: number,
  period: number,
  ...
  const scratch = allocateAokanaBitmap((bitmap.width + 1) | 0, (bitmap.height + 1) | 0, 5);
  let dy = centerY | 0;
  ...
  const result = deriveAokanaHeightDisplacement(bitmap, scratch, 65536);
  scratch.storage?.release();
  return result;
}

/** 033890: finite-radius sine heights, preserving the wrapped doubled radius. */
export function fillAokanaCurvedDisplacement(
  bitmap: AokanaBitmap,
  centerX: number,
  centerY: number,
  strength: number,
  ...
  const result = deriveAokanaHeightDisplacement(bitmap, scratch, Math.floor(0x40000000 / strength));
  scratch.storage?.release();
  return result;
}

/** 033580: row-X and column-Y WORD tables, with separate signed-positive loops. */
export function fillAokanaSineDisplacement(
  bitmap: AokanaBitmap,
  periodX: number,
  phaseX: number,
  amplitudeX: number,
  ...
  const rows = new AokanaBitmapStorage(new Uint8Array((bitmap.height >>> 0) * 2), false);
  const columns = new AokanaBitmapStorage(new Uint8Array((bitmap.width >>> 0) * 2), false);
...
export function fillAokanaPointDisplacement(
  bitmap: AokanaBitmap,
  startX: number,
  startY: number,
  endX: number,
