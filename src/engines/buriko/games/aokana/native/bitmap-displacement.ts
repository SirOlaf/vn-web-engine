...
function relativeOrigin(source: AokanaBitmap, bounds: AokanaBitmap): [number, number] {
  if (
    source.storage === null ||
    bounds.storage === null ||
    source.storage.bytes.buffer !== bounds.storage.bytes.buffer
  )
    throw new Error('Aokana displacement requires cropped and full descriptors of the same bitmap');
  const stride = source.stride | 0;
  if (stride === 0)
    throw new RangeError('Aokana displacement requires a nonzero source row stride');
  const difference =
...
function point(
  ...
  if (coefficients === undefined)
    throw new RangeError('Aokana displacement table lacks the requested coefficient');
