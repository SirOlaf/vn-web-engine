import {AokanaDisplayMap} from './display-map.js';
import type {AokanaDisplayManager} from './display-manager.js';

/** 084070..0844F0, over the actual shared Map pool and display lists. */
export class AokanaMapDisplays {
  constructor(readonly manager: AokanaDisplayManager) {}

  private map(handle: number): AokanaDisplayMap | null {
    const object = this.manager.find('map', handle);
    if (object === null) return null;
    if (!(object instanceof AokanaDisplayMap))
      throw new Error('Aokana map pool contains another display class');
    return object;
  }
  create(): number {
    return this.manager.createSimple(
      'map',
      (order) => new AokanaDisplayMap(this.manager.environment, this.manager.surfaces, order),
    );
  }
