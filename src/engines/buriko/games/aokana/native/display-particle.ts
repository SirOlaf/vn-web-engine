...
import {AokanaParticleController} from './particle-controller.js';
import type {AokanaParticleVariants} from './particle-images.js';
import type {AokanaCrtRandom} from './system-timing.js';

export type AokanaParticleDwordReader = ((index: number) => number) | null;
...
export class AokanaParticleDisplayObject extends AokanaDisplayObject {
  readonly controller: AokanaParticleController;
  private layers: AokanaBitmap[] = [];
  ...
  private applyLayerOffsets(
    count: number,
    read: AokanaParticleDwordReader,
    ...
    for (let i = 0; i < count; i++) {
      if (read === null) throw new Error('Aokana particle layer offset pointer is null');
  ...
  configureLayers(
    count: number,
    readDepth: AokanaParticleDwordReader,
    readOffset: AokanaParticleDwordReader,
  ): number {
    this.check();
    count >>>= 0;
    if (count === 1) {
    ...
    let depth = 0;
    for (let i = 0; i < count; i++) {
      if (readDepth === null) throw new Error('Aokana particle layer depth pointer is null');
  ...
  configureCamera(values: readonly number[]): boolean {
    if (values.length !== 9) throw new Error('Aokana particle camera requires nine DWORDs');
  ...
  refreshParticle(): void {
    ...
    for (const layer of this.layers) fillAokanaBitmap(layer, 0);
  ...
  override draw(destination: AokanaBitmap, rectangle: AokanaBitmapRectangle, key: number): void {
    this.check();
    if (this.layers[0]?.storage == null) return;
