...
import {validateAokanaBmvHeader} from './bmv-frame.js';
...
export class AokanaBmvHeaderLoadProcess extends AokanaProcedure {
  ...
  override async poll(): Promise<number> {
    ...
    if (this.stage === 2) {
      ...
      if (this.result.value === 0x40 && validateAokanaBmvHeader(this.header) === 0) {
      ...
      if (this.result.value === this.tableLength()) {
        ...
        if (size.size === null)
          throw new AokanaUndefinedResourceRead(
            'Aokana BMV partial loader reads an unwritten stored-size output',
          );
  ...
  override dispose(): void {
    if (this.finalStatus === null)
      throw new AokanaUndefinedResourceRead(
        'Aokana BMV partial loader reads an unwritten completion status',
      );
    ...
    if (this.name === null)
      throw new AokanaUndefinedResourceRead(
        'Aokana BMV partial loader frees unwritten native name pointers',
      );
    this.archive = null;
    this.name = null;
