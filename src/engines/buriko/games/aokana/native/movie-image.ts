import {bitmapStorage, type AokanaBitmap, type AokanaBitmapStorage} from './bitmap.js';

export interface AokanaMovieMediaType {
...
export interface AokanaMovieSample {
  readonly storage: AokanaBitmapStorage;
  readonly offset: number;
}
export class AokanaMovieImageConfiguration {
...
function mediaInfo(type: AokanaMovieMediaType): DataView {
  if (type.format.length < 60) throw new RangeError('Aokana movie reads beyond VIDEOINFOHEADER storage');
...
export class AokanaMovieImage {
  ...
  bottomUp = false;
  averageFrameTime = 0n;
  constructor(readonly configuration: AokanaMovieImageConfiguration) {}
  get width(): number {return this.configuration.dimensionMode === 1 ? this.requestedWidth : this.sourceWidth;}
  get height(): number {return this.configuration.dimensionMode === 1 ? this.requestedHeight : this.sourceHeight;}
  ...
  setMediaType(type: AokanaMovieMediaType): number {
    const info = mediaInfo(type), width = info.getInt32(52, true), height = info.getInt32(56, true);
    this.sourceWidth = width;
    const sign = height >> 31;
    this.sourceHeight = ((height ^ sign) - sign) | 0;
    this.sourceStride = this.selectedFormat === 0 ? Math.imul(width, 4) :
      this.selectedFormat === 1 ? Math.imul((width + 1) | 0, 3) & ~3 :
      this.selectedFormat === 2 || this.selectedFormat === 3 ? Math.imul(width, 2) : 0;
  ...
  copySample(destination: AokanaBitmap, sample: AokanaMovieSample): number {
    ...
    for (let y = 0; y < height; y++) {
      ...
      const put = (pixel: number): void => {
        ...
        target.view.setUint32(output, pixel >>> 0, true); target.written(output, 4); output += 4;
      };
      if (this.selectedFormat === 0) {
        let x = 0;
        const separated = destination.storage !== source ||
          input + (width - 1) * 4 < output || output + (width - 1) * 4 < input;
        if (width >= 16 && separated) {
          ...
          const store = (bytes: Uint8Array): void => {
            ...
            target.bytes.set(bytes, output); target.written(output, 16); output += 16;
          };
          for (; x < (width & ~15); x += 16, input += 64) {
            const first = load(input), second = load(input + 16);
            store(first);
            const third = load(input + 32); store(second);
            const fourth = load(input + 48); store(third); store(fourth);
          }
        }
        ...
        for (; x + 4 <= width; x += 4, input += 12) {
          ...
          const secondLow = read(input + 3, 1), secondHigh = read(input + 4, 2);
          put(secondLow | (secondHigh << 8));
          const thirdLow = read(input + 6, 2), thirdHigh = read(input + 8, 1);
          put(thirdLow | (thirdHigh << 16));
          put(read(input + 8, 4) >>> 8);
        }
        for (; x < width; x++, input += 3) {
          // Tail instructions fetch bytes1,2,0, in that order.
          const green = read(input + 1, 1), red = read(input + 2, 1), blue = read(input, 1);
  ...
  orientDestination(bitmap: AokanaBitmap): AokanaBitmap {
