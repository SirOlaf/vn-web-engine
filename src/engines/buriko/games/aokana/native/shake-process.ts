...
import {aokanaShakeTarget} from './shake-math.js';

function divide(numerator: number, denominator: number): number {
  numerator |= 0;
  denominator |= 0;
  if (denominator === 0 || (numerator === -2147483648 && denominator === -1))
    throw new Error('Aokana shake native signed division fault');
...
export class AokanaShakeProcess extends AokanaProcedure {
  active = 0;
  capture = 0;
  mode = 0;
  amplitude = 0;
