...
import {
  AokanaDisplayControlProcess,
  AokanaSplineDisplayControlProcess,
...
export function createGroup90DisplayControl(
  ...
  const install = (context: AokanaBpOpcodeContext, process: AokanaDisplayControlProcess): 2 => {
    const node =
      context.thread === scheduler.root.state
        ? scheduler.root
        : scheduler.findById(context.thread.id);
    if (node === null || node.state !== context.thread)
      throw new Error('Aokana display control thread is not linked to its scheduler');
  ...
  const initialize = (
    ...
    const process = new AokanaDisplayControlProcess(
      context.thread,
      procedures,
      clock,
      manager,
  ...
  add(0x24, 0x1400dc3e0, 'ControlDisplaySpline', (context) => {
    ...
    if (manager.resolve(handle) === null)
      return fatal(context, '無効なオブジェクトハンドルが指定されました');
    const process = new AokanaSplineDisplayControlProcess(
        context.thread,
        procedures,
        clock,
        manager,
