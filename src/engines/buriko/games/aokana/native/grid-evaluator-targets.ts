...
import type {AokanaGridEvaluatorRecords} from './grid-evaluator-records.js';
...
export function chooseGridEvaluatorTarget(
  ...
  if (actor.word(0x95) === 1) {
    for (let i = 0; i < count; i++) {
      ...
      if (grid.copyPosition(pointer(), candidate.id) !== 0)
        throw new Error('Aokana grid evaluator target has uninitialized grid coordinates');
      let x = values.getInt32(0, true),
        y = values.getInt32(4, true),
        cost = 0x7fffffff;
      if (grid.copyNeighborPaths(pointer(8), pointer(32), actor.id, x, y) !== 0)
        throw new Error('Aokana grid evaluator target reads uninitialized neighbor paths');
      for (let direction = 0; direction < 6; direction++) {
        ...
        if (grid.copyMetric(pointer(96), actor.id, nx, ny) !== 0)
          throw new Error('Aokana grid evaluator target reads uninitialized neighbor cost');
    ...
    if (grid.copyReachable({bytes: positions, offset: 0}, pointer(), actor.id, true) !== 0)
      throw new Error('Aokana grid evaluator target reads uninitialized reachable count');
    ...
    for (let positionIndex = 0; positionIndex < length; positionIndex++) {
      ...
      if (grid.copyMetric(pointer(), actor.id, x, y) !== 0)
        throw new Error('Aokana grid evaluator target reads uninitialized movement cost');
      visibility.collect(
  ...
  if (selected >= 0) {
    if (selected >= costs.length)
      throw new Error('Aokana grid evaluator target index outside stack cost array');
    if (costs[selected]! > 0) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (target === undefined)
      throw new Error('Aokana grid evaluator target reads uninitialized destination');
    selected = i;
    selectedType = type;
    selectedCost = cost;
    actor.setWord(3, i);
