...
import {readAokanaPixelPair, writeAokanaPixelPair, saturateAokanaByte} from './bitmap-pairs.js';
...
function average(
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (; x + 1 < width; x += 2) {
      const pair = readAokanaPixelPair(source, offset + x * 4);
      add(pair[0]);
      add(pair[1]);
    }
...
function packed(lanes: Lanes): number {
  return (
    (saturateAokanaByte(lanes[0]) |
      (saturateAokanaByte(lanes[1]) << 8) |
      (saturateAokanaByte(lanes[2]) << 16) |
      (saturateAokanaByte(lanes[3]) << 24)) >>>
    0
  );
}
...
function mosaicBlocks(
  ...
  if (side === 0 && (averaged || height !== 0))
    throw new RangeError('Aokana mosaic has a zero native block divisor/step');
      ...
      for (let y = 0; y < blockHeight; y++) {
        ...
        if (transparency === 0) {
          ...
          for (; x + 1 < blockWidth; x += 2) {
            const pair = readAokanaPixelPair(destination, offset + x * 4);
            writeAokanaPixelPair(destination, offset + x * 4, [
