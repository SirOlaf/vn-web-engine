...
import {createGroupB0CursorPolicy} from './group-b0-main.js';
import {
...
/** Current graph owns these cached/style/filter lowers; size rebuild and live D3D mode query remain unbound. */
const boundDeviceSecondaries = new Set([0x0b, 0x66, 0x6d, 0x6f]);
/** Child presentation and font drawing require a selected canvas/font host. */
const boundChildState = new Set([0x10, 0x11, 0x12, 0x14, 0x15, 0x16, 0x17, 0x1c, 0x1e, 0x1f]);
/** Additional volume changes retain inactive voices; playback controls need later owners. */
const boundAudioControls = new Set([0x1c, 0x2c]);
/** Device reconfiguration and setup are outside this display-state slice. */
  ...
  nativeDefinitions(): AokanaNativeSlotDefinition[] {
    const definitions = [
      ...
      ...createGroup80DisplayService(this.graph.controller, this.graph.resource.errors).filter(
        ({secondary}) => boundDisplayService.has(secondary),
      ),
      ...
      ...(this.graph.fileEnumeration.metadata.profile.namespace === undefined
        ? []
      ...
      ...createGroup81Device(this.graph.controller).filter((definition) =>
      ...
      ...createGroupA0AudioControls(
        this.graph.resource.channels,
        this.graph.resource.errors,
      ).filter(({secondary}) => boundAudioControls.has(secondary)),
      ...
      ...createGroupB0Children(this.graph.children, this.graph.resource.errors).filter(
        ({secondary}) => boundChildState.has(secondary),
      ),
