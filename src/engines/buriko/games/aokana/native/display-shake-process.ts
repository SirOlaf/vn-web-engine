import {AokanaDisplayControlProcess} from './display-control-process.js';

function divideUnsigned(numerator: number, denominator: number): number {
  denominator >>>= 0;
  if (denominator === 0) throw new Error('Aokana display shake native unsigned division fault');
...
export class AokanaDisplayShakeProcess extends AokanaDisplayControlProcess {
  private mode = 0;
  ...
  protected override update(forceFinish: boolean): boolean {
    ...
    if (!finished) {
      ...
      if (phase === undefined || amplitude === undefined)
        throw new RangeError('Aokana display shake reads beyond its native phase/amplitude tables');
