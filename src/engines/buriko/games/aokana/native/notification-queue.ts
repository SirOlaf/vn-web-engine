export interface AokanaNativeNotification {
  readonly type: number;
  readonly value1: number;
  readonly value2: number;
}
interface NotificationNode extends AokanaNativeNotification {next: NotificationNode | null;}

/** 0fa3b0/0fa300/0fa2a0: native12-byte notifications in a separate FIFO from VM thread messages. */
export class AokanaNativeNotifications {
  ...
  push(type: number, value1: number, value2: number): void {
    const node: NotificationNode = {type: type >>> 0, value1: value1 >>> 0, value2: value2 >>> 0, next: null};
    if (this.last === null) this.first = node;
    else this.last.next = node;
    this.last = node;
  }
  take(): AokanaNativeNotification | null {
  ...
  clear(): void {this.first = this.last = null;}
}
