...
import {allocateAokanaBitmap, fillAokanaBitmap, type AokanaBitmap} from './bitmap.js';
import type {AokanaBitmapCompositor} from './bitmap-compositor.js';
import {mixAokanaBitmaps} from './bitmap-mix.js';
import {aokanaParticleRandom, type AokanaParticleVector} from './particle-air.js';
import {AokanaParticleImages, AokanaParticleVariants} from './particle-images.js';
...
function divide(numerator: number, denominator: number): number {
  ...
  if (denominator === 0 || (numerator === -2147483648 && denominator === -1))
    throw new RangeError('Aokana particle signed division is outside its numerical domain');
  return Math.trunc(numerator / denominator) | 0;
}
function required(value: number | undefined): number {
  if (value === undefined) throw new Error('Aokana particle reads an unconfigured numeric field');
  return value;
}

/** Native controller-owned 38h movement record; only six constructor words are initialized. */
export class AokanaFireflyMovement {
  ...
  configure(values: readonly number[]): void {
    if (values.length !== 14)
      throw new Error('Aokana firefly movement requires fourteen DWORD values');
    for (let i = 0; i < 14; i++) {
...
export class AokanaParticle {
  active = 0;
  protected point: AokanaParticleVector | undefined;
  ...
  constructor(
    ...
    protected readonly images: AokanaParticleImages,
    protected readonly random: AokanaCrtRandom,
  ) {
    this.frameIncrement =
      (aokanaParticleRandom(random, images.durationSpread) + images.duration) | 0;
    this.specialFrameIncrement = this.frameIncrement;
    this.specialFrameCount = images.count;
  }
  position(): AokanaParticleVector {
    if (this.point === undefined) throw new Error('Aokana particle reads unconfigured position');
    return {...this.point};
  }
  blendMode(): number {
  ...
  configureSpecialFrames(images: AokanaParticleImages): void {
    this.specialFrameIncrement =
      (aokanaParticleRandom(this.random, images.durationSpread) + images.duration) | 0;
    this.specialFrameCount = images.count;
  }
  /** 095c50 applies current environment displacement only to active particles. */
  applyAir(vector: AokanaParticleVector): void {
    if (this.active === 0) return;
    const point = this.point,
      scale = required(this.airScale);
    if (point === undefined) throw new Error('Aokana particle air reads unconfigured position');
  ...
  image(_scale: number): AokanaBitmap | null {
    return null;
  }
  releaseImage(): number {
    return 0;
...
export class AokanaSnowParticle extends AokanaParticle {
  private velocity: AokanaParticleVector | undefined;
  constructor(variant: number, variants: AokanaParticleVariants, random: AokanaCrtRandom) {
    const images = variants.snowImages[variant >>> 0];
    if (images === undefined)
      throw new RangeError('Aokana snow variant is outside its native table');
    super(0, variant >>> 0, images, random);
    const p = variants.snowParameters[variant >>> 0]!;
    if (p[0] === 0) return;
    const around = (center: number, spread: number): number =>
      (aokanaParticleRandom(random, Math.imul(spread, 2)) + (center - spread)) | 0;
  ...
  override image(scale: number): AokanaBitmap | null {
    scale >>>= 0;
    if (scale >= 32) return null;
    const bitmap = this.images.scales[scale]?.[this.frame()];
    if (bitmap === undefined) throw new Error('Aokana snow image descriptor is absent');
    return bitmap.storage === null ? null : bitmap;
  }
}

/** DCPFirefly, 09a5f0: lifespan, interpolated movement, fades and optional mixed image. */
export class AokanaFireflyParticle extends AokanaParticle {
  private lifetime: number | undefined;
  private age: number | undefined;
  private firstVelocity: AokanaParticleVector | undefined;
  private nextVelocity: AokanaParticleVector | undefined;
  ...
  private mixedImage: AokanaBitmap | null = null;
  ...
  constructor(
    variant: number,
    private readonly variants: AokanaParticleVariants,
    random: AokanaCrtRandom,
    readonly movement: AokanaFireflyMovement,
    private readonly compositor: AokanaBitmapCompositor,
  ) {
    const images = variants.fireflyImages[variant >>> 0];
    if (images === undefined)
      throw new RangeError('Aokana firefly variant is outside its native table');
    super(1, variant >>> 0, images, random);
    const p = variants.fireflyParameters[variant >>> 0]!;
    if (p[0] === 0) return;
    this.lifetime = (aokanaParticleRandom(random, p[2]!) + p[1]!) >>> 0;
    this.age = 0;
    this.point = {
  ...
  private around(center: number, spread: number): number {
    return (aokanaParticleRandom(this.random, Math.imul(spread, 2)) + (center - spread)) | 0;
  }
  private randomVelocity(): AokanaParticleVector {
  ...
  private normalizeVelocity(adjustLifetime: boolean): void {
    ...
    const magnitude = (aokanaParticleRandom(this.random, p.get(5)) + p.get(4)) | 0;
  ...
  private beginTransition(): void {
    ...
    this.transitionDuration = (aokanaParticleRandom(this.random, global[13]!) + global[12]!) | 0;
  ...
  override image(scale: number): AokanaBitmap | null {
    ...
    if (base === undefined) throw new Error('Aokana firefly image descriptor is absent');
    ...
    if (special === undefined) throw new Error('Aokana special firefly image descriptor is absent');
    const width = Math.max(base.width >>> 0, special.width >>> 0),
      height = Math.max(base.height >>> 0, special.height >>> 0);
    const output = allocateAokanaBitmap(width, height, base.format);
    ...
    if (special.width === width && special.height === height)
      mixAokanaBitmaps(output, output, special, option, this.compositor.processing);
    else {
      const expanded = allocateAokanaBitmap(width, height, base.format);
      fillAokanaBitmap(expanded, 0);
      this.compositor.draw(
        expanded,
      ...
      mixAokanaBitmaps(output, output, expanded, option, this.compositor.processing);
