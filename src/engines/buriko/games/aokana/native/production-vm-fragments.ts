...
export class AokanaProductionVmFragments {
  constructor(readonly core: AokanaProductionVmCore) {
    if (!(core instanceof AokanaProductionVmCore))
      throw new TypeError('Aokana VM fragments require the actual production VM core');
  }
  ...
  nativeDefinitions(): AokanaNativeSlotDefinition[] {
    ...
    if (
      ...
      !(data.independentIcon instanceof AokanaIndependentIconState) ||
      !(data.textSelection instanceof AokanaSelectionState) ||
      !(data.splines instanceof AokanaNativeSplines) ||
    ...
    const definitions = [
      ...
      ...createAokanaBitmapLoadingSlots(
        graph.bitmapLoading,
        scheduler,
        data.procedureState,
        graph.clock,
    ...
    for (const definition of definitions) {
      ...
      if (
        ...
        throw new Error(`Aokana VM fragments have an invalid inventory address ${key}`);
      if (keys.has(key)) throw new Error(`Aokana VM fragments duplicate ${key}`);
      keys.add(key);
    }
    return definitions.map((definition) => ({
      ...
      execute: (context) => {
        ...
        if (context.memory !== data.memory || context.diagnostics !== this.core.diagnostics)
          throw new Error('Aokana VM fragments require the core BP memory and diagnostics');
