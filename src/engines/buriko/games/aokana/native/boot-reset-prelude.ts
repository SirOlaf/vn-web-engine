 ...
 */
export class AokanaBootResetPrelude {
  ...
  constructor(
    ...
    if (
      ...
      !(gate instanceof AokanaBootTerminationGate) ||
      gate.loading !== graph.resource.loading
    )
      throw new TypeError('Aokana reset prelude requires the shared production graph and gate');
  }

  /** Consume the restart latch before applying ECB90 calls FF520 through 0375D0. */
  run(): 'reset-prefix-complete' | null {
    if (this.phase === 'audio-running') throw new Error('Aokana reset audio is still running');
  ...
  async resetAudio(actor = this.graph.allocator.currentActor): Promise<'audio-reset-complete'> {
    ...
    if (
      ...
      throw new Error('Aokana reset audio requires active graph channels after the reset prefix');
