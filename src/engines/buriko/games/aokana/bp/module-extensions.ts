import type {AokanaBpOpcodeContext, AokanaBpModuleResourceSource} from '../native/types.js';
import {readU8} from './decode.js';
import {AOKANA_BP_MODULE_NO_SPACE, attachModule, detachLastModule} from './modules.js';
...
function readName(pointer: AokanaBpPointer | null, nullable = false): Uint8Array | null {
  if (pointer === null) {
    if (nullable) return null;
    throw new Error('Aokana module extension dereferenced a null resource name');
  }
  const end = pointer.bytes.indexOf(0, pointer.offset);
  if (end < 0) throw new RangeError('Unterminated Aokana module extension resource name');
...
export class AokanaBpModuleExtensions {
  private readonly resources = new Map<number, ExtensionResource>();

  constructor(private readonly source: AokanaBpModuleResourceSource) {}
  ...
  execute(context: AokanaBpOpcodeContext): 0 | Promise<0> {
    ...
    if (entry === undefined) throw new Error(`Unregistered Aokana module extension ${secondary}`);
    const base = attachModule(thread, 'Mediation program', mediationModule());
    if (base === AOKANA_BP_MODULE_NO_SPACE)
      throw new Error('Aokana module extension mediation does not fit');
    if (attachModule(thread, entry.name, entry.module) === AOKANA_BP_MODULE_NO_SPACE) {
      // Native fatal path leaves the first attachment in place.
      throw new Error(`Aokana module extension ${secondary} does not fit`);
    }
    if (thread.frameCursor + 4 > thread.frameLimit)
      throw new RangeError('Aokana module extension frame overflow');
  ...
  private validateIndex(index: number): void {
    if (!Number.isInteger(index) || index < 0 || index >= 0xf0) {
      throw new RangeError(`Invalid Aokana module extension ${index}`);
    }
  }
}
