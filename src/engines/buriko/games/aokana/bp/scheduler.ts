...
import type {AokanaSharedLoaderWorker} from '../native/shared-loader-worker.js';
...
export class AokanaBpScheduledThread {
  ...
  constructor(
    readonly state: AokanaBpThread,
    private readonly owner: AokanaBpScheduler | null = null,
  ) {}
  ...
  installProcess(process: AokanaBpWaitProcess | null): void {
    if (this.process?.hasOutstandingExternalBorrow?.())
      throw new Error('Aokana process replacement retains a borrowed BP destination');
  ...
  private finishPoll(value: number): number {
    ...
    if (result === -1 || result === 1) {
      if (this.process?.hasOutstandingExternalBorrow?.())
        throw new Error('Aokana completed process retains a borrowed BP destination');
...
export class AokanaBpScheduler {
  ...
  private dispatchingInstruction: AokanaBpThread | null = null;
  ...
  private sharedLoaderWorker: AokanaSharedLoaderWorker | null = null;
  private executeInstruction: ((thread: AokanaBpThread) => AokanaBpInstructionResult) | null;

  constructor(
    root: AokanaBpThread,
    executeInstruction?: (thread: AokanaBpThread) => AokanaBpInstructionResult,
    private readonly onThreadRemoved?: (thread: AokanaBpScheduledThread) => void,
  ) {
    this.root = new AokanaBpScheduledThread(root, this);
  ...
  isDispatchingInstructionFor(thread: AokanaBpThread): boolean {
  ...
  bindProcessPollGuard(pending: () => boolean): void {
    if (this.pendingNativeCallbacks !== null || this.finalAdmissionClosed)
      throw new Error('Aokana scheduler process-poll guard is already bound');
  ...
  private beginProcessPoll(invocationToken?: object): object {
    ...
    if (invocationToken === undefined && this.finalAdmissionClosed)
      throw new Error('Aokana scheduler process-poll admission is closed');
    if (
      invocationToken === undefined
        ? this.invocationToken !== null
        : this.invocationToken !== invocationToken || !this.running
    )
      throw new Error('Aokana scheduler invocation excludes a direct process poll');
    if (this.processPollToken !== null)
      throw new Error('Aokana scheduler process poll is already active');
    if (this.pendingNativeCallbacks?.())
      throw new Error('Aokana scheduler process poll overlaps native or frame work');
    const token = {};
  ...
  private endProcessPoll(token: object): void {
    if (this.processPollToken !== token)
      throw new Error('Aokana scheduler process-poll token is stale');
  ...
  private beginInvocation(): object {
    if (this.finalAdmissionClosed)
      throw new Error('Aokana scheduler invocation admission is closed');
    if (this.invocationToken !== null || this.processPollToken !== null)
      throw new Error('Aokana scheduler invocation overlaps a process poll');
    if (this.pendingNativeCallbacks?.())
      throw new Error('Aokana scheduler invocation overlaps native or frame work');
    const token = {};
  ...
  private endInvocation(token: object): void {
    if (this.invocationToken !== token)
      throw new Error('Aokana scheduler invocation token is stale');
    this.invocationToken = null;
  ...
  bindInstructionExecutor(
    ...
    if (this.executeInstruction !== null)
      throw new Error('Aokana scheduler instruction executor is already bound');
    if (typeof executeInstruction !== 'function')
      throw new TypeError('Aokana scheduler instruction executor must be a function');
  ...
  attachSharedLoaderWorker(worker: AokanaSharedLoaderWorker): void {
    if (this.sharedLoaderWorker !== null && this.sharedLoaderWorker !== worker)
      throw new Error('Aokana scheduler already has its shared loader worker');
    this.sharedLoaderWorker = worker;
  }

  append(thread: AokanaBpThread): AokanaBpScheduledThread {
    const node = new AokanaBpScheduledThread(thread, this);
    let last = this.root;
    ...
    while (node !== null) {
      if (node === thread) {
        if (node.process?.hasOutstandingExternalBorrow?.())
          throw new Error('Aokana child removal retains a borrowed BP destination');
  ...
  removeAllChildren(): void {
    if (this.running || this.invocationToken !== null || this.processPollToken !== null)
      throw new Error('Aokana child-tree teardown requires an idle scheduler');
    for (let node = this.root.next; node !== null; node = node.next)
      if (node.process?.hasOutstandingExternalBorrow?.())
        throw new Error('Aokana child-tree teardown retains a borrowed BP destination');
    while (this.root.next !== null) {
      let last = this.root.next;
      while (last.next !== null) last = last.next;
      if (!this.remove(last)) throw new Error('Aokana child-tree teardown lost its linked tail');
    }
  }
  ...
  private destroy(node: AokanaBpScheduledThread): void {
    if (
      ...
      throw new Error('Aokana child destruction requires live-storage process retirement');
  ...
  async run(): Promise<AokanaBpSchedulerResult> {
    ...
    if (executeInstruction === null)
      throw new Error('Aokana scheduler instruction executor is not bound');
  ...
  private async runInvocation(
    executeInstruction: (thread: AokanaBpThread) => AokanaBpInstructionResult,
    invocationToken: object,
  ): Promise<AokanaBpSchedulerResult> {
    ...
    while (node !== null) {
      ...
      for (let count = 0; count < AOKANA_BP_BURST_INSTRUCTIONS; count++) {
        let instruction: AokanaBpInstructionResult;
