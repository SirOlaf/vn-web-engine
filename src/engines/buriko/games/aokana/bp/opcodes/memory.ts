...
function watchedStore(h: AokanaBpOpcodeContext, reverse: boolean): 0 {
...
function calculateStore(h: AokanaBpOpcodeContext, immediate: boolean): 0 {
...
export const memoryOpcodes: Readonly<Record<number, AokanaBpOpcodeHandler>> = {
  0x08: (h) => {
  ...
  0x0c: (h) => {
    ...
    if (count === 0) return 0;
    if (type > 2) throw new Error('Aokana ._bp scalar sequence reads undefined native stride');
  ...
  0x70: (h) => {
    const size = pop32(h.thread);
    if ((size - 1) >>> 0 >= 0x10000000) throw new Error('Aokana ._bp invalid allocation size');
    if (h.thread.heap === null)
      throw new Error('Aokana ._bp allocation through a null native heap manager');
    const base = h.thread.heap.allocate(size);
    if ((base + size) >>> 0 > 0x10000000)
      throw new Error('Aokana ._bp allocation exceeds tagged address range');
    push32(h.thread, base + 0x30000000);
    return 0;
  },
  0x71: (h) => {
    const address = pop32(h.thread);
    if ((address & 0xf0000000) !== 0x30000000)
      throw new Error('Aokana ._bp free requires a thread heap address');
    if (h.thread.heap === null)
      throw new Error('Aokana ._bp free through a null native heap manager');
    if (!h.thread.heap.free(address & 0x0fffffff)) throw new Error('Aokana ._bp invalid heap free');
    push32(h.thread, 1);
    return 0;
  },
  0xec: (h) => {
