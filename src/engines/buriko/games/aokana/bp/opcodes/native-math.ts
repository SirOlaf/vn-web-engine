...
function reduceLargeAngle(magnitude: number): {quadrant: number; head: number; tail: number} {
  ...
  if (high === 0n) throw new Error('Aokana large reduction reaches an undefined native BSR input');
  ...
  if (middle === 0n)
    throw new Error('Aokana large reduction reaches an undefined native BSR input');
...
export function nativeSineFirstQuadrant(radians: number): number {
  if (!Number.isFinite(radians) || radians < 0 || radians > 3.141592653589793 / 2)
    throw new RangeError('Aokana font sine argument outside its verified first-quadrant domain');
...
export function nativeSpatialSineCosine(angle: number): {sine: number; cosine: number} {
  if (!Number.isFinite(angle) || Math.abs(angle) > 8)
    throw new RangeError('Aokana spatial trigonometry outside its native argument domain');
...
export function nativeWaveSineRadians(radians: number): number {
  ...
  if (Math.abs(radians) > 0x80000000 * 6.283185307179586)
    throw new RangeError('Aokana wave sine argument exceeds its native phase/period domain');
  return sineBoundedRadians(radians);
}
...
export function nativeSpatialAngle(y: number, x: number): number {
  ...
  if (!Number.isFinite(y) || !Number.isFinite(x))
    throw new RangeError('Aokana spatial angle requires finite native coordinates');
...
export function nativeCursorInterpolation(
  ...
  progress >>>= 0;
  steps >>>= 0;
  if (steps === 0) throw new Error('Aokana native cursor interpolation division by zero');
...
export const nativeMathOpcodes: Readonly<Record<number, AokanaBpOpcodeHandler>> = {
  0x43: (h) => {
