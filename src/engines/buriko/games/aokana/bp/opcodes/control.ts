...
function checkTarget(h: AokanaBpOpcodeContext, address: number, forbidZero: boolean): void {
  if ((forbidZero && address >>> 0 === 0) || !validCodeAddress(h.thread, address >>> 0)) {
    throw new Error(`Aokana ._bp invalid code target 0x${(address >>> 0).toString(16)}`);
  }
}

function enterCall(h: AokanaBpOpcodeContext, returnAddress: number): void {
  const t = h.thread;
  if (t.frameCursor + 4 > t.frameLimit) throw new Error('Aokana ._bp call frame overflow');
  t.callSites.push(t.instructionStart);
  writeFrame32(t, t.frameCursor, returnAddress);
  t.frameCursor = (t.frameCursor + 4) >>> 0;
}

function comparisonBranch(h: AokanaBpOpcodeContext, immediate: boolean): 0 {
...
export const controlOpcodes: Readonly<Record<number, AokanaBpOpcodeHandler>> = {
  0x00: (h) => {
  ...
  0x05: (h) => {
    ...
    push32(h.thread, (h.thread.instructionStart + relative) | 0x10000000);
    return 0;
  },
  0x06: (h) => {
    const relative = readI16(h.thread);
  ...
  0x11: (h) => {
    ...
    if (cursor > h.thread.frameLimit) throw new Error('Aokana ._bp invalid frame cursor');
  ...
  0x15: (h) => {
    ...
    switch (control & 7) {
      ...
      case 5:
        ...
        break;
      default:
        throw new Error('Aokana ._bp branch selector reads undefined native stack data');
    }
    if (taken) {
      checkTarget(h, target, false);
      setPc(h.thread, target);
