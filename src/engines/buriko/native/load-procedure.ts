...
import {
  BurikoResourceLoadingState,
  type BurikoResourceBuffer,
  type BurikoResourceResult,
} from './resource-loading.js';
import {terminatedNativeBytes} from './program-files.js';
import type {BurikoBpOpcodeContext} from './types.js';

export interface BurikoLoadProcedureOptions {
...
export abstract class BurikoLoadProcedure extends BurikoProcedure {
  readonly output: BurikoResourceBuffer = {bytes: null};
  readonly result: BurikoResourceResult = {value: 0};
  ...
  get archiveName(): Uint8Array | null {
    return this.archive[0] === 0 ? null : this.archive;
  }
    ...
    if (step === 0) {
      ...
      if (this.cacheSelected && this.cacheMiss) {
        if (this.output.bytes === null)
          throw new Error('Buriko load procedure has no successful resource bytes');
