...
export class BurikoFontResources {
  ...
  async load(
    ...
    if (archive === null) {
      ...
      if (loaded.bytes === null)
        throw new Error('Buriko font resource dereferences a null successful load');
      if (loaded.bytes.length < size)
        throw new RangeError('Buriko font resource reads past the loaded allocation');
  ...
  async enumerate(charset: number, japanese: boolean): Promise<BurikoFontEnumeration> {
    charset |= 0;
    ...
    for (const wide of await this.fonts.browser.enumerate(charset, japanese)) {
      for (let index = 0; index < 2; index++)
        seen[index] ||= burikoCrtWidePrefixEqual(wide, wideDefaults[index]!);
