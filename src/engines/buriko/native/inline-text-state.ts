...
export function burikoInlineTextWidth(value: string): number {
  let width = 0;
  for (let index = 0; index < value.length; index++) {
    ...
    width = (width + Number(burikoWideCharacter(character) !== 0) + 1) | 0;
  }
  return width;
}

export interface BurikoInlineTextSpecification {
  ...
  readonly nativeRectangle: BurikoNativeRectangle;
...
export class BurikoInlineTextState {
  ...
  visible = 0; // 1401e7110
  rectangle: BurikoNativeRectangle = [0, 0, 0, 0];
  ...
  setInitial(source: BurikoBpPointer | null): void {
  ...
  character(
    ...
    if (character === 0x16n) {
      if (clipboard !== null) {
        ...
        if (copied.length > 1023)
          throw new RangeError('Buriko inline text clipboard overwrites native wide stack scratch');
        incoming = burikoInlineTextWidth(copied);
      }
    } else if (character < 0x20n) {
      if (character === 3n || character === 8n || character === 24n) return 'default';
      return 'consume';
    } else incoming = Number(burikoWideCharacter(Number(BigInt.asUintN(32, character))) !== 0) + 1;
    const nul = current.indexOf('\0');
    const copied = (nul < 0 ? current : current.slice(0, nul)).slice(0, 1023);
    const existing = burikoInlineTextWidth(copied);
    ...
    if (end > start) {
      if (end > copied.length + 1)
        throw new Error('Buriko inline selection copies unwritten native wide stack bytes');
      incoming = (incoming - burikoInlineTextWidth((copied + '\0').slice(start, end))) | 0;
    }
  ...
  specification(
    ...
    limit: number,
    focus: number,
  ): {result: 1 | 2 | 3 | 4} | {result: 0; value: BurikoInlineTextSpecification} {
    width >>>= 0;
    height >>>= 0;
    size >>>= 0;
    ...
    const rectangle: BurikoNativeRectangle = [x | 0, y | 0, (x + width) | 0, (y + height) | 0];
    return {
      result: 0,
      value: {
        font: name,
