...
const divide = (value: number, divisor: number): number => {
  if (divisor === 0 || (value === -2147483648 && divisor === -1))
    throw new RangeError('Buriko selection text has an undefined native signed quotient');
  return Math.trunc(value / divisor) | 0;
};

/** 07D530: draw the actual item strings and retain their measured selection rectangles. */
export function drawBurikoSelectionText(
  ...
  const fontId = window.fontId,
    ...
    text = new BurikoBitmapText(surfaces.fonts, surfaces.compositor),
    rectangles: BurikoBitmapRectangle[] = [];
  window.clearText();
  for (let index = 0, column = 0; index < items.length; index++, column = (column + 1) % columns) {
    const source = items[index]!;
    if (source === null) throw new Error('Buriko selection renderer reads a null item string');
    const bitmap = allocateBurikoBitmap(Math.imul(Math.imul(textLength(source), size), 2), size, 1);
    ...
    if (published === 0)
      throw new Error('Buriko selection renderer consumes undefined glyph width');
