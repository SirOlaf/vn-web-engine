...
import type {BurikoAudioMusicResources} from './audio/resource-music.js';
...
export const burikoPairMissingWaveDiagnostic = literal(
...
export const burikoPairInvalidWaveDiagnostic = literal(
...
export function createGroupA0PairedMusicLoad(
  music: BurikoAudioMusicResources,
  ...
  return [
    {
      ...
      execute: async (h): Promise<0> => {
        ...
        const name = (pointer: typeof first) => () => {
            if (pointer === null) throw new Error('Buriko paired music dereferences a null name');
        ...
        if (status === 12 || status === 14)
          return errors.threadFatal(
            ...
            formatBurikoAudioNames(
              status === 12 ? burikoPairMissingWaveDiagnostic : burikoPairInvalidWaveDiagnostic,
