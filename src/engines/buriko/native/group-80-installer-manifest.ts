...
import type {BurikoInstallerManifestActions} from './installer-manifest-actions.js';
...
function argumentsFor(context: BurikoBpOpcodeContext): {
  root: Uint8Array;
  entries: Uint8Array[];
} {
  ...
  if (addresses === null || root === null)
    throw new Error('Buriko installer manifest dereferences a null argument');
  let count = 0;
  while (pointerView(addresses, (count + 1) * 4).getUint32(count * 4, true) !== 0) count++;
  const resolved = resolveBurikoAddressArray(memory, thread, addresses, count + 1);
  const entries = resolved.slice(0, count).map((pointer: BurikoBpPointer | null) => {
    if (pointer === null) throw new Error('Buriko installer manifest has a null list entry');
...
export function createGroup80InstallerManifest(
  actions: BurikoInstallerManifestActions,
