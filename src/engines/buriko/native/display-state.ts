...
export type BurikoNativeRectangle = readonly [number, number, number, number];
...
export class BurikoNativeDisplayState {
  ...
  monitors: BurikoNativeRectangle[] = []; // Monitor rectangles populated by the window host.
  ...
  get logicalWidth(): number {
    ...
    if (width === undefined) throw new RangeError('Buriko native display preset outside table');
    return width;
  }
  get logicalHeight(): number {
    const height = this.presetHeights[this.selectedSizePreset >>> 0];
    if (height === undefined) throw new RangeError('Buriko native display preset outside table');
    return height;
  }
  ...
  adjustedDesktopSize(): readonly [number, number] {
    ...
    if (width === 0 || height === 0) throw new Error('Buriko native display division by zero');
  ...
  refreshPointerStep(): void {
    ...
    if (logicalWidth === 0 || logicalHeight === 0 || width === 0 || height === 0)
      throw new Error('Buriko native pointer tolerance division by zero');
    this.pointerStepX =
      width > logicalWidth
