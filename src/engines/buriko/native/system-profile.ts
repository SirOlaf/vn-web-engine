...
export interface BurikoWindowsVersion {
  major: number;
  minor: number;
  build: number;
  platform: number;
...
export interface BurikoPhysicalMemoryStatus {
  total: bigint;
  available: bigint;
}
/**
 ...
 */
export interface BurikoSystemProfileHost {
  readUserName(): Uint8Array | null;
  readComputerName(): Uint8Array | null;
  readVersion(): BurikoWindowsVersion | null;
  readLegacyPhysicalMemory(): BurikoPhysicalMemoryStatus | null;
  readPhysicalMemory(): BurikoPhysicalMemoryStatus | null;
}

function ansiResult(bytes: Uint8Array | null, capacity: number): Uint8Array | null {
  if (bytes === null) return null;
  if (bytes.length >= capacity || bytes.includes(0))
    throw new RangeError('Buriko system host returned an invalid ANSI profile field');
...
export class BurikoSystemProfile {
  private initializedVersion = false;
  private readonly versionRecord = new Uint8Array(148);

  constructor(readonly host: BurikoSystemProfileHost) {}
