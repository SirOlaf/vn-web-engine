import {BurikoDisplayAdapters, type BurikoAdapterDesktopMode} from './display-adapters.js';
import {BurikoDisplayDevice} from './display-device.js';

export interface BurikoDisplayCapabilitiesRecord {
  caps: number;
  caps2: number;
...
export class BurikoDisplayCapabilities {
  ...
  constructor(
    ...
    if (device.adapter !== adapters)
      throw new Error('Buriko capability queries require the actual device adapter owner');
  }
  ...
  readMode(adapter: number, output: BurikoAdapterDesktopMode | null): boolean {
  ...
  readIdentifier(adapter: number, output: Uint8Array): boolean {
    ...
    if (output.length < 0x450)
      throw new RangeError('Buriko adapter identifier output is shorter than the native record');
  ...
  readCapabilities(
    adapter: number,
    deviceType: number,
    output: BurikoDisplayCapabilitiesRecord,
  ): number {
  ...
  checkDeviceType(
    ...
    return this.active &&
      this.adapters.records[adapter >>> 0] !== undefined &&
      deviceType >>> 0 === 1 &&
      displayFormat >>> 0 === 22 &&
      backBufferFormat >>> 0 === 22 &&
      (windowed >>> 0 === 0 || windowed >>> 0 === 1)
      ? 0
      : NOT_AVAILABLE;
  }
  checkDeviceFormat(
    ...
    return this.active &&
      ...
      deviceType >>> 0 === 1 &&
      adapterFormat >>> 0 === 22 &&
      (usage >>> 0 === 0 || usage >>> 0 === 0x200) &&
      resourceType >>> 0 === 3 &&
      (format >>> 0 === 21 || format >>> 0 === 22)
      ? 0
      : NOT_AVAILABLE;
  }
}
