...
function divide(numerator: number, denominator: number): number {
  ...
  if (denominator === 0 || (numerator === -2147483648 && denominator === -1))
    throw new RangeError('Buriko font layout native integer division fault');
  return Math.trunc(numerator / denominator);
}

/** 14003f5c0 copies native coverage into only the intersecting raster dimensions. */
export function rasterBurikoGlyph(
  bitmap: BurikoBitmap,
  raster: BurikoFontRaster,
  character: number,
  color: number,
...
function proportionalGlyph(bitmap: BurikoBitmap, glyph: BurikoGlyph, size: number): number {
  ...
  if (padding === undefined)
    throw new RangeError('Buriko proportional glyph spacing reads outside native stack table');
  return padding;
}

/** Native font rendering through actual browser glyphs and the executable's bitmap compositor. */
export class BurikoBitmapText {
  constructor(
  ...
  draw(
    ...
    if (source === null) throw new Error('Buriko font text decoder dereferences a null source');
    ...
    for (let index = 0; index < text.length; index++) {
      ...
      if (font.raster === null)
        throw new Error('Buriko font drawing reads glyph state after failed native initialization');
      const glyph = rasterBurikoGlyph(scratch, font.raster, character, color);
      const view = {...scratch};
      const padding = proportional === 0 ? 0 : proportionalGlyph(view, glyph, font.size);
      if (proportional === 0)
        view.width = burikoWideCharacter(character) === 0 ? divide(advance, 2) : advance;
      if (
        wrap !== 0 &&
  ...
  drawHex(
    ...
    const font = this.fonts.find(fontId);
    if (font === null) return 0;
    const line = allocateBurikoBitmap(
    ...
    while (rows !== 0) {
      rows = (rows - 1) | 0;
      if (line.format === 0) fillBurikoBitmap16(line, 1);
      else if (line.format === 1) fillBurikoBitmap32(line, 0);
      else if (line.format === 2) fillBurikoBitmap32(line, 0xa0000000);
