...
function addBurikoAlphaIntoRgbPixels(
  ...
  const pixel = (input: number, old: number): number => {
    ...
    for (let shift = 0; shift < 24; shift += 8) {
      const contribution = saturateBurikoByte(
        burikoSignedProduct16((input >>> shift) & 255, coefficient) >> 7,
      );
      result |=
        saturateBurikoByte(((old >>> shift) & 255) + (subtract ? -contribution : contribution)) <<
        shift;
    }
    return result >>> 0;
  };
  ...
  const pixel = (input: number, old: number): number => {
    ...
    for (let shift = 0; shift < 32; shift += 8) {
      ...
      const contribution = premultiplied
        ...
        : burikoSignedProduct16(channel, shift === 24 ? alphaCoefficient : rgbCoefficient) >> 7;
      result |=
        saturateBurikoByte(((old >>> shift) & 255) + saturateBurikoByte(contribution)) << shift;
    }
    return result >>> 0;
  };
  visitBurikoPixelPairs(
...
function multiplyBurikoRgbPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  opacity: number,
): void {
  ...
  const pixel = (input: number, old: number): number => {
    ...
    for (let shift = 0; shift < 24; shift += 8) {
      ...
      const difference = burikoSignedProduct16(((input >>> shift) & 255) - 256, coefficient);
      result |= saturateBurikoByte(target + (Math.imul(difference, target * 2) >> 16)) << shift;
    }
    return result >>> 0;
  };
  visitBurikoPixelPairs(
...
function multiplyBurikoRgbProductPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  opacity: number,
  ...
  const pixel = (input: number, old: number): number => {
    ...
    for (let shift = 0; shift < 24; shift += 8) {
      ...
      result |=
        saturateBurikoByte(target + (burikoSignedProduct16(product - target, coefficient) >> 7)) <<
        shift;
    }
    return result >>> 0;
  };
...
function multiplyBurikoMaskPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  opacity: number,
): void {
...
function dimBurikoAlphaIntoRgbPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  transparency: number,
): void {
  const pixel = (input: number, old: number): number => {
    const coefficient = burikoAlphaHalfCoefficient(input >>> 24);
    let result = old & 0xff000000;
    for (let shift = 0; shift < 24; shift += 8) {
      const target = (old >>> shift) & 255;
      const dimmed = Math.imul((input >>> shift) & 255, 256 - transparency) >>> 8;
      result |=
        saturateBurikoByte(target + (burikoSignedProduct16(dimmed - target, coefficient) >> 7)) <<
        shift;
    }
    return result >>> 0;
  };
...
export const addBurikoAlphaIntoRgb = withBurikoBitmapText(addBurikoAlphaIntoRgbPixels, {
  opacity: (args) => args[2] / 256,
});

export const addBurikoAlpha = withBurikoBitmapText(addBurikoAlphaPixels, {
  opacity: (args) => args[2] / 256,
});

export const multiplyBurikoRgb = withBurikoBitmapText(multiplyBurikoRgbPixels, {
  opacity: (args) => args[2] / 256,
});

export const multiplyBurikoRgbProduct = withBurikoBitmapText(multiplyBurikoRgbProductPixels, {
  opacity: (args) => args[2] / 256,
});

export const multiplyBurikoMask = withBurikoBitmapText(multiplyBurikoMaskPixels, {
  opacity: (args) => args[2] / 256,
});

export const dimBurikoAlphaIntoRgb = withBurikoBitmapText(dimBurikoAlphaIntoRgbPixels, {
  opacity: (args) => (256 - args[2]) / 256,
});

export const dimBurikoAlpha = withBurikoBitmapText(dimBurikoAlphaPixels, {
  opacity: (args) => (256 - args[2]) / 256,
});
