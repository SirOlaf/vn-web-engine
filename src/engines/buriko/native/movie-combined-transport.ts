...
import {BurikoMovieVideoPump, type BurikoMovieVideoPumpResult} from './movie-video-pump.js';
import {BurikoMovieReceivePin} from './movie-receive.js';
import {BurikoMovieFilterEvents, type BurikoMovieFilterEvent} from './movie-filter-events.js';
import {BurikoMovieRenderer} from './movie-renderer.js';
...
export interface BurikoCombinedMovieEvent extends BurikoMovieFilterEvent {
 ...
 */
export class BurikoMovieCombinedTransport {
  private readonly events: BurikoMovieFilterEvents;
  private readonly pin: BurikoMovieReceivePin;
  private readonly audioToken = {};
  private pump: BurikoMovieVideoPump | null = null;
  private readonly creationAbort = new AbortController();
  private readonly removers: Array<() => void> = [];
  private readonly eventObservers = new Set<(event: BurikoCombinedMovieEvent) => void>();
  private readonly failureObservers = new Set<(error: unknown) => void>();
  private completion: BurikoCombinedMovieEvent | null = null;
  ...
  private constructor(
    private readonly audio: BurikoMovieAudioTransport,
    messages: BurikoWindowMessages,
    renderer: BurikoMovieRenderer | BurikoTraditionalMovieRenderer,
    generation: number,
  ) {
    this.generation = generation;
    this.events = new BurikoMovieFilterEvents(messages);
    this.pin = new BurikoMovieReceivePin(renderer, this.events, audio.clock);
  ...
  static async create(
    ...
    signal?: AbortSignal,
  ): Promise<BurikoMovieCombinedTransport> {
    if (
      !(audio instanceof BurikoMovieAudioTransport) ||
      !audio.usesMovie(movie) ||
      !movie.tracks.includes(track) ||
      !(messages instanceof BurikoWindowMessages) ||
      (!(renderer instanceof BurikoMovieRenderer) &&
        !(renderer instanceof BurikoTraditionalMovieRenderer))
    )
    ...
    BurikoMovieCombinedTransport.requireFreshAudio(status, status.generation, seek);
    const owner = new BurikoMovieCombinedTransport(audio, messages, renderer, status.generation);
    const abort = (): void => {
      owner.terminal = true;
    ...
    try {
      owner.check();
      owner.pump = await BurikoMovieVideoPump.create(
        audio,
        owner.pin,
        movie,
        track,
      ...
      owner.check();
      BurikoMovieCombinedTransport.requireFreshAudio(
  ...
  private static requireFreshAudio(
    status: BurikoMoviePcmStatus | null,
    generation: number,
  ...
  private observeAudioCompletion(status: BurikoMoviePcmStatus): void {
    if (
      !this.terminal &&
  ...
  private actualPump(): BurikoMovieVideoPump {
  ...
  get effectiveStop(): BurikoIsoRational {
    return this.audio.effectiveStop;
  }
  onEvent(observer: (event: BurikoCombinedMovieEvent) => void): () => void {
  ...
  private drainEvents(): void {
    for (;;) {
      ...
      const value: BurikoCombinedMovieEvent = {...event, generation: this.generation};
  ...
  private acceptResult(result: BurikoMovieVideoPumpResult): void {
  ...
  seek(position: BurikoIsoRational, resume = this.desiredRunning): Promise<void> {
    this.check();
