...
import {BurikoDiskImagePixels, type BurikoGdiLockedPixels} from './disk-image-pixels.js';
...
export class BurikoDiskImageService {
  constructor(
    readonly pixels: BurikoDiskImagePixels,
    readonly files: BurikoProgramFiles,
    readonly text: BurikoNativeText,
    readonly codec: WindowsGdiImageCodecHost,
  ) {}

  private filename(pointer: BurikoBpPointer): string {
    const name = this.text.decodeAuto(pointer);
    if (name.length > 783)
      throw new RangeError('Buriko disk image filename exceeds native wide scratch');
    return name;
  }

  /** Returns the exact lower status; the native wrapper maps it to script values. */
  async import(index: number, filename: BurikoBpPointer, mode: number): Promise<number> {
    ...
    try {
      ...
      if (locked.width !== image.width || locked.height !== image.height)
        throw new Error('Buriko GDI+ LockBits changed image dimensions');
      return this.pixels.importLocked(index, selected, locked as BurikoGdiLockedPixels) === 0
        ? 0xffffffff
        : 0;
    } finally {
      image.dispose();
  ...
  async export(
    ...
    if (encoder === undefined) throw new Error('Buriko GDI+ save has no initialized encoder CLSID');
