...
function required(value: BurikoBpPointer | null): BurikoBpPointer {
  if (value === null) throw new Error('Buriko font service dereferences a null native string');
  return value;
}
function nullableBytes(value: BurikoBpPointer | null): Uint8Array | null {
  return value === null ? null : textBytes(value);
}

/** Name registration uses the graph font registry without loading host font resources. */
export function createGroupB0FontNames(fonts: BurikoNativeFonts): BurikoNativeSlotDefinition[] {
  return [
    {
      primary: 0xb0,
      secondary: 0xc0,
...
export function createGroupB0Fonts(
  ...
  const enumerate = async (h: BurikoBpOpcodeContext, charset: number): Promise<0> => {
