...
import type {BurikoBpScheduledThread, BurikoBpScheduler} from '../bp/scheduler.js';
...
import type {BurikoNativeSlotDefinition, BurikoBpOpcodeHandler} from './types.js';

/** Persistent executable globals; these are not reset when a module/thread finishes. */
export class BurikoVmControlState {
  ...
  return [
    {
      ...
      execute: ({thread}) => {
        ...
        if (node === null || node.state !== thread)
          throw new Error('Buriko wait thread is not linked to its root');
        node.installProcess(
          new BurikoWaitTimingEx(
            thread,
            procedures,
            clock,
            input,
...
export function createGroup80Threads(
  ...
  procedures: BurikoProcedureState,
  windowMessages: BurikoWindowMessages,
  dialogs: BurikoDiagnosticDialogs,
): BurikoNativeSlotDefinition[] {
  const node = (thread: BurikoBpThread): BurikoBpScheduledThread => {
    ...
    if (found === null || found.state !== thread)
      throw new Error('Buriko current thread is not linked to its root');
    return found;
  };
  const target = (id: number): BurikoBpScheduledThread => {
    const found = scheduler.findById(id);
    if (found === null) throw new Error(`Buriko message target thread ${id >>> 0} does not exist`);
    return found;
  };
  const remaining = (thread: BurikoBpThread): number =>
    Math.max(0, (node(thread).deadline - Number(BigInt.asUintN(32, clock.read()))) | 0);
  const handlers: readonly [number, number, string, BurikoBpOpcodeHandler][] = [
    [
      0x41,
      0x1400e8db0,
      'UnloadLastModule',
      ({thread}) => {
        ...
        if (count === 0 && thread.storageOwner === thread)
          throw new Error('Buriko cannot unload the final module of a root-owned thread');
        push32(thread, count);
        return 0;
      },
    ],
    ...
    [
      ...
      ({thread, memory}) => {
        ...
        if (count < 1) throw new RangeError(`Buriko thread message count ${count} is not positive`);
    ...
    [
      ...
      ({thread}) => {
        ...
        node(thread).installProcess(
          new BurikoWaitWindowMessage(thread, procedures, clock, windowMessages, dialogs, message),
        );
        return 2;
      },
    ],
    ...
    [
      ...
      ({thread}) => {
        ...
        if (duration !== 0)
          node(thread).installProcess(new BurikoWaitTiming(thread, procedures, clock, duration));
