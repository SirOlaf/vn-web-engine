...
function copiedRecordName(
  ...
  if (name.length + 1 > allocationSize)
    throw new RangeError('Buriko font name writes outside native record allocation');
  ...
  if (end < 0 || end >= 96 + fields.length * 4)
    throw new Error('Buriko font name reads undefined native record fields');
  return storage.slice(0, end);
}

/** CFontManager and the executable's raw-name/alias registries share this title-local owner. */
export class BurikoNativeFonts {
  readonly rasterSettings = new BurikoFontRasterSettings();
  readonly records: BurikoFontRecord[] = [];
  ...
  constructor(
    ...
    readonly browser: BurikoFontProvider = new BurikoBrowserFonts(),
  ) {}
  ...
  private async initialize(
    ...
    const valid = validateBurikoFont(input.length, record.size, record.widthPercent, false);
    if (valid !== 0) return valid;
    const transform = initialAttempt
      ? (this.transforms.find((entry) => equal(entry.name, record.name))?.value ?? null)
      : null;
    const geometry = burikoFontGeometry(
      record.size,
      record.widthPercent,
      transform,
      this.rasterSettings,
    ...
    if (
      ...
      if (fallbackPath.some((name) => equal(name, fallback)) || equal(input, fallback))
        throw new Error('Buriko native font alias fallback recurses indefinitely');
      return this.initialize(record, fallback, false, [...fallbackPath, input]);
    }
    record.raster = new BurikoFontRaster(geometry, face, this.rasterSettings, capacity);
  ...
  async get(
    ...
    if (input === null) {
      if (
        ...
        throw new Error('Buriko font lookup compares a null native name');
  ...
  async createTransient(
    ...
    bold: number,
    italic: number,
  ): Promise<{result: number; record: BurikoFontRecord | null}> {
    const name = rawName(input);
    const record: BurikoFontRecord = {
      id: 0,
      name,
  ...
  releaseTransient(record: BurikoFontRecord): void {
    if (record.id !== 0)
      throw new Error('Buriko font manager releases a published record as transient');
    record.raster?.clear();
    record.raster = null;
  }

  find(id: number): BurikoFontRecord | null {
  ...
  private async replaceRaster(record: BurikoFontRecord): Promise<void> {
    record.raster?.clear();
    record.raster = null;
    record.field44 = 0;
    record.field48 = 0;
  ...
  async setTransform(
    ...
    transform: BurikoFontTransform,
  ): Promise<number> {
    const result = validateBurikoFontTransform(transform);
  ...
  setExtra(input: Uint8Array, value: number): number {
    const result = validateBurikoFontTransform([0, 0, 0, 0, value]);
    if (result !== 0) return result;
