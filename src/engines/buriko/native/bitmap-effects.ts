...
function intersectDimensions(
  ...
  source: BurikoBitmap,
  wordRows: boolean,
): readonly [BurikoBitmap, BurikoBitmap] {
...
function screenBurikoBitmapPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  opacity: number,
  ...
  const pixel = (sourcePixel: number, destinationPixel: number): number => {
    ...
    for (let shift = 0; shift < 32; shift += 8) {
      ...
      result |= saturateBurikoByte(channel + old - (Math.imul(channel, old) >>> 8)) << shift;
    }
    return result >>> 0;
  };
  visitBurikoPixelPairs(
    ...
    (pixels, offset) => {
      ...
      const old = readBurikoPixelPair(target, offset);
      writeBurikoPixelPair(target, offset, [pixel(pixels[0], old[0]), pixel(pixels[1], old[1])]);
    },
    (value, offset) => {
 ...
 */
function overlayBurikoBitmapPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  opacity: number,
  sourceHasAlpha: boolean,
  ...
  const pixel = (sourcePixel: number, destinationPixel: number): number => {
    ...
    for (let shift = 0; shift < 32; shift += 8) {
      ...
      result |= saturateBurikoByte(channel) << shift;
    }
    return result >>> 0;
  };
  visitBurikoPixelPairs(
...
function eraseBurikoAlphaFromRgbPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  opacity: number,
): void {
...
function eraseBurikoAlphaPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  opacity: number,
): void {
  ...
  visitBurikoPixelPairs(
    ...
    (pixels, offset) => {
      if (pixels[0] >>> 24 === 0 && pixels[1] >>> 24 === 0) return;
      const old = readBurikoPixelPair(target, offset);
      writeBurikoPixelPair(target, offset, [
...
function eraseBurikoAlphaFromMaskPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  opacity: number,
): void {
  ...
  for (let y = 0; y < input.height >>> 0; y++) {
    ...
    let x = 0;
    for (; x + 1 < width; x += 2) {
      const pixels = readBurikoPixelPair(input, sourceRow + x * 4);
...
function tintBurikoBitmap32Pixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  color: number,
  opacity: number,
  ...
  const [target, input] = intersectDimensions(destination, source, false);
  writeBurikoMappedPairs(target, input, (value) => {
...
function dimBurikoRgbPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  transparency: number,
): void {
  writeBurikoMappedPairs(destination, source, (value) => {
    let result = 0;
    for (let shift = 0; shift < 24; shift += 8)
...
export const screenBurikoBitmap = withBurikoBitmapText(screenBurikoBitmapPixels, {
  opacity: (args) => args[2] / 256,
});

export const overlayBurikoBitmap = withBurikoBitmapText(overlayBurikoBitmapPixels, {
  opacity: (args) => args[2] / 256,
});

export const eraseBurikoAlphaFromRgb = withBurikoBitmapText(eraseBurikoAlphaFromRgbPixels, {
  source: 0,
  replace: true,
});

export const eraseBurikoAlpha = withBurikoBitmapText(eraseBurikoAlphaPixels, {
  source: 0,
  replace: true,
});

export const eraseBurikoAlphaFromMask = withBurikoBitmapText(eraseBurikoAlphaFromMaskPixels, {
  source: 0,
  replace: true,
});

export const tintBurikoBitmap32 = withBurikoBitmapText(tintBurikoBitmap32Pixels, {
  replace: true,
  color: (color, args) => {
    let output = 0;
    for (let shift = 0; shift < 24; shift += 8) {
