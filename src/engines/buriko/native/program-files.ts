...
export type BurikoDriveTypeHost = WindowsDriveTypeHost;

/** A concrete selected 26-drive profile; no drive type is inferred from mounted paths. */
export class BurikoDriveTypeProfile implements BurikoDriveTypeHost {
  private readonly types: Uint32Array;
  constructor(types: ArrayLike<number>) {
    if (types.length !== 26) throw new RangeError('Buriko drive profile requires 26 entries');
  ...
  readDriveType(root: string): number {
    ...
    if (
      ...
      throw new RangeError(
        'Buriko drive profile requires an uppercase root from A:\\ through Z:\\',
      );
...
export interface BurikoDriveGeometryHost {
...
export class BurikoDriveGeometryProfile implements BurikoDriveGeometryHost {
  private readonly sectors: readonly (number | null)[];
  constructor(sectors: ArrayLike<number | null>) {
    if (sectors.length !== 26)
      throw new RangeError('Buriko drive-geometry profile requires 26 entries');
    this.sectors = Array.from(sectors, (value) => {
      if (value === null) return null;
      if (!Number.isInteger(value) || value <= 0 || value > 0xffffffff)
        throw new RangeError(
          'Buriko drive-geometry values must be positive unsigned 32-bit integers',
        );
      return value >>> 0;
    });
  }
  readBytesPerSector(root: string): number | null {
    ...
    if (
      ...
      throw new RangeError(
        'Buriko drive-geometry profile requires a lowercase root from a:\\ through z:\\',
      );
...
export interface BurikoDiskFreeSpaceHost {
...
export class BurikoDiskFreeSpaceProfile implements BurikoDiskFreeSpaceHost {
  ...
  constructor(entries: Iterable<readonly [string, bigint | null]>) {
    for (const [path, value] of entries) {
      if (value !== null && (value < 0n || value > 0xffffffffffffffffn))
        throw new RangeError('Buriko disk-free-space values must be unsigned 64-bit integers');
...
export class BurikoProgramMedia {
  ...
  setDriveType(index: number, type: number): void {
    if (!Number.isInteger(index) || index < 0 || index >= 26)
      throw new RangeError('Invalid Buriko drive index');
  ...
  refreshDriveTypes(host: BurikoDriveTypeHost): void {
  ...
  writeDriveClassifications(host: BurikoDriveTypeHost, output: BurikoBpPointer | null): number {
...
export class BurikoProgramOutputFile {
  ...
  static async create(
  ...
  ): Promise<BurikoProgramOutputFile | null> {
    ...
    try {
      ...
      if (preserveExisting && files instanceof BurikoMountedFileMetadata)
    ...
    const output = new BurikoProgramOutputFile(files, path);
  ...
  async write(bytes: Uint8Array): Promise<number> {
    ...
    if (end > BigInt(Number.MAX_SAFE_INTEGER))
      throw new RangeError(
        'Buriko output file position exceeds the mounted byte-source numerical domain',
      );
...
export class BurikoProgramFiles {
  /** The composition owner shares this service with E0 and the native system-file wrappers. */
  specialFolders: BurikoSpecialFolders | null = null;
  readonly metadata: BurikoMountedFileMetadata | null;
  constructor(
    private readonly files: FileSystem,
    readonly text: BurikoNativeText,
    readonly media: BurikoProgramMedia,
    readonly paths: BurikoMountedProgramPaths | null = null,
  ) {
    this.metadata = files instanceof BurikoMountedFileMetadata ? files : null;
  }
  ...
  async readDriveFile(
    ...
    outputSize: BurikoBpPointer | null,
    path: Uint8Array,
    ...
    if (roundedLength === 0)
      throw new RangeError('Buriko drive-file sector doubling cannot progress from zero');
    ...
    while (roundedLength < requestedLength) {
      ...
      if (doubled <= roundedLength)
        throw new RangeError('Buriko drive-file sector doubling overflows its DWORD');
  ...
  async open(bytes: Uint8Array): Promise<BurikoOpenResult> {
    return this.openWide(this.path(bytes));
  }

  async openWide(path: string): Promise<BurikoOpenResult> {
    try {
  ...
  async createOutput(
    ...
    if (wide.length > 783)
      throw new RangeError('Buriko output file path overwrites its native wide stack buffer');
    try {
      return BurikoProgramOutputFile.create(
        this.files,
