...
import {BurikoLoadSoundProcess} from './load-sound-process.js';
import type {BurikoProcedureState} from './procedure.js';
import {BurikoRegisterSoundProcess} from './register-sound-process.js';
...
export function createGroupA0SoundProcesses(
  ...
  const legacy = worker.audio.staticResources.channels.context.abi.compatibility === '1.69';
  scheduler.attachSharedLoaderWorker(worker);
  const install = (
    context: BurikoBpOpcodeContext,
    process: BurikoLoadSoundProcess | BurikoRegisterSoundProcess,
  ): 2 => {
    const node =
    ...
    if (node === null || node.state !== context.thread)
      throw new Error('Buriko sound process thread is not linked to its scheduler');
  ...
  const validateChannel = (
    ...
    channel >>> 0 >= worker.audio.staticResources.channels.staticCapacity
  ...
  const resourceSlot = (
    ...
    execute: async (context): Promise<2> => {
      const legacy = context.memory.abi.compatibility === '1.69';
      const speed =
        kind === 'variable'
          ...
          : kind === 'zero'
            ...
            : kind === 'double'
              ? 2
              : 1;
      const gain = kind === 'zero' ? (legacy ? 1 : 0) : (pop32(context.thread) | 0) / 65536;
      ...
      const process = await BurikoLoadSoundProcess.create(
        context,
        procedures,
        clock,
        worker.loading,
  ...
  return [
    resourceSlot(
      0x20,
      0x1400e56b0,
      legacy ? 'LoadStaticSoundDefault' : 'LoadStaticSoundZeroSpeed',
      'zero',
    ),
    ...
    resourceSlot(
      0x23,
      0x1400e54c0,
      legacy ? 'LoadStaticSoundAlternatingBlocks' : 'LoadStaticSoundDoubleSpeed',
      'double',
    ),
    resourceSlot(0x27, 0x1400e52a0, 'LoadStaticSoundVariableSpeed', 'variable'),
    {
      ...
      execute: (context) => {
        ...
        return install(
          context,
          new BurikoRegisterSoundProcess(
            context,
            procedures,
            clock,
            worker.loading,
