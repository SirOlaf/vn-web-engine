...
import {burikoDisplayScaleSize, burikoDisplayTransformRectangle} from './display-geometry.js';
...
export class BurikoInlineTextControl {
  readonly state = new BurikoInlineTextState();
  ...
  async create(
    ...
    this.show(1);
    const [left, top, right, bottom] = burikoDisplayTransformRectangle(this.host.display, [
      x,
      y,
      (x + width) | 0,
      (y + height) | 0,
    ...
    const [fontWidth, fontHeight] = burikoDisplayScaleSize(
      this.host.display,
      logicalFontWidth,
      size,
    );
  ...
  read(output: BurikoBpPointer | null): number {
