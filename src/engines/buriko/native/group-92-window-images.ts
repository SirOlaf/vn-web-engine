...
export function createGroup92WindowImages(
  ...
  const invalidate = (object: BurikoWindowDisplayObject): void => {
    if (object.inputActive() !== 0) object.invalidate();
  };
  const draw = (h: BurikoBpOpcodeContext, text: boolean): 0 | Promise<never> => {
    ...
    throw new Error('Buriko Window image facade reads an unwritten native status');
  };
