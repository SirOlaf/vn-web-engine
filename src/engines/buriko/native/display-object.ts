...
import {BurikoDisplayDamage, burikoRectangleContained} from './display-damage.js';
import {BurikoMemoryDx} from './memory-dx.js';

export type BurikoDisplayPoint = {x: number; y: number};
export type BurikoDisplayCoordinates = {x: number; y: number; z: number};
export interface BurikoDisplayContext {
  bitmap: BurikoBitmap;
  bounds: BurikoBitmapRectangle;
}
/** Shared native globals used by CDspObj, including the independent 1d1d20 point. */
export class BurikoDisplayObjectEnvironment {
  readonly origin = {x: 0, y: 0};
  /** Native 0871a0 returns a zero descriptor while the display-manager global is null. */
  displayContext: BurikoDisplayContext | null = null;
  displayBitmap(): BurikoBitmap {
...
export class BurikoDisplayObject {
  ...
  sortBias = 0;
  sortMode = 0;
  readonly memory = new BurikoMemoryDx();
  ...
  hitEnabled = 1;
  parent: BurikoDisplayObject | null = null;
  ...
  protected hitMask: BurikoBitmapStorage | null = null;
  ...
  protected check(): void {
    if (this.disposed) throw new Error('Buriko accesses a deleted CDspObj');
  }
  ...
  diagnosticPropertyWord(offset: number): number {
    ...
    switch (offset) {
      ...
      default:
        throw new Error('Buriko object diagnostic references an unsupported field');
    }
  }
  ...
  getOffset(): BurikoDisplayPoint {
    this.check();
    return {...this.offset};
  }
  /** 055df0 returns the stored +50 point. */
  getSecondaryOffset(): BurikoDisplayPoint {
    this.check();
  ...
  *children(): IterableIterator<BurikoDisplayObject> {
    this.check();
  ...
  draw(_destination: BurikoBitmap, _rectangle: BurikoBitmapRectangle, _key: number): void {
    this.check();
  }
  sortKey(): number {
    this.check();
  ...
  localRectangle(): BurikoBitmapRectangle {
    this.check();
    return {
      left: 0,
      top: 0,
  ...
  inputRectangle(_reference: 0 | BurikoBitmapRectangle): BurikoBitmapRectangle {
  ...
  effectivePosition(): BurikoDisplayPoint {
    this.check();
  ...
  coordinates(): BurikoDisplayCoordinates {
    this.check();
  ...
  getProperty(selector: number, output: BurikoDisplayPropertyDestination): number {
    this.check();
  ...
  setSecondaryVisibility(value: number): void {
    ...
    if (this.propagateSecondaryVisibility !== 0)
      for (const child of this.children())
        BurikoDisplayObject.prototype.setSecondaryVisibility.call(child, value);
  }
  ...
  setTransparency(value: number): void {
    ...
    for (const child of this.children())
      BurikoDisplayObject.prototype.setTransparency.call(child, value);
  }
  setOpacityScale(value: number): void {
    this.check();
    this.opacityScale = value | 0;
    for (const child of this.children())
      BurikoDisplayObject.prototype.setOpacityScale.call(child, value);
  }
  ...
  addChild(object: BurikoDisplayObject, x: number, y: number): 0 | 1 {
    this.check();
    object.check();
  ...
  removeChild(object: BurikoDisplayObject): 0 | 1 {
    this.check();
  ...
  updateChildOffset(object: BurikoDisplayObject): boolean {
    this.check();
    for (let child = this.firstChild; child !== null; child = child.next)
      if (child.object === object) {
  ...
  setHitMask(source: BurikoBitmap | null): void {
    this.check();
    ...
    if (source !== null) {
      ...
      this.hitMask = new BurikoBitmapStorage(
  ...
  drawClipped(
    ...
    if (!intersectBurikoBitmapRectangle(rectangle, selected.bounds)) return 0;
    const contained = burikoRectangleContained(rectangle, clip) ? 1 : 0;
    if (intersectBurikoBitmapRectangle(rectangle, clip)) {
      const destination = {...selected.bitmap};
      if (alternate !== null)
        translateBurikoBitmapRectangle(rectangle, -alternate.bounds.left, -alternate.bounds.top);
