...
export class BurikoScopedLock {
  private readonly section: BurikoDisplayCriticalSection | null;
  private count = 0;
  private disposed = false;
  constructor(
    private readonly currentActor: () => object,
    private readonly parent: BurikoScopedLock | null = null,
  ) {
    if (parent === null)
      this.section = new BurikoDisplayCriticalSection(currentActor); // 094c30.
    else {
      this.section = null;
      parent.enter();
    } // 094b30.
  ...
  private check(): void {
    if (this.disposed) throw new Error('Buriko DCLock is no longer available');
  }
  get depth(): number {
    return this.count;
  }
  ...
  scope(): BurikoScopedLock {
    this.check();
    return new BurikoScopedLock(this.currentActor, this);
  }
