import {burikoBitmapTextCompositor, withBurikoBitmapText} from './bitmap-dom-text.js';
import {
...
import {
  ...
  runBurikoBitmapRectangleOperation,
  ...
  for (let y = 0; y < destination.height >>> 0; y++) {
    ...
    for (let x = 0; x < destination.width >>> 0; x++) {
      ...
      for (let channel = 0; channel < 3; channel++) {
        ...
        pixel |=
          saturateBurikoByte(target + (burikoSignedProduct16(value - target, coefficient) >> 7)) <<
          bits;
      }
...
function blurPass(
  ...
  const pixel = (sum: readonly number[], start: number): number => {
    let value = 0;
    for (let i = 0; i < 4; i++)
      value |= saturateBurikoByte(Math.imul(sum[start + i]!, reciprocal) >>> 16) << (i * 8);
    return value >>> 0;
  };
  if (!vertical) {
...
function applyBurikoEffectorBlurPixels(
  compositor: BurikoBitmapCompositor,
  destination: BurikoBitmap,
  source: BurikoBitmap,
  selector: number,
  ...
  const temporary = allocateBurikoBitmap(destination.width, destination.height, destination.format);
...
function vectorPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  primary: BurikoBitmap,
  secondary: BurikoBitmap | null,
  level: number,
  bilinear: boolean,
    ...
    for (let x = 0; x < destination.width >>> 0; x += 2) {
      ...
      const pixels = samples.map((values, i) => {
        ...
        for (let bits = 0; bits < 32; bits += 8) {
          ...
          result |= saturateBurikoByte(value) << bits;
        }
        return result >>> 0;
      });
      for (let i = 0; i < count; i++)
...
function applyBurikoEffectorVectorMapPixels(
  compositor: BurikoBitmapCompositor,
  destination: BurikoBitmap,
  source: BurikoBitmap,
  primary: BurikoBitmap,
  ...
  const bounds = burikoBitmapRectangle(source);
...
export function clearBurikoEffectorBuffer(bitmap: BurikoBitmap): void {
  clearBurikoBitmap(bitmap);
}

export const applyBurikoFilterMaskColor = withBurikoBitmapText(applyBurikoFilterMaskColorPixels, {
  source: [0, 2],
  replace: true,
  applied: (_, args) => args[0].format === 1,
});

export const applyBurikoEffectorBlur = withBurikoBitmapText(applyBurikoEffectorBlurPixels, {
  alternateArgs: (args) => {
    const alternate = [...args] as Parameters<typeof applyBurikoEffectorBlurPixels>;
    alternate[0] = burikoBitmapTextCompositor(args[0]);
    return alternate;
  },
  destination: 1,
  source: 2,
...
export const applyBurikoEffectorVectorMap = withBurikoBitmapText(
  applyBurikoEffectorVectorMapPixels,
  {
    alternateArgs: (args) => {
      const alternate = [...args] as Parameters<typeof applyBurikoEffectorVectorMapPixels>;
      alternate[0] = burikoBitmapTextCompositor(args[0]);
      return alternate;
    },
    destination: 1,
