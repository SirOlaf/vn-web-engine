...
export interface BurikoArchiveResource {
...
export class BurikoProgramArchives {
  private root = node();
  ...
  private async openIndex(target: OrdinaryArchive, path: Uint8Array): Promise<boolean> {
    ...
    if (stored.length !== target.count * recordSize) {
      throw new BurikoUndefinedResourceRead(
        'Buriko archive index includes unwritten allocation bytes',
      );
    }
  ...
  async registerComplex(
    logical: BurikoBpPointer | null,
    components: readonly (BurikoBpPointer | null)[],
  ): Promise<0 | 1> {
    return this.serialized(async () => {
      if (logical === null) return 0;
      const path = this.normalize(textBytes(logical, true));
      if (path.length > 780)
        throw new RangeError('Buriko complex logical name exceeds its native path field');
      ...
      for (const pointer of components) {
        ...
        if (converted.length > original.length)
          throw new RangeError(
            'Buriko complex component exceeds its native original-byte allocation',
          );
  ...
  private async match(
    ...
    if (context) {
      ...
      if (prefix.length > original.length)
        throw new RangeError('Buriko complex prefix exceeds its native original-byte allocation');
  ...
  private componentPath(prefix: Uint8Array, component: Uint8Array): Uint8Array {
    ...
    if (path.length > 784)
      throw new RangeError('Buriko complex component path exceeds native scratch storage');
  ...
  async enumerateNames(path: Uint8Array): Promise<Uint8Array[] | null> {
    ...
    if (archive.kind === 'complex')
      throw new BurikoUndefinedResourceRead(
        'Buriko complex index enumeration reads an unwritten inherited count',
      );
    const names: Uint8Array[] = [];
    for (let index = 0; index < archive.count; index++)
  ...
  private async readFrom(
    ...
    if (archive.kind === 'complex') {
      ...
      let result: BurikoArchiveResource = {result: 0x80000010, bytes: null};
      for (const component of archive.components) {
  ...
  async resource(
    ...
    let stored: BurikoArchiveResource;
    try {
