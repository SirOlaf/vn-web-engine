...
import type {BurikoTraditionalMovieAudioPolicy} from './movie-traditional-audio-policy.js';
...
export function createTraditionalMovieSlots(
  session: BurikoBrowserTraditionalMovieSession,
  frames: BurikoDisplayFrames,
  audio: BurikoTraditionalMovieAudioPolicy,
  errors: BurikoEngineErrors,
): BurikoNativeSlotDefinition[] {
  if (session.audio !== audio || session.fullscreen !== frames.fullscreenMovie)
    throw new Error('Buriko traditional movie callbacks require one graph and frame owner');
  const pointer = (h: BurikoBpOpcodeContext) => h.memory.resolve(h.thread, pop32(h.thread));
  const invalidSize = (h: BurikoBpOpcodeContext, width: number, height: number): Promise<never> =>
    errors.threadFatal(
      h.thread,
      h.diagnostics,
  ...
  const executeCreate = async (h: BurikoBpOpcodeContext, withArchive: boolean): Promise<0> => {
