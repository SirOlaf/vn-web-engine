...
export class BurikoMaskedBackdrop extends BurikoBackdrop {
  private first = -1;
  private second = -1;
  private mask = -1;
  ...
  override position(): {x: number; y: number} {
    ...
    if (this.firstPosition === undefined)
      throw new Error('Buriko masked backdrop reads unwritten position');
  ...
  override drawContent(destination: BurikoBitmap, rectangle: BurikoBitmapRectangle): 0 | 1 {
    ...
    if (this.firstPosition === undefined || this.secondPosition === undefined)
      throw new Error('Buriko masked backdrop draws with unwritten positions');
    const firstX = -(this.firstPosition.x + rectangle.left) | 0,
      firstY = -(this.firstPosition.y + rectangle.top) | 0;
    let mode = 0x80,
      fill = false,
      background = 0;
    let second: BurikoBitmap | null = null;
    ...
    else if (fill) fillBurikoBitmap(destination, background);
    if (mask !== null) {
      if (this.maskParameter === undefined)
        throw new Error('Buriko masked backdrop reads unwritten mask parameter');
      cropBurikoBitmap(mask, rectangle);
      transitionBurikoBitmap(
        destination,
        firstX,
        firstY,
        first,
