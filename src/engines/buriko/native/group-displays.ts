import {BurikoDisplayGroup} from './display-group.js';
...
export class BurikoGroupDisplays {
  constructor(readonly manager: BurikoDisplayManager) {}
  private group(handle: number): BurikoDisplayGroup | null {
    const object = this.manager.find('group', handle);
    if (object === null) return null;
    if (!(object instanceof BurikoDisplayGroup))
      throw new Error('Buriko group pool contains another display class');
    return object;
  }
  create(): number {
    return this.manager.createSimple(
      'group',
      (order) => new BurikoDisplayGroup(this.manager.environment, order),
    );
  }
