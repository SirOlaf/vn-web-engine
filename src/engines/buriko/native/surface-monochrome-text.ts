...
export interface BurikoMonochromeTextArguments {
  surface: number;
  x: number;
  y: number;
...
export class BurikoMonochromeSurfaceText {
  private font: BurikoMonochromeFont | null = null;
  ...
  async draw(
    args: BurikoMonochromeTextArguments,
    ...
    if (
      ...
      const candidate = new BurikoMonochromeFont(this.surfaces.fonts);
    ...
    if (font === null) throw new Error('Buriko monochrome font was not initialized');
  ...
  private render(
    ...
    const size = font.size,
      scratch = allocateBurikoBitmap(size, size, 1);
    let metric = 0;
    try {
      ...
      for (;;) {
        ...
        if (!Number.isFinite(character))
          throw new Error('Buriko mono text reads outside prepared UTF16 storage');
        if (character >= 0xd800 && character < 0xdc00) {
          const second = wide.charCodeAt(wideAt + 1);
          if (!Number.isFinite(second))
            throw new Error('Buriko mono text reads outside prepared surrogate storage');
        ...
        if (first < 0x20) {
          ...
          recordBurikoBitmapText(scratch, String.fromCodePoint(character), {
            size,
          ...
          const glyph = {...scratch};
          if (burikoWideCharacter(character) === 0) glyph.width >>>= 1;
