 ...
 */
export function burikoRosettaSseReciprocal(input: number): number {
  floatBits.setFloat32(0, input, true);
  const bits = floatBits.getUint32(0, true);
  const exponent = (bits >>> 23) & 255;
  if (exponent === 0 || exponent === 255)
    throw new RangeError('Buriko reciprocal is outside its normal finite SSE domain');
...
export function burikoRosettaSseReciprocalSqrt(input: number): number {
  floatBits.setFloat32(0, input, true);
  const bits = floatBits.getUint32(0, true);
  const exponentBits = (bits >>> 23) & 255;
  if (bits >>> 31 || exponentBits === 0 || exponentBits === 255)
    throw new RangeError('Buriko reciprocal square root is outside its positive-normal SSE domain');
