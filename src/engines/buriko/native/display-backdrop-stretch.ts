...
export class BurikoStretchBackdrop extends BurikoBackdrop {
  private source = -1;
  ...
  override position(): {x: number; y: number} {
    ...
    if (this.origin === undefined)
      throw new Error('Buriko stretch backdrop reads unwritten Q16 position');
    return {...this.origin};
  }
  ...
  override drawContent(destination: BurikoBitmap, _rectangle: BurikoBitmapRectangle): 0 | 1 {
    ...
    if (this.origin === undefined || this.extent === undefined || this.target === undefined)
      throw new Error('Buriko stretch backdrop draws before base and target window configuration');
