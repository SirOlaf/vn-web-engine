...
import {burikoWindowPositionAllowed} from './browser-main-window.js';
...
export class BurikoMainWindowSizeEffects {
  constructor(readonly graph: BurikoProductionDisplayResourceGraph) {
    if (
      ...
      throw new Error('Buriko size effects require the shared production window/audio owners');
  }
  ...
  async runInitializedMinimize(actor = this.graph.allocator.currentActor): Promise<void> {
    ...
    if (
      ...
      throw new Error('Buriko initialized size effects require live shared channels and window');
  ...
  async runInitializedRestore(
    ...
    if (graph.display.windowMoveImmediate !== 0 && graph.display.fullscreen !== 1) {
      ...
      if (!burikoWindowPositionAllowed(graph.display, x, y)) graph.host.center();
      if (graph.fullscreenMovie.query() === 0) {
        const control = graph.fullscreenMovie.displayControl;
        if (control === null)
          throw new Error(
            'Buriko initialized MF WM_SIZE restore requires a retained display control',
          );
