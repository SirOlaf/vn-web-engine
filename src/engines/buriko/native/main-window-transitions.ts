...
export class BurikoMainWindowTransitions {
  ...
  constructor(
    ...
    if (
      ...
      throw new Error('Buriko main transition requires selected ordered host and dispatch owners');
  ...
  private run(operation: () => Promise<boolean>): Promise<boolean> {
    if (this.closed || this.pending !== null || this.dispatcher.hasPendingDispatch)
      throw new Error('Buriko main transition requires idle live admission');
