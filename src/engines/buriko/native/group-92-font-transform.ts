...
import {validateBurikoFontTransform, type BurikoFontTransform} from './font-raster.js';
...
export function createGroup92FontTransform(
  ...
  const nameBytes = (pointer: BurikoBpPointer | null): Uint8Array => {
    if (pointer === null) throw new Error('Buriko font transform reads a null name');
  ...
  return [
    {
      ...
      execute: async (context): Promise<0> => {
        ...
        const extra = count === 5 ? word(4) : 0,
          ...
          offsetY = word(3),
          scaleY = word(1);
        const transform: BurikoFontTransform = [scaleX, scaleY, offsetX, offsetY, extra];
        // Validation precedes native name traversal, including for null names.
        let status = validateBurikoFontTransform(transform);
