...
import type {BurikoDiskImageService} from './disk-image-service.js';
  ...
  return [
    {
      ...
      execute: async (h): Promise<0> => {
        ...
        if (filename === null) throw new Error('Buriko GDI+ import consumes a null filename');
    ...
    },
    {
      ...
      execute: async (h): Promise<0> => {
        const index = pop32(h.thread),
          ...
          filename = pointer(h);
        if (index >>> 0 >= 0x4000) return invalid(h, index);
        if (filename === null) throw new Error('Buriko GDI+ export consumes a null filename');
