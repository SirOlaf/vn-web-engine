...
function bounded(path: string, capacity = 784, checkDomain = true): string {
  if (path.length >= capacity)
    throw new RangeError('Buriko availability path exceeds native wide scratch');
  if (checkDomain) assertBurikoPathDomain(path);
  return path;
}
...
export class BurikoProgramResources implements BurikoBpModuleResourceSource {
  ...
  constructor(
    ...
    readonly mainProcessing: BurikoDistributedProcessing,
  ) {
    this.archives = new BurikoProgramArchives(files, errors, mainProcessing);
  }
  ...
  async setPrimaryRoot(path: BurikoBpPointer): Promise<0 | 1> {
    ...
    if (original.length + 2 > 784)
      throw new RangeError('Buriko primary root exceeds native byte buffer');
  ...
  async enumerateArchiveNames(
    packedNames: BurikoBpPointer | null,
    outputValue: BurikoBpPointer,
  ...
  async requestMediaRetry(diagnostic: Uint8Array): Promise<void> {
    if (this.configuration.secondaryRoot.length === 0)
      throw new Error('Buriko media retry reads absent secondary-root storage');
  ...
  private async mediaRetryDialog(): Promise<void> {
    ...
    if (answer === 2) {
      if (
        ...
        throw new BurikoNativeExit(0x7fffffff, 'Buriko resource retry cancelled');
      }
    }
  ...
  private async relativePath(root: string, name: BurikoBpPointer): Promise<string | null> {
    ...
    assertBurikoPathDomain(decoded);
    if (decoded.length >= 784)
      throw new RangeError('Buriko relative name exceeds native wide scratch');
  ...
  async findRelativeFile(root: string, name: BurikoBpPointer): Promise<Uint8Array | null> {
  ...
  async locateArchiveEntry(
    ...
    if (physical === null)
      throw new BurikoUndefinedResourceRead(
        'Buriko archive locator consumes unwritten physical path',
      );
    const base = await this.archives.payloadBase(physical);
    if (base === null)
      throw new BurikoUndefinedResourceRead(
        'Buriko archive locator consumes unwritten payload base',
      );
    const view = new DataView(record.buffer, record.byteOffset, record.byteLength);
    view.setUint32(96, (view.getUint32(96, true) + base) >>> 0, true);
    return {path: physical, record};
  }
  async isAvailable(archive: BurikoArchiveName | null, name: BurikoBpPointer): Promise<number> {
    const pointer = name,
      files = this.files,
  ...
  async readModule(
    ...
    if (result.bytes === null) {
      ...
      throw new BurikoUndefinedResourceRead(
        `Buriko module reader consumes archive error 0x${result.result.toString(16)} as a resource`,
      );
    }
    return result.bytes;
  }
