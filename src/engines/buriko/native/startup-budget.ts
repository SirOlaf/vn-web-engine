...
import {burikoStartupLogHigh, burikoStartupLogLow} from './startup-math-tables.js';
...
export function burikoStartupCpuLog(count: number): number {
  count >>>= 0;
  if (count === 0) throw new RangeError('Buriko startup requires a positive native CPU count');
  if (count === 1) return 0;
  ...
  const correction = exponent * 5.7699990475432854e-8 - (first + second),
    tail = burikoStartupLogLow[index]! + correction,
    head = burikoStartupLogHigh[index]! + exponent * 0.6931471228599548;
  return head + tail;
}

const divideUnsigned = (numerator: number, denominator: number): number => {
  denominator >>>= 0;
  if (denominator === 0) throw new RangeError('Buriko startup pixel-budget division by zero');
...
export function burikoDisplayRenderPixelBudget(
  ...
  const logarithm = burikoStartupCpuLog(count) / burikoStartupCpuLog(2);
