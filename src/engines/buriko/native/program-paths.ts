...
export interface BurikoProgramPathMount {
...
export class BurikoMountedProgramPaths {
  ...
  constructor(mounts: readonly BurikoProgramPathMount[], currentDirectory: string) {
    this.mounts = mounts
      .map((mount) => {
        const native = normalized(mount.native).replace(/\\$/, '');
        return {native, folded: burikoRegistryFold(native), mounted: filePath(mount.mounted)};
      })
  ...
  mountSnapshot(): BurikoProgramPathMount[] {
  ...
  resolve(value: string): string {
    const path = this.resolveNative(value),
      folded = burikoRegistryFold(path);
