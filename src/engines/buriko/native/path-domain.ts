...
export function assertBurikoPathDomain(value: string, search = false): void {
  const path = value.replaceAll('/', '\\');
  const unsupported = (): never => {
    throw new Error('Buriko mounted path profile does not support this Windows path form');
  };
