...
import {BurikoResourceCache} from './resource-cache.js';
import {BurikoBitmapPreloadCache} from './bitmap-preload-cache.js';
import {BurikoLoaderMetadata} from './loader-metadata.js';

export interface BurikoResourceBuffer {
  bytes: Uint8Array | null;
  /** Owned-private publication retains the native decoder's defined-byte mask. */
  initialized?: Uint8Array;
}
export interface BurikoResourceResult {
  value: number;
}
interface ResourceJob {
  readonly output: BurikoResourceBuffer | null;
  readonly direct: BurikoBpPointer | null;
...
export class BurikoResourceLoadingState {
  readonly metadata: BurikoLoaderMetadata;
  readonly ranges: BurikoResourceRanges;
  readonly cache: BurikoResourceCache;
  readonly preloaded: BurikoBitmapPreloadCache;
  ...
  constructor(readonly resources: BurikoProgramResources) {
    this.metadata = new BurikoLoaderMetadata(resources.mainProcessing.allocator);
    this.ranges = new BurikoResourceRanges(resources);
    this.cache = new BurikoResourceCache(resources.files.text);
    this.preloaded = new BurikoBitmapPreloadCache(resources.files.text);
  }
  ...
  enqueue(
    output: BurikoResourceBuffer | null,
    direct: BurikoBpPointer | null,
  ...
  processNext(actor = this.resources.mainProcessing.allocator.currentActor): Promise<boolean> {
    ...
    this.pending = Promise.resolve()
      ...
      .then(() => {
        this.metadata.run(actor, () => {
          if (this.jobs[0] !== job)
            throw new Error('Buriko resource loader lost its borrowed head');
          this.jobs.shift();
        });
        return true;
      })
  ...
  discardPendingResources(actor = this.metadata.allocator.currentActor): void {
    if (this.pending !== null)
      throw new Error('Buriko resource loader worker must join before queue shutdown');
    this.metadata.run(actor, () => {
      this.jobs.length = 0;
    });
  }
