...
export class BurikoDisplayMap extends BurikoDisplayObject {
  private cache: BurikoBitmap | null = null;
  ...
  private mapWidth = 0;
  private mapHeight = 0;
  private tileDescriptors: BurikoBitmap[] | null = null;
  private tileSurface = -1;
  private tileImageId = -1;
  private viewEnabled = false;
  private mapX = -1;
  private mapY = -1;
  private view: BurikoBitmapRectangle = {left: -1, top: -1, right: -1, bottom: -1};
  private dirty = false;
  ...
  configureGrid(columns: number, rows: number, tileWidth: number, tileHeight: number): 0 | 2 | 3 {
    ...
    this.cache = allocateBurikoBitmap(
      (columns + 1) * tileWidth,
      (rows + 1) * tileHeight,
      format === 1 ? 2 : format,
    );
    clearBurikoBitmap(this.cache);
    ...
    if (mapX !== this.mapX || mapY !== this.mapY) {
      ...
      for (let y = 0; y <= this.rows; y++, sourceY++) {
        ...
        for (let x = 0; x <= this.columns; x++, sourceX++) {
          ...
          if (word === undefined)
            throw new RangeError('Buriko map reads outside native map allocation');
  ...
  invalidateTile(tile: number): boolean {
    ...
    for (let i = 0; i < this.cachedCells.length; i++, source++) {
      if (this.cachedCells[i] === tile) {
        ...
        if (word === undefined)
          throw new RangeError('Buriko map reads outside native selected-cell allocation');
  ...
  override draw(destination: BurikoBitmap, rectangle: BurikoBitmapRectangle, _key: number): void {
    ...
    cropBurikoBitmap(source, this.view);
