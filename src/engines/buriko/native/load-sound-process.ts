...
export class BurikoLoadSoundProcess extends BurikoLoadProcedure {
  /** Native +694 is unwritten until the first FE330 enqueue. */
  readonly staticResult = {} as BurikoResourceResult;
  private phase = 0;
  ...
  private constructor(
    ...
    queues: BurikoAudioLoaderQueues,
    channel: number,
    fade: number,
    gain: number,
    speed: number,
    ...
    if (queues.loading !== loading)
      throw new Error('Buriko load-sound process must share the actual loading count');
  ...
  static async create(
  ...
  ): Promise<BurikoLoadSoundProcess> {
    if (namePointer === null)
    ...
    return new BurikoLoadSoundProcess(
      context,
      procedures,
      clock,
      loading,
  ...
  private fatal(template: Uint8Array): Promise<never> {
    return this.loading.resources.errors.threadFatal(
      ...
      formatBurikoAudioNames(template, [this.archive, this.name], 256),
    );
  }
}
