...
export class BurikoParticleImages {
  count = 0;
  duration = 0;
  durationSpread = 0;
  readonly scales: Array<BurikoBitmap[] | null> = Array.from({length: 32}, () => null);
  ...
  configure(
    frames: readonly BurikoBitmap[] | null,
    count: number,
    duration: number,
    ...
    if (first === undefined) throw new Error('Buriko particle image descriptor is absent');
    if ((first.format - 1) >>> 0 > 1) return 0x80000003;
    for (let i = 0; i < count; i++) {
      const frame = frames[i];
      if (frame === undefined) throw new Error('Buriko particle image descriptor is absent');
      if (
    ...
    for (let scale = 0; scale < 32; scale++) {
      ...
      const scaled: BurikoBitmap[] = [];
      this.scales[scale] = scaled;
      for (let i = 0; i < count; i++) {
        const destination = allocateBurikoBitmap(width, height, first.format);
        scaled.push(destination);
        if (destination.storage !== null) scaleBurikoBitmap(destination, frames[i]!, factor);
      }
    }
    return 0;
  }
...
export class BurikoParticleVariants {
  readonly snowImages = Array.from({length: 64}, () => new BurikoParticleImages());
  readonly fireflyImages = Array.from({length: 64}, () => new BurikoParticleImages());
  readonly specialFireflyImages = Array.from({length: 64}, () => new BurikoParticleImages());
  ...
  configureSnow(
    ...
    if (values.length !== 10)
      throw new Error('Buriko snow configuration requires ten DWORD values');
  ...
  configureFirefly(
    ...
    if (variant >= 64) return false;
    if (values.length !== 17)
      throw new Error('Buriko firefly configuration requires seventeen DWORD values');
