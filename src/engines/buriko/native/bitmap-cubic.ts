...
import {runBurikoBitmapFloatPointOperation} from './bitmap-operation-jobs.js';
...
function cubicPixels(
  ...
  for (let y = 0; y < destination.height >>> 0; y++) {
    ...
    for (let x = 0; x < destination.width >>> 0; x++) {
      ...
      const seed = total === 0 ? 1 / total : burikoRosettaSseReciprocal(total);
...
function stretchBurikoBitmapCubicPixels(
  ...
  if (
    parallel &&
    runBurikoBitmapFloatPointOperation(
...
export const stretchBurikoBitmapCubic = withBurikoBitmapText(stretchBurikoBitmapCubicPixels, {
  alternateArgs: (args) => {
    const alternate = [...args] as Parameters<typeof stretchBurikoBitmapCubicPixels>;
    alternate[9] = false;
    return alternate;
  },
  destination: 1,
