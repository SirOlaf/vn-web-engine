import type {BurikoProductionDataOwners} from './production-data-owners.js';
...
import type {BurikoInstallationService} from './installation.js';
import {createBurikoCdSlots} from './cd-audio.js';
...
import {createGroup80CursorMotion, BurikoBrowserCursorPosition} from './cursor-motion.js';
...
import {BurikoInstallerModal} from './installer-modal.js';
import {createGroup80InstallerShortcutTransaction} from './group-80-installer-shortcut-transaction.js';
import {BurikoInstallerShortcutTransaction} from './installer-shortcut-transaction.js';
...
export class BurikoProductionNativeFragments {
  constructor(
    readonly graph: BurikoProductionDisplayResourceGraph,
    readonly data: BurikoProductionDataOwners,
    readonly scheduler: BurikoBpScheduler | null = null,
    readonly installation: BurikoInstallationService | null = null,
  ) {
    if (data.graph !== graph)
      throw new Error('Buriko native fragments require the same production graph');
    if (
      ...
      throw new Error('Buriko touch fragments require the selected main-window ingress');
    if (
      (graph.cdMediaHost === null) !== (graph.cdAudio === null) ||
      (graph.cdAudio !== null && graph.receiver.cdAudio !== graph.cdAudio)
    )
      throw new Error('Buriko CD fragments require the selected medium and main receiver');
    if (
      graph.fileAssociations.host !== graph.fileAssociationHost ||
      graph.fileAssociations.text !== graph.text
    )
      throw new Error(
        'Buriko file-association fragments require the graph platform and text owners',
      );
    if (
      ...
      throw new Error('Buriko installer-shortcut fragments require selected graph shell owners');
    if (
      ...
      throw new Error('Buriko process and media fragments require the selected graph hosts');
    if (
      ...
      throw new Error('Buriko DLL fragments require the selected graph host and resources');
    if (
      ...
      throw new Error('Buriko wallpaper fragments require the selected desktop and registry');
    if ((scheduler === null) !== (installation === null))
      throw new Error('Buriko installer fragments require scheduler and service together');
    if (
      installation !== null &&
      ...
      throw new Error('Buriko installer fragments require the shared VM and graph owners');
    if (
      ...
      throw new Error('Buriko system-profile fragments require the selected host owner');
    if (
      ...
      throw new Error(
        'Buriko device-power fragments require selected graph system and file owners',
      );
    if (
      ...
      throw new Error('Buriko gamepad fragments require selected graph input and queue owners');
    if (
      (graph.namedMutexHost === null) !== (graph.namedMutexes === null) ||
      (graph.namedMutexes !== null && graph.namedMutexes.host !== graph.namedMutexHost)
    )
      throw new Error('Buriko named-mutex fragments require the selected host owner');
    if (graph.installerManifest.resources !== graph.resource.resources)
      throw new Error('Buriko installer fragments require the graph resource owner');
    if (
      ...
      throw new Error(
        'Buriko installer queries require the graph registry, folder and file owners',
      );
    if (
      ...
      throw new Error('Buriko shortcut cleanup requires the graph folder, text and file owners');
    if (graph.frames.metrics.clock !== graph.clock || graph.frames.metrics.raster !== graph.device)
      throw new Error('Buriko metric fragments require the graph frame clock and device');
    if (
      ...
      !(graph.cursorPosition instanceof BurikoBrowserCursorPosition) ||
      ...
      throw new Error('Buriko display-toggle fragments require shared controller and error owners');
    if (
      ...
      throw new Error('Buriko synthetic mouse fragments require the main input/message owners');
    if (
      ...
      throw new Error('Buriko hotkey fragments require the main keyboard/message owners');
    if (
      ...
      throw new Error('Buriko file fragments require the graph resource and metadata owners');
    if (
      ...
      throw new Error('Buriko picker fragments require the selected host and graph owners');
    if (
      ...
      throw new Error('Buriko modal fragments require the graph dialog, text and title owners');
    if (
      ...
      throw new Error(
        'Buriko form fragments require the graph DOM, dialog, text and language owners',
      );
    if (
      ...
      throw new Error('Buriko E0 diagnostics require the selected engine caption and input owners');
    if (
      graph.resource.temporaryDirectoryProbe !== null &&
      graph.resource.temporaryDirectoryProbe.files !== graph.resource.files
    )
      throw new Error('Buriko temporary-directory probe requires the graph file owner');
    if (
      ...
      throw new Error('Buriko drive fragments require the shared file, media and host owners');
    if (graph.localized.text !== graph.text || graph.localized.importedMessages.text !== graph.text)
      throw new Error('Buriko imported-text fragments require the graph text owner');
    if (
      ...
      throw new Error('Buriko path fragments require the graph mounted files and text owners');
    if (
      graph.folders.text !== graph.text ||
      ...
      throw new Error(
        'Buriko settings fragments require shared resource, text and registry owners',
      );
    if (
      ...
      throw new Error('Buriko child fragments require the graph window and bitmap owners');
    if (
      ...
      throw new Error('Buriko audio fragments require the graph channel, lock and actor owners');
  }
  ...
  nativeDefinitions(): BurikoNativeSlotDefinition[] {
    const definitions = [
      ...
      ...(this.graph.shellShortcuts === null
        ...
        : createGroup80InstallerShortcutTransaction(
            new BurikoInstallerShortcutTransaction(this.graph.shellShortcuts),
          )),
      ...
      ...(this.graph.cdAudio === null ? [] : createBurikoCdSlots(this.graph.cdAudio)),
    ...
    for (const definition of definitions) {
      ...
      if (
        ...
        throw new Error(`Buriko partial native fragments have an invalid inventory address ${key}`);
      if (keys.has(key)) throw new Error(`Buriko partial native fragments duplicate ${key}`);
      keys.add(key);
    }
    return definitions.map((definition) => ({
      ...
      execute: (context) => {
        if (context.memory !== this.data.memory)
          throw new Error('Buriko native fragments require the aggregate BP memory');
