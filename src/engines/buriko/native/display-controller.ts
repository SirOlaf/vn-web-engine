...
import {BurikoDisplayAdapters} from './display-adapters.js';
import {BurikoBrowserMainWindow, burikoWindowCenteredPosition} from './browser-main-window.js';
import {BurikoCpuProfile} from './cpu-profile.js';
...
import {BurikoNativeExit} from './engine-dialogs.js';
...
export class BurikoDisplayController {
  readonly capabilities: BurikoDisplayCapabilities;
  ...
  constructor(
    ...
    readonly cpu: BurikoCpuProfile,
    readonly ticks: BurikoSystemTicks,
    readonly mouseTrails: BurikoDisplayMouseTrails,
    ...
    if (
      ...
      throw new Error(
        'Buriko display controller requires the shared device, window and adapter owners',
      );
    this.capabilities = new BurikoDisplayCapabilities(device, adapters);
  }
  get display() {
  ...
  async createDevice(fullscreen: number): Promise<number> {
    ...
    throw new Error('Buriko display creation loop exhausted without returning its status');
  }
  ...
  async reconfigure(
    ...
    if (width === undefined || height === undefined)
      throw new RangeError('Buriko reconfiguration reads beyond its logical-mode tables');
    ...
    if ((fullscreen | 0) === 0) {
      ...
      for (;;) {
        ...
        if (forceCreate === 0) forceCreate = 1;
          ...
          throw new BurikoNativeExit(0x7fffffff, 'Buriko display initialization retry declined');
      }
      if (display.windowMoveImmediate === 0) {
        this.adapters.readMonitorOrigin();
        const centered = burikoWindowCenteredPosition(display);
    ...
    const budget = burikoDisplayRenderPixelBudget(this.cpu, display);
  ...
  configureModeToggle(enabled: number, source: BurikoBpPointer | null): 0 | 1 {
