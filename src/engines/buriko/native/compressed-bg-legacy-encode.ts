...
export class BurikoLegacyBgEncoder {
  ...
  encode(output: BurikoBpPointer, count: BurikoBpPointer, source: BurikoBpPointer): number {
    ...
    const read = (bytes: Uint8Array, offset: number): number => {
      ...
      if (value === undefined)
        throw new Error('Buriko legacy BG encoder reads outside owned storage');
      return value;
    };
    const write = (bytes: Uint8Array, offset: number, value: number): void => {
      if (offset < 0 || offset >= bytes.length)
        throw new Error('Buriko legacy BG encoder writes outside owned storage');
      bytes[offset] = value;
    };
