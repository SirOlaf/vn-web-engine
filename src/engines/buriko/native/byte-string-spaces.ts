...
export function normalizeBurikoAsciiSpaces(source: BurikoBpPointer): Uint8Array {
  const input = textBytes(source).slice();
  if (input.length === 0)
    throw new Error('Buriko native space normalization requires a nonempty source record');
  const output: number[] = [];
  let space = false;
