import {BurikoMovieAudioTransport} from './movie-audio-transport.js';
import {BurikoMovieVideoOnlyTimeline} from './movie-video-only-timeline.js';
import {BurikoMovieReceivePin} from './movie-receive.js';
import {BurikoMovieVideoSamples, type BurikoMovieVideoSample} from './movie-video-samples.js';
import type {BurikoMovieVideoOutputLimits} from './movie-video-decoder.js';
import type {BurikoMovieMediaType} from './movie-image.js';
import type {BurikoIsoMovie, BurikoIsoTrack} from './movie-iso-samples.js';
import {
  burikoIsoTime as time,
  burikoIsoReferenceTime,
  type BurikoIsoRational,
} from './movie-iso-timeline.js';
import type {BurikoMoviePcmStatus} from './movie-pcm-protocol.js';

interface BurikoMovieVideoPumpResultBase {
  readonly kind: 'running' | 'paused' | 'delivered' | 'submitted-video-eos' | 'empty-video-eos';
}
export type BurikoMovieVideoPumpResult = BurikoMovieVideoPumpResultBase &
  (
    | {readonly mode: 'audio'; readonly audioGeneration: number}
    | {readonly mode: 'video-only'; readonly videoGeneration: number}
  );
function sameType(left: BurikoMovieMediaType, right: BurikoMovieMediaType): boolean {
  return (
 ...
 */
export class BurikoMovieVideoPump {
  private source: BurikoMovieVideoSamples | null = null;
  private creation: Promise<BurikoMovieVideoSamples> | null = null;
  private prepared: BurikoMovieVideoSample | null = null;
  private sourceEnded = false;
  private eosSubmitted = false;
  private previousStart: bigint | null = null;
  private mediaType: BurikoMovieMediaType | null = null;
  ...
  private stepWork: Promise<BurikoMovieVideoPumpResult> | null = null;
  ...
  private constructor(
    private readonly audio: BurikoMovieAudioTransport | null,
    readonly pin: BurikoMovieReceivePin,
    private readonly movie: BurikoIsoMovie,
    private readonly track: BurikoIsoTrack,
    private readonly limits: BurikoMovieVideoOutputLimits,
    private readonly maxRgb32Bytes: number,
    private generation: number,
    private seekPosition: BurikoIsoRational,
    private readonly timeline: BurikoMovieVideoOnlyTimeline | null = null,
  ) {}

  private static requireUntouchedPin(pin: BurikoMovieReceivePin): void {
    if (
      pin.state !== 0 ||
      pin.connected ||
      pin.flushing ||
  ...
  static async create(
    audio: BurikoMovieAudioTransport,
    pin: BurikoMovieReceivePin,
    movie: BurikoIsoMovie,
    track: BurikoIsoTrack,
    limits: BurikoMovieVideoOutputLimits,
    maxRgb32Bytes: number,
    signal?: AbortSignal,
  ): Promise<BurikoMovieVideoPump> {
    if (
      !(audio instanceof BurikoMovieAudioTransport) ||
      !(pin instanceof BurikoMovieReceivePin) ||
      pin.clock !== audio.clock
    )
      throw new Error('Movie video pump requires the actual shared audio clock and receive pin');
    BurikoMovieVideoPump.requireUntouchedPin(pin);
  ...
  static async createVideoOnly(
    timeline: BurikoMovieVideoOnlyTimeline,
    pin: BurikoMovieReceivePin,
    limits: BurikoMovieVideoOutputLimits,
    maxRgb32Bytes: number,
    signal?: AbortSignal,
    ...
    if (
      !(timeline instanceof BurikoMovieVideoOnlyTimeline) ||
      !(pin instanceof BurikoMovieReceivePin) ||
    ...
    const pump = new BurikoMovieVideoPump(
      null,
      pin,
  ...
  private releaseSample(sample: BurikoMovieVideoSample | null): void {
    try {
      sample?.release();
    } catch (error) {
      this.recordFailure(error);
  ...
  private validateAudio(fresh: boolean, generation = this.generation): BurikoMoviePcmStatus {
  ...
  private result(kind: BurikoMovieVideoPumpResult['kind']): BurikoMovieVideoPumpResult {
  ...
  private async prepareSource(): Promise<void> {
    ...
    const work = BurikoMovieVideoSamples.create(this.movie, this.track, {
      outputLimits: this.limits,
      maxRgb32Bytes: this.maxRgb32Bytes,
      timestampMode: 'absolute',
    });
    this.creation = work;
    let source: BurikoMovieVideoSamples;
    try {
      source = await work;
      this.source = source;
    } finally {
    ...
    try {
      ...
      source.seek(burikoIsoReferenceTime(this.seekPosition));
  ...
  private connectPrepared(): void {
    ...
    this.pin.sourceStopPosition =
      this.timeline === null
        ? burikoIsoReferenceTime(this.audio!.effectiveStop)
  ...
  run(): Promise<BurikoMovieVideoPumpResult> {
    if (this.timeline !== null)
  ...
  runVideoOnly(graphStart: bigint): Promise<BurikoMovieVideoPumpResult> {
    if (this.timeline === null)
  ...
  private start(graphStart: bigint): Promise<BurikoMovieVideoPumpResult> {
    this.check();
  ...
  step(): Promise<BurikoMovieVideoPumpResult> {
    this.check();
  ...
  private async performStep(epoch: number): Promise<BurikoMovieVideoPumpResult> {
    ...
    if (this.prepared === null && !this.sourceEnded) {
      ...
      let sample: BurikoMovieVideoSample | null;
      try {
  ...
  beginSeek(position: BurikoIsoRational): Promise<void> {
    this.check();
    if (this.timeline !== null) {
