...
export function burikoWindowsScanCode(code: string): number {
...
export type BurikoKeyboardEvent = Pick<
  KeyboardEvent,
...
export interface BurikoKeyboardHotkeys {
...
export class BurikoKeyboardMessages {
  hotkeys: BurikoKeyboardHotkeys | null = null;
  ...
  post(target: BurikoWindowTarget, event: BurikoKeyboardEvent): void {
    if (event.type !== 'keydown' && event.type !== 'keyup')
      throw new TypeError('Buriko keyboard translator requires a key transition');
  ...
  private transition(
    ...
    if (!Number.isInteger(key) || key < 0 || key > 255)
      throw new RangeError('Buriko browser virtual key exceeds the native key array');
    ...
    const scan = burikoWindowsScanCode(code);
