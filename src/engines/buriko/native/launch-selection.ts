...
import {copyText, textByte, textBytes, type BurikoNativeText} from './text.js';
...
function launchTokens(bytes: Uint8Array): Uint8Array[] {
  ...
  while (tokens.length < 2) {
    ...
    for (;;) {
      ...
      if (value === 0 || value === 10)
        throw new RangeError('Buriko launch token consumed an unclosed native quote');
      if (token.length >= CAPACITY - 1)
        throw new RangeError('Buriko launch token exceeds its 784-byte native buffer');
      token.push(value);
      offset++;
    }
    if (quoted) {
      if (token.length === 0)
        throw new RangeError('Buriko launch parser repeats an empty native quote');
      offset++;
    }
...
export class BurikoLaunchSelection {
  private readonly pathService: BurikoPathFileDirectory;
  ...
  constructor(
    ...
    if (
      ...
      throw new Error('Buriko launch selection requires the mounted resource and save-root owners');
    this.pathService = new BurikoPathFileDirectory(files);
  }
  ...
  private lastBackslash(path: Uint8Array): number | null {
    ...
    for (let offset = 0; textByte(path, offset) !== 0;) {
      ...
      if (character.length === 0)
        throw new RangeError('Buriko launch path has an unsupported native encoding');
