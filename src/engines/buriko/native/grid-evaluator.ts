...
import {BurikoGridEvaluatorRecords} from './grid-evaluator-records.js';
import {prepareGridEvaluation} from './grid-evaluator-jobs.js';
import type {BurikoGridEvaluationCandidates} from './grid-evaluator-jobs.js';
...
export class BurikoGridEvaluator extends BurikoGridEvaluatorRecords {
  private readonly pool: BurikoDistributedProcessing;
  private readonly workerGrids: (BurikoLogicalGridManager | null)[];
  private candidates: BurikoGridEvaluationCandidates | null = null;
  private selected = 0;
  private cursor = 0;

  constructor(allocator: BurikoDistributedAllocator, capacity: number) {
    super();
    this.pool = new BurikoDistributedProcessing(allocator, capacity);
  ...
  private work(worker: number): number {
    ...
    if (candidates === null)
      throw new Error('Buriko grid evaluator worker uses released candidates');
    ...
    if (grid == null) throw new Error('Buriko grid evaluator worker dereferences null copied grid');
