...
export interface BurikoParticleVector {
  x: number;
  y: number;
  z: number;
}
const zero = (): BurikoParticleVector => ({x: 0, y: 0, z: 0});

/** 095d10 consumes two CRT random values, including when the requested range is zero. */
export function burikoParticleRandom(random: BurikoCrtRandom, maximum: number): number {
  maximum |= 0;
  const combined = (random.next() << 15) | random.next();
  const denominator = Math.abs((maximum + 1) | 0);
  if (denominator === 0) throw new RangeError('Buriko particle random range divides by zero');
...
export class BurikoParticleAir {
  interval = 0;
  enabled = 0;
  private vector = zero();
  private center: BurikoParticleVector | undefined;
  private spread: BurikoParticleVector | undefined;
  ...
  private first: BurikoParticleVector | undefined;
  private second: BurikoParticleVector | undefined;

  constructor(private readonly random: BurikoCrtRandom) {}

  /** 099350 exposes zero coordinates while the independent enable flag is clear. */
  current(): {enabled: number; vector: BurikoParticleVector} {
  ...
  private randomEndpoint(): BurikoParticleVector {
    if (this.center === undefined || this.spread === undefined)
      throw new Error('Buriko particle air reads unconfigured endpoint fields');
    const coordinate = (axis: keyof BurikoParticleVector): number =>
      (burikoParticleRandom(this.random, this.spread![axis] * 2) +
  ...
  private beginHold(initial: boolean): void {
    if (this.steps === undefined)
      throw new Error('Buriko particle air reads unconfigured derived durations');
    this.beginPhase(0, (burikoParticleRandom(this.random, this.steps[1]) + this.steps[0]) | 0);
    if (initial) this.first = this.randomEndpoint();
    else {
      if (this.second === undefined)
        throw new Error('Buriko particle air reads an unconfigured target');
  ...
  advance(): void {
    ...
    if (
      ...
      throw new Error('Buriko particle air reads an unconfigured transition');
    if (this.phase === 0) this.vector = {...this.first};
    else if (this.phase === 1) {
      const remaining = (this.duration - this.progress) | 0;
      const interpolate = (axis: keyof BurikoParticleVector): number => {
        const numerator =
        ...
        if (this.duration === 0 || (numerator === -0x80000000 && this.duration === -1))
          throw new RangeError(
            'Buriko particle air interpolation divides outside signed DWORD range',
          );
    ...
    if (this.duration >>> 0 <= this.progress >>> 0) {
      if (this.phase === 0)
        this.beginPhase(1, (burikoParticleRandom(this.random, this.steps[3]) + this.steps[2]) | 0);
