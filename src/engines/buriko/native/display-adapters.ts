import type {BurikoDisplayAdapterProfile} from './display-device.js';
import {BurikoNativeDisplayState, type BurikoNativeRectangle} from './display-state.js';

export interface BurikoAdapterDesktopMode {
  width: number;
  height: number;
  refreshRate: number;
  format: number;
}
export interface BurikoConfiguredDisplayAdapter {
  ...
  readonly mode: BurikoAdapterDesktopMode;
}
...
export class BurikoDisplayAdapters implements BurikoDisplayAdapterProfile {
  constructor(
    readonly display: BurikoNativeDisplayState,
    readonly records: readonly BurikoConfiguredDisplayAdapter[],
    readonly primaryMonitor: number,
    readonly readWindowRectangle: () => BurikoNativeRectangle,
  ) {
    if (records.length === 0 || display.monitors[primaryMonitor] === undefined)
      throw new Error('Buriko display requires an explicit configured adapter and primary monitor');
    for (const record of records)
      if (display.monitors[record.monitor] === undefined)
        throw new Error('Buriko adapter references a monitor absent from the shared list');
      else if (record.identifier !== undefined && record.identifier.length !== 0x450)
        throw new RangeError('Buriko adapter identifier requires its native 0x450-byte record');
  }
  ...
  currentMonitor(): number {
    ...
    return selected >= 0
      ? selected
      : this.display.geometryChanged !== 0
        ? nearest
        : this.primaryMonitor;
  }
  ...
  queryDesktopMode(output: BurikoAdapterDesktopMode | null = null): boolean {
  ...
  queryAdapterMode(index: number, output: BurikoAdapterDesktopMode | null = null): boolean {
  ...
  desktopSizeChanged(): boolean {
    ...
    return (
      this.queryDesktopMode(mode) &&
      (mode.width !== this.display.desktopWidth >>> 0 ||
        mode.height !== this.display.desktopHeight >>> 0)
    );
  }
