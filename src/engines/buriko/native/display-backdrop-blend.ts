...
export class BurikoBlendBackdrop extends BurikoBackdrop {
  private first = -1;
  private second = -1;
  ...
  override drawContent(destination: BurikoBitmap, rectangle: BurikoBitmapRectangle): 0 | 1 {
    ...
    if (this.firstImageId === undefined)
      throw new Error('Buriko blended backdrop reads unwritten first image ID');
    ...
    if ((this.second - 0x7000) >>> 0 < 2) mode = this.second === 0x7000 ? 0xc0 : 0xc1;
      ...
      if (this.secondImageId === undefined)
        throw new Error('Buriko blended backdrop reads unwritten second image ID');
      if (this.secondImageId !== this.surfaces.imageId(this.second)) return 0;
      cropBurikoBitmap(second, rectangle);
      this.environment.compositor.composite(destination, second, 0x80, 0, true);
      mode = 0xf0;
    }
    cropBurikoBitmap(first, rectangle);
