import {BurikoWindowMessages, type BurikoWindowTarget} from './window-messages.js';

export interface BurikoMovieFilterEvent {
...
export class BurikoMovieFilterEvents {
  private readonly renderers = new Set<object>();
  private readonly completed = new Set<object>();
  private readonly events: BurikoMovieFilterEvent[] = [];
  private target: BurikoWindowTarget | null = null;
  private message = 0;
  private instance = 0n;
  ...
  onAvailable(observer: () => void): () => void {
    if (this.disposed) throw new Error('Buriko movie observes a released event queue');
  ...
  addRenderer(renderer: object): void {
    if (this.disposed) throw new Error('Buriko movie adds a renderer to a released graph');
  ...
  take(): BurikoMovieFilterEvent | null {
