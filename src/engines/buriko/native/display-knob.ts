...
export interface BurikoKnobEvent {
...
export class BurikoDisplayKnob extends BurikoDisplayObject {
  private currentX = 0;
  private currentY = 0;
  ...
  constructor(
    ...
    readonly target: BurikoDisplayObject,
  ) {
    super(environment, 10, creationOrder, 1);
    if (target.parent === null) target.parent = this;
    this.blendMode = target.blendMode;
    BurikoDisplayObject.prototype.setBlendValue.call(this, target.getBlendValue());
  ...
  override localRectangle(): BurikoBitmapRectangle {
    this.check();
  ...
  private mappedValue(): BurikoDisplayPoint {
    return {
      x:
        this.precisionX > 0
