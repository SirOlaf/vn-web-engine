...
function copyBurikoMaskedAlphaPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  mask: BurikoBitmap,
): void {
...
function blendBurikoMaskedAlphaIntoRgbPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  mask: BurikoBitmap,
  transparency: number,
  ...
  for (let y = 0; y < source.height >>> 0; y++) {
    ...
    for (let x = 0; x < source.width >>> 0; x++) {
      ...
      for (let shift = 0; shift < 24; shift += 8) {
        ...
        result |=
          saturateBurikoByte(before + (burikoSignedProduct16(delta, coefficient) >> 7)) << shift;
      }
...
export const copyBurikoMaskedAlpha = withBurikoBitmapText(copyBurikoMaskedAlphaPixels, {
  replace: true,
});

export const blendBurikoMaskedAlphaIntoRgb = withBurikoBitmapText(
  blendBurikoMaskedAlphaIntoRgbPixels,
