...
interface BurikoMovieRecord {
  readonly id: number;
  readonly renderer: BurikoMovieRenderer;
  next: BurikoMovieRecord | null;
}
interface BurikoMovieRetirement {
  readonly id: number;
...
export class BurikoMovieRegistry {
  private head: BurikoMovieRecord | null = null;
  private nextId = 0;
  private nextNotificationId = 0;
  private readonly retirements = new Set<BurikoMovieRetirement>();
  /** 03fd00 links before renderer initialization, including later failed-initialization IDs. */
  append(renderer: BurikoMovieRenderer): number {
  ...
  find(id: number): BurikoMovieRenderer | null {
    for (let node = this.head; node !== null; node = node.next)
      if (node.id === (id | 0)) return node.renderer;
    return null;
  }
  remove(id: number): 0 | 1 {
    let previous: BurikoMovieRecord | null = null;
    for (let node = this.head; node !== null; node = node.next) {
      if (node.id === (id | 0)) {
  ...
  joinSlotRetirements(slot: number): Promise<void> {
    if (!Number.isInteger(slot) || slot < 0 || slot >= 0x4000)
      throw new RangeError('Buriko movie retirement slot is outside the surface table');
  ...
  detachSlot(surfaces: BurikoSurfaces, slot: number): number {
  ...
  removeFinished(surfaces: BurikoSurfaces): void {
    let node = this.head;
    while (node !== null) {
