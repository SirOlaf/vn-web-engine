...
import type {BurikoDisplayCapabilities} from './display-capabilities.js';
...
import {
  ...
  burikoCubicPresentationSample,
  burikoPresentationLinearRgbBlendInto,
  burikoPresentationLinearRgbInto,
  burikoPresentationLinearRgbRowInto,
  burikoPresentationTextureSampleInto,
  type BurikoPresentationColor,
  type BurikoPresentationSampler,
} from './presentation-sampling.js';

/** The selected Windows-compatible adapter record, supplied by the title's host profile. */
export interface BurikoDisplayAdapterProfile {
...
export function burikoPresentationVertices(display: BurikoNativeDisplayState): Uint8Array {
  const [physicalWidth, physicalHeight] = burikoDisplayTextureSize(
 ...
 */
export class BurikoDisplayDevice {
  readonly numericalProfile = BURIKO_PRESENTATION_NUMERICAL_PROFILE;
  readonly nativeLock: BurikoScopedLock; // The one 27caf0 DCLock, separate from engine/object locks.
  private context: CanvasRenderingContext2D | null = null;
  private frame: ImageData | null = null;
  private source: BurikoDisplayTexture | null = null; // 1e6af8, system memory.
  private sampled: BurikoDisplayTexture | null = null; // 1e6b10, default pool.
  private dynamic: BurikoDisplayTexture | null = null; // 1e65e0, movie transfer.
  ...
  private rasterSampler: BurikoPresentationSampler = 'point';
  ...
  constructor(
    ...
    readonly adapter: BurikoDisplayAdapterProfile,
    ...
    if (presentationMode !== 'canvas' && presentationMode !== 'none')
      throw new TypeError('Buriko display requires a selected presentation mode');
    this.nativeLock = new BurikoScopedLock(() => manager.surfaces.allocator.currentActor);
  ...
  get dynamicTexture(): BurikoDisplayTexture | null {
    return this.dynamic;
  }
  ...
  private createTextures(): number {
    ...
    const [width, height] = burikoDisplayTextureSize(
    ...
    this.source = null;
    try {
      this.source = new BurikoDisplayTexture(width, height, format);
    } catch {
      return 5;
    }
    ...
    this.sampled = null;
    try {
      this.sampled = new BurikoDisplayTexture(width, height, format);
    } catch {
      return 7;
    }
    ...
    this.dynamic = null;
    try {
      this.dynamic = new BurikoDisplayTexture(width, height, 22);
    } catch {
      return 9;
    }
    return 0;
  ...
  private createVertices(): number {
    if (!this.isPresent()) return 4;
    this.vertices = null;
    this.baseVertices = burikoPresentationVertices(this.display);
  ...
  create(fullscreen: number, capabilities: BurikoDisplayCapabilities | null = null): number {
    this.nativeLock.enter(); // b3080.
    try {
      if (capabilities === null) this.release();
      else {
        if (capabilities.device !== this)
          throw new Error('Buriko device creation requires its own shared interface');
  ...
  private moveQuad(x: number, y: number): void {
    ...
    if (this.vertices === null || this.baseVertices === null)
      throw new Error('Buriko display quad is absent');
  ...
  prepare(
    count: number,
    rectangles: readonly BurikoBitmapRectangle[] | null,
    x: number,
    y: number,
  ): void {
    ...
    if (source === null || sampled === null || this.vertices === null)
      throw new Error('Buriko display resources are absent');
    if (rectangles === null)
  ...
  drawMovieTexture(
    ...
    if (this.lost || !this.isPresent()) return;
    if (this.vertices === null || this.baseVertices === null)
      throw new Error('Buriko movie draw uses an absent display quad');
    this.rasterValid = false;
    this.sampler = 'linear';
    const [width, height] = burikoDisplayTextureSize(
      this.display.logicalWidth,
  ...
  private rasterizeQuad(
    ...
    const colorBuffer: BurikoPresentationColor = [0, 0, 0, 0];
    const sampleScratch = {
      a: [0, 0, 0, 0] as BurikoPresentationColor,
      b: [0, 0, 0, 0] as BurikoPresentationColor,
      c: [0, 0, 0, 0] as BurikoPresentationColor,
      d: [0, 0, 0, 0] as BurikoPresentationColor,
    };
        ...
        if (fractionX !== null && finiteColumns && Number.isFinite(ny)) {
          ...
          if (topSourceY !== ny) {
            if (bottomSourceY === ny) {
              ...
              burikoPresentationLinearRgbRowInto(sampled, sourceX, fractionX, ny, topRow);
              topSourceY = ny;
            }
          }
          if (bottomSourceY !== ny + 1) {
            burikoPresentationLinearRgbRowInto(sampled, sourceX, fractionX, ny + 1, bottomRow);
            bottomSourceY = ny + 1;
          }
          burikoPresentationLinearRgbBlendInto(
            topRow,
    ...
    for (let row = firstY; row < lastY; row++) {
      ...
      for (let column = firstX; column < lastX; column++) {
        const u = Math.fround(Math.fround(Math.fround(column - left) / width) * uMax);
        const color = cubic
          ? burikoCubicPresentationSample(sampled, logicalWidth, logicalHeight, u, v, sampler)
          : burikoPresentationTextureSampleInto(
              sampled,
              u,
              v,
