...
export class BurikoSharedLoaderWorker {
  readonly actor = {};
  private running = false;
  private automatic = false;
  private scheduled: ReturnType<typeof setTimeout> | null = null;
  private iteration: Promise<BurikoSharedLoaderJob> | null = null;
  ...
  constructor(
    ...
    if (
      ...
      throw new Error('Buriko shared loader requires the actual resource, audio and script owners');
  }
  ...
  /** Unexpected host/worker failures are distinct from decoded native result codes.
   * Observe them from the production frame while stop() retires borrowed queues. */
  checkWorker(): void {
  ...
  start(options: {automatic?: boolean} = {}): void {
    if (this.running || (this.stopping !== null && !this.stopped))
      throw new Error('Buriko shared loader is already running or joining');
    this.stopping = null;
  ...
  processOne(): Promise<BurikoSharedLoaderJob> {
    if (!this.running) throw new Error('Buriko shared loader is not running');
  ...
  stop(): Promise<void> {
    ...
    if (!this.running) throw new Error('Buriko shared loader has not been started');
    this.running = false;
  ...
  closeProgramScripts(actor = this.loading.metadata.allocator.currentActor): Promise<void> {
    if (this.scriptClosing !== null) {
      ...
      throw new Error('Buriko script close has already completed for this worker start');
    }
    if (!this.running) throw new Error('Buriko script shutdown requires a running loader');
    if (!this.scripts.hasLiveSection)
      throw new Error('Buriko script section is already closed; restart does not reinitialize it');
  ...
  async join(): Promise<void> {
    if (this.stopping === null) throw new Error('Buriko shared loader has not been stopped');
    let failed = false;
