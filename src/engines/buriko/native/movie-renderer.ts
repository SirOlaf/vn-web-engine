...
export class BurikoMovieMediaGraph {
  ...
  listen(type: string, listener: EventListener): void {
    if (this.disposed) throw new Error('Buriko movie listens on a released media graph');
    this.video.addEventListener(type, listener);
    this.listeners.push({type, listener});
  }
  get currentTime(): bigint {
    return BigInt(Math.trunc(this.video.currentTime * 10000000));
  }
  get stopTime(): bigint {
    return BigInt(Math.trunc(this.video.duration * 10000000));
  }
  get state(): 0 | 1 | 2 {
  ...
  async run(): Promise<number> {
    try {
      ...
      if (error instanceof DOMException) return 0x80004005;
      throw error;
    }
  }
  pause(): number {
  ...
  seek(time: bigint): number {
    try {
      this.video.currentTime = Number(time) / 10000000;
      return 0;
    } catch (error) {
      if (error instanceof DOMException) return 0x80004005;
      throw error;
...
export class BurikoMovieRenderer {
  initialized = false;
  suspended = 0;
  started = 0;
  paused = 0;
  ...
  deliveredFrames = 0;
  notificationId = -1;
  private graph: BurikoMovieMediaGraph | null = null;
  ...
  setMediaType(type: BurikoMovieMediaType): number {
    if (this.retired) return 0x80004005;
    // The initial connection runs before 040430 publishes movieId. A later
    // renegotiation would reenter release/registry removal on this same renderer.
    if (this.surfaces.record(this.slot)?.movieId !== -1) return 0x80004005;
    const result = this.image.setMediaType(type);
    if ((result | 0) < 0) return result;
    if (
      this.surfaces.allocate(this.slot, this.image.width, this.image.height, 1) === 0 ||
      this.surfaces.lock(this.slot) === 0
    )
      return 0x80004005;
    const bitmap = this.surfaces.snapshot(this.slot);
    if (bitmap === null)
      throw new Error('Buriko movie reads an unwritten locked bitmap descriptor');
  ...
  deliver(sample: BurikoMovieSample | null): number {
    if (this.retired) return 0x80004005;
  ...
  attachGraph(graph: BurikoMovieMediaGraph, repeat: number, notificationId: number): void {
    if (this.retired || this.initialized)
      throw new Error('Buriko movie renderer requires one graph lifetime');
    this.graph = graph;
  ...
  framePosition(): {status: number; value?: number} {
    ...
    return {
      status: 0,
      value:
        this.image.averageFrameTime > 0n
          ? Number(BigInt.asIntN(32, this.graph.currentTime / this.image.averageFrameTime))
          : 0,
    };
  }
  ...
  isPlaying(): 0 | 1 {
    ...
    return this.started !== 0 &&
      graph !== null &&
      (this.repeat !== 0 || (graph.currentTime < graph.stopTime && graph.state !== 0))
      ? 1
      : 0;
  }
  volume(value: number): number {
    ...
    const db =
      value === 0 ? -10000 : Math.trunc(-(((Math.imul(value, -100) + 12800) >>> 0) / 2.6666666666));
    const result = this.graph?.volume(db) ?? 0;
    return result === 0x80070057
      ? 0x80000003
      : result === 0 || result === 0x80004001
        ? 0
        : 0x80000001;
  }
  ...
  /** Synchronous fence precedes any future asynchronous graph retirement. */
  beginRetirement(): Promise<void> {
    ...
    this.retirement = completion;
    try {
      // The current HTML owner disposes synchronously. The completion remains
      // available for the later concrete asynchronous graph adapter.
      void Promise.resolve(graph?.dispose()).then(resolve, reject);
    } catch (error) {
      reject(error);
    }
    return completion;
  }
  /** Direct owner teardown retains the same idempotent completion. */
