...
export function burikoCrtWideSpace(character: number): boolean {
  if (character < 0x100)
    return (
...
export function burikoCrtWideDigit(character: number): number {
...
export interface BurikoCrtIntegerResult {
...
export function parseBurikoCrtWideInteger(
  text: string,
  base: 10 | 16,
  signed: boolean,
): BurikoCrtIntegerResult {
  let index = 0;
  while (burikoCrtWideSpace(wchar(text, index))) index++;
  const negative = wchar(text, index) === 45;
  if (negative || wchar(text, index) === 43) index++;
  if (
    base === 16 &&
    burikoCrtWideDigit(wchar(text, index)) === 0 &&
  ...
  let overflow = false;
  for (;;) {
    const digit = burikoCrtWideDigit(wchar(text, index));
...
export interface BurikoCrtFloatResult {
  readonly value: number;
...
function floating(bits: bigint, end: number, rangeError = false): BurikoCrtFloatResult {
...
export function parseBurikoCrtWideFloat(text: string): BurikoCrtFloatResult {
  let index = 0;
  while (burikoCrtWideSpace(wchar(text, index))) index++;
  const negative = wchar(text, index) === 45;
...
export function parseBurikoPropertyNumber(
  kind: 0 | 1 | 2 | 3,
  input: string,
): {result: number; value?: number} {
  if (kind !== 3) {
    const parsed = parseBurikoCrtWideInteger(input, kind === 2 ? 16 : 10, kind === 0);
    return parsed.end === 0 ? {result: 0x80000008} : {result: 0, value: parsed.value | 0};
  }
  const parsed = parseBurikoCrtWideFloat(input);
