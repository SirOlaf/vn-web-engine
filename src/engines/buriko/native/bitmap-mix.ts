...
function mixRgb(first: number, second: number, factor: number): number {
  ...
  for (let shift = 0; shift < 32; shift += 8) {
    ...
    result |= saturateBurikoByte(value) << shift;
  }
  return result >>> 0;
}
function mixAlpha(first: number, second: number, factor: number): number {
  ...
  if (coefficient > 128)
    throw new RangeError('Buriko alpha mix coefficient exceeds its native table');
  let result = saturateBurikoByte(signed16(alpha >> 8)) << 24;
  for (let shift = 0; shift < 24; shift += 8) {
    const base = (second >>> shift) & 255;
    const product = burikoSignedProduct16(((first >>> shift) & 255) - base, coefficient);
...
function fusedPremultipliedChannel(
  ...
  return saturateBurikoByte((mixed + retained) | 0);
}
...
function blendMixedBurikoBitmapsIntoRgbPixels(
  destination: BurikoBitmap,
  first: BurikoBitmap,
  second: BurikoBitmap,
  factor: number,
  ...
  transparency |= 0;
  const firstView = initializedBurikoBitmapView(first, width, height),
    secondView = initializedBurikoBitmapView(second, width, height),
    destinationView = initializedBurikoBitmapView(destination, width, height);
  if (
    firstView !== null &&
    secondView !== null &&
...
function mixRows(
  ...
  for (let row = 0; row < height; row++) {
    ...
    let column = 0;
    for (; column + 1 < width; column += 2) {
      const a = readBurikoPixelPair(first, left + column * 4);
      const b = readBurikoPixelPair(second, right + column * 4);
      writeBurikoPixelPair(destination, target + column * 4, [
        mix(a[0], b[0], factor),
        mix(a[1], b[1], factor),
      ]);
    }
...
function mixBurikoBitmapsPixels(
  destination: BurikoBitmap,
  first: BurikoBitmap,
  second: BurikoBitmap,
  factor: number,
  ...
  if (
    distributed !== 0 &&
    runBurikoBitmapOperation(processing, [destination, first, second], first, 0, (bitmaps) => {
      mixBurikoBitmaps(bitmaps[0]!, bitmaps[1]!, bitmaps[2]!, factor, processing, 0);
    })
  )
    return 0;
...
export const mixBurikoBitmaps = withBurikoBitmapText(mixBurikoBitmapsPixels, {
  sourceOpacity: (source, args) =>
    ...
    args[1].stride === args[2].stride
      ...
      : (source === 1 ? 256 - args[3] : args[3]) / 256,
  alternateArgs: (args) =>
    [args[0], args[1], args[2], args[3], null, 0] as Parameters<typeof mixBurikoBitmapsPixels>,
  source: [1, 2],
  replace: true,
  applied: (result) => result === 0,
});
