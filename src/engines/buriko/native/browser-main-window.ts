...
import type {BurikoNativeDisplayState, BurikoNativeRectangle} from './display-state.js';
import type {BurikoNativeInput} from './input.js';
import type {BurikoWindowMessages} from './window-messages.js';
import type {BurikoMainDomInput} from './main-dom-input.js';
...
export interface BurikoMainWindowCallbacks {
...
export function burikoWindowCenteredPosition(
  display: BurikoNativeDisplayState,
...
export class BurikoBrowserMainWindow {
  closePolicy = 1; // 1c945c: the executable's initialized DWORD is 1.
  closeMenuEnabled = true;
  private closeInput: BurikoNativeInput | null = null;
  private closeMessages: BurikoWindowMessages | null = null;
  private detached = false;
  private readScreenMapping: (() => BurikoViewportScreenMapping) | null = null;
  private lastRestoredOuterRectangle: BurikoNativeRectangle | null = null;
  private outerRectangleMinimized = false;
  private minimized = false;
  private inputIngress: BurikoMainDomInput | null = null;
  ...
  constructor(
    ...
    readonly callbacks: BurikoMainWindowCallbacks,
  ...
  get display(): BurikoNativeDisplayState {
    return this.manager.displayState;
  }

  /** B2FD0's host primitive reads the actual scoped outer border box in screen coordinates. */
  bindViewportScreenMapping(read: () => BurikoViewportScreenMapping): void {
    if (this.readScreenMapping !== null)
      throw new Error('Buriko main window screen mapping is already bound');
  ...
  mapCanvasViewportPoint(
    ...
    if (this.detached || this.readScreenMapping === null)
      throw new Error('Buriko canvas pointer requires a live screen mapping');
    ...
    if (
      ...
      throw new RangeError('Buriko canvas pointer requires measured geometry and screen mapping');
  ...
  private measureOuterScreenRectangle(): BurikoNativeRectangle {
    if (this.detached) throw new Error('Buriko main window has been detached');
    if (this.readScreenMapping === null)
      throw new Error('Buriko main window requires a viewport-to-screen mapping');
    const mapping = this.readScreenMapping(),
    ...
    if (
      ...
      throw new RangeError(
        'Buriko main window requires a measured outer border box and screen mapping',
      );
    ...
    if (right <= left || bottom <= top)
      throw new RangeError('Buriko main window outer rectangle collapses in native screen pixels');
    return [left, top, right, bottom];
  }

  /** MonitorFromWindow uses the last restored outer rectangle while iconic. */
  readRestoredOuterScreenRectangle(): BurikoNativeRectangle {
    if (this.detached) throw new Error('Buriko main window has been detached');
    if (this.outerRectangleMinimized) {
      if (this.lastRestoredOuterRectangle === null)
        throw new Error('Buriko minimized main window has no restored outer rectangle');
  ...
  readClientNativeSize(): readonly [number, number] {
    if (this.readScreenMapping === null)
      throw new Error('Buriko main client requires a screen mapping');
    ...
    if (width < 0 || height < 0)
      throw new RangeError('Buriko main client has a negative measured size');
    return [width, height];
  }

  /** Bind the native Close menu policy to the live HWND/message owners. */
  bindCloseMenu(input: BurikoNativeInput, messages: BurikoWindowMessages): void {
    if (
      input.display !== this.display ||
      messages.input !== input ||
      messages.mainTarget() === null
    )
      throw new Error('Buriko main Close menu requires the shared live main-window owners');
    if (this.closeInput !== null) {
      if (this.closeInput !== input || this.closeMessages !== messages)
        throw new Error('Buriko main Close menu is already bound to another owner');
      return;
    }
  ...
  setClosePolicy(value: number): void {
    if (this.closeInput === null)
      throw new Error('Buriko main Close policy has no live menu owner');
  ...
  setCloseMenuEnabled(enabled: boolean): void {
    if (this.closeInput === null) throw new Error('Buriko main Close menu has no live owner');
    this.closeMenuEnabled = enabled;
  }

  /** FF770 WM_SIZE's menu/input tail, after separately owned initialized-engine effects. */
  applySizeMenuTail(fullWParam: number | bigint, input: BurikoNativeInput): void {
    if (
      this.closeInput !== input ||
      ...
      throw new Error('Buriko size menu tail requires the shared live main-window owners');
  ...
  bindInputIngress(ingress: BurikoMainDomInput): void {
    if (this.detached || this.inputIngress !== null || ingress.host !== this)
      throw new Error('Buriko main input ingress requires the live scoped host');
  ...
  configureMonitorProfile(
    monitors: readonly BurikoNativeRectangle[],
    selected: number,
    ...
    if (current === undefined)
      throw new RangeError('Buriko main window requires an existing selected monitor');
    this.display.monitors = monitors.map(
      (rectangle) => [...rectangle] as unknown as BurikoNativeRectangle,
    );
  ...
  applyWindowStyle(styleValue: number): void {
    this.parent.setAttribute('data-buriko-window-style', (styleValue >>> 0).toString(16));
  }
  ...
  center(): 0 | 1 {
    ...
    this.applyPosition(...burikoWindowCenteredPosition(this.display));
    return 1;
  }
  move(x: number, y: number): 0 | 1 {
    if (this.display.fullscreen !== 0 || !burikoWindowPositionAllowed(this.display, x, y)) return 0;
  ...
  private blitBitmap(x: number, y: number, bitmap: BurikoBitmap): void {
    ...
    const [width, height] = burikoDisplayScaleSize(this.display, bitmap.width, bitmap.height);
    const [left, top, right, bottom] = burikoDisplayViewport(this.display);
    ...
    if (input === null || context === null)
      throw new Error('Buriko main window cannot acquire its drawing context');
  ...
  invalidateInline(rectangle: BurikoNativeRectangle): void {
