...
function synchronous<T>(value: T): T {
  if (
    ...
    throw new TypeError('Buriko native DLL host operation must complete synchronously');
  return value;
}

/** B7990/B7920/B78C0's global linked DLL registry and synchronous callback ABI. */
export class BurikoExternalLibraries {
  private nextId = 0;
  private entries: LibraryEntry[] = [];
  private closed = false;

  constructor(
    readonly resources: Pick<BurikoProgramResources, 'configuration'>,
  ...
  load(name: Uint8Array, output: BurikoBpPointer | null): 0 | 1 | 2 {
    if (this.closed) throw new Error('Buriko DLL registry admission is closed');
    const library = synchronous(this.host.loadLibraryA(this.path(name)));
    if (library === null) return 1;
    if (typeof library !== 'object')
      throw new TypeError('Buriko DLL host returned an invalid module identity');
    ...
    if (typeof call !== 'object')
      throw new TypeError('Buriko DLL host returned an invalid export identity');
    ...
    if (setWindow !== null) {
      ...
      if (handle !== null && typeof handle !== 'object')
        throw new TypeError('Buriko DLL host returned an invalid window identity');
  ...
  call(
    ...
    if (typeof result !== 'bigint')
      throw new TypeError('Buriko DLL callback must return a native-width integer');
