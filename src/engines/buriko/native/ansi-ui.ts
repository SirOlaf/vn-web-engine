...
export class BurikoAnsiUi {
  constructor(readonly text: BurikoNativeText) {}

  decode(pointer: BurikoBpPointer): string {
  ...
  initial(pointer: BurikoBpPointer | null, requestedLimit: number): {value: string; bytes: number} {
    ...
    if (count > 1024)
      throw new RangeError('Buriko ANSI initial text overwrites its 1024-byte native stack buffer');
    if (count === 1024)
      throw new Error('Buriko ANSI initial text scans beyond its unterminated native stack buffer');
