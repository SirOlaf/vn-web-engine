...
import {BurikoIndependentIconEEx} from './independent-icon-eex.js';
...
export function createGroup91IndependentIconMotion(
  ...
  const subtype = (icon: object): number => {
    ...
    if (type === undefined) throw new Error('Buriko Icon motion consumes absent subtype');
    return type;
  };
  return [
    {
      ...
      execute: (h) => {
        ...
        if (icon === null || icon.category !== 0x80) status = 1;
          ...
          if (!(icon instanceof BurikoIndependentIconEx))
            throw new Error('Buriko visibility requires actual IconEx owner');
          ...
          else throw new Error('Buriko visibility returns unwritten status');
        }
    ...
    },
    {
      ...
      execute: (h) => {
        ...
        if (!(window instanceof BurikoWindowDisplayObject))
          throw new Error('Buriko IconEEx requires actual Window owner');
        push32(
          h.thread,
          shared.register(
            new BurikoIndependentIconEEx(
              shared,
              window,
              input,
              priorities,
    ...
    },
    {
      ...
      execute: (h) => {
        ...
        if (icon === null || icon.category !== 0x80) status = 1;
          ...
          if (!(icon instanceof BurikoIndependentIconEEx))
            throw new Error('Buriko motion requires actual IconEEx owner');
          ...
          switch (result) {
            ...
            case 0x80000006:
              ...
              break;
            default:
              throw new Error('Buriko motion facade returns unwritten status');
          }
        }
        push32(h.thread, status);
        return 0;
