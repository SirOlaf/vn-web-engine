...
import {extractBurikoBitmapAlpha} from './bitmap-alpha-extraction.js';
  ...
  return [
    {
      ...
      execute: (context) => {
        ...
        if (destination === null)
          throw new Error('Buriko alpha extraction has no published output descriptor');
        const status = extractBurikoBitmapAlpha(destination, x, y, first, second, level);
        if (status === 1)
          return fatal(
            `参照元に指定されたビットマップ [ ${firstId | 0} ] 及び [ ${secondId | 0} ] のピクセルモードが一致しません`,
          );
        if (status !== 0)
          throw new Error('Buriko alpha extraction consumes an unwritten native status');
        return 0;
      },
    },
  ];
