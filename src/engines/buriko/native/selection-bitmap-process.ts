...
export class BurikoBitmapSelectionProcess extends BurikoProcedure {
  protected count = 0;
  protected records: number[][] = [];
  protected children: BurikoVirtualDisplayObject[] = [];
  ...
  constructor(
    ...
    private readonly waits: BurikoWindowMessages,
    private readonly notifications: BurikoNativeNotifications,
    private readonly settings: BurikoBitmapSelectionState,
  ...
  configureKeys(source: BurikoBpPointer): void {
    const keys: number[] = [];
  ...
  protected copyRecord(source: BurikoBpPointer | null, index: number): void {
    this.records[index] = Array.from({length: 4}, (_, i) => read(source, index * 16 + i * 4));
  }
  protected sourceId(source: BurikoBpPointer | null, index: number): number {
    return read(source, index * 16 + 8);
  }
  protected configureMask(
    child: BurikoVirtualDisplayObject,
    bitmap: BurikoBitmap,
    _source: BurikoBpPointer | null,
    _index: number,
  ): void {
  ...
  protected record(index: number, word: number): number {
    ...
    if (value === undefined) throw new Error('Buriko bitmap selection consumes unwritten record');
    return value;
  }
  initialize(count: number, source: BurikoBpPointer | null, hitMask: number): number {
    this.clearChildren();
    ...
    for (let i = 0; i < count; i++) {
      ...
      const child = new BurikoVirtualDisplayObject(this.window.environment, 0, this.window);
  ...
  private child(index: number): BurikoVirtualDisplayObject {
    const child = this.children[index];
    if (child === undefined)
      throw new Error('Buriko bitmap selection dereferences unwritten child');
    return child;
  }
  protected damage(area: BurikoBitmapRectangle): void {
  ...
  private keys(): number {
    ...
    for (let i = 0; i < this.count; i++) {
      ...
      if (key === undefined) throw new Error('Buriko bitmap selection consumes unwritten key');
  ...
  poll(): number {
    ...
    if (message === null)
      throw new Error('Buriko bitmap selection consumes unwritten window-message result');
    ...
    if (this.relativeX === undefined)
      throw new Error('Buriko bitmap selection consumes unwritten relative X');
    push32(this.thread, this.relativeX);
    if (this.relativeY === undefined)
      throw new Error('Buriko bitmap selection consumes unwritten relative Y');
...
export class BurikoExtendedBitmapSelectionProcess extends BurikoBitmapSelectionProcess {
  private readonly extended: (number | undefined)[] = Array(1024);
  protected override sourceId(source: BurikoBpPointer | null, index: number): number {
    return read(source, index * 64 + 8);
  }
  protected override copyRecord(source: BurikoBpPointer | null, index: number): void {
  ...
  private word(index: number): number {
    ...
    if (value === undefined)
      throw new Error('Buriko extended bitmap selection consumes unwritten contiguous record');
    return value;
  }
  protected override renderSelection(next: number): void {
    let area: BurikoBitmapRectangle | undefined;
    for (let i = 0; i < 8; i++) {
    ...
    if (next === -1) this.window.disableOverlays();
      ...
      for (let i = 0; i < 8; i++) {
        ...
        if (bitmap === null) this.window.setOverlayEnabled(i, 0);
          ...
          if (area === undefined)
            throw new Error(
              'Buriko extended bitmap selection consumes unwritten overlay rectangle',
            );
          this.damage(area);
        }
      }
