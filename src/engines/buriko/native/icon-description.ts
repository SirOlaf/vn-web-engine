...
export class BurikoIconWords {
  constructor(readonly words: (number | undefined)[]) {}
  read(index: number): number {
    const value = this.words[index];
    if (value === undefined)
      throw new Error('Buriko icon consumes an unwritten native record field');
    return value | 0;
  }
  write(index: number, value: number): void {
    this.words[index] = value | 0;
  }
  clone(): BurikoIconWords {
    return new BurikoIconWords(this.words.slice());
  }
}
export interface BurikoIconRow {
  data: BurikoIconWords;
  icons: BurikoIconWords[];
}
export interface BurikoIconDescription {
  header: BurikoIconWords;
  rows: BurikoIconRow[];
}
...
): {status: 0; description: BurikoIconDescription} | {status: 2 | 3} {
  const read = (p: BurikoBpPointer | null, i: number) => iconPointerWord(p, i).getInt32(0, true);
  const header = new BurikoIconWords(Array(10));
