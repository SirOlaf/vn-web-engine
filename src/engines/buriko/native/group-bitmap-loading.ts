...
import {
  BurikoLoadBitmapProcess,
  BurikoPreloadBitmapProcess,
  burikoBitmapMessage,
} from './bitmap-load-process.js';
...
export function createBurikoBitmapLoadingSlots(
  loading: BurikoBitmapLoading,
  scheduler: BurikoBpScheduler,
  procedures: BurikoProcedureState,
  clock: BurikoNativeClock,
): BurikoNativeSlotDefinition[] {
  const resources = loading.resources.resources,
    text = resources.files.text;
  const fatal = (h: BurikoBpOpcodeContext, message: Uint8Array): Promise<never> =>
    resources.errors.threadFatal(h.thread, h.diagnostics, message);
  const install = (h: BurikoBpOpcodeContext, process: BurikoBpWaitProcess): void => {
    const scheduled =
      h.thread === scheduler.root.state ? scheduler.root : scheduler.findById(h.thread.id);
    if (scheduled === null || scheduled.state !== h.thread)
      throw new Error('Buriko bitmap loader thread is not linked to its scheduler');
  ...
  return [
    {
      ...
      execute: async (h): Promise<0 | 2> => {
        ...
        if (namePointer === null)
          throw new Error('Buriko cached bitmap lookup dereferences a null filename');
        ...
        if (text.findCharacter(namePointer, 47) !== null || loading.policy.skipLoadWait() === 0) {
          const process = await BurikoLoadBitmapProcess.create(
            h,
            procedures,
            clock,
            loading.resources,
        ...
        const templates: Record<number, string> = {
          ...
          0x80000004:
            '指定されたBG/BMPファイル [ %s : %s ] はサポート外のビットカウントのデータです',
        ...
        if (templates[result] !== undefined) {
          const message = burikoBitmapMessage(text, templates[result], [
    ...
    },
    {
      ...
      execute: async (h): Promise<2> => {
        ...
        const archive = archivePointer === null ? null : textBytes(archivePointer).slice();
        const name = textBytes(namePointer).slice();
        const process = await BurikoPreloadBitmapProcess.create(
          h,
          procedures,
          clock,
          loading.resources,
          loading.surfaces.compositor,
          archive,
          name,
        );
        install(h, process);
    ...
    },
    {
      primary: 0x92,
      secondary: 0x15,
      nativeAddress: 0x1400e4950,
      name: 'ClearPreloadedBitmaps',
      execute: (): 0 => {
        loading.resources.preloaded.clear();
        return 0;
      },
    },
  ];
