...
export function createGroupSystemProfile(
  ...
  return [
    ...
    ).map(([secondary, nativeAddress, name, read]): BurikoNativeSlotDefinition => ({
      primary: 0x81,
      secondary,
      nativeAddress,
      name,
    ...
    },
    {
      ...
      execute: (h) => {
        ...
        if (total === null) throw new Error('Buriko physical memory null total destination');
        pointerView(total, 4).setUint32(0, values[0], true);
        if (available === null)
          throw new Error('Buriko physical memory null available destination');
