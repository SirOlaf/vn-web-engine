...
function reciprocal(value: number): number {
  const seed = value === 0 ? Infinity : burikoRosettaSseReciprocal(value);
...
function read(values: Float64Array, index: number): number {
  if (index < 0 || index >= values.length)
    throw new Error('Buriko native spatial-density array access is outside its allocation');
  return values[index]!;
}
...
export class BurikoLogicalSpatialDensity {
  ...
  query(
    ...
    if (output === null) throw new Error('Buriko spatial-density null vector output');
