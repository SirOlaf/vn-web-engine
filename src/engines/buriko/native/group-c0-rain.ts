...
export function createGroupC0Rain(
  rain: BurikoRainDisplays,
  redraw: BurikoDisplayRedraw,
  ...
  const invalid = (h: BurikoBpOpcodeContext, result: number): 0 | Promise<never> =>
  ...
  handlers[0x40] = (h) => {
    ...
    if (result.result !== 0)
      throw new Error('Buriko rain create reads its unwritten native output handle');
