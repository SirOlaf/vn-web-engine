...
export class BurikoGridEvaluatorRecord {
  ...
  word(index: number): number {
    ...
    if (offset >= 0x858 && offset < 0x968)
      throw new Error('Buriko grid evaluator reads a native map pointer as an integer');
    return this.view.getInt32(offset, true);
  }
  setWord(index: number, value: number): void {
    const offset = index * 4;
    if (offset >= 0x858 && offset < 0x968)
      throw new Error('Buriko grid evaluator overwrites a native map pointer as an integer');
  ...
  copy(): BurikoGridEvaluatorRecord {
    const result = new BurikoGridEvaluatorRecord();
  ...
  map(ability: number, extended: boolean): Uint8Array | null {
    ...
    if (index < 0 || index >= this.maps.length)
      throw new Error('Buriko grid evaluator map pointer address outside actor');
    return this.maps[index]!;
  }
}

/** 1400a8df0/1400a8d90/1400a8a70 own copied grid state and precomputed actor maps. */
export class BurikoGridEvaluatorRecords {
  grid: BurikoLogicalGridManager | null = null;
  readonly records: BurikoGridEvaluatorRecord[] = [];
  ...
  requireGrid(): BurikoLogicalGridManager {
    if (this.grid === null) throw new Error('Buriko grid evaluator null copied grid');
    return this.grid;
  }
  record(index: number): BurikoGridEvaluatorRecord {
    const record = this.records[index];
    if (record === undefined)
      throw new Error('Buriko grid evaluator actor address outside allocation');
    return record;
  }
  replaceGrid(grid: BurikoLogicalGridManager | null): number {
    this.grid?.dispose();
  ...
  initialize(
    grid: BurikoLogicalGridManager | null,
    count: number,
    input: BurikoBpPointer | null,
  ): number {
    if (grid === null) {
      this.clear();
      return 0;
  ...
  update(index: number, input: BurikoBpPointer | null, recompute: number): number {
    index >>>= 0;
    if (index >= this.records.length) return 0x90000003;
    const record = this.records[index]!;
    record.bytes.set(gridRead(input, 0x834));
    if (input === null) throw new Error('Buriko grid evaluator null actor source');
    ...
    if (grid.cells === null)
      throw new Error('Buriko grid evaluator reads uninitialized grid dimensions');
  ...
  findId(record: BurikoGridEvaluatorRecord, records = this.records): number {
    return records.findIndex((entry) => entry.id !== 0 && entry.id === record.id);
  }
  findPosition(x: number, y: number, records = this.records): BurikoGridEvaluatorRecord | null {
