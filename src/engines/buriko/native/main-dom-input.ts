...
import type {BurikoKeyboardMessages} from './keyboard-messages.js';
import type {BurikoWindowMessages} from './window-messages.js';
import {BurikoMainMouseInput, type BurikoMainWheelTranslator} from './main-mouse-input.js';
import {BurikoMainTouchInput, type BurikoBrowserTouchWindow} from './main-touch-input.js';
...
export class BurikoMainDomInput {
  private disposed = false;
  private readonly originalTabIndex: number;
  private readonly view: Window | null;
  readonly mouse: BurikoMainMouseInput;
  readonly touch: BurikoMainTouchInput | null;
  ...
  constructor(
    ...
    wheel: BurikoMainWheelTranslator | null = null,
    touch: BurikoNativeTouch | null = null,
    touchWindow: BurikoBrowserTouchWindow | null = null,
  ) {
    if (
      ...
      throw new Error('Buriko main DOM input requires the shared live main-window owners');
    ...
    if ((touch === null) !== (touchWindow === null))
      throw new Error('Buriko main DOM input requires touch and touch window together');
    this.touch =
      touch !== null && touchWindow !== null
        ? new BurikoMainTouchInput(host, input, messages, touch, touchWindow)
        : null;
    this.mouse = new BurikoMainMouseInput(
      host,
      input,
      messages,
      wheel,
