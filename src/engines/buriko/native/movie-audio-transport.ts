import {BurikoMovieAudioProducer, type BurikoMovieAudioCommonStop} from './movie-audio-producer.js';
import type {BurikoMovieAudioOutputLimits} from './movie-audio-decoder.js';
...
import {BurikoMoviePcmGraphClock} from './movie-pcm-clock.js';
...
type Output = BurikoBrowserMoviePcmOutput | BurikoMemoryMoviePcmOutput;

/** Exclusive audio-only transport. It never activates an AudioContext or drives memory render. */
export class BurikoMovieAudioTransport {
  readonly clock: BurikoMoviePcmGraphClock;
  private producer: BurikoMovieAudioProducer | null = null;
  private creation: Promise<BurikoMovieAudioProducer> | null = null;
  ...
  private constructor(
    ...
    private readonly limits: BurikoMovieAudioOutputLimits,
    private generation: number,
    private readonly commonStop: BurikoMovieAudioCommonStop | undefined,
  ) {
    this.clock = new BurikoMoviePcmGraphClock(output);
    ...
    if (output instanceof BurikoBrowserMoviePcmOutput)
  ...
  static async create(
  ...
  ): Promise<BurikoMovieAudioTransport> {
    const copiedStop =
    ...
    const transport = new BurikoMovieAudioTransport(
      movie,
      track,
      timeline,
      output,
  ...
  private async createProducer(seek: BurikoIsoRational): Promise<void> {
    this.check();
    const work = BurikoMovieAudioProducer.create(
      this.movie,
      this.track,
      this.timeline,
      this.output,
  ...
  usesMovie(movie: BurikoIsoMovie): boolean {
  ...
  private applied(status: BurikoMoviePcmStatus, generation = this.generation): void {
  ...
  private observe(status: BurikoMoviePcmStatus): void {
  ...
  dispose(): Promise<void> {
    ...
    void (async () => {
      ...
      try {
        if (this.output instanceof BurikoBrowserMoviePcmOutput)
