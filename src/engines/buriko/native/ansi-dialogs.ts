...
export interface BurikoAnsiDialogField {
  readonly output: BurikoBpPointer | null;
  readonly label: BurikoBpPointer | null;
  readonly initial: BurikoBpPointer | null;
  readonly limit: number;
  readonly numeric: number;
}

/** b00e0/afb70/aebd0 and their concrete native edit/combobox control paths. */
export class BurikoAnsiDialogs {
  constructor(
    readonly document: Document,
    readonly parent: HTMLElement,
    readonly dialogs: BurikoEngineDialogs,
    readonly ansi: BurikoAnsiUi,
    readonly language: BurikoNativeLanguage,
  ) {}
  ...
  pair(
    kind: number,
    title: BurikoBpPointer | null,
    first: BurikoAnsiDialogField,
    second: BurikoAnsiDialogField,
  ): Promise<number> {
    return this.dialogs.withNativeModal(
      () =>
        new Promise((resolve, reject) => {
  ...
  private acceptName(output: BurikoBpPointer | null, value: string): boolean {
    const bytes = this.ansi.getText(value, 11);
    for (let index = 0; ; index += 2) {
      if (index >= bytes.length)
        throw new Error(
          'Buriko name validation reads uninitialized bytes beyond its ANSI text terminator',
        );
  ...
  nameAndBirthday(
    last: BurikoBpPointer | null,
    first: BurikoBpPointer | null,
    nickname: BurikoBpPointer | null,
    firstPerson: BurikoBpPointer | null,
    monthOutput: BurikoBpPointer | null,
    dayOutput: BurikoBpPointer | null,
  ): Promise<number> {
    return this.dialogs.withNativeModal(
      () =>
        new Promise((resolve, reject) => {
          ...
          const inputs = outputs.map((output, index) => {
            if (output === null)
              throw new Error('Buriko name initialization scans a null native string');
            ...
            try {
              ...
              if (count === undefined)
                throw new RangeError(
                  'Buriko birthday callback indexes outside its native month table',
                );
