...
import type {BurikoBpProcessMessage, BurikoBpWaitProcess} from './types.js';

/** Executable globals shared by every CProcedure and the outer native controller. */
export class BurikoProcedureState {
  nextId = 0;
  enabled = 1;
...
export abstract class BurikoProcedure implements BurikoBpWaitProcess {
  readonly id: number;
  deadline = 0;
  completionRequested = false;
  private stopped = false;
  private enabledOverride = false;
  private readonly messages: BurikoBpProcessMessage[] = [];

  constructor(
    readonly thread: BurikoBpThread,
    protected readonly shared: BurikoProcedureState,
  ...
  protected consumeMessages(): void {
    let message: BurikoBpProcessMessage | undefined;
  ...
  protected handleMessage(_message: BurikoBpProcessMessage): void {}
...
export class BurikoWaitTiming extends BurikoProcedure {
  constructor(
...
export class BurikoWaitTimingEx extends BurikoProcedure {
...
export class BurikoWindowMessages {
  ...
  register(thread: BurikoBpThread, message: number): void {
    this.records.unshift({thread, message: message >>> 0, value1: 0n, value2: 0n, received: false});
  }

  unregister(thread: BurikoBpThread, message: number): void {
  ...
  consume(thread: BurikoBpThread, message: number): WindowMessageRecord | null {
...
export class BurikoWaitWindowMessage extends BurikoProcedure {
  constructor(
    thread: BurikoBpThread,
    shared: BurikoProcedureState,
    clock: BurikoNativeClock,
    private readonly registry: BurikoWindowMessages,
    private readonly dialogs: BurikoDiagnosticDialogs,
