...
import type {BurikoNativeCursor} from './engine-dialogs.js';
...
export function readBurikoCursorResource(executable: Uint8Array): Uint8Array | null {
  const resource = new PeCursorReader(executable).read(106);
  if (resource === undefined) return null;
  if (resource.kind !== 'static')
    throw new Error('Buriko cursor resource106 requires a static browser CUR profile');
  return resource.bytes;
}

/** Five native handles share the same physical cursor used by modal/auto-hide owners. */
export class BurikoCursorShapes {
  private selected = 0;
  ...
  select(index: number): void {
    if (index >>> 0 >= 5) throw new RangeError('Buriko cursor index exceeds five native handles');
  ...
  defaultClientCursor(hitTest: number): 1 {
    if (hitTest !== 1)
      throw new Error('Buriko browser cursor profile requires the scoped client area');
