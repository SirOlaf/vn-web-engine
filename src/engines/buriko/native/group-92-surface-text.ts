...
import {drawBurikoSurfaceText} from './surface-text.js';
...
export function createGroup92SurfaceText(
  ...
  const execute = async (context: BurikoBpOpcodeContext, wrap: 0 | 1): Promise<0> => {
    ...
    const result = await drawBurikoSurfaceText(surfaces, {
      surface,
      x,
      y,
      source,
    ...
    if (result.metric === undefined)
      throw new Error('Buriko surface text wrapper consumes an unwritten metric');
