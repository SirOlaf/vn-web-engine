...
import {
  BurikoMfMovieSourceCandidates,
  type BurikoMfArchiveSourceCandidate,
  type BurikoMfDirectSourceCandidate,
} from './movie-mf-source-candidates.js';

/** The exact physical MF region, retaining its local file backing when available. */
export interface BurikoMfMovieDocument {
  readonly blob: Blob;
  readonly kind: 'direct' | 'archive';
}

export class BurikoMfMovieDocuments {
  constructor(
    readonly candidates: BurikoMfMovieSourceCandidates,
    readonly files: BurikoProgramFiles,
    readonly byteBudget: number,
  ) {
    if (candidates.resources.files !== files)
      throw new Error('Buriko MF source candidates require the selected file owner');
    if (!Number.isSafeInteger(byteBudget) || byteBudget < 1 || byteBudget > 0xffffffff)
      throw new RangeError('Invalid Buriko MF browser document byte budget');
  }

  direct(name: BurikoBpPointer): Promise<BurikoMfDirectSourceCandidate | null> {
  ...
  async read(
    candidate: BurikoMfDirectSourceCandidate | BurikoMfArchiveSourceCandidate,
    signal: AbortSignal,
  ): Promise<BurikoMfMovieDocument> {
    ...
    if (source === null) throw new Error('Buriko MF selected physical source cannot be opened');
    ...
    if (
      ...
      throw new RangeError('Buriko MF source exceeds its physical region or browser byte budget');
    ...
    for (let at = 0; at < length;) {
      ...
      if (chunk.length === 0 || chunk.length > length - at)
        throw new Error('Buriko MF source read did not fill its selected region');
      bytes.set(chunk, at);
      at += chunk.length;
    }
    if (signal.aborted) throw new DOMException('MF movie source was retired', 'AbortError');
