...
function requirePointer(pointer: BurikoBpPointer | null): BurikoBpPointer {
  if (pointer === null) throw new Error('Buriko native text null pointer');
  return pointer;
}
...
export function nativeWordSimilarity(
  left: BurikoBpPointer | null,
  right: BurikoBpPointer | null,
): number {
  ...
  if (equalCount > 0x7fffffff || costCount > 0x7fffffff) {
    throw new RangeError('Buriko native similarity allocation size overflows signed DWORD');
  }
...
export function createGroup81Text(text: BurikoNativeText): BurikoNativeSlotDefinition[] {
  return [
    {
      primary: 0x81,
      secondary: 0x00,
