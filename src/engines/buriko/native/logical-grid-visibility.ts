...
import {BurikoNativeSpline, truncateSplineInteger} from './spline.js';
...
export class BurikoLogicalGridVisibility {
  constructor(private readonly manager: BurikoLogicalGridManager) {}
  ...
  collect(
    output: BurikoBpPointer | null,
    distances: BurikoBpPointer | null,
    ...
    for (let row = 0; row < height; row++)
      for (let column = 0; column < width; column++) {
        ...
        if (elevation < 0 || elevation >= 32)
          throw new Error('Buriko grid visibility reads outside native height-threshold stack');
        if (
