...
export class BurikoMainWindowShowState {
  ...
  constructor(
    ...
    if (
      ...
      !(input instanceof BurikoNativeInput) ||
      !(messages instanceof BurikoWindowMessages) ||
      !(dispatcher instanceof BurikoQueuedWindowDispatcher) ||
      ...
      throw new Error('Buriko show state requires the shared main window and paint owners');
  }
  ...
  async setShowState(rawState: number): Promise<void> {
    if (this.applying) throw new Error('Buriko main show state is already applying');
    this.applying = true;
    try {
