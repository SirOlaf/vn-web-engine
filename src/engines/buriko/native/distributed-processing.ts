...
class WorkEvent {
  ...
  check(): void {
    if (this.closed) throw new Error('Buriko work manager uses a closed event');
  }
}

export type BurikoWorkYield =
  {readonly kind: 'event'; readonly event: WorkEvent} | {readonly kind: 'cooperate'};
export type BurikoWorkContinuation = Generator<BurikoWorkYield, number, number>;
export type BurikoWorkResult = number | BurikoWorkContinuation;
export type BurikoAsyncWorkerCallback<T> = (
  context: T,
  worker: number,
  actor: object,
...
interface WorkThread {
  ...
  parked: boolean;
  phase: WorkerPhase;
  continuation: BurikoWorkContinuation | null;
...
export class BurikoDistributedAllocator {
  private readonly active: BurikoDistributedProcessing[] = [];
  private readonly runningPools: BurikoDistributedProcessing[] = [];
  ...
  constructor(readonly processorCount: number) {
    if (!Number.isInteger(processorCount) || processorCount <= 0 || processorCount > 0xffffffff)
      throw new RangeError('Buriko work-manager processor count must be a positive DWORD');
  }
  ...
  beginRun(pool: BurikoDistributedProcessing): void {
    this.runningPools.push(pool);
  }

  endRun(pool: BurikoDistributedProcessing): void {
    const index = this.runningPools.indexOf(pool);
    if (index < 0) throw new Error('Buriko cooperative pool has no active run');
  ...
  update(pool: BurikoDistributedProcessing, enabled: boolean): void {
    if (!this.initialized) throw new Error('Buriko work allocator critical section was deleted');
    let total = 0;
    ...
    let assigned = 0,
      bestPriority = 0,
      bestQuota = 0;
    let best: BurikoDistributedProcessing | undefined;
    ...
    if (assigned < this.processorCount) {
      if (best === undefined)
        throw new Error('Buriko work allocator has no native highest-priority pool');
 ...
 */
export class BurikoDistributedProcessing {
  ...
  private callback: ((context: unknown) => BurikoWorkResult) | null = null;
  private workerCallback: ((context: unknown, worker: number) => BurikoWorkResult) | null = null;
  private asyncWorkerCallback: BurikoAsyncWorkerCallback<unknown> | null = null;
  ...
  constructor(
    ...
    workerCount >>>= 0;
    if (workerCount === 0)
      throw new Error('Buriko work constructor writes worker zero in an empty allocation');
  ...
  setCallback<T>(callback: ((context: T) => BurikoWorkResult) | null, context: T): void {
    this.check();
    if (this.asyncWorkerCallback !== null)
      throw new Error('Buriko asynchronous indexed callback is already installed');
    this.callback = callback === null ? null : (value) => callback(value as T);
    this.context = context;
  }

  setWorkerCallback<T>(
    callback: ((context: T, worker: number) => BurikoWorkResult) | null,
    context: T,
  ): void {
    this.check();
    if (this.asyncWorkerCallback !== null)
  ...
  enterShared(): 0 | 1 {
    ...
    if (this.sharedOwner !== null && this.sharedOwner !== actor)
      throw new Error(
        'Buriko cooperative callback yielded while holding another worker’s critical section',
      );
  ...
  leaveShared(force = 0): void {
    ...
    if (this.sharedOwner !== this.allocator.currentActor || this.sharedDepth === 0)
      throw new Error('Buriko native work callback leaves an unowned critical section');
  ...
  wake(count: number): void {
    ...
    if (this.wakeAllLatch === undefined)
      throw new Error('Buriko work callback reads its uninitialized wake-all flag');
  ...
  *park(id: number): BurikoWorkContinuation {
    this.check();
    id >>>= 0;
  ...
  private check(): void {
    if (!this.alive) throw new Error('Buriko work manager has been destroyed');
  }
      ...
      if (worker.continuation === null) {
        ...
        if (resumedActivation || worker.id >= this.activeCapacity) {
          if (worker.activation === null)
            throw new Error('Buriko worker zero waits on a null activation event');
        ...
        if (this.callback === null && this.workerCallback === null)
          throw new Error('Buriko work callback became null during its native activation wait');
        const result =
          this.callback !== null
  ...
  *cooperate(): Generator<BurikoWorkYield, void, number> {
  ...
  run(distributedFlag: number, actor = this.allocator.currentActor): void {
    this.check();
    if (this.running) throw new Error('Buriko work manager entered a recursive native run barrier');
    this.running = true;
    ...
    while (!this.mainFinished()) {
      if (!this.allocator.advance())
        throw new Error('Buriko native work run is blocked with no signaled worker event');
    }
    if (distributed) {
      ...
      while (this.workers.some((worker) => worker.phase !== 'finishing')) {
        if (!this.allocator.advance())
          throw new Error('Buriko native work completion is blocked with no signaled worker event');
      }
      for (const worker of this.workers.slice(1)) {
        worker.gate3 = 'worker';
        worker.gate0 = null;
  ...
  private async invokeAsync(worker: WorkThread): Promise<boolean> {
    ...
    if (callback === null)
      throw new Error('Buriko asynchronous indexed callback became null during its run');
    worker.executing = true;
    worker.phase = 'callback';
    try {
  ...
  async runWorkerCallbackAsync<T>(
    callback: BurikoAsyncWorkerCallback<T>,
    context: T,
    ...
    if (this.callback !== null || this.workerCallback !== null || this.asyncWorkerCallback !== null)
      throw new Error('Buriko work manager already has an installed callback');
    ...
    try {
      ...
      while (!this.mainFinished()) {
        if (!(await this.advanceAsync()))
          throw new Error('Buriko asynchronous work run is blocked with no runnable worker');
      }
      if (distributed) {
        this.allocator.update(this, false);
        registered = false;
        ...
        while (this.workers.some((worker) => worker.phase !== 'finishing')) {
          if (!(await this.advanceAsync()))
            throw new Error('Buriko asynchronous work completion has no runnable worker');
        }
      }
    } finally {
  ...
  workerState(id: number): Readonly<{
    ...
    if (worker === undefined) throw new RangeError('Buriko work-thread index outside capacity');
    return {
  ...
  dispose(): void {
    this.check();
    if (this.running) throw new Error('Buriko work manager destroyed during a native run barrier');
    this.allocator.update(this, false);
    this.alive = false;
