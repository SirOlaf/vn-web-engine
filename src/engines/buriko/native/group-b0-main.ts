...
import {BurikoShakeProcess} from './shake-process.js';
...
export function createGroupB0InlineText(
  inline: BurikoInlineTextControl,
  errors: BurikoEngineErrors,
): BurikoNativeSlotDefinition[] {
...
export function createGroupB0Geometry(host: BurikoBrowserMainWindow): BurikoNativeSlotDefinition[] {
  const slots: BurikoNativeSlotDefinition[] = [];
  const add = (
    secondary: number,
    nativeAddress: number,
...
export function createGroupB0Shake(
  ...
  add(0x08, 0x1400d5d60, 'ShakeScreen', (h) => {
    const capture = pop32(h.thread),
      ...
      mode = pop32(h.thread);
    const process = new BurikoShakeProcess(
      h.thread,
      procedures,
      cursor.clock,
      cursor.input,
    ...
    if (scheduled === null || scheduled.state !== h.thread)
      throw new Error('Buriko shake thread is not linked to its scheduler');
