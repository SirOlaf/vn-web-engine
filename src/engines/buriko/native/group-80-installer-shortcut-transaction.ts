...
import type {BurikoInstallerShortcutTransaction} from './installer-shortcut-transaction.js';
...
export function createGroup80InstallerShortcutTransaction(
  shortcuts: BurikoInstallerShortcutTransaction,
  ...
  return [
    {
      ...
      execute: async ({thread, memory}): Promise<0> => {
        ...
        if (
          ...
          throw new Error('Buriko installer shortcuts dereference a null path or filename');
        push32(
          thread,
