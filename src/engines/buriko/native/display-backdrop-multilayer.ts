...
import {transformBurikoBitmap, blendTransformedBurikoBitmap} from './bitmap-affine.js';
...
export class BurikoMultilayerBackdrop extends BurikoBackdrop {
  private selected = 0;
  ...
  layerActivation(index: number): number {
    const layer = this.layerRecord(index);
    if (layer === null) throw new Error('Buriko multilayer activation reads invalid layer');
    return layer[ACTIVE]!;
  }
  ...
  override position(): BurikoDisplayPoint {
  ...
  override setCoordinates(x: number, y: number, z: number): void {
    BurikoDisplayObject.prototype.setCoordinates.call(this, x, y, z);
  ...
  override drawContent(destination: BurikoBitmap, rectangle: BurikoBitmapRectangle): 1 {
    this.check();
    ...
    for (let index = 0; index < 8; index++) {
      ...
      if (index === 0 && !drawn) clearBurikoBitmap(destination);
    }
    return 1;
  }
}
