...
export class BurikoInstallerManifestActions {
  ...
  async deleteUnlisted(root: Uint8Array, retain: readonly Uint8Array[]): Promise<0 | 1> {
    ...
    if (metadata === null)
      throw new Error('Buriko installer deletion requires the shared mounted metadata owner');
    ...
    for (let start = 0; start < bytes.length && bytes[start] !== 0;) {
      ...
      if (end < 0) throw new Error('Buriko installer deletion reads an unterminated manifest line');
