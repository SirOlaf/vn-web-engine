...
interface DifferenceSource {
  index: number;
  imageId: number;
  tables: (BurikoBitmapRectangle[] | null)[];
}

/** CDspObjBackD:057CE0, ordered pair difference tables057A40/0576C0. */
export class BurikoDifferenceBackdrop extends BurikoBackdrop {
  private sources: DifferenceSource[] = Array.from({length: 32}, () => ({
    index: -1,
    imageId: 0,
    tables: new Array<BurikoBitmapRectangle[] | null>(32).fill(null),
  }));
  ...
  setSurfaces(count: number, indices: readonly number[]): 0 | 0x80000001 | 0x80000002 {
    ...
    for (let index = 0; index < count; index++) {
      ...
      if (surface === undefined)
        throw new Error('Buriko difference backdrop reads unwritten source array');
  ...
  private buildDifferences(
    ...
    for (let cellY = 0; cellY < 24; cellY++) {
      let run: BurikoBitmapRectangle | null = null;
      for (let cellX = 0; cellX < 32; cellX++) {
        const rowStart = (bitmap: BurikoBitmap): number =>
          bitmap.offset +
        ...
        if (run !== null) run.right = (run.right + width) | 0;
          ...
          if (rectangles.length >= 384)
            throw new RangeError('Buriko difference table exceeds native rectangle allocation');
          run = {
  ...
  override invalidate(): void {
    ...
    if (this.count === undefined || this.previous === undefined)
      throw new Error('Buriko difference backdrop invalidates before source configuration');
  ...
  override drawContent(destination: BurikoBitmap, rectangle: BurikoBitmapRectangle): 0 | 1 {
    ...
    if (this.count === undefined)
      throw new Error('Buriko difference backdrop draws before source configuration');
    const current = this.getBlendValue() >>> 0;
