...
export interface BurikoHorizontalTextEffect {
...
export interface BurikoHorizontalTextCursor {
  x: number;
  y: number;
}

export interface BurikoHorizontalTextLayoutNode {
  ...
  fontSize: number;
  bitmap: BurikoBitmap;
  auxiliary: BurikoBitmap;
  annotationKey: Uint8Array | null;
  annotationBaseExtent: number;
  kind: number;
  next: BurikoHorizontalTextLayoutNode | null;
...
export interface BurikoHorizontalTextLayoutOptions {
  readonly source: BurikoBpPointer;
  readonly readingEnabled: number;
  readonly annotations: BurikoRubyAnnotations;
...
export interface BurikoHorizontalTextLayoutResult {
  readonly result: 0 | 1;
  readonly nodes: BurikoHorizontalTextLayoutNode[];
...
interface MutableFont {
  ...
  bold: number;
  italic: number;
  record: BurikoFontRecord;
  transient: boolean;
}

export interface BurikoHorizontalMeasuredText {
...
function divide(numerator: number, denominator: number): number {
  ...
  if (denominator === 0 || (numerator === -0x80000000 && denominator === -1))
    throw new RangeError('Buriko horizontal text layout integer division fault');
  return Math.trunc(numerator / denominator) | 0;
}

function cellWidth(font: MutableFont | BurikoFontRecord): number {
...
function parseTag(text: string, index: number): ParsedTag | null {
  ...
  const body = burikoCrtWideLower(text.slice(index + 1, close));
...
function cropToGlyph(bitmap: BurikoBitmap, glyph: BurikoDrawnGlyph): void {
...
function fillRectangle(bitmap: BurikoBitmap, width: number, height: number, color: number): void {
  const view = {...bitmap};
  if (
    cropBurikoBitmap(view, {
      left: 0,
      top: 0,
      right: (width - 1) | 0,
      bottom: (height - 1) | 0,
    ...
    fillBurikoBitmap(view, color);
}

function copyBitmap(state: BurikoTextLayoutState, source: BurikoBitmap): BurikoBitmap {
...
export function releaseBurikoHorizontalTextLayout(
...
function fontBitmapDimensions(font: BurikoFontRecord): {width: number; height: number} {
  if (font.raster === null)
    throw new Error('Buriko horizontal text layout reads an uninitialized font raster');
  return {width: font.raster.geometry.width, height: font.raster.geometry.height};
}

function alphaScratchFormat(state: BurikoTextLayoutState): number {
...
function fontExtra(font: MutableFont): number {
  if (font.record.raster === null)
    throw new Error('Buriko horizontal metrics read an undefined native font raster');
  return font.record.raster.extraPixels;
}

function glyphMetrics(
  state: BurikoTextLayoutState,
  glyph: BurikoDrawnGlyph,
...
export function measureBurikoHorizontalWideText(
  state: BurikoTextLayoutState,
  text: string,
 ...
 */
export async function buildBurikoHorizontalTextLayout(
  state: BurikoTextLayoutState,
  options: BurikoHorizontalTextLayoutOptions,
): Promise<BurikoHorizontalTextLayoutResult> {
  ...
  if (base.raster === null)
    throw new Error('Buriko horizontal text builder found an uninitialized native font');
  ...
  const installFont = async (
    ...
    italic |= 0;
    effectiveBold |= 0;
    let selected: BurikoFontRecord | null = null;
    if (
      size === base.size &&
  ...
  while (index < text.length) {
    ...
    if (state.field1D1E40 !== 0 && character > 0x7f && character !== 0x3000) {
      ...
      if (!covered) {
        ...
        if (missingGlyph === null) {
          ...
          throw new Error('Buriko missing-glyph modal callback is not connected');
        }
    ...
    if (!dropWrappedSpace) {
      ...
      if ((options.effect.mode | 0) === 0) {
        ...
        const shadow = allocateBurikoBitmap(glyphBitmap.width, glyphBitmap.height, format);
        clearBurikoBitmap(shadow);
        recolorAlpha(shadow, glyphBitmap, options.effect.color);
        recordBurikoBitmapText(shadow, '', {
          decorative: true,
        ...
        const outline = allocateBurikoBitmap(
        ...
        recordBurikoBitmapText(outline, '', {
          decorative: true,
          color: options.effect.color,
        });
        runAsActor(() =>
