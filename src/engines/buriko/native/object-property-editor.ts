...
export function createBurikoObjectPropertyTab(
  ...
  const refresh: BurikoPropertyCallback = (_row, context) =>
    Number(manager.resolve(context as number) !== null);
  const raw = (owner: BurikoDisplayObject, offset: number): number =>
    BurikoDisplayObject.prototype.diagnosticPropertyWord.call(owner, offset);
  const current = (): BurikoDisplayObject => {
    const value = manager.resolve(handle);
    if (value === null)
      throw new Error('Buriko object-property callback dereferences a missing object');
    return value;
  };
  const add = (
    name: string,
  ...
  if (category === 2) {
    if (!(object instanceof BurikoDisplaySprite))
      throw new Error('Buriko property diagnostic requires the concrete Sprite field owner');
