...
export type BurikoCdTrack = CdAudioTrack;
export type BurikoCdMedium = CdAudioMedium;
export type BurikoCdMediumLease = CdAudioMediumLease;
export type BurikoCdMediaHost = CdAudioMediaHost;

/** The native 0x1400edc30 mapping of MCI mode constants. */
export function burikoCdMode(mode: number): number {
  switch (mode) {
    case 0x20c:
      return 0;
    case 0x20d:
...
/** Buriko's optional CD-audio device, with the native successful-notification repeat. */
export class BurikoCdAudio {
  private opened = false;
  private disposed = false;
  private medium: BurikoCdMedium | null;
  private readonly host: BurikoCdMediaHost | null;
  private lease: BurikoCdMediumLease | null = null;
  private mode = 0x20c;
  ...
  constructor(
    mediumOrHost: BurikoCdMedium | BurikoCdMediaHost | null,
    ...
    this.medium = this.host === null ? (mediumOrHost as BurikoCdMedium | null) : null;
    if (this.medium !== null) this.validateMedium(this.medium);
  }

  private validateMedium(medium: BurikoCdMedium): void {
    this.starts.length = 1;
    if (medium.tracks.length < 1 || medium.tracks.length > 99) {
      throw new RangeError('Buriko CD medium must contain 1..99 tracks');
    }
    for (const track of medium.tracks) {
      if (!Number.isInteger(track.frames) || track.frames < 1) {
        throw new RangeError('Buriko CD track length must be a positive count of CD frames');
      }
      const pcm = track.pcm;
      if (
        pcm !== null &&
        ...
        throw new RangeError('Buriko CD audio must be stereo 44100 Hz PCM aligned to CD frames');
      }
  ...
  get status(): number | null {
    return this.opened ? burikoCdMode(this.mode) : null;
  }
...
export function createBurikoCdSlots(cd: BurikoCdAudio): readonly BurikoNativeSlotDefinition[] {
  return [
    {
      primary: 0xa0,
      secondary: 0x80,
