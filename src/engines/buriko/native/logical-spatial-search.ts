 ...
 */
export class BurikoLogicalSpatialSearch {
  ...
  private goal = -1;
  private workerPool: BurikoDistributedProcessing | null = null;
  ...
  constructor(
    private readonly manager: BurikoLogicalSpatialManager,
    private readonly globalPool: BurikoDistributedProcessing,
  ) {}
  ...
  private *worker(id: number): BurikoWorkContinuation {
    if (id === 0) {
      if (this.active) {
  ...
  query(
    _output: BurikoBpPointer | null,
    _count: BurikoBpPointer | null,
    sourceIndex: number,
    targetIndex: number,
    range: number,
    step: number,
    ...
    range = f32(range);
    const seed = burikoRosettaSseReciprocal(step),
