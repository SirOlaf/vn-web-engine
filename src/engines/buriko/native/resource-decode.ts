...
export type BurikoResourceDecodeStatus = 0 | 2 | 3 | 5 | 6;
export interface BurikoResourceDecodeResult {
  readonly status: BurikoResourceDecodeStatus;
  readonly bytes: Uint8Array | null;
  readonly initialized?: Uint8Array;
}

interface BurikoNativeResourceInput {
...
export async function decodeBurikoResourcePointer(
  ...
  let format: BurikoNativeResourceInput['format'];
...
export async function decodeBurikoResource(
  stored: Uint8Array,
  mainProcessing: BurikoDistributedProcessing,
  offset = 0,
  length = 0,
  ...
  const target = (extent: number) => {
    requireBurikoResourceRange(destination!.bytes.length, 0, extent);
    if (destination!.initialized !== undefined)
      requireBurikoResourceRange(destination!.initialized.length, 0, extent);
    return {
  ...
  try {
    if (
      ...
      requireBurikoResourceRange(stored.length, 20, 4);
      ...
      if (nativeInput === undefined) requireBurikoResourceRange(stored.length, 46, 2);
      ...
      const version =
        nativeInput === undefined
          ...
          : nativeInput.format === 'cbg-v2'
            ...
            : 1;
      if (version === 2) {
        const processing = new BurikoDistributedProcessing(
    ...
    if (
      ...
      throw new BurikoUndefinedResourceRead(error.message);
    }
    throw error;
  }
  ...
  if (offset + length > decoded.length) {
    throw new BurikoUndefinedResourceRead(
      'Buriko resource slice wraps beyond its native allocation',
    );
  }
  if (directImage)
  ...
  if (offset + length > initializedLength) {
    throw new BurikoUndefinedResourceRead(
      'Buriko CompressedBG version2 slice includes unwritten allocation bytes',
    );
  }
