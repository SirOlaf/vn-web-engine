...
import {
  ...
  burikoBfVarint as unsignedVarint,
  burikoBfTree as movieFrequencyTree,
  burikoBfSymbol,
  burikoBfSignedBits as movieSignedBits,
  type BurikoBfTree,
} from './bf-entropy.js';
...
function colorLookup(base: number, index: number): number {
  ...
  if (value === undefined)
    throw new BurikoUndefinedResourceRead(
      'Buriko BF color lookup reads outside initialized native lookup storage',
    );
  return value;
}

/** Actual 104010/103C70 pair, also used by the native encoder's coefficient verification. */
export function decodeBurikoBfCoefficients(
  bytes: Uint8Array,
  count: number,
  limit: number,
  dcTree: BurikoBfTree,
  ...
  for (let index = 0; index < count && Math.floor(acBits.position / 8) < limit; index += 64) {
    for (let order = 1; order < 64;) {
      ...
      if (target === undefined)
        throw new BurikoUndefinedResourceRead('Buriko BF AC run reads beyond native zigzag table');
...
export function decodeBurikoBfFrame(
  ...
  if (depth === 32) {
    ...
    if (version === 0x10000) alphaMode = 1;
      ...
      if (version !== 0x10001)
        throw new BurikoResourceCodecException(9, `Buriko native BF version exception: ${version}`);
