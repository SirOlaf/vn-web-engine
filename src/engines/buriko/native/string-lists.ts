...
interface StringEntry {
  ...
  readonly value: BurikoBpPointer;
}
class StringList {
  ...
  entry(index: number): StringEntry {
    ...
    if (entry === undefined) throw new Error('Buriko string list reads an undefined native entry');
    return entry;
  }
}

/** F9060..F94E0, the one numbered string-list registry at1CA4F0. */
export class BurikoStringLists {
  ...
  private copyEntry(source: BurikoBpPointer, hash: number): StringEntry {
  ...
  contains(id: number, source: BurikoBpPointer | null): 0 | 1 | 0x80000001 {
    const list = this.find(id);
    if (list === null) return 0x80000001;
    if (source === null) throw new RangeError('Buriko string membership consumed a null string');
    const hash = burikoNamedValueHash(source);
    for (let i = 0; i < (list.count | 0); i++) {
      const entry = list.entry(i);
      if (entry.hash === hash && burikoCompareNamedBytes(source, entry.value) === 0) return 0;
    }
    return 1;
  }

  /** F94E0 links the replacement and records its full count before copying each input string. */
  replace(id: number, count: number, source: BurikoBpPointer | null): 0 | 1 {
    id >>>= 0;
    count |= 0;
    if (count === 0) {
      this.destroy(id);
    ...
    while (capacity < count) {
      ...
      if (capacity <= 0)
        throw new RangeError('Buriko string-list capacity exceeds its native numerical domain');
    }
  ...
  append(id: number, source: BurikoBpPointer | null): number {
    ...
    for (let index = 0; index < (list.count | 0); index++) {
      ...
      if (entry.hash === hash && burikoCompareNamedBytes(source, entry.value) === 0) return index;
    }
  ...
  copyAll(output: BurikoBpPointer | null, id: number): number {
    const list = this.find(id);
  ...
  read(
    ...
    lengthOutput: BurikoBpPointer | null,
    id: number,
    index: number,
  ): number {
    const list = this.find(id);
