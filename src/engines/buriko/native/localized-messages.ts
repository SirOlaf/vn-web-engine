...
import {BurikoNamedValueMap, burikoCompareNamedBytes} from './named-value-map.js';

interface LanguageSection {
  languages: number[];
  readonly strings: BurikoNamedValueMap;
...
const separator = (byte: number): boolean =>
  byte === 9 || byte === 32 || byte === 44 || byte === 61;
...
function languageId(source: BurikoBpPointer): number {
  ...
  for (;;) {
    const byte = read(),
      letter = byte | 32,
      digit =
        byte >= 48 && byte <= 57 ? byte - 48 : letter >= 97 && letter <= 122 ? letter - 87 : -1;
...
export class BurikoLocalizedMessages {
  ...
  constructor(
    ...
    readonly importedMessages: BurikoImportedTextMaps,
  ) {}
  ...
  loadPeResources(resources: readonly PeResource[], resourceLanguage?: number): 0 | 1 {
    ...
    if (matches.length > 1)
      throw new Error('Buriko localized resource requires the selected Windows resource language');
  ...
  load(stored: Uint8Array | null): 0 | 1 {
    ...
    bytes.set(decoded);
    this.clear();
    this.first = {languages: [], strings: new BurikoNamedValueMap(0), next: null};
    const mode = this.text.detectEncoding(bytes);
    let offset = 0;
    const advance = (): void => {
      const count = this.text.readCharacter(bytes, offset, mode).length;
      if (count === 0) throw new Error('Buriko localized text character does not advance');
      offset += count;
    };
    for (;;) {
      ...
      if (burikoCompareNamedBytes(key, languageIdKey) !== 0) continue;
      ...
      if (this.first.strings.readByIndex(null, 0) === 0)
        this.first = {languages: [], strings: new BurikoNamedValueMap(0), next: this.first};
  ...
  lookup(key: BurikoBpPointer): BurikoBpPointer | null {
