...
function unsignedDivide(numerator: number, denominator: number): number {
  denominator >>>= 0;
  if (denominator === 0) throw new RangeError('Buriko bitmap scale divides by zero');
...
function bilinear(source: BurikoBitmap, x: number, y: number): number {
  x >>>= 0;
  y >>>= 0;
  const column = x >>> 16,
    row = y >>> 16;
...
function weightedBox(
  ...
  for (let row = top >>> 0; row < bottom;) {
    ...
    for (let column = left >>> 0; column < right;) {
      ...
      if (factor > 256)
        throw new RangeError('Buriko scale coverage indexes outside its native coefficient table');
 ...
 */
function scaleBurikoBitmapPixels(
...
export const scaleBurikoBitmap = withBurikoBitmapText(scaleBurikoBitmapPixels, {
  replace: true,
  region: (args) => {
