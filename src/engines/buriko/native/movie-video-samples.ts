...
import type {BurikoMovieTimedSample} from './movie-receive.js';
import {BurikoMovieVideoDecoder, type BurikoMovieVideoOutputLimits} from './movie-video-decoder.js';
import {
  createBurikoIsoTimeline,
  type BurikoIsoPresentation,
  type BurikoIsoTimeline,
} from './movie-iso-timeline.js';
import type {BurikoIsoMovie, BurikoIsoTrack} from './movie-iso-samples.js';

export interface BurikoMovieVideoSampleOptions {
  readonly outputLimits?: BurikoMovieVideoOutputLimits;
...
export function burikoMovieRgb32Type(
  width: number,
  height: number,
...
export interface BurikoMovieVideoSample {
  readonly sample: BurikoMovieTimedSample;
  readonly type: BurikoMovieMediaType;
...
export async function burikoCopyMoviePicture(
  frame: VideoFrame,
  presentation: BurikoIsoPresentation,
  averageFrameTime: bigint,
  discontinuity: boolean,
  maxRgb32Bytes?: number,
): Promise<BurikoMovieVideoSample> {
  ...
  const type = burikoMovieRgb32Type(width, height, averageFrameTime);
 ...
 */
export class BurikoMovieVideoSamples {
  ...
  private selection = new Map<number, BurikoIsoPresentation>();
  ...
  static async create(
    ...
    options: BurikoMovieVideoSampleOptions = {},
  ): Promise<BurikoMovieVideoSamples> {
    ...
    const decoder = await BurikoMovieVideoDecoder.create(movie, track, outputLimits);
    try {
      return new BurikoMovieVideoSamples(decoder, maxRgb32Bytes, timestampMode);
    } catch (error) {
      decoder.dispose();
      throw error;
    }
  ...
  private select(position: bigint): void {
    const edits = new Map<number, BurikoIsoPresentation[]>();
  ...
  async next(): Promise<BurikoMovieVideoSample | null> {
    const generation = this.generation;
    this.check(generation);
    if (this.reading === generation) throw new Error('Concurrent Buriko movie splitter reads');
    ...
    try {
      for (;;) {
        ...
        let selected: BurikoIsoPresentation | undefined;
        if (edit.presentations[0]!.rate <= 0) {
        ...
        for (;;) {
          ...
          try {
            const output = await burikoCopyMoviePicture(
              picture.frame,
              presentation,
              this.averageFrameTime,
              this.firstSample,
