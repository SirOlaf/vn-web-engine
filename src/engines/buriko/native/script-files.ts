...
export interface BurikoScriptFileRecord {
  readonly id: number;
  readonly normalizedName: BurikoBpPointer;
  readonly file: BurikoNativeFile;
  readonly mode: number;
  busy: number;
  next: BurikoScriptFileRecord | null;
}
interface ScriptFileJob {
  readonly output: BurikoBpPointer | null;
  readonly id: number;
  readonly buffer: BurikoBpPointer | null;
...
export class BurikoScriptFiles {
  readonly section = new BurikoAsyncCriticalSection();
  private enabled = 0;
  private sectionLive = false;
  private closing = false;
  private counter = 0;
  private first: BurikoScriptFileRecord | null = null;
  ...
  find(id: number): BurikoScriptFileRecord | null {
  ...
  async open(
    ...
    try {
      ...
      for (let record = this.first; record !== null; record = record.next)
        if (burikoCompareNamedBytes(normalized, record.normalizedName) === 0) return 0x80000002;
  ...
  private remove(id: number): number {
    let previous: BurikoScriptFileRecord | null = null;
    for (let record = this.first; record !== null; record = record.next) {
      if (record.id === id >>> 0) {
  ...
  private async enqueue(
    ...
    await this.section.enter(actor);
    try {
      if (this.closing && !shutdownClose) throw new Error('Buriko script files are closing');
  ...
  async shutdown(
    ...
    await this.section.enter(actor);
    try {
      if (this.closing) throw new Error('Buriko script files are already closing');
      this.closing = true;
      for (let record = this.first; record !== null; record = record.next)
    ...
    while (this.first !== null) {
      if (workerRunning !== undefined && !workerRunning())
        throw new Error('Buriko script close worker stopped before draining queued files');
      await this.sleep(1);
    }
