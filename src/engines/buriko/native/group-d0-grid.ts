...
export function createGroupD0Grid(
  managers: BurikoLogicalGridManagers,
): BurikoNativeSlotDefinition[] {
  const pointer = (h: BurikoBpOpcodeContext) => h.memory.resolve(h.thread, pop32(h.thread));
  const use = (id: number, operation: (manager: BurikoLogicalGridManager) => number) => {
    const manager = managers.get(id);
    return burikoLogicalStatus(manager === undefined ? 0x80000000 : operation(manager));
  };
  const entries: [number, number, string, BurikoBpOpcodeHandler][] = [
    [
      0x00,
      0x1400d3e60,
      'CreateLogicalGrid',
    ...
    [
      ...
      (h) => {
        ...
        push32(
          h.thread,
          use(id, (manager) =>
            new BurikoLogicalGridVisibility(manager).collect(
              output,
              distances,
              count,
              x,
