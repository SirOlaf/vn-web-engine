...
export class BurikoNativeRecordBuffers {
  ...
  write(
    indexOutput: BurikoBpPointer | null,
    id: number,
    index: number,
    ...
    if (index < set.capacity) set.records.delete(index);
      ...
      do {
        ...
        if (doubled === 0)
          throw new Error(
            'Buriko native record-table DWORD growth reaches a nonterminating zero capacity',
          );
        capacity = doubled;
  ...
  read(
    ...
    sizeOutput: BurikoBpPointer | null,
    id: number,
    index: number,
  ): number {
    const set = this.find(id);
