...
import {BurikoRawSurfaceExport} from './raw-surface-export.js';
...
export class BurikoCompressedSurfaceEncoder {
  readonly legacy: BurikoLegacyBgEncoder;
  readonly raw: BurikoRawSurfaceExport;
  constructor(
    readonly surfaces: BurikoSurfaces,
    readonly processing: BurikoDistributedProcessing,
    readonly ticks: BurikoSystemTicks,
  ) {
    this.legacy = new BurikoLegacyBgEncoder(ticks);
    this.raw = new BurikoRawSurfaceExport(surfaces);
  }
  encode(
    ...
    if (mode >>> 0 === 0) {
      ...
      if (quality >>> 0 > 100) status = 0x80000018;
        ...
        const result = encodeBurikoCompressedBgV2(
          packed,
          encoded,
          quality,
          this.processing,
    ...
    if (count === null)
      throw new Error('Buriko compressed surface encoder writes a null count pointer');
