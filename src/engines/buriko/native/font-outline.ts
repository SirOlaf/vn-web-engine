...
type BurikoOutlineReader = (x: number, y: number) => number;
type BurikoOutlineContribution = (value: number, dx: number, dy: number) => number;
...
export function burikoTextOutlineWeightTables(): readonly Uint32Array[] {
...
function drawOutlinePixels(
  ...
  sourceWidth: number,
  sourceHeight: number,
  read: BurikoOutlineReader,
  radiusX: number,
  radiusY: number,
  color: number,
  contribution: BurikoOutlineContribution,
): void {
...
export function drawBurikoCachedGlyphOutline(
  ...
  if (raster === null) throw new Error('Buriko text outline reads an uninitialized ordinary font');
  ...
  recordBurikoBitmapText(destination, '', {
...
export function drawBurikoByteMaskOutline(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  radiusX: number,
  radiusY: number,
  ...
  recordBurikoBitmapText(destination, '', {decorative: true, color});
}
