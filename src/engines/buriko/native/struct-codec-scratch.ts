...
export class BurikoStructCodecScratch {
  ...
  constructor(readonly pageSize = 4096) {
    if (!Number.isInteger(pageSize) || pageSize <= 0 || this.capacity % pageSize !== 0)
      throw new RangeError('Buriko codec scratch requires an explicit compatible page size');
    this.section.initialize();
  }
  commit(bytes: number, actor: object): BurikoCodecPointer | null {
    if (!this.reserved || this.section.owner !== actor)
      throw new Error('Buriko codec scratch commit requires its live owned reservation');
    if (this.committed !== null) throw new Error('Buriko codec scratch still has committed pages');
    bytes >>>= 0;
  ...
  decommit(actor: object): void {
    if (this.section.owner !== actor) throw new Error('Buriko codec scratch decommit is not owned');
    this.committed = null;
  }
