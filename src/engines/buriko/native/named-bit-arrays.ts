...
interface NamedBitArray {
  ...
  readonly name: BurikoBpPointer;
  bitCount: number;
  data: Uint8Array;
  next: NamedBitArray | null;
}

export interface BurikoNamedBitArraySnapshot {
...
export function burikoNamedBitByteCount(bitCount: number): number {
...
export class BurikoNamedBitArrays {
  ...
  private findMoveToFront(name: BurikoBpPointer): NamedBitArray | null {
    const hash = burikoNamedValueHash(name);
    let previous: NamedBitArray | null = null,
      entry = this.first;
    while (entry !== null) {
      if (entry.hash === hash && burikoCompareNamedBytes(name, entry.name) === 0) {
        if (previous !== null) {
  ...
  createOrResize(name: BurikoBpPointer, bitCount: number): 0 | 0x80000001 {
    bitCount >>>= 0;
    const byteCount = burikoNamedBitByteCount(bitCount);
  ...
  writeBit(name: BurikoBpPointer | null, bitIndex: number, value: number): number {
    const entry = this.findMoveToFront(name!);
  ...
  writeRange(name: BurikoBpPointer | null, start: number, value: number, count: number): number {
    const entry = this.findMoveToFront(name!);
    if (entry === null) return 0x80000002;
    start >>>= 0;
    value >>>= 0;
  ...
  readBit(output: BurikoBpPointer | null, name: BurikoBpPointer | null, bitIndex: number): number {
    const entry = this.findMoveToFront(name!);
    if (entry === null) return 0x80000002;
    bitIndex >>>= 0;
    if (bitIndex >= entry.bitCount) return 0x80000003;
  ...
  replacePacked(source: BurikoBpPointer): 0 {
  ...
  copyPacked(output: BurikoBpPointer | null): number {
    let size = 4,
      count = 0;
  ...
  mergePacked(source: BurikoBpPointer): 0 {
    const records = this.packedRecords(source);
    for (let index = records.length - 1; index >= 0; index--) {
      ...
      const resized = this.findMoveToFront(record.name)!,
        byteCount = burikoNamedBitByteCount(record.bitCount),
  ...
  read(name: BurikoBpPointer): BurikoNamedBitArraySnapshot | null {
    const entry = this.findMoveToFront(name);
    return entry === null
      ? null
      : {
  ...
  snapshots(): BurikoNamedBitArraySnapshot[] {
    const result: BurikoNamedBitArraySnapshot[] = [];
    for (let entry = this.first; entry !== null; entry = entry.next)
      result.push({
        name: textBytes(entry.name, true).slice(),
        bitCount: entry.bitCount,
