...
import {applyBurikoEffectorVectorMap} from './bitmap-display-filters.js';
...
export class BurikoVectorBackdrop extends BurikoBackdrop {
  private source = -1;
  private primary = -1;
  private secondary = -1;
  ...
  private acceptsMap(bitmap: BurikoBitmap): boolean {
  ...
  override drawContent(destination: BurikoBitmap, rectangle: BurikoBitmapRectangle): 0 | 1 {
    ...
    cropBurikoBitmap(primary, rectangle);
    if (secondary !== null) cropBurikoBitmap(secondary, rectangle);
    const effect = this.effectiveBlendValue();
    if (this.sampling === undefined)
      throw new Error('Buriko vector backdrop reads unwritten sampling option');
