import {BurikoDisplayEffector} from './display-effector.js';
import {BurikoDisplayFilter} from './display-filter.js';
...
export class BurikoFilterDisplays {
  constructor(readonly manager: BurikoDisplayManager) {}

  private filter(handle: number): BurikoDisplayFilter | null {
    const object = this.manager.find('filter', handle);
    if (object === null) return null;
    if (!(object instanceof BurikoDisplayFilter))
      throw new Error('Buriko filter pool contains a different native display class');
    return object;
  }

  private effector(handle: number): BurikoDisplayEffector | null {
    const object = this.manager.find('effector', handle);
    if (object === null) return null;
    if (!(object instanceof BurikoDisplayEffector))
      throw new Error('Buriko effector pool contains a different native display class');
    return object;
  }
  ...
  createFilter(): number {
    return this.manager.createSimple(
      'filter',
      (order) => new BurikoDisplayFilter(this.manager.environment, this.manager.surfaces, order),
    );
  }
  ...
  configureFilter(
    ...
    if (result !== 0) {
      ...
      throw new Error('Buriko Filter returned an unknown native configuration status');
    }
  ...
  private finishEffectorConfiguration(effector: BurikoDisplayEffector): void {
  ...
  configureVectorEffector(
    ...
    if (result !== 0) {
      ...
      throw new Error('Buriko vector Effector returned an unknown native status');
    }
  ...
  configureBlurEffector(
    ...
    if (result !== 0) {
      ...
      throw new Error('Buriko blur Effector returned an unknown native status');
    }
    this.finishEffectorConfiguration(effector);
    return 0;
  }
  ...
  configureDisplacementEffector(
    ...
    if (result !== 0) {
      ...
      throw new Error('Buriko displacement Effector returned an unknown native status');
    }
    this.finishEffectorConfiguration(effector);
    return 0;
  }
  ...
  configureTransformEffector(
    ...
    if (result !== 0) {
      ...
      throw new Error('Buriko transform Effector returned an unknown native status');
    }
    this.finishEffectorConfiguration(effector);
    return 0;
  }
