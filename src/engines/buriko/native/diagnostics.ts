...
export interface BurikoBpWriteWatch {
...
export interface BurikoBpWriteWatchNotice {
  readonly watch: BurikoBpWriteWatch;
...
export class BurikoBpDiagnostics {
  writeWatchEnabled = false;
  errorCode = 0;
  errorDetail: Uint8Array | null = null;
  private readonly watches: BurikoBpWriteWatch[] = [];

  constructor(
    private readonly reportWriteWatch: (notice: BurikoBpWriteWatchNotice) => void,
  ...
  formatThreadMessage(thread: BurikoBpThread, message: Uint8Array): Uint8Array {
    ...
    if (opcode === undefined)
      throw new RangeError('Buriko diagnostic instruction is outside module storage');
    let instruction = opcode;
    if (
      this.abi.compatibility === '1.69'
        ? opcode >= 0x80
        : (opcode >= 0x7f && opcode <= 0xe0) || opcode === 0xff
    ) {
      const secondary = thread.moduleMemory[thread.instructionStart + 1];
      if (secondary === undefined)
        throw new RangeError('Buriko diagnostic secondary byte is outside module storage');
    ...
    if (this.abi.compatibility === '1.69') {
      if (modules.length === 0) {
        text(
          `Thread No. [ ${thread.id | 0} ] , IP [ $${hex(thread.instructionStart)} ] , Instruction No. [ $${hex(instruction, 1)} ]\n\n`,
        );
      } else {
        const current = modules.find((module) => thread.instructionStart >= module.base);
        if (!current) throw new RangeError('Buriko diagnostic code address precedes every module');
        text(`Thread No. [ ${thread.id | 0} ] , Program [ `);
        chunks.push(current.name);
        text(
          ` ]\n\nIP [ Thread : $${hex(thread.instructionStart)} / Program : $${hex(thread.instructionStart - current.base)} ] , Instruction No. [ $${hex(instruction, 1)} ] , SP [ $${hex(thread.frameCursor)} ]\n\n`,
        );
      }
      ...
      const find = (address: number) => {
        ...
        throw new RangeError('Buriko diagnostic code address precedes every module');
      };
  ...
  checkWrite(thread: BurikoBpThread, address: number, size: number): void {
