...
import {mirrorBurikoBitmap} from './bitmap-mirror.js';
import {reduceBurikoBitmapHalf} from './bitmap-reduce.js';
import {stretchCenteredBurikoBitmap} from './bitmap-centered-stretch.js';
import {rotateCenteredBurikoBitmap} from './bitmap-centered-rotation.js';
import {scaleBurikoBitmap} from './bitmap-scale.js';
import {scaleBurikoTrueColorBitmapNearest} from './bitmap-scale-nearest.js';
...
import {makeBurikoBitmapOpaque, removeBurikoBitmapMatte} from './bitmap-import.js';
...
import {BurikoSurfaceCoefficientTables} from './surface-coefficients.js';
import {convertBurikoBitmapToMask, invertBurikoBitmapMask} from './bitmap-mask-conversion.js';
import {splatBurikoBitmap} from './bitmap-splat.js';
import {
...
/** An attached native producer lends pixels while the slot keeps its own allocation. */
export interface BurikoSurfaceBitmapAttachment {
  snapshot(): BurikoBitmap | null;
  /** Begin retirement synchronously; the producer owner joins its asynchronous work. */
  retire(): void;
}
interface SurfaceSlot extends BurikoSurfaceRecord {
  owner: BurikoBitmapStorage | null;
  attachment: BurikoSurfaceBitmapAttachment | null;
...
export class BurikoSurfaces {
  readonly capacity = 0x4000;
  /** The same eight-record owner used by every sprite/filter coefficient consumer. */
  readonly coefficientTables = new BurikoSurfaceCoefficientTables();
  readonly toneCurves = new BurikoSurfaceToneCurves();
  deviceIndex = 0;
  preserveImageIds = 0;
  private nextImageId = 0;
  private movies: BurikoMovieRegistry | null = null;
  ...
  applyToneCurve(
    ...
    const result = applyBurikoBitmapTone(
      this.compositor,
      destination,
      source,
      monochrome,
  ...
  scaleSurface(
    ...
    const destination = this.snapshot(destinationIndex)!;
    if ((sampling | 0) !== 0) scaleBurikoBitmap(destination, source, scaleX, scaleY);
    else scaleBurikoTrueColorBitmapNearest(destination, source, scaleX, scaleY);
    return 0;
  }
  ...
  stretchCenteredSurface(
    ...
    intersectBurikoBitmapRectangle(rectangle, {
      left: x | 0,
      top: y | 0,
      right: (x + width - 1) | 0,
      bottom: (y + height - 1) | 0,
    });
    cropBurikoBitmap(destination, rectangle);
    ...
    const result = stretchCenteredBurikoBitmap(
      this.compositor,
      destination,
      source,
      sourceX << 16,
    ...
    throw new Error('Buriko centered stretch consumes an indeterminate native result');
  }
  ...
  rotateCenteredSurface(
    ...
    const source = this.snapshot(sourceIndex);
    if (source === null) return 2;
    const result = rotateCenteredBurikoBitmap(this.compositor, destination, source, scale, angle);
    if (result === 0) return 0;
    if (result === 1) return 3;
    if (result === 0x13) return 4;
    throw new Error('Buriko centered rotation consumes an indeterminate native result');
  }
  ...
  mirrorSurface(destinationIndex: number, sourceIndex: number, mode: number): number {
    ...
    if (this.allocate(destinationIndex, source.width, source.height, source.format) === 0)
      return 0x80000009;
    return mirrorBurikoBitmap(this.snapshot(destinationIndex)!, source, mode);
  }
  ...
  reduceSurfaceHalf(destinationIndex: number, sourceIndex: number): number {
    ...
    if (
      ...
      return 0x80000009;
    reduceBurikoBitmapHalf(this.snapshot(destinationIndex)!, source);
    return 0;
  }

  /** Resolves the construction cycle before any movie is attached to a slot. */
  attachMovies(movies: BurikoMovieRegistry): void {
    if (this.movies !== null && this.movies !== movies)
      throw new Error('Buriko surface movie registry is already attached');
    this.movies = movies;
  }
  /** Read-only identity check for lowers that borrow the one attached movie registry. */
  usesMovieRegistry(movies: BurikoMovieRegistry): boolean {
  ...
  record(index: number): BurikoSurfaceRecord | null {
  ...
  createMask(destinationIndex: number, sourceIndex: number): 0 | 9 | 10 {
    ...
    convertBurikoBitmapToMask(this.snapshot(destinationIndex)!, source);
    return 0;
  }
  ...
  invertMask(index: number): 0 | 7 | 11 {
    ...
    return invertBurikoBitmapMask(bitmap) ? 0 : 7;
  }
  ...
  getMetadata(output: BurikoBpPointer | null, index: number): 0 | 1 {
    const record = this.record(index);
    if (record === null) return 0;
    const x = record.metadataX,
      y = record.metadataY;
    if (output === null) throw new Error('Buriko surface metadata dereferences a null output');
  ...
  replaceColor(index: number, search: number, replacement: number): 0 | 1 | 2 {
    ...
    replaceBurikoBitmapColor(bitmap, search, replacement);
    return 0;
  }
  /** 036AD0 clears the DWORD before memmove reads the selected source pixel. */
  readPixel(output: BurikoBpPointer | null, index: number, x: number, y: number): 0 | 1 | 2 | 3 {
    const bitmap = this.snapshot(index);
    if (bitmap === null) return 1;
  ...
  descriptor(index: number): BurikoBitmap | null {
  ...
  snapshot(index: number): BurikoBitmap | null {
    const bitmap = this.descriptor(index);
    if (bitmap === null) return null;
    return {...(this.slot(index)!.attachment?.snapshot() ?? bitmap)};
  }
  /** 1.69 Flash getters borrow the producer DIB without replacing the CMemoryDX backing. */
  attachBitmap(index: number, attachment: BurikoSurfaceBitmapAttachment): void {
    const slot = this.slot(index);
    if (!slot?.owner) throw new Error('Buriko bitmap producer requires an allocated surface');
    if (slot.attachment !== null) throw new Error('Buriko surface already owns a bitmap producer');
    slot.attachment = attachment;
  }
  detachBitmap(index: number, attachment: BurikoSurfaceBitmapAttachment): void {
    const slot = this.slot(index);
    if (slot?.attachment === attachment) slot.attachment = null;
  }
  /** 1400404d0 returns -1 for an unused or invalid slot. */
  imageId(index: number): number {
    const slot = this.slot(index);
    if (!slot?.owner) return -1;
    if (slot.imageId === null) throw new Error('Buriko surface reads an unwritten native image ID');
    return slot.imageId;
  }
  ...
  lock(index: number): 0 | 1 {
    ...
    if (slot.lockOwner !== null && slot.lockOwner !== actor)
      throw new Error(
        'Buriko surface critical section requires its current actor to resume before entry',
      );
  ...
  unlock(index: number): 0 | 1 {
    ...
    if (slot.lockDepth === 0 || slot.lockOwner !== this.allocator.currentActor)
      throw new Error('Buriko surface releases a native critical section it does not own');
  ...
  release(index: number): 0 | 1 {
    ...
    if (slot.movieId !== -1) {
      if (this.movies === null)
        throw new Error('Buriko movie was attached before its concrete registry');
  ...
  fill(index: number, color: number): 0 | 1 {
    ...
    color >>>= 0;
    if (color === 0) clearBurikoBitmap(bitmap);
    else if (bitmap.format === 0) fillBurikoBitmap16(bitmap, color);
    else if (bitmap.format === 1 || bitmap.format === 2)
      fillBurikoBitmap32(bitmap, bitmap.format === 1 ? color & 0xffffff : color);
    else if (bitmap.format === 3) fillBurikoBitmap(bitmap, color);
    return 1;
  }
  ...
  importRaw(
    ...
    if (data === null) throw new Error('Buriko raw bitmap import dereferences a null source');
    ...
    } else copyBurikoBitmapRows(destination, source);
    removeBurikoBitmapMatte(destination, this.compositor.importMatteColor);
    if (metadata !== null) {
      const record = this.record(index)!;
  ...
  splatSurface(
    ...
    splatBurikoBitmap(destination, source, scaleX, scaleY, level);
    return 0;
  }
  ...
  transformSurface(
    ...
    const source = this.snapshot(sourceIndex);
    if (source === null) return 2;
    const result = (blend ? blendTransformedBurikoBitmap : transformBurikoBitmap)(
      this.compositor,
      destination,
      source,
      transform,
    ...
    throw new Error('Buriko surface transform reads an unwritten native result');
  }
  ...
  recolorSurface(destinationIndex: number, sourceIndex: number, color: number): 0 | 1 | 2 {
    ...
    if (source === null) return 2;
    const rectangle = burikoBitmapRectangle(destination);
    intersectBurikoBitmapRectangle(rectangle, burikoBitmapRectangle(source));
    cropBurikoBitmap(destination, rectangle);
    cropBurikoBitmap(source, rectangle);
    recolorBurikoBitmapAlpha(destination, source, color);
    return 0;
  }
  ...
  applySurfaceColorEffect(
    ...
    const source = this.snapshot(sourceIndex);
    if (source === null) return 2;
    return applyBurikoBitmapColorEffect(
      this.compositor,
      destination,
      source,
      selector,
  ...
  applySurfaceMask(
    ...
    const rectangle = burikoBitmapRectangle(destination),
      bounds = burikoBitmapRectangle(mask);
    translateBurikoBitmapRectangle(bounds, -x | 0, -y | 0);
    if (!intersectBurikoBitmapRectangle(rectangle, bounds)) return 0;
    cropBurikoBitmap(destination, rectangle);
    translateBurikoBitmapRectangle(rectangle, x | 0, y | 0);
    cropBurikoBitmap(mask, rectangle);
    if (mask.format === 2) applyBurikoAlphaMask(destination, destination, mask, 0);
    else if (mask.format === 3) applyBurikoBitmapMask(destination, destination, mask);
    else return 3;
    return 0;
  }
  ...
  duplicateSurface(destinationIndex: number, sourceIndex: number): 0 | 1 | 2 {
    ...
    copyBurikoBitmapRows(this.snapshot(destinationIndex)!, source);
