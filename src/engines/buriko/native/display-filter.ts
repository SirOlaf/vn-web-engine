...
import {applyBurikoFilterMaskColor} from './bitmap-display-filters.js';
...
export type BurikoDisplayFilterStatus = 0 | 0x80000001 | 0x80000002 | 0x80000003;

/** CDspObjFilter, the size-188 full-display color/mask filter constructed by 05BDB0. */
export class BurikoDisplayFilter extends BurikoDisplayObject {
  filterMode = 0;
  operation = 0;
  color = 0;
  maskSurface = -1;
  ...
  configureMask(maskSurface: number, shift: number): BurikoDisplayFilterStatus {
    this.check();
    maskSurface |= 0;
    if (maskSurface === -1) {
      this.filterMode = 0;
  ...
  override draw(destination: BurikoBitmap, rectangle: BurikoBitmapRectangle, _key: number): void {
    ...
    applyBurikoFilterMaskColor(destination, this.color, mask, this.maskShift, blendValue);
  }
}
