...
import {blendBurikoMaskedAlphaIntoRgb, copyBurikoMaskedAlpha} from './bitmap-mask.js';
import {BurikoDisplayObject, BurikoDisplayObjectEnvironment} from './display-object.js';
import {BurikoRain} from './rain.js';
...
export class BurikoRainDisplayState {
  enabled = 1;
...
export class BurikoRainDisplayObject extends BurikoDisplayObject {
  ...
  private rain: BurikoRain | null = null;
  private rainBitmap: BurikoBitmap = {
    storage: null,
    offset: 0,
    stride: 0,
    width: 0,
  ...
  constructor(
    ...
    private readonly surfaces: BurikoSurfaces,
    private readonly state: BurikoRainDisplayState,
  ...
  configureRain(width: number, height: number): number {
    ...
    this.rainBitmap = allocateBurikoBitmap(width, height, nativeFormat === 1 ? 2 : nativeFormat);
    clearBurikoBitmap(this.rainBitmap);
    return 0;
  }
  ...
  start(elapsed: number): void {
    this.check();
    this.rain?.dispose();
    this.rain = new BurikoRain(this.random, this.ticks);
  ...
  override draw(destination: BurikoBitmap, rectangle: BurikoBitmapRectangle, _key: number): void {
    ...
    cropBurikoBitmap(source, rectangle); // Native ignores unsuccessful clipping here.
    ...
    if (this.maskImageId === undefined)
      throw new Error('Buriko rain reads its unwritten mask image ID');
    ...
    const temporary = allocateBurikoBitmap(source.width, source.height, source.format);
    copyBurikoMaskedAlpha(temporary, source, mask);
    const value = this.effectiveBlendValue();
