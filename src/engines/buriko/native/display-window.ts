...
import {BurikoDisplayObject, type BurikoDisplayPoint} from './display-object.js';
import {BurikoWindowDisplayState} from './display-window-state.js';
import {BurikoMemoryDx} from './memory-dx.js';
import {configureBurikoOwnedBitmap} from './owned-bitmap.js';
import {BurikoInnerDisplayObjectManager} from './object-manager.js';
...
export class BurikoWindowDisplayObject extends BurikoDisplayObject {
  private ready = 0;
  private configuredWidth: number | undefined;
  private configuredHeight: number | undefined;
  private readonly composedOwner = new BurikoMemoryDx();
  private readonly backgroundOwner = new BurikoMemoryDx();
  private readonly textOwner = new BurikoMemoryDx();
  ...
  private textRectangle: BurikoBitmapRectangle | null = null;
  private cursor: BurikoDisplayPoint | null = null;
  ...
  private order = [0, 1, 2];
  private inner: BurikoInnerDisplayObjectManager | null = null;
  private innerObjects: (BurikoDisplayObject | null)[] = [];
  private innerOrigin: BurikoDisplayPoint | null = null;
  ...
  get compositionBitmap(): BurikoBitmap {
    this.check();
    return {...this.composed};
  }
  get backgroundBitmap(): BurikoBitmap {
    this.check();
    return {...this.background};
  }
  get textBitmap(): BurikoBitmap {
    this.check();
    return {...this.text};
  }

  /** 0670c0 crops both local descriptors with the composed bitmap's native bounds. */
  copyCompositionTo(destination: BurikoBitmap): number {
    this.check();
    if (this.ready === 0) return 0;
    const target = {...destination},
      source = {...this.composed};
    const targetBounds = burikoBitmapRectangle(target),
      sourceBounds = burikoBitmapRectangle(source);
    intersectBurikoBitmapRectangle(targetBounds, sourceBounds);
    cropBurikoBitmap(target, sourceBounds);
    cropBurikoBitmap(source, sourceBounds);
  ...
  configureInitial(width: number, height: number): 0 | 1 {
    ...
    configureBurikoOwnedBitmap(this.composedOwner, this.composed, width, height, compositor);
    this.setBackgroundEnabled(0);
    configureBurikoOwnedBitmap(this.backgroundOwner, this.background, width, height, compositor);
    clearBurikoBitmap(this.background);
    this.setTextEnabled(0);
    configureBurikoOwnedBitmap(this.textOwner, this.text, width, height, compositor);
    this.textRectangle = burikoBitmapRectangle(this.text);
  ...
  composeRectangle(rectangle: BurikoBitmapRectangle): number {
    this.check();
    if (this.ready === 0) return 0;
    const clipped = {...rectangle};
    if (!intersectBurikoBitmapRectangle(clipped, burikoBitmapRectangle(this.composed)))
      return this.ready;
    const destination = {...this.composed},
      compositor = this.environment.compositor;
    cropBurikoBitmap(destination, clipped);
    for (const stage of this.order) {
      if (stage === 0) {
        if (this.backgroundEnabled === 0) clearBurikoBitmap(destination);
        else {
          const background = {...this.background};
          cropBurikoBitmap(background, clipped);
          compositor.composite(destination, background, 0x80, 0, true);
        }
      } else if (stage === 1) {
        if (this.textEnabled !== 0) {
          const text = {...this.text};
          cropBurikoBitmap(text, clipped);
          if (this.textMaskMode === 0)
            compositor.composite(destination, text, 1, this.textTransparency, true);
          else {
            const temporary = allocateBurikoBitmap(text.width, text.height, 1);
  ...
  composeAll(): number {
    this.check();
    return this.ready === 0 ? 0 : this.composeRectangle(burikoBitmapRectangle(this.composed));
  }
  /** 066cf0. */
  private composeImageArea(x: number, y: number, bitmap: BurikoBitmap): void {
  ...
  private overlay(index: number): WindowLayer {
    ...
    if (layer === undefined)
      throw new RangeError('Buriko window overlay indexes outside native storage');
    return layer;
  }
  ...
  setOverlayBitmap(index: number, source: BurikoBitmap): 0 | 4 {
    const compositor = this.environment.compositor;
    if (!burikoBitmapFormatsCompatible(compositor.defaultFormat, source.format)) return 4;
    this.composeLayer(index, 0);
    const layer = this.overlay(index);
    configureBurikoOwnedBitmap(
      layer.owner,
      layer.bitmap,
      source.width,
      source.height,
  ...
  /** 067870. */
  overlayRectangle(index: number): BurikoBitmapRectangle | null {
    const layer = this.overlay(index);
    if (layer.enabled === 0 || layer.bitmap.storage === null) return null;
    const rectangle = burikoBitmapRectangle(layer.bitmap),
  ...
  setBackgroundSurface(surface: number): 0 | 1 | 2 {
    ...
    surface |= 0;
    if (surface === -1) clearBurikoBitmap(this.background);
    else {
  ...
  drawBackgroundSurface(
    ...
    Object.assign(output, burikoBitmapRectangle(this.background));
    const area = burikoBitmapRectangle(source);
    translateBurikoBitmapRectangle(area, x, y);
    intersectBurikoBitmapRectangle(output, area);
    const position = this.effectivePosition();
    translateBurikoBitmapRectangle(output, position.x, position.y);
    this.composeImageArea(x, y, source);
    return 0;
  }

  /** 067980 copies the raw text rectangle. 067960 optionally extends its right edge. */
  getTextRectangle(extended = false): BurikoBitmapRectangle {
    this.check();
    if (this.textRectangle === null)
      throw new Error('Buriko window text rectangle is not initialized');
  ...
  setTextRectangle(rectangle: BurikoBitmapRectangle): 0 | 1 {
    this.check();
  ...
  getTextCursor(): BurikoDisplayPoint {
    this.check();
    if (this.cursor === null) throw new Error('Buriko window text cursor is not initialized');
    return {...this.cursor};
  }
  ...
  textAdvanceFits(distance: number): number {
    ...
    throw new Error('Buriko window text fit returns an undefined native value');
  }
  /** 067650. */
  ...
  textLineCount(): number {
    ...
    if (advance === 0 || (height === -2147483648 && advance === -1))
      throw new RangeError('Buriko window line count has an undefined native quotient');
  ...
  setTextParameters(values: Uint32Array | null): void {
    ...
    if (values !== null && values.length < 16)
      throw new RangeError('Buriko window text parameters need sixteen DWORDs');
  ...
  saveText(): void {
    this.check();
    this.backup.storage?.release();
    this.backup = allocateBurikoBitmap(this.text.width, this.text.height, this.text.format);
    copyBurikoBitmapRows(this.backup, this.text);
  }
  restoreText(): 0 | 10 {
    this.check();
    if (this.text.width !== this.backup.width || this.text.height !== this.backup.height) return 10;
    copyBurikoBitmapRows(this.text, this.backup);
    return 0;
  }
  ...
  drawTextBitmap(
    ...
    cropBurikoBitmap(destination, region);
    const result = this.environment.compositor.draw(
      destination,
      (x - region.left) | 0,
      (y - region.top) | 0,
  ...
  textBitmapRectangle(): BurikoBitmapRectangle {
    this.check();
    const rectangle = burikoBitmapRectangle(this.text),
      point = this.effectivePosition();
    translateBurikoBitmapRectangle(rectangle, point.x, point.y);
    return rectangle;
  }
  ...
  createInnerSprite(
    ...
    const sprite = new BurikoDisplaySprite(
      this.environment,
    ...
    if (this.innerOrigin === null || this.innerProjection === undefined)
      throw new Error('Buriko Window consumes unwritten inner projection');
  ...
  composeInnerObject(output: BurikoBitmapRectangle, index: number): 0 | 1 {
    this.check();
    index >>>= 0;
    ...
    translateBurikoBitmapRectangle(output, point.x, point.y);
    return 1;
  }

  /** 0679D0 composes the old area while temporarily deactivating the inner object. */
  eraseInnerObject(output: BurikoBitmapRectangle, index: number): 0 | 1 {
  ...
  private innerSprite(index: number): BurikoDisplaySprite | null {
    this.check();
    index >>>= 0;
    if (index >= this.innerObjects.length) return null;
    const object = this.innerObjects[index]!;
    if (object !== null && !(object instanceof BurikoDisplaySprite))
      throw new Error('Buriko Window inner operation requires its actual Sprite');
    return object;
  }
  ...
  refreshDisplayGeometry(): void {
    ...
    if (this.innerOrigin === null) throw new Error('Buriko Window consumes unwritten inner origin');
  ...
  configureInnerObjects(count: number): void {
    ...
    this.inner?.dispose();
    this.innerObjects = Array<BurikoDisplayObject | null>(count).fill(null);
    if (count === 0) this.inner = null;
    else {
      const bitmap = {...this.composed},
        context = {bitmap, bounds: burikoBitmapRectangle(bitmap)},
        manager = this.windowState.manager;
      this.inner = new BurikoInnerDisplayObjectManager(
        count,
        context,
        manager.surfaces,
        manager.effectors,
