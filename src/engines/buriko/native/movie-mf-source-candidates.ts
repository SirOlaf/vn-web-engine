...
export interface BurikoMfArchiveSourceCandidate {
...
export class BurikoMfMovieSourceCandidates {
  constructor(readonly resources: BurikoProgramResources) {}

  async direct(name: BurikoBpPointer): Promise<BurikoMfDirectSourceCandidate | null> {
    ...
    assertBurikoPathDomain(primary);
    ...
    for (const [index, root] of [primary, secondary].entries()) {
      ...
      if (index === 1) assertBurikoPathDomain(secondary);
      const wide = root + decoded;
      if (wide.length >= 784)
        throw new RangeError('Buriko MF direct path exceeds native wide scratch');
  ...
  private boundedBytes(path: Uint8Array): Uint8Array {
    if (path.length > 784)
      throw new RangeError('Buriko MF path exceeds native caller output storage');
    return path;
  }
}
