...
export function createGroup90Surfaces(
  ...
  const invalidIndex = (h: BurikoBpOpcodeContext, index: number): Promise<never> =>
  ...
  add(0x16, 0x1400dd620, 'ReadBitmapDescriptor', (h) => {
    ...
    if (bitmap !== null) {
      if (output === null) throw new Error('Buriko bitmap descriptor writes through null');
