...
function samePixels(first: BurikoBitmap, second: BurikoBitmap): boolean {
...
function applyBurikoAlphaMaskPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  mask: BurikoBitmap,
  transparency: number,
...
function mask8(destination: BurikoBitmap, source: BurikoBitmap, mask: BurikoBitmap): void {
...
function applyBurikoBitmapMaskPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  mask: BurikoBitmap,
): MaskStatus {
  if (mask.format >>> 0 === 2) return applyBurikoAlphaMask(destination, source, mask, 0);
...
export const applyBurikoAlphaMask = withBurikoBitmapText(applyBurikoAlphaMaskPixels, {
  replace: true,
  opacity: (args) => (256 - args[3]) / 256,
  applied: (result) => result === 0,
});

export const applyBurikoBitmapMask = withBurikoBitmapText(applyBurikoBitmapMaskPixels, {
  replace: true,
  applied: (result) => result === 0,
});
