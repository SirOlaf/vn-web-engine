...
function splatBurikoBitmapPixels(
  ...
  const accumulate = (offset: number, weight: number, pixel: number): void => {
    ...
    if (coefficient === undefined)
      throw new RangeError('Buriko splat reads outside its native coefficient stack');
...
export const splatBurikoBitmap = withBurikoBitmapText(splatBurikoBitmapPixels, {
  replace: true,
