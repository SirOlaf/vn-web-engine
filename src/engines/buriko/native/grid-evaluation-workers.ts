...
import {BurikoGridEvaluator} from './grid-evaluator.js';
...
class GridEvaluationThread {
  ...
  constructor(
    readonly id: number,
    readonly evaluator: BurikoGridEvaluator,
  ) {}
  notify(): void {
...
export class BurikoGridEvaluationWorkers {
  private nextId = 0;
  ...
  closeAndJoin(): Promise<void> {
    ...
    this.finalClosing = (async () => {
      ...
      if (!this.finalQuiesced && !failed) {
        firstError = new Error('Buriko grid evaluator workers did not quiesce');
        failed = true;
      }
  ...
  private checkAdmission(): void {
    if (this.finalAdmissionClosed)
      throw new Error('Buriko grid evaluator worker admission is closed');
    this.checkFailure();
  }
  ...
  create(output: BurikoBpPointer | null): number {
    ...
    const thread = new GridEvaluationThread(
      this.nextId,
      new BurikoGridEvaluator(this.allocator, this.mainProcessing.capacity),
    );
    this.schedule(thread);
  ...
  setTypeCount(id: number, value: number): number {
    const thread = this.find(id);
    if (thread === undefined) return 1;
    return thread.busy !== 0 ? 14 : burikoLogicalStatus(thread.evaluator.setTypeCount(value));
  }
  takeStatus(output: BurikoBpPointer | null, id: number): number {
    const thread = this.find(id);
    if (thread === undefined) return 1;
  ...
  initialize(
    ...
    if (thread.busy !== 0) return 14;
    if (asynchronous === 0)
      return burikoLogicalStatus(thread.evaluator.initialize(grid, count, input));
  ...
  replaceGrid(id: number, gridId: number): number {
    ...
    const grid = this.grids.get(gridId);
    if (grid === undefined) return 1;
    return thread.busy !== 0 ? 14 : burikoLogicalStatus(thread.evaluator.replaceGrid(grid));
  }
  update(
    id: number,
    index: number,
    ...
    if (thread.busy !== 0) return 14;
    if (asynchronous === 0)
      return burikoLogicalStatus(thread.evaluator.update(index, input, recompute));
  ...
  copyRecord(output: BurikoBpPointer | null, id: number, index: number): number {
    const thread = this.find(id);
    if (thread === undefined) return 1;
    return thread.busy !== 0 ? 14 : burikoLogicalStatus(thread.evaluator.copyRecord(output, index));
  }
}
