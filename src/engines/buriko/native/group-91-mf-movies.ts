...
import {BurikoMfMovieProcess} from './movie-mf-process.js';
...
export function createGroup91MfMovies(
  session: BurikoBrowserMfMovieSession,
  volume: BurikoMfMovieVolumePolicy,
  scheduler: BurikoBpScheduler,
  procedures: BurikoProcedureState,
  clock: BurikoNativeClock,
): BurikoNativeSlotDefinition[] {
  if (session.volume !== volume)
    throw new Error('Buriko MF callbacks require the session volume owner');
  ...
  return [
    {
      ...
      execute: (h) => {
        const rawVolume = pop32(h.thread),
          name = pointer(h),
          archive = pointer(h);
        const process = new BurikoMfMovieProcess(
          h.thread,
          procedures,
          clock,
          session,
        ...
        if (node === null || node.state !== h.thread)
          throw new Error('Buriko MF movie thread is not linked to its scheduler');
