...
import {BurikoProgramFiles} from './program-files.js';

/** 129450/1292e0/129110 bounded, untransformed CAsyncStream file source. */
export class BurikoMovieFileStream {
  ...
  private check(): void {
    if (this.disposed) throw new Error('Buriko movie uses a released file stream');
  }
  private regionLength(): bigint {
    if (this.length === null)
      throw new Error('Buriko movie reads an unwritten native region length');
    return this.length;
  }
  ...
  unlock(actor: object): void {
    ...
    if (this.owner !== actor || this.lockDepth === 0)
      throw new Error("Buriko movie releases another worker's stream lock");
  ...
  async read(
    ...
    try {
      ...
      if (this.base === null) throw new Error('Buriko movie reads an unwritten native region base');
      ...
      if (scratch === null || defined === null)
        throw new Error('Buriko movie reads an absent native scratch allocation');
      if (bytes.length > scratch.length)
        throw new RangeError('Buriko movie ReadFile writes beyond its native scratch allocation');
      scratch.set(bytes);
      ...
      if (count > scratch.length)
        throw new RangeError('Buriko movie memcpy reads beyond its native scratch allocation');
      for (let index = 0; index < count; index++)
        if (defined[index] === 0)
          throw new Error('Buriko movie memcpy reads unwritten native scratch bytes');
      if (count !== 0) {
        if (
          ...
          throw new RangeError('Buriko movie memcpy writes beyond its destination');
  ...
  dispose(): void {
    ...
    if (this.lockDepth !== 0 || this.waiters.length !== 0)
      throw new Error('Buriko movie deletes a busy file-stream critical section');
    this.source = null;
    this.scratch = null;
    this.scratchDefined = null;
    this.path = null;
