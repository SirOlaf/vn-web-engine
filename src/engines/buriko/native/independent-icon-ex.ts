...
import {BurikoIconWords, type BurikoIconRow} from './icon-description.js';
import {BurikoIndependentIcon} from './independent-icon.js';
import {requireBurikoResourceRange} from './bf-entropy.js';

interface Animation {
  present: number;
  visible: number;
  unit: BurikoIconWords | null;
  index: number;
  layer: number;
  frame: number;
...
interface Group {
  words: BurikoIconWords;
  units: BurikoIconWords[];
  animations: Animation[];
}
const u64 = (value: bigint): bigint => BigInt.asUintN(64, value);
const scratch = (): BurikoBitmapRectangle => ({left: 0, top: 0, right: 0, bottom: 0});

/** 094190/type1 and vtable17ED88, over the actual inherited controller and Window owners. */
export class BurikoIndependentIconEx extends BurikoIndependentIcon {
  override readonly type: number = 1;
  private groupCount = 0;
  private groups: Group[] | null = null;
  private sorted: number[] = [];
  private readonly writtenRectangles = new WeakSet<BurikoBitmapRectangle>();

  private group(index: number): Group {
    const group = this.groups?.[index];
    if (group === undefined) throw new Error('Buriko IconEx consumes an absent native group');
    return group;
  }
  private animation(row: number, column: number): Animation {
    const animation = this.group(row).animations[column];
    if (animation === undefined) throw new Error('Buriko IconEx indexes outside animation storage');
    return animation;
  }
  private unit(animation: Animation): BurikoIconWords {
    if (animation.unit === null) throw new Error('Buriko IconEx dereferences an absent unit');
    return animation.unit;
  }
  private readonly bitmapScratch = new WeakMap<BurikoBitmapRectangle, number>();
  private selectedBitmap(value: number | undefined): number {
    if (value === undefined) throw new Error('Buriko IconEx consumes unwritten bitmap scratch');
    return value;
  }
  private compose(animation: Animation, output: BurikoBitmapRectangle): void {
    ...
    if (!this.writtenRectangles.has(output))
      throw new Error('Buriko IconEx consumes unwritten inner-object rectangle');
    this.damage(output);
  }
  private erase(animation: Animation, output: BurikoBitmapRectangle): void {
    if (this.window.eraseInnerObject(output, animation.index) !== 0)
      this.writtenRectangles.add(output);
    if (!this.writtenRectangles.has(output))
      throw new Error('Buriko IconEx consumes unwritten erased-object rectangle');
    this.damage(output);
  }
  private replace(
    animation: Animation,
    source: number | undefined,
    area: BurikoBitmapRectangle,
  ): void {
  ...
  initializeGroups(description: BurikoBitmapGroupDescription): number {
    this.reset();
    ...
    if ((count - 1) >>> 0 < 256) {
      ...
      for (let r = 0; r < count; r++) {
        ...
        if (width === 0) throw new Error('Buriko IconEx grid divides by zero');
      ...
      for (let r = 0; r < count; r++) {
        const original = description.groups[r]!,
          words = new BurikoIconWords(Array.from(original.words)),
          data = new BurikoIconWords(Array(16)),
          group: Group = {words, units: [], animations: []},
          row: BurikoIconRow = {data, icons: []};
        ...
        for (let c = 0; c < n; c++) {
          requireBurikoResourceRange(original.units.length, c * 0xc4, 0xc4);
          const view = new DataView(
              original.units.buffer,
              original.units.byteOffset + c * 0xc4,
              0xc4,
            ),
            unit = new BurikoIconWords(
              Array.from({length: 49}, (_, i) => view.getInt32(i * 4, true)),
            ),
            base = new BurikoIconWords(Array(18)),
          ...
          const entry = {
            ...
            column: c,
            data: base,
            child: null as BurikoVirtualDisplayObject | null,
          };
          ...
          const child = new BurikoVirtualDisplayObject(this.window.environment, index, this.window);
          entry.child = child;
          child.setUnbackedDimensions(bitmap.width, bitmap.height);
          BurikoDisplayObject.prototype.setSecondaryVisibility.call(child, animation.visible);
  ...
  protected override clickOutside(): void {
    ...
    if (point === null) throw new Error('Buriko IconEx consumes an unwritten click');
    let [x, y] = point;
    if (this.modal === 0) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        if (bitmap === null) this.window.setOverlayEnabled(layer, 0);
          ...
          if (!this.writtenRectangles.has(area))
            throw new Error('Buriko IconEx consumes unwritten overlay rectangle');
          this.damage(area);
        }
        this.mark();
      }
  ...
  setIconVisibility(row: number, column: number, value: number): number {
    return BurikoIndependentIconEx.prototype.changeIcon.call(this, row, column, value);
  }
    ...
    if (a.present !== 0) {
      for (let i = 0; ; i++) {
        ...
        if (e.row === row && e.column === column) {
          BurikoDisplayObject.prototype.setSecondaryVisibility.call(e.child!, value);
          break;
        }
      }
  ...
  protected override reset(): void {
    ...
    if (this.groupCount > 0 && this.groups === null)
      throw new Error('Buriko IconEx dereferences freed native groups');
    this.groups = null;
    this.sorted = [];
    super.reset();
  }
