...
export class BurikoNativeSplines {
  ...
  initialize(id: number, count: number, points: BurikoBpPointer | null, duration: number): number {
    count >>>= 0;
    duration >>>= 0;
    if (count < 2) return 2;
  ...
  sample(output: BurikoBpPointer | null, id: number, time: number): number {
    const entry = this.entries.find((entry) => entry.id === id >>> 0);
    if (entry === undefined) return 1;
    if (entry.duration === undefined)
      throw new Error('Buriko spline sample reads uninitialized registry duration');
