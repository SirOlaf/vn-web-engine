...
export interface BurikoBitmapGroupDescription {
  /** Original VM DWORD fields; the tagged pointer field is replaced by groups below when consumed. */
  readonly words: Uint32Array;
  readonly groups: readonly BurikoBitmapUnitGroup[];
}
export interface BurikoBitmapUnitGroup {
  /** Original 0x40-byte-stride VM record, including the full DWORD at offset zero. */
  readonly words: Uint32Array;
  readonly units: Uint8Array;
}
function read(pointer: BurikoBpPointer | null, word: number): number {
  if (pointer === null)
    throw new Error('Buriko bitmap-group conversion dereferences a null description');
...
export function copyBurikoBitmapGroupDescription(
  source: BurikoBpPointer | null,
  memory: BurikoBpMemory,
  thread: BurikoBpThread,
): {result: 0; description: BurikoBitmapGroupDescription} | {result: 2 | 3} {
...
export function formatBurikoBitmapGroups(
