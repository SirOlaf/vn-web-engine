...
export function createGroup90DisplayImmediate(
  ...
  const slots: BurikoNativeSlotDefinition[] = [],
    fatal = (context: BurikoBpOpcodeContext, message: string): Promise<never> =>
      errors.threadFatal(
        context.thread,
        context.diagnostics,
  ...
  const scalar =
    (apply: (handle: number, value: number) => boolean, validate = false): BurikoBpOpcodeHandler =>
    (context) => {
  ...
  const point =
    (apply: (handle: number, x: number, y: number) => boolean): BurikoBpOpcodeHandler =>
    (context) => {
