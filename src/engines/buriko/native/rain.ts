...
import {drawBurikoRainLine} from './rain-line.js';
...
function divide(numerator: number, denominator: number): number {
  ...
  if (denominator === 0 || (numerator === -0x80000000 && denominator === -1))
    throw new RangeError('Buriko rain native signed division fault');
...
export class BurikoRain {
  ...
  private painter: {
    ...
    clip: BurikoBitmapRectangle | undefined;
  ...
  constructor(
    ...
    private readonly ticks: BurikoSystemTicks,
  ) {}
  private check(): void {
    if (this.disposed) throw new Error('Buriko accesses a deleted rain object');
  }
  private now(): number {
  ...
  exchangeParameters(values: Int32Array): Int32Array {
    this.check();
    if (values.length < 16) throw new RangeError('Buriko rain reads sixteen parameter DWORDs');
  ...
  exchangeTransform(values: Int32Array): Int32Array {
    this.check();
    if (values.length < 7) throw new RangeError('Buriko rain reads seven transform DWORDs');
  ...
  private spawnDrops(): void {
    ...
    for (let index = 0; index < this.parameters[13]!; index++) {
      ...
      if (xRange === 0) throw new RangeError('Buriko rain divides by a zero X extent');
      const x = (this.parameters[0]! + (xRandom % xRange)) | 0;
      const y = (this.parameters[1]! - (this.random.next() % 100)) | 0;
      const zRandom = this.random.next();
      if (zRange === 0) throw new RangeError('Buriko rain divides by a zero Z extent');
  ...
  update(): number {
    ...
    if (this.previousTick === undefined)
      throw new Error('Buriko rain reads its unwritten start time');
  ...
  draw(bitmap: BurikoBitmap): void {
    ...
    for (const drop of this.drops) {
      ...
      if (painter.clip === undefined)
        throw new Error('Buriko rain painter reads unwritten clipping bounds');
      if (destination === null)
        throw new Error('Buriko rain painter dereferences an unconfigured pixel buffer');
      drawBurikoRainLine(destination, painter.clip, painter.color, x1, y1, x2, y2);
    }
  }
