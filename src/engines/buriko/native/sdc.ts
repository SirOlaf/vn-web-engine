...
export function decodeBurikoSdc(bytes: Uint8Array): Uint8Array | null {
  const magic = 'SDC FORMAT 1.00\0';
  for (let i = 0; i < magic.length; i++) {
    if (i >= bytes.length)
      throw new RangeError('Buriko SDC signature exceeds native source storage');
...
export function encodeBurikoSdcInto(
...
export function encodeBurikoSdc(input: Uint8Array, seed: number): Uint8Array | null {
  ...
  const restored = decodeBurikoSdc(encoded);
