...
import {BurikoProgramFiles, type BurikoProgramOutputFile} from './program-files.js';
...
export class BurikoNativeFile {
  private static readonly openFiles = new WeakMap<BurikoProgramFiles, Set<BurikoNativeFile>>();
  private source: ByteSource | null = null;
  private output: BurikoProgramOutputFile | null = null;
  private position = 0n;
  ...
  private pathIdentity(wide: string): string {
    ...
    if (this.files.paths !== null) {
      try {
        identity = burikoRegistryFold(this.files.paths.resolveNative(wide));
      } catch (error) {
  ...
  private reserve(path: string, write: boolean): boolean {
    let opened = BurikoNativeFile.openFiles.get(this.files);
    if (opened === undefined) {
      opened = new Set();
      BurikoNativeFile.openFiles.set(this.files, opened);
    }
  ...
  private release(): void {
    BurikoNativeFile.openFiles.get(this.files)?.delete(this);
    this.openPath = null;
    this.mountedPath = null;
  }
  /** 0695F0 -> 08CAF0: OPEN_EXISTING and share-read; failure updates +10. */
  async openRead(path: BurikoBpPointer): Promise<0 | 1> {
  ...
  async openWrite(path: BurikoBpPointer, preserve: number): Promise<0 | 1> {
    if (this.openPath !== null) return 0;
  ...
  async read(output: BurikoBpPointer, count: number, initialized?: Uint8Array): Promise<number> {
  ...
  async getTimes(): Promise<BurikoFileTimes | null> {
  ...
  async setTimes(times: BurikoFileTimes): Promise<0 | 1> {
