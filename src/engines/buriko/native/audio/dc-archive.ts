...
export function burikoAudioWideLower(value: string): string {
  let result = '';
...
export function validateBurikoDcArchiveHeader(header: BurikoBpPointer): number {
...
export interface BurikoDcArchiveInfo {
...
export class BurikoDcArchive {
  private readonly section = new BurikoAsyncCriticalSection();
  ...
  private check(): void {
    if (this.disposed) throw new Error('Buriko DCArchive accesses released owner');
  }
  ...
  async memberInfo(name: string, actor: object): Promise<BurikoDcArchiveInfo> {
    this.check();
    await this.section.enter(actor);
    try {
      const status = await this.openIndex(this.path, actor);
      if (status !== 0) return {status};
      const entry = this.entries.get(burikoAudioWideLower(name));
      ...
      for (;;) {
        if ((await file.openReadWide(path)) !== 0) {
          ...
          try {
            if (!entry.live)
              throw new Error(
                'Buriko DCArchive consumes an entry invalidated while its section was released',
              );
            ...
            if (this.payloadBase === undefined)
              throw new Error('Buriko DCArchive reads unwritten payload base');
