...
export class BurikoStreamSpeaker extends BurikoStaticSpeaker {
  ...
  protected override makeDescriptor(model: BurikoSpeakerModel): BurikoSpeakerDescriptor {
  ...
  override async attach(model: BurikoSpeakerModel): Promise<number> {
    ...
    this.unsubscribe = buffer.onNotification((event) => {
      if (event.index < 0 || event.index >= this.events.length)
        throw new RangeError('Buriko speaker notification exceeds event array');
  ...
  override async detach(): Promise<BurikoSpeakerModel | null> {
    this.cancel = true;
    this.wake?.();
    if (this.worker !== null) {
      await this.worker;
