import {BurikoAudioLevels, burikoAudioPan} from '../audio-levels.js';
import {BurikoAsyncCriticalSection} from '../async-critical-section.js';
import type {BurikoNativeLocks, BurikoLockActors} from '../exclusion-locks.js';
import type {BurikoSystemTicks} from '../system-ticks.js';
import {BurikoSpeakerContext, BurikoStaticSpeaker} from './speaker.js';
import {BurikoStreamSpeaker} from './stream-speaker.js';
import {BurikoSpeakerModel} from './speaker-model.js';
import {BurikoWaveStream, createBurikoWaveStream} from './wave-stream.js';
import {createBurikoWaveStatic, type BurikoWaveStatic} from './wave-static.js';

export class BurikoAudioChannel {
  active = 0;
  model: BurikoSpeakerModel | null = null;
  readonly levels = new BurikoAudioLevels();
  constructor(readonly speaker: BurikoStaticSpeaker) {
    this.levels.master = this.levels.additional = 128;
    this.levels.volume.current = this.levels.envelope.current = 128;
  }
}
export interface BurikoAudioOutputProfile {
...
export class BurikoAudioChannels {
  readonly stream: BurikoAudioChannel[] = [];
  readonly static: BurikoAudioChannel[] = [];
  readonly streamMaster = new Uint32Array(16);
  readonly staticMaster: Uint32Array;
  /**2745B0: actual separate BSS header records, shared with future F4D50 publication. */
  readonly staticHeaders: Uint8Array;
  readonly staticHeadersInitialized: Uint8Array;
  get staticCapacity(): number {
    return this.context.abi.compatibility === '1.69' ? 64 : 128;
  }
  readonly section = new BurikoAsyncCriticalSection();
  flags = 0;
  muted = 0;
  ...
  constructor(
    readonly context: BurikoSpeakerContext,
    readonly locks: BurikoNativeLocks,
    readonly actors: BurikoLockActors,
    readonly ticks: BurikoSystemTicks,
    readonly output: BurikoAudioOutputProfile,
  ) {
    this.staticMaster = new Uint32Array(this.staticCapacity);
    this.staticHeaders = new Uint8Array(this.staticCapacity * 64);
    this.staticHeadersInitialized = new Uint8Array(this.staticCapacity * 64).fill(1);
    if (context.abi.compatibility === '1.69') this.output = {...output, prefer24Bit: false};
  ...
  initialize(window: object): number {
    if ((this.flags & 1) !== 0) throw new Error('Buriko audio channels already initialized');
    this.flags = 0;
    this.window = window;
    for (let index = 0; index < 16; index++)
      this.stream.push(new BurikoAudioChannel(new BurikoStreamSpeaker(this.context)));
    for (let index = 0; index < this.staticCapacity; index++)
      this.static.push(new BurikoAudioChannel(new BurikoStaticSpeaker(this.context)));
    if (!this.context.initialized) throw new Error('Buriko audio backend initialization failed');
    this.flags |= 1;
    return 0;
  }
  ...
  private records(stream: boolean): BurikoAudioChannel[] {
  ...
  async mute(actor = this.actors.currentActor): Promise<void> {
    await this.engine(actor, async () => {
      ...
      for (let index = 0; index < this.staticCapacity; index++)
        await this.setMaster(false, index, 0, actor);
    });
  }
  ...
  async attachStream(
    ...
    if (record === undefined) throw new RangeError('Buriko raw stream attachment exceeds registry');
    const wave = await createBurikoWaveStream(
      bytes,
      {gain, prefer24Bit: this.output.prefer24Bit, abi: this.context.abi},
  ...
  async publishInitializedStream(
    ...
    const record = this.stream[index >>> 0];
    if (record === undefined)
      throw new RangeError('Buriko raw stream publication exceeds registry');
  ...
  async attachStatic(
    ...
    if (record === undefined) throw new RangeError('Buriko raw static attachment exceeds registry');
  ...
  async publishInitializedStatic(
    index: number,
    wave: BurikoWaveStatic,
    fadeMilliseconds: number,
    speed: number,
  ): Promise<number> {
    const record = this.static[index >>> 0];
    if (record === undefined) throw new RangeError('Buriko static publication exceeds registry');
    wave.fadeIn(fadeMilliseconds);
    const previous = await record.speaker.detach();
    await previous?.dispose();
    if (this.context.abi.compatibility === '1.69')
      record.speaker.exchangeLegacyAcceleration(Number(speed !== 1));
    else record.speaker.exchangeRate(speed);
    const model = new BurikoSpeakerModel(wave);
    record.model = model;
    const status = await record.speaker.attach(model);
    if (status !== 0) return 22;
  ...
  async setPanRaw(stream: boolean, index: number, value: number): Promise<number> {
    ...
    if (status !== 0) return status;
    const pan = stream ? burikoAudioPan(value) : ((value - 64) | 0) * 2;
    ...
    return this.withEngineControl(async (actor) => {
      ...
      if (publishLoop !== null)
        await this.internal(actor, () => {
          ...
          if (!(wave instanceof BurikoWaveStream))
            throw new Error('Buriko stream query consumes an absent native WaveBox');
  ...
  async releaseStatic(index: number, operationActor = this.actors.currentActor): Promise<number> {
    return this.withEngineControl(async (actor) => {
      ...
      if (offset + 64 > this.staticHeaders.length)
        throw new RangeError('Buriko static header release exceeds its native array');
