...
export class BurikoBufferProcessor {
  private readonly core: BurikoAudioBufferRenderCore;
  private closed = false;
  private failure: string | null = null;
  constructor(
    format: BurikoAudioBufferFormat,
    outputRate: number,
    private readonly send: (response: BurikoAudioBufferResponse) => void,
  ) {
    this.core = new BurikoAudioBufferRenderCore(format, outputRate);
  }
  receive({id, command}: BurikoAudioBufferRequest): void {
    try {
