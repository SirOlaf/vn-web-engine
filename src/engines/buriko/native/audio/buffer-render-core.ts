 ...
 */
export class BurikoAudioBufferRenderCore {
  ...
  private events: BurikoAudioBufferNotification[] = [];
  ...
  constructor(
    ...
    if (
      ...
      throw new RangeError('Unsupported Buriko browser PCM format');
    this.format = {...format};
    ...
    if (
      ...
      throw new RangeError('Buriko browser buffer requires complete PCM frames');
  ...
  status(): BurikoAudioBufferStatus {
    return {playing: this.playing, byteCursor: this.cursor, renderFrame: this.frame};
  }
  takeNotifications(): BurikoAudioBufferNotification[] {
  ...
  command(command: BurikoAudioBufferCommand): BurikoAudioBufferStatus {
    if (this.disposed) throw new Error('Buriko browser audio buffer is disposed');
    switch (command.kind) {
      case 'write': {
        ...
        if (
          ...
          throw new RangeError('Buriko browser audio write exceeds ring storage');
      ...
      case 'notifications':
        for (const offset of command.offsets)
          if (
            ...
            throw new RangeError('Buriko browser notification is outside buffer');
      ...
      case 'seek':
        if (
          ...
          throw new RangeError('Buriko browser seek exceeds buffer');
      ...
      case 'volume':
        if (
          ...
          throw new RangeError('Buriko browser attenuation is outside DirectSound range');
      ...
      case 'pan':
        if (
          ...
          throw new RangeError('Buriko browser pan is outside DirectSound range');
        this.left =
          command.decibels >= 10000
            ? 0
            : command.decibels > 0
  ...
  private sample(offset: number): number {
    ...
    for (let i = 0; i < width; i++) {
      ...
      if (at < 0 || at >= this.bytes.length || this.initialized[at] === 0)
        throw new Error('Buriko audio render consumes unwritten or out-of-range PCM');
  ...
  render(output: readonly Float32Array[]): void {
    ...
    if (
      ...
      throw new RangeError('Buriko audio render output layout mismatch');
