...
import {
  ...
  type BurikoWaveBoxCheckpoint,
} from './wavebox-header.js';

export interface BurikoWaveBoxDecoder {
...
export type BurikoWaveEffect<T> = T | Promise<T>;
type Operation<T> = Generator<BurikoWaveEffect<number>, T, number>;
/** Runs synchronous memory effects immediately; resumes live effects only after actual I/O. */
function run<T>(operation: Operation<T>): BurikoWaveEffect<T> {
  const step = (value: number): BurikoWaveEffect<T> => {
    for (;;) {
...
export interface BurikoLiveWaveBoxDecoder extends Omit<
...
> {
  ...
  reset(actor?: object): BurikoWaveEffect<void>;
  restartLoop(actor?: object): BurikoWaveEffect<void>;
  dispose(): BurikoWaveEffect<void>;
}
export interface BurikoWaveBoxDecoderOptions {
  /** Binary64 multiplier stored at native model +78 and applied before the output FIFO. */
  readonly gain: number;
}

export interface BurikoAdpcmState {
  predictor: number;
  step: number;
}
...
export function decodeBurikoAdpcm4Sample(code: number, state: BurikoAdpcmState): number {
...
export function decodeBurikoHfAdpcm8Sample(
  code: number,
  state: BurikoAdpcmState,
  shiftParameter: number,
): number {
  const threshold = 1 << ((((shiftParameter + 1) >>> 0) >>> 1) & 31);
  const divisor = (threshold * 2) >>> 0;
  if (divisor === 0) throw new RangeError('Buriko HFADPCM8 native integer division by zero');
...
class NativeBuffer {
  ...
  require(offset: number, length: number): void {
    if (
      ...
      throw new RangeError('Buriko WaveBox memory access exceeds native allocation');
    for (let index = offset; index < offset + length; index++)
      if (this.defined[index] === 0)
        throw new Error('Buriko WaveBox reads unwritten native allocation');
  }
  ...
  storeSample(offset: number, value: number): void {
    if (offset < 0 || offset + 2 > this.bytes.length)
      throw new RangeError('Buriko WaveBox sample write exceeds native allocation');
  ...
  moveWithin(start: number, length: number): void {
    if (start < 0 || length < 0 || start + length > this.bytes.length)
      throw new RangeError('Buriko WaveBox refill exceeds native allocation');
...
interface WaveSource {
  seek(offset: number, actor?: object): BurikoWaveEffect<number>;
  read(
    buffer: NativeBuffer,
    offset: number,
    count: number,
    actor?: object,
  ): BurikoWaveEffect<number>;
  dispose(): BurikoWaveEffect<void>;
}
class LiveSource implements WaveSource {
  constructor(readonly storage: BurikoLiveAudioStorage) {}
  seek(offset: number, actor?: object): BurikoWaveEffect<number> {
...
abstract class CustomDecoder implements BurikoLiveWaveBoxDecoder {
  ...
  protected divide(numerator: number, denominator: number): number {
    if (denominator === 0) throw new RangeError('Buriko WaveBox native integer division by zero');
  ...
  initialize(actor?: object): BurikoWaveEffect<void> {
...
class Pcm16Decoder extends CustomDecoder {
  protected *readOperation(
    ...
    let totalBytes = 0;
    while (remaining !== 0) {
      if (blockSize === 0) throw new Error('Buriko PCM16 native frame reader cannot make progress');
...
abstract class AdpcmDecoder extends CustomDecoder {
  readonly left: BurikoAdpcmState = {predictor: 0, step: 127};
  readonly right: BurikoAdpcmState = {predictor: 0, step: 127};
  protected restore(left: BurikoWaveBoxCheckpoint, right: BurikoWaveBoxCheckpoint): void {
...
class Adpcm4Decoder extends AdpcmDecoder {
  ...
  protected *readOperation(
    ...
    while (completed < frames) {
      if (blockFrames === 0)
        throw new Error('Buriko ADPCM4 native frame reader cannot make progress');
  ...
  protected *loopOperation(actor?: object): Operation<void> {
    ...
    if (this.channels === 1 && (this.framePosition & 1) !== 0) {
      ...
      this.pending = decodeBurikoAdpcm4Sample(byte.byte(0) >>> 4, this.left);
...
class HfAdpcm8Decoder extends AdpcmDecoder {
  ...
  protected *readOperation(
    ...
    while (completed < frames) {
      if (blockFrames === 0)
        throw new Error('Buriko HFADPCM8 native frame reader cannot make progress');
...
function createCustom(
  ...
  throw new TypeError('OGG WaveBox resources require the Buriko OGG decoder');
}
function synchronous<T>(value: BurikoWaveEffect<T>): T {
...
export function createBurikoCustomWaveBoxDecoder(
  bytes: Uint8Array,
  options: BurikoWaveBoxDecoderOptions,
): BurikoWaveBoxDecoder {
  const decoder = createCustom(
    parseBurikoWaveBoxHeader(bytes),
...
export async function createBurikoLiveCustomWaveBoxDecoder(
  storage: BurikoLiveAudioStorage,
  selectedCodec: 0 | 1 | 2,
  options: BurikoWaveBoxDecoderOptions,
  actor: object,
): Promise<BurikoLiveWaveBoxDecoder> {
  const source = new LiveSource(storage);
  try {
    if (storage.size >>> 0 < 64)
      throw new BurikoWaveBoxError(0x10000003, 'Buriko WaveBox input is shorter than model header');
    ...
    if (view.getUint32(4, true) !== 0x20207762)
      throw new BurikoWaveBoxError(0x11000001, 'Buriko WaveBox bw marker does not match');
    requireBurikoWaveHeaderBytes(defined, 48, 4);
    if (view.getUint32(48, true) !== selectedCodec)
      throw new BurikoWaveBoxError(0x11000004, 'Buriko WaveBox selected model rejects codec');
    requireBurikoWaveHeaderBytes(defined, 0, 64);
    const decoder = createCustom(parseBurikoWaveBoxHeader(raw), source, options.gain);
