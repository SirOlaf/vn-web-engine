export class BurikoWaveBoxError extends Error {
  constructor(
    readonly nativeCode: number,
    message: string,
  ) {
    super(message);
    this.name = 'BurikoWaveBoxError';
  }
}

export interface BurikoWaveBoxCheckpoint {
  readonly predictor: number;
  readonly step: number;
}

export interface BurikoWaveBoxHeader {
  ...
  readonly loopLeft: BurikoWaveBoxCheckpoint;
  readonly loopRight: BurikoWaveBoxCheckpoint;
...
export function parseBurikoWaveBoxHeader(bytes: Uint8Array): BurikoWaveBoxHeader {
  if (bytes.length < 64)
    throw new BurikoWaveBoxError(0x0e, 'Buriko WaveBox header is shorter than 64 bytes');
  ...
  if (codec !== 0 && codec !== 1 && codec !== 2 && codec !== 3)
    throw new BurikoWaveBoxError(0x0e, 'Buriko WaveBox selector has an invalid codec');
...
export function requireBurikoWaveHeaderBytes(
  mask: Uint8Array,
  offset: number,
  length: number,
): void {
  if (offset + length > mask.length || mask.subarray(offset, offset + length).includes(0))
    throw new Error('Buriko WaveBox consumes unwritten header bytes');
}
