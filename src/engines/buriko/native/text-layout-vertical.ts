...
import {burikoTextCodes, type BurikoRubyAnnotations} from './text-annotations.js';
...
import type {
  BurikoHorizontalTextCursor,
  BurikoHorizontalTextEffect,
  BurikoHorizontalTextLayoutNode,
  BurikoHorizontalTextLayoutOptions,
...
function divide(a: number, b: number): number {
  ...
  if (b === 0 || (a === -0x80000000 && b === -1))
    throw new RangeError('Buriko vertical layout native integer division fault');
...
function allocate(state: BurikoTextLayoutState, width: number, height: number): BurikoBitmap {
  const format = state.surfaces.compositor.defaultFormat;
  const bitmap = allocateBurikoBitmap(width, height, format === 1 ? 2 : format);
  clearBurikoBitmap(bitmap);
  return bitmap;
}
function empty(): BurikoBitmap {
...
function rotateBurikoVerticalGlyphPixels(bitmap: BurikoBitmap): boolean {
  if (bitmap.width !== bitmap.height) return false;
  const temporary = allocateBurikoBitmap(bitmap.width, bitmap.height, bitmap.format);
  try {
    ...
    for (let y = 0; y < bitmap.height >>> 0; y++)
      for (let x = 0; x < bitmap.width >>> 0; x++) {
        ...
        if (bitmap.storage === null || temporary.storage === null)
          throw new TypeError('Buriko vertical glyph rotation dereferences null storage');
    ...
    copyBurikoBitmapRows(bitmap, temporary);
  } finally {
...
export const rotateBurikoVerticalGlyph = withBurikoBitmapText(rotateBurikoVerticalGlyphPixels, {
  source: 0,
  destination: 0,
  replace: true,
...
function glyph(
  ...
  const draw = (destination: BurikoBitmap, selectedColor: number, decorative = false): void => {
    if (wide === 0) return;
    if (reading) {
      if (font.raster === null)
        throw new Error('Buriko vertical reading font raster is uninitialized');
      rasterBurikoGlyph(destination, font.raster, wide, selectedColor, {
        vertical: true,
        decorative,
      });
    } else
  ...
  try {
    draw(main, color);
    if (rotation) rotateBurikoVerticalGlyph(main);
    if ((effect.mode | 0) !== 0) {
      ...
      try {
        if (effect.color >>> 0 === 0)
          ...
          if (rotation && !reading) rotateBurikoVerticalGlyph(shadow);
        }
        recordBurikoBitmapText(shadow, '', {decorative: true, vertical: true});
...
function relink(nodes: BurikoHorizontalTextLayoutNode[]): void {
...
export function buildBurikoVerticalTextLayout(
  state: BurikoTextLayoutState,
  options: BurikoHorizontalTextLayoutOptions,
): {result: 0 | 1; nodes: BurikoHorizontalTextLayoutNode[]; outputCount: number} {
...
export async function addBurikoVerticalReadings(
  state: BurikoTextLayoutState,
  nodes: BurikoHorizontalTextLayoutNode[],
  fontId: number,
  color: number,
  ...
  if (font === null) throw new Error('Buriko vertical reading font was not published');
  for (let index = 0; index < nodes.length; index++) {
    ...
    const count = annotation.readingLength | 0,
      ...
      readings: BurikoHorizontalTextLayoutNode[] = [];
    let y =
        (parent.y +
          (size >> 3) +
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (code === undefined)
        throw new RangeError('Buriko vertical reading code exceeds its native array');
...
export function alignBurikoVerticalTextNodes(
  state: BurikoTextLayoutState,
  nodes: BurikoHorizontalTextLayoutNode[],
