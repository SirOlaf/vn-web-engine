...
export const BURIKO_INSTALLER_TITLE = 'BURIKO General Interpreter Integrated Installer';

/** C7A60/C79F0 and their two dialog callbacks over one selected modal host. */
export class BurikoInstallerDialogs {
  constructor(
  ...
  private async checkDestination(path: string, suffix: string | null): Promise<string> {
    ...
    const drive = /^[a-z]:\\/i.exec(value),
      type =
        drive === null
          ? 0
          : this.files.media.driveTypes[drive[0]!.toLowerCase().charCodeAt(0) - 97];
    const legal =
      drive !== null &&
    ...
    if (!legal) {
      await this.dialogs.show(
        this.files.text.encodeWide('An illegal path was selected.', 1),
        null,
        0x30,
      );
      return value;
    ...
    if (type !== 3) {
      await this.dialogs.show(
        this.files.text.encodeWide('Please choose a stationary harddrive.', 1),
        null,
        0x30,
      );
      return value;
    ...
    const tree = new BurikoDirectoryTree(this.files);
    try {
  ...
  async chooseDestination(
    outputPath: BurikoBpPointer | null,
    outputA: BurikoBpPointer | null,
    outputB: BurikoBpPointer | null,
    ...
    if (outputA === null || outputB === null)
      throw new Error('Buriko installer dialog dereferences a null option output');
    if (editable) {
      // GetDlgItemTextW(..., 0x30c) retains at most 779 UTF-16 code units.
      const path = await this.checkDestination(
        selected.path.slice(0, 779),
        this.decode(rootSuffix),
      );
