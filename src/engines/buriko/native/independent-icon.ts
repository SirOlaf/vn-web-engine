...
import {BurikoVirtualDisplayObject} from './display-virtual.js';
import type {BurikoWindowDisplayObject} from './display-window.js';
import {
  BurikoIconWords,
  type BurikoIconDescription,
  type BurikoIconRow,
} from './icon-description.js';
import {
  BurikoIndependentProcedure,
  type BurikoIndependentProcedures,
...
export class BurikoIndependentIconState {
  foregroundOnly = 0;
...
interface FlatIcon {
  ...
  row: number;
  column: number;
  data: BurikoIconWords;
  child: BurikoVirtualDisplayObject | null;
}
const rectangle = (): BurikoBitmapRectangle => ({left: 0, top: 0, right: 0, bottom: 0});
function prepare(window: BurikoWindowDisplayObject): void {
...
export function drawBurikoIconDescription(
  window: BurikoWindowDisplayObject,
  description: BurikoIconDescription,
): number {
  prepare(window);
...
export class BurikoIndependentIcon extends BurikoIndependentProcedure {
  readonly type: number = 0;
  protected readonly helper: BurikoVirtualDisplayObject;
  protected running = 0;
  protected rows: BurikoIconRow[] = [];
  ...
  constructor(
    ...
    readonly window: BurikoWindowDisplayObject,
    protected readonly input: BurikoNativeInput,
    protected readonly priorities: BurikoProcedureState,
    protected readonly clock: BurikoNativeClock,
    protected readonly cursor: BurikoCursorPolicy,
    protected readonly settings: BurikoIndependentIconState,
  ) {
    super(shared, window);
    settings.initialize();
  ...
  protected createChild(
    ...
    y: number,
    activate: boolean,
  ): BurikoVirtualDisplayObject {
    const child = new BurikoVirtualDisplayObject(this.window.environment, order, this.window);
  ...
  protected damage(r: BurikoBitmapRectangle): void {
  ...
  private defined(value: number | undefined): number {
    if (value === undefined) throw new Error('Buriko Icon consumes uninitialized controller state');
    return value;
  }
  protected row(index: number): BurikoIconRow {
    const row = this.rows[index];
    if (row === undefined) throw new Error('Buriko Icon indexes outside its native row array');
    return row;
  }
  protected item(index: number): FlatIcon {
    const item = this.flat[index];
    if (item === undefined)
      throw new Error('Buriko Icon indexes outside its native flattened array');
    return item;
  }
  ...
  protected childFor(row: number, column: number): BurikoVirtualDisplayObject | null {
  ...
  protected installObject(object: BurikoVirtualDisplayObject | BurikoWindowDisplayObject): void {
    this.input.installObjectCapture(object.sortKey(), [0, 0, 0, 0], object);
  }
  initialize(description: BurikoIconDescription): number {
    this.reset();
    prepare(this.window);
    let status = 0x80000001;
    const n = description.header.read(0);
    if ((n - 1) >>> 0 < 256) {
      ...
      for (const row of description.rows) {
        ...
        if (width === 0) throw new Error('Buriko Icon grid divides by zero native width');
      ...
      for (let r = 0; r < n; r++) {
        ...
        const row: BurikoIconRow = {data, icons: []};
        this.rows.push(row);
        for (let c = 0; c < data.read(0); c++) {
          ...
          const copied = bitmap === null ? new BurikoIconWords(Array(18)) : icon.clone();
      ...
      if (this.flat.length !== total)
        throw new Error('Buriko Icon native flattened allocation differs from row traversal');
  ...
  protected bitmapFor(
    ...
    if (i === undefined) throw new Error('Buriko Icon reads a negative native icon index');
    let id = i.read(3);
  ...
  protected redrawIcon(
    icon: BurikoIconWords,
    next: number | undefined,
    old: number | undefined,
  ): number {
    if (next === undefined || old === undefined)
      throw new Error('Buriko Icon consumes unwritten bitmap selection scratch');
  ...
  protected updateOverlays(data: BurikoIconWords, firstWord: number, firstLayer: number): void {
    const inset = this.window.getTextRectangle();
    for (let i = 0; i < 2; i++) {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (bitmap === null) this.window.setOverlayEnabled(layer, 0);
        ...
        if (area === null) throw new Error('Buriko Icon consumes unpublished overlay rectangle');
        this.damage(area);
      }
      this.mark();
    }
    ...
    if (index === -1) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        if (bitmap === null) this.window.setOverlayEnabled(layer, 0);
          ...
          this.window.setOverlayEnabled(layer, 1);
          const area = this.window.overlayRectangle(layer);
          if (area === null) throw new Error('Buriko Icon consumes unpublished hover rectangle');
          this.damage(area);
        }
        this.mark();
      }
    ...
    if (this.inputEnabled !== 0) {
      if (this.hover !== -1 && this.row(this.item(this.hover).row).data.read(6) !== 0) {
        ...
        if (this.pointerPrevious === undefined)
          throw new Error('Buriko Icon reads unwritten prior pointer');
    ...
    switch (action) {
      ...
      case 12:
        if (selectedRow !== null) {
          ...
          if (w === 0) throw new Error('Buriko Icon navigation divides by zero');
  ...
  override dispose(): void {
    ...
    BurikoIndependentIcon.prototype.reset.call(this);
