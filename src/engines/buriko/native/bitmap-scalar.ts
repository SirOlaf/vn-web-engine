...
export function bitmapRead8(bitmap: BurikoBitmap, offset: number): number {
  return bitmapStorage(bitmap, offset, 1, true).bytes[offset]!;
}
export function bitmapRead16(bitmap: BurikoBitmap, offset: number): number {
  return bitmapStorage(bitmap, offset, 2, true).view.getUint16(offset, true);
}
export function bitmapRead32(bitmap: BurikoBitmap, offset: number): number {
  return bitmapStorage(bitmap, offset, 4, true).view.getUint32(offset, true);
}
export function bitmapWrite8(bitmap: BurikoBitmap, offset: number, value: number): void {
  const storage = bitmapStorage(bitmap, offset, 1, false);
  storage.bytes[offset] = value;
  storage.written(offset, 1);
}
export function bitmapWrite16(bitmap: BurikoBitmap, offset: number, value: number): void {
  const storage = bitmapStorage(bitmap, offset, 2, false);
  storage.view.setUint16(offset, value, true);
  storage.written(offset, 2);
}
export function bitmapWrite32(bitmap: BurikoBitmap, offset: number, value: number): void {
...
function copyBurikoTransparent16Pixels(destination: BurikoBitmap, source: BurikoBitmap): void {
...
function addBuriko16Pixels(destination: BurikoBitmap, source: BurikoBitmap, opacity: number): void {
  for (let y = 0; y < source.height >>> 0; y++)
    for (let x = (source.width >>> 0) - 1; x >= 0; x--) {
      const sourceOffset = source.offset + y * source.stride + x * 2;
      const destinationOffset = destination.offset + y * destination.stride + x * 2;
...
function eraseBuriko16Pixels(destination: BurikoBitmap, source: BurikoBitmap): void {
  for (let y = 0; y < source.height >>> 0; y++)
    for (let x = (source.width >>> 0) - 1; x >= 0; x--)
...
function addBurikoOpaque32Pixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  opacity: number,
): void {
...
function isolateBurikoChannelPixels(
  destination: BurikoBitmap,
  source: BurikoBitmap,
  channel: number,
): void {
...
export const copyBurikoTransparent16 = withBurikoBitmapText(copyBurikoTransparent16Pixels);

export const mixBuriko16 = withBurikoBitmapText(mixBuriko16Pixels, {
  opacity: (args) => (256 - args[2]) / 256,
});

export const addBuriko16 = withBurikoBitmapText(addBuriko16Pixels, {
  opacity: (args) => args[2] / 256,
});

export const addBurikoOpaque32 = withBurikoBitmapText(addBurikoOpaque32Pixels, {
  opacity: (args) => args[2] / 256,
});

export const tintBuriko16 = withBurikoBitmapText(tintBuriko16Pixels, {replace: true});

export const isolateBurikoChannel = withBurikoBitmapText(isolateBurikoChannelPixels, {
  replace: true,
});

export const eraseBuriko16 = withBurikoBitmapText(eraseBuriko16Pixels, {source: 0, replace: true});

export const eraseBuriko32 = withBurikoBitmapText(eraseBuriko32Pixels, {source: 0, replace: true});
