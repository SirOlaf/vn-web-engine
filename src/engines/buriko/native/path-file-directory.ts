...
function required(pointer: BurikoBpPointer | null): BurikoBpPointer {
  if (pointer === null) throw new RangeError('Buriko path service consumed a null native path');
  return pointer;
}
...
export class BurikoPathFileDirectory {
  readonly metadata: BurikoMountedFileMetadata;

  constructor(readonly files: BurikoProgramFiles) {
    if (files.metadata === null)
      throw new Error('Buriko path services require the shared mounted file metadata owner');
    this.metadata = files.metadata;
  }

  private path(pointer: BurikoBpPointer | null): string {
    return this.files.mountedPath(this.files.text.decodeAuto(required(pointer)));
  }

  async createDirectory(path: BurikoBpPointer | null): Promise<number> {
    try {
  ...
  async removeDirectory(path: BurikoBpPointer | null): Promise<number> {
    try {
  ...
  async isDirectory(path: BurikoBpPointer | null): Promise<number> {
    try {
  ...
  async getAttributes(path: BurikoBpPointer | null): Promise<number> {
    try {
      const attributes = await this.metadata.getAttributes(this.path(path));
      if (attributes === null)
        throw new Error('Buriko imported file attributes are unavailable in the mounted profile');
      return attributes;
    } catch (error) {
  ...
  async setAttributes(path: BurikoBpPointer | null, attributes: number): Promise<number> {
    try {
  ...
  splitPath(
    drive: BurikoBpPointer | null,
    directory: BurikoBpPointer | null,
    ...
    if (wide.length > 783)
      throw new RangeError('Buriko split path overwrites its native wide stack buffer');
    let driveText = '';
