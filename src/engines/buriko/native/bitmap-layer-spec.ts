...
export interface BurikoBitmapLayer {
  needsLoad: boolean;
...
function number(
  ...
  for (;;) {
    ...
    if (digit >= 48 && digit <= 57)
      value = (Math.imul(value, hexadecimal ? 16 : 10) + digit - 48) | 0;
    else if (hexadecimal && digit >= 97 && digit <= 102)
      value = (Math.imul(value, 16) + digit - 97) | 0;
    else break;
    found = true;
  }
...
export function parseBurikoBitmapLayers(
  text: BurikoNativeText,
  name: Uint8Array,
): {result: 0; layers: BurikoBitmapLayer[]} | {result: 0x80000001; part: number} {
  const bytes = terminatedNativeBytes(name).slice(),
    layers: BurikoBitmapLayer[] = [];
  ...
  while (start !== null) {
    ...
    const x = number(text, bytes, cursor),
      y = number(text, bytes, cursor);
    const mode = number(text, bytes, cursor),
      opacity = number(text, bytes, cursor);
    layers.push({
      needsLoad: true,
      name: filename,
      positioned: x !== null && y !== null,
      x: x ?? 0,
      y: y ?? 0,
      mode: mode === null ? 0 : mode >>> 0 <= 7 ? mode + 32 : 128,
...
export function burikoJapaneseNumber(value: number): string {
  value >>>= 0;
  if (value === 0) return '零';
