import {burikoIsoAvcConfiguration} from './movie-iso-codecs.js';
import {
...
import {burikoIsoDecodeStart} from './movie-iso-timeline.js';

/** Explicit retained-output profile; not a bound on opaque browser decoder allocations. */
export interface BurikoMovieVideoOutputLimits {
...
function copyOutputLimits(
  limits: BurikoMovieVideoOutputLimits | undefined,
): BurikoMovieVideoOutputLimits | undefined {
...
function admitGeometry(frame: VideoFrame, limits: BurikoMovieVideoOutputLimits): void {
  const rectangle = frame.visibleRect;
  if (rectangle === null) throw new BurikoIsoSampleError('AVC output has no visible rectangle');
  for (const [width, height] of [
    ...
    if (
      ...
      throw new BurikoIsoSampleError('AVC output exceeds its declared geometry profile');
  }
  if (
    ...
    throw new BurikoIsoSampleError('AVC visible rectangle exceeds its coded geometry');
}

export interface BurikoDecodedMoviePicture {
 ...
 */
export class BurikoMovieVideoDecoder {
  private decoder: VideoDecoder;
  private readonly pictures: BurikoDecodedMoviePicture[] = [];
  ...
  private constructor(
    ...
    private readonly limits: BurikoMovieVideoOutputLimits | undefined,
  ) {
  ...
  static async create(
  ...
  ): Promise<BurikoMovieVideoDecoder> {
    ...
    for (const sample of track.samples) {
      ...
      if (description === undefined)
        throw new BurikoIsoSampleError('AVC sample has no description');
      const config = burikoIsoAvcConfiguration(description);
    ...
    return new BurikoMovieVideoDecoder(movie, track, configurations, copiedLimits);
  }
      ...
      output: (frame) => {
        ...
        try {
          ...
          if (
            ...
            throw new BurikoIsoSampleError('AVC decoder returned an unknown access-unit timestamp');
  ...
  async next(): Promise<BurikoDecodedMoviePicture | null> {
    const generation = this.generation;
    this.check(generation);
    if (this.readingGeneration === generation) throw new Error('Concurrent Buriko AVC reads');
    ...
    try {
      for (;;) {
        ...
        if (sample.description !== this.configuredDescription) {
          ...
          if (config === undefined)
            throw new BurikoIsoSampleError('AVC description was not negotiated');
        ...
        if (!Number.isSafeInteger(duration))
          throw new BurikoIsoSampleError('AVC duration exceeds the browser time range');
  ...
  seek(sampleIndex: number, endIndex = this.track.samples.length): number {
    ...
    const index = burikoIsoDecodeStart(this.track, sampleIndex);
    if (
      !Number.isInteger(endIndex) ||
      endIndex <= sampleIndex ||
      endIndex > this.track.samples.length
    )
      throw new BurikoIsoSampleError('AVC edit decode extent is outside its sample table');
    this.generation++;
