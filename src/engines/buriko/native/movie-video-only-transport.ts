...
export interface BurikoVideoOnlyMovieEvent extends BurikoMovieFilterEvent {
 ...
 */
export class BurikoMovieVideoOnlyTransport {
  ...
  private readonly eventObservers = new Set<(event: BurikoVideoOnlyMovieEvent) => void>();
  private readonly failureObservers = new Set<(error: unknown) => void>();
  private completion: BurikoVideoOnlyMovieEvent | null = null;
  ...
  private constructor(
    ...
    this.pin = new BurikoMovieReceivePin(renderer, this.events, timeline.clock, timeline.clock);
  ...
  static async create(
    ...
    signal?: AbortSignal,
  ): Promise<BurikoMovieVideoOnlyTransport> {
    if (
    ...
    const owner = new BurikoMovieVideoOnlyTransport(timeline, messages, renderer);
    const abort = (): void => {
      owner.terminal = true;
    ...
    try {
      owner.check();
      owner.pump = await BurikoMovieVideoPump.createVideoOnly(
        timeline,
        owner.pin,
        {...limits},
        maxRgb32Bytes,
  ...
  onEvent(observer: (event: BurikoVideoOnlyMovieEvent) => void): () => void {
  ...
  private drainEvents(): void {
    for (;;) {
      ...
      const value: BurikoVideoOnlyMovieEvent = {...event, generation: this.generation};
