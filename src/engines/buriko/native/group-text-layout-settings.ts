...
import {
  ...
  createBurikoHorizontalTextEffect,
  drawBurikoHorizontalTextWindowFacade,
  drawBurikoRegisteredHorizontalTextToSurface,
  type BurikoRegisteredSurfaceHorizontalTextOptions,
...
export function createTextLayoutSettings(
  ...
  const readingFont = (context: BurikoBpOpcodeContext, extended: boolean): 0 => {
...
export function createHorizontalTextLayoutServices(
  ...
  const fatal = (context: BurikoBpOpcodeContext, message: string): Promise<never> =>
    errors.threadFatal(context.thread, context.diagnostics, state.text.encodeWide(message, 0));
  const pointer = (context: BurikoBpOpcodeContext): BurikoBpPointer | null =>
    context.memory.resolve(context.thread, pop32(context.thread));
  const requiredPointer = (value: BurikoBpPointer | null, owner: string): BurikoBpPointer => {
  ...
  const drawSurface = async (
    ...
    const result = await drawBurikoRegisteredHorizontalTextToSurface(state, options);
    if (result === 0x80000001)
    ...
    if (result !== 0)
      throw new Error('Buriko surface horizontal text returned an unknown native status');
  ...
  const drawWindow = async (
    ...
    color: number,
    readingColor: number,
    effect: Parameters<typeof drawBurikoHorizontalTextWindowFacade>[7],
  ): Promise<0> => {
    const result = await drawBurikoHorizontalTextWindowFacade(
      windows,
      handle,
      source,
      readingEnabled,
  ...
  return [
    {
      ...
      execute: (context) => {
        const wrapping = pop32(context.thread),
          ...
          source = requiredPointer(pointer(context), 'Buriko window horizontal text'),
    ...
    },
    {
      ...
      execute: (context) => {
        ...
        return drawWindow(
          ...
          color,
          color,
          disableEffect === 0 ? state.defaultEffect : BURIKO_DISABLED_HORIZONTAL_TEXT_EFFECT,
        );
      },
    },
    {
      ...
      execute: (context) => {
        const opacity = pop32(context.thread),
          ...
          source = requiredPointer(pointer(context), 'Buriko window horizontal text'),
          handle = pop32(context.thread),
          effect = createBurikoHorizontalTextEffect(mode, radiusX, radiusY, effectColor, opacity);
        return drawWindow(
          context,
          handle,
          source,
    ...
    },
    {
      ...
      execute: async (context): Promise<0> => {
        const color = pop32(context.thread),
          ...
          readingEnabled = pop32(context.thread),
          source = requiredPointer(pointer(context), 'Buriko surface horizontal text'),
          y = pop32(context.thread),
          x = pop32(context.thread),
          surface = pop32(context.thread),
