...
export interface BurikoMovieQuality {
  type: 0 | 1;
  proportion: number;
  late: bigint;
  timestamp: bigint;
...
export interface BurikoMovieRenderDecision {
...
const divide32 = (value: number, divisor: number): number => {
  ...
  if (divisor === 0 || (value === -0x80000000 && divisor === -1)) {
    throw new RangeError('Buriko movie quality integer division faults');
  }
...
/**
 * The exact renderer policy in the reference BGI executable. The codec reports the actual
 ...
 */
export class BurikoMovieRenderTiming {
  ...
  quality(lateness: bigint, streamTime: bigint): BurikoMovieQuality {
