...
import type {BurikoGdbRestore} from './gdb-restore.js';
import type {BurikoNativeSlotDefinition} from './types.js';

/** Bank 81:80 resolves five live VM pointers and collapses the lower status to boolean. */
export function createGroup81GdbRestore(restore: BurikoGdbRestore): BurikoNativeSlotDefinition[] {
  return [
    {
      primary: 0x81,
      secondary: 0x80,
      ...
      execute: (h) => {
        ...
        if (source === null) throw new Error('Buriko GDB restore dereferences a null SDC source');
        push32(
          h.thread,
          Number(
            restore.restore(
