...
import {
  ...
  type BurikoDisplayPropertyDestination,
...
import {applyBurikoAlphaMask, applyBurikoBitmapMask} from './bitmap-alpha-mask.js';
import {
...
import {
  ...
  type BurikoMeshScanline,
} from './bitmap-mesh.js';
import {blendMixedBurikoBitmapsIntoRgb, mixBurikoBitmaps} from './bitmap-mix.js';
...
import {
  BurikoDisplayObject,
  type BurikoDisplayObjectEnvironment,
  type BurikoDisplayPoint,
} from './display-object.js';
import {burikoRectangleContained} from './display-damage.js';
import {BurikoSpriteEffects} from './sprite-effects.js';
import {burikoSpriteBounds} from './sprite-bounds.js';
...
export type BurikoSpriteConfigurationStatus =
  ...
  | 0x80000009
  | 0x8000000a;
export type BurikoSpriteMaskAssociationStatus =
  0 | 0x8000000a | 0x8000000b | 0x8000000c | 0x8000000d | 0x8000000e;

export interface BurikoSpriteAffineConfiguration {
...
export interface BurikoSpriteAffineBlendConfiguration {
  readonly sourceSurface: number;
...
export interface BurikoSpriteMeshConfiguration {
  readonly sourceSurface: number;
  readonly secondarySurface?: number;
  readonly mixValue?: number;
  readonly blendSelector?: number;
...
const rectangleWidth = (rectangle: BurikoBitmapRectangle): number =>
  (rectangle.right - rectangle.left + 1) | 0;
const rectangleHeight = (rectangle: BurikoBitmapRectangle): number =>
  (rectangle.bottom - rectangle.top + 1) | 0;

function releaseOwned(bitmap: BurikoBitmap | null): null {
  bitmap?.storage?.release();
  return null;
}

function crop(bitmap: BurikoBitmap, rectangle: BurikoBitmapRectangle): BurikoBitmap | null {
  const selected = {...bitmap};
  return cropBurikoBitmap(selected, rectangle) ? selected : null;
}
...
export class BurikoDisplaySprite extends BurikoDisplayObject {
  mode: BurikoSpriteMode = 0;
  sourceSurface = -1;
  secondarySurface = -1;
  staticMaskSurface = -1;
  ...
  wavePhase = 0;
  waveAmplitude = 0;
  readonly effects: BurikoSpriteEffects;
  ...
  private dynamicMask: BurikoDisplaySprite | null = null;
  private maskOwner: BurikoDisplaySprite | null = null;
  ...
  private waveLevel = 0;
  private revealMask: BurikoBitmap | null = null;
  ...
  private meshConfiguration: BurikoSpriteMeshConfiguration | null = null;
  private meshRecords: readonly BurikoMeshScanline[] | null = null;
  private meshFirstRow = 0;
  private meshBounds: BurikoBitmapRectangle = {left: 0, top: 0, right: 0, bottom: 0};
  ...
  constructor(
    ...
    value120 = 1,
    private readonly referencePoint: BurikoDisplayPoint | null = null,
  ) {
    super(environment, 2, creationOrder, value120);
    this.effects = new BurikoSpriteEffects(environment.compositor);
  }
  ...
  private current(surface: number, imageId: number): BurikoBitmap | null {
    const bitmap = this.surfaces.snapshot(surface);
    return bitmap !== null && this.surfaces.imageId(surface) === imageId ? bitmap : null;
  }

  private currentSource(): BurikoBitmap | null {
    return this.current(this.sourceSurface, this.sourceImageId);
  }

  private currentSecondary(): BurikoBitmap | null {
  ...
  private displayAnchor(): BurikoDisplayPoint {
  ...
  initializeSimple(
    ...
    blendValue: number,
    layer: number,
  ): BurikoSpriteConfigurationStatus {
  ...
  /** 064DE0. */
  configureSimple(sourceSurface: number): BurikoSpriteConfigurationStatus {
  ...
  configureAffine(configuration: BurikoSpriteAffineConfiguration): BurikoSpriteConfigurationStatus {
  ...
  configureReveal(sourceSurface: number, maskSurface: number): BurikoSpriteConfigurationStatus {
    const source = this.stable(sourceSurface);
    if (source === null) return 0x80000001;
    ...
    this.clearModeCaches();
    const owned = allocateBurikoBitmap(source.bitmap.width, source.bitmap.height, 3);
  ...
  /** 064650, sharing the one eight-record owner on BurikoSurfaces. */
  ...
  configureMesh(configuration: BurikoSpriteMeshConfiguration): BurikoSpriteConfigurationStatus {
    const source = this.stable(configuration.sourceSurface);
    if (source === null) return 0x80000001;
    const secondarySurface = configuration.secondarySurface ?? -1,
      secondary = secondarySurface === -1 ? null : this.stable(secondarySurface);
  ...
  private installAffine(
    configuration: BurikoSpriteAffineConfiguration,
    bounds: ReturnType<typeof burikoSpriteBounds>,
  ...
  private reduceChain(source: BurikoBitmap, destination: (BurikoBitmap | null)[]): void {
    let previous = source;
    for (let index = 0; index < destination.length; index++) {
      ...
      reduceBurikoBitmapHalf(reduced, previous);
  ...
  private updateMipmapRegion(region: BurikoBitmapRectangle): void {
    let primary: BurikoBitmap | null = this.currentSource(),
      secondary: BurikoBitmap | null = this.currentSecondary(),
      current = {...region};
    const reduceRegion = (
      source: BurikoBitmap | null,
      destination: BurikoBitmap | null,
      sourceRectangle: BurikoBitmapRectangle,
      destinationRectangle: BurikoBitmapRectangle,
    ): void => {
      ...
      if (selectedDestination !== null) reduceBurikoBitmapHalf(selectedDestination, selectedSource);
    };
  ...
  private affineDamageBounds(region: BurikoBitmapRectangle): BurikoBitmapRectangle | null {
  ...
  private changedRegionDamageBounds(region: BurikoBitmapRectangle): BurikoBitmapRectangle | null {
  ...
  notifySourceRegionChanged(region: BurikoBitmapRectangle): 0 | 1 {
  ...
  private selectSurface(source: BurikoBitmap, mipmaps: (BurikoBitmap | null)[]): SelectedBitmap {
  ...
  private selectedSecondary(level: number): BurikoBitmap | null {
  ...
  private rebuildMesh(): void {
    ...
    const geometry: BurikoMeshGeometry = {
  ...
  setStaticMaskSurface(surface: number): BurikoSpriteMaskAssociationStatus | 0x80000001 {
    surface |= 0;
    if (surface === -1) {
  ...
  private updateMaskChild(mask: BurikoDisplaySprite | null, token: number): boolean {
  ...
  private acceptMaskOwner(
    owner: BurikoDisplaySprite | null,
    token: number,
  ...
  setDynamicMask(mask: BurikoDisplaySprite | null): BurikoSpriteMaskAssociationStatus {
    if (mask !== null) {
  ...
  replaceSource(sourceSurface: number): BurikoSpriteConfigurationStatus {
    switch (this.mode) {
      case 0:
  ...
  override effectivePosition(): BurikoDisplayPoint {
  ...
  private alignedMask(rectangle: BurikoBitmapRectangle): AlignedMask | null {
    if (this.staticMaskEnabled === 0) return null;
    let source: BurikoBitmap | null,
      position: BurikoDisplayPoint,
      spritePosition: BurikoDisplayPoint,
      dynamic = false;
    ...
    if (dynamic && this.dynamicMask !== null) {
      ...
      if (burikoRectangleContained(globalRectangle, bounds)) {
        const borrowed = {...source};
        cropBurikoBitmap(borrowed, {
    ...
    const aligned = allocateBurikoBitmap(
      rectangleWidth(rectangle),
      rectangleHeight(rectangle),
      source.format,
    );
    clearBurikoBitmap(aligned);
  ...
  private applyExternalMask(bitmap: BurikoBitmap, rectangle: BurikoBitmapRectangle): void {
    ...
    else applyBurikoBitmapMask(bitmap, bitmap, mask.bitmap);
  ...
  private temporary(
    ...
    const format = forceAlpha && source.format === 1 ? 2 : source.format,
      target = allocateBurikoBitmap(rectangleWidth(rectangle), rectangleHeight(rectangle), format);
    clearBurikoBitmap(target);
    return target;
  }

  private finishTemporary(
    destination: BurikoBitmap,
    temporary: BurikoBitmap,
    rectangle: BurikoBitmapRectangle,
    mask: boolean,
    opacity = this.effectiveBlendValue(),
  ): void {
    let output = temporary,
      converted: BurikoBitmap | null = null;
    if (mask && output.format !== 2) {
      converted = allocateBurikoBitmap(output.width, output.height, 2);
  ...
  private simpleMode(): BurikoSpriteMode {
    if (
      this.mode === 2 &&
  ...
  private drawSimple(destination: BurikoBitmap, rectangle: BurikoBitmapRectangle): void {
    const source = this.currentSource();
    if (source === null) return;
  ...
  private drawBlend(destination: BurikoBitmap, rectangle: BurikoBitmapRectangle): void {
    ...
    if (
      ...
      blendMixedBurikoBitmapsIntoRgb(
        destination,
        left,
        right,
        this.mixValue,
  ...
  private directAffineEligible(destination: BurikoBitmap, source: BurikoBitmap): boolean {
    return (
      this.staticMaskEnabled === 0 &&
      !this.effects.active &&
      destination.format === 1 &&
  ...
  private drawReveal(destination: BurikoBitmap, rectangle: BurikoBitmapRectangle): void {
    const source = this.currentSource(),
    ...
    const temporary = allocateBurikoBitmap(
      rectangleWidth(rectangle),
      rectangleHeight(rectangle),
      2,
    );
  ...
  private drawDisplacement(destination: BurikoBitmap, rectangle: BurikoBitmapRectangle): void {
    const source = this.currentSource(),
  ...
  private drawMesh(destination: BurikoBitmap, rectangle: BurikoBitmapRectangle): void {
    const selected = this.selectedRenderedSource(),
  ...
  override getProperty(selector: number, output: BurikoDisplayPropertyDestination): number {
  ...
  override dispose(): void {
    if (this.disposedSprite) throw new Error('Buriko accesses a deleted CDspObjSprite');
