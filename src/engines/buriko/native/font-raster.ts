...
export type BurikoFontTransform = readonly [number, number, number, number, number];
export interface BurikoFontGeometry {
...
export class BurikoFontRasterSettings {
  textOut = true;
  quality = 1;
  sampleScale = 4;
  sampleShift = 2;
...
function divide(value: number, divisor: number): number {
  if (divisor === 0 || (value === -2147483648 && divisor === -1))
    throw new RangeError('Buriko native font integer division fault');
...
export function validateBurikoFont(
...
export function validateBurikoFontTransform(transform: BurikoFontTransform): number {
...
export function burikoFontGeometry(
  size: number,
  widthPercent: number,
  transform: BurikoFontTransform | null,
  settings: BurikoFontRasterSettings,
): BurikoFontGeometry {
...
export function burikoWideCharacter(value: number): number {
  value >>>= 0;
...
export class BurikoFontRaster {
  ...
  extraPixels: number;
  constructor(
    readonly geometry: BurikoFontGeometry,
    readonly face: BurikoFontFace,
    readonly settings: BurikoFontRasterSettings,
    readonly cacheCapacity: number,
  ) {
    if (cacheCapacity < 2)
      throw new RangeError('Buriko font cache capacity is below native minimum');
  ...
  glyph(character: number): BurikoGlyph {
    character >>>= 0;
  ...
  private raster(original: number, pixels: Uint8Array): BurikoGlyph {
    const geometry = this.geometry;
    const settings = this.settings;
    const put = (offset: number, value: number): void => {
      if (offset < 0 || offset >= pixels.length)
        throw new RangeError('Buriko glyph writes outside native cache allocation');
      pixels[offset] = value;
    };
      ...
      if (mapped.text === null || character === 0x20 || character === 0x3000) pixels.fill(0);
        ...
        const read = (x: number, y: number): number => {
          ...
          if (index < 0 || index >= dib.bytes.length)
            throw new RangeError('Buriko glyph reads outside native DIB allocation');
    ...
    return {
      character: original,
      fullWidth: burikoWideCharacter(original),
      pixels,
      left,
      top: 0,
      right,
      ...
      abc: normalized,
      wideExtent: Number(
        burikoWideCharacter(character) !== 0 &&
