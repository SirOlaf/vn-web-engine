...
export interface BurikoLockActors {
...
class RecursiveSection {
  ...
  constructor(private readonly actors: BurikoLockActors) {}
  tryEnter(actor = this.actors.currentActor): boolean {
    if (!this.alive) throw new Error('Buriko exclusion section is no longer available');
  ...
  enter(): void {
    while (!this.tryEnter()) {
      if (this.actors.advance?.() !== true)
        throw new Error('Buriko exclusion wait has no runnable owning-actor continuation');
    }
  }
  ...
  leave(actor = this.actors.currentActor): void {
    if (!this.alive || this.depth === 0 || this.owner !== actor)
      throw new Error('Buriko exclusion section is not owned by its current actor');
    if (--this.depth === 0) {
  ...
  runAs<T>(actor: object, operation: () => T): T {
    ...
    if (!this.tryEnter(actor))
      throw new Error('Buriko async admission encountered a suspended registry section');
    try {
      return operation();
    } finally {
      this.leave(actor);
  ...
  dispose(): void {
    ...
    for (const waiter of this.waiters.splice(0))
      waiter.reject(
        new Error('Buriko pending exclusion acquisition targets a removed native section'),
      );
  }
}

/** Shared atomic DWORD 1d2818. Both native DCExclusionControl instances consume it. */
export class BurikoExclusionIds {
  private last = 0;
  next(): number {
...
export type BurikoExclusionResult = 0 | 0x80000001 | 0x80000002 | 0x80000003 | 0x80000004;

/** DCExclusionControl, 08c910: registry section +08 and newest-first record list +30. */
export class BurikoExclusionRegistry {
  private readonly registry: RecursiveSection;
  private head: LockRecord | null = null;
  constructor(
    private readonly ids: BurikoExclusionIds,
  ...
  enter(id: number): BurikoExclusionResult {
  ...
  async enterAsync(id: number, actor: object): Promise<BurikoExclusionResult> {
  ...
  /** 08c700's record behavior; BurikoNativeLocks exposes it only on the script instance. */
  tryEnter(id: number): BurikoExclusionResult {
    return this.registry.run(() => {
  ...
  leave(id: number, actor?: object): BurikoExclusionResult {
    const operation = (): BurikoExclusionResult => {
      const record = this.find(id);
      if (record === null) return 0x80000001;
  ...
  remove(id: number, force = 0): BurikoExclusionResult {
    return this.registry.run(() => {
...
export class BurikoNativeLocks {
  readonly ids = new BurikoExclusionIds();
  readonly engine: BurikoExclusionRegistry;
  readonly script: BurikoExclusionRegistry;
  private readonly engineIds = new Uint32Array(5);
  constructor(actors: BurikoLockActors) {
    this.engine = new BurikoExclusionRegistry(this.ids, actors);
    this.script = new BurikoExclusionRegistry(this.ids, actors);
  }
  ...
  engineId(index: number): number {
    ...
    if (id === undefined) throw new RangeError('Buriko engine lock index is outside its five IDs');
    return id;
  }
...
export function createBurikoDisplayLocks(actors: BurikoLockActors): BurikoNativeLocks {
