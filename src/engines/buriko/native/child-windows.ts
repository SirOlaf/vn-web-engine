...
import {BurikoBitmapText, type BurikoFontTextOutput} from './font-bitmap.js';
import {BurikoSurfaces} from './surfaces.js';
import {BurikoNativeText, textBytes} from './text.js';
import {BurikoChildScroll} from './child-scroll.js';
import {BurikoChildScrollbarControl} from './child-scrollbar.js';
import {presentBurikoChildBitmap} from './child-bitmap.js';
...
import {writeBurikoClipboard} from './modal.js';
...
export interface BurikoChildWindowMetrics {
...
interface ChildWindow {
  ...
  readonly bars: readonly BurikoChildScrollbarControl[];
  minimized: boolean;
  normalHeight: string;
}
interface ChildRecord {
  ...
  scroll: BurikoChildScroll;
  field4c: number;
}
function empty(): ChildRecord {
  const scroll = new BurikoChildScroll(0);
  scroll.wheelStep = 0;
  return {
    window: null,
...
export class BurikoChildWindows {
  ...
  create(
    ...
    record.bitmap = allocateBurikoBitmap(width, height, this.compositor.defaultFormat);
    fillBurikoBitmap(record.bitmap, 0);
    if (title === null)
      throw new Error(
        'Buriko child creation decodes a null native title after allocating its bitmap',
      );
    const caption = this.text.decodeAuto(title);
    const scroll = new BurikoChildScroll(flags);
    ...
    try {
      ...
      const bars: BurikoChildScrollbarControl[] = [];
      for (const axis of [0, 1] as const)
        if ((flags & (1 << axis)) !== 0) {
          const bar = new BurikoChildScrollbarControl(
            this.document,
            axis,
            scroll.bars[axis],
            this.messages,
  ...
  getPosition(output: BurikoBpPointer | null, handle: number): 0 | 1 {
    const record = this.record(handle);
    if (record === null) return 0;
    if (record.window === null)
      throw new Error(
        'Buriko child GetWindowRect reads an uninitialized RECT after its null HWND fails',
      );
  ...
  setClipboard(handle: number, source: BurikoBpPointer | null): number {
    const record = this.record(handle);
    if (record === null) return 0xffffffff;
    record.clipboard = null;
    if (source === null)
      throw new Error(
        'Buriko child clipboard replacement scans null after freeing its previous text',
      );
  ...
  getScrollProperty(output: BurikoBpPointer | null, handle: number, selector: number): number {
    const record = this.record(handle);
    if (record === null) return 0xffffffff;
  ...
  fill(handle: number, color: number): 0 | 1 {
    const record = this.record(handle);
    if (record === null) return 0;
    fillBurikoBitmap(record.bitmap, color);
    this.present(record);
    return 1;
  }
  copy(
  ...
  async handleMessage(message: BurikoWindowMessage): Promise<boolean> {
    ...
    const required = (): ChildRecord => {
      if (record === undefined)
        throw new Error('Buriko child WndProc dereferences a missing native window record');
      return record;
    };
