...
function checkedLength(length: number): number {
  if (length >= 0x10000)
    throw new RangeError('Buriko VM formatter exceeds its 65536-wchar allocation');
  return length;
}
...
export function formatVmConversion(format: string, value: number | string): string {
  let state = 0,
    output = '';
    ...
    if (state === 0) output += character;
      ...
      do {
        ...
        if (number > 0x7fffffff)
          throw new RangeError('Buriko CRT formatter decimal field overflows int32');
        index++;
...
export function formatVmText(
  ...
  for (let index = 0; index < wideFormat.length;) {
    ...
    if (end - index + 2 > 256)
      throw new RangeError('Buriko VM formatter exceeds its 256-wchar conversion buffer');
    const kind = wideFormat[end];
    if (kind === undefined) throw new Error('Buriko VM formatter reached an unfinished conversion');
    if (kind === '%') wideOutput += '%';
    else {
      ...
      if (kind === 's') {
        ...
        if (source === null)
          throw new Error('Buriko text encoding detector dereferences a null %s operand');
      ...
      else throw new Error(`Buriko VM formatter rejects conversion ${kind}`);
      wideOutput += formatVmConversion(wideFormat.slice(index, end + 1), value);
    }
    checkedLength(wideOutput.length);
    index = end + 1;
  }
  ...
  for (let offset = 0; textByte(output, offset) !== 0;) {
    if (output[offset] === 0x1a) {
      ...
      if (value === undefined)
        throw new Error('Buriko VM formatter dereferences an empty raw-character list');
      output[offset++] = value;
