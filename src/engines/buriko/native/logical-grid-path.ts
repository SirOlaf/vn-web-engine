...
export function gridAllocation(
  ...
  if (count < 0) throw new Error('Buriko logical-grid native allocation size overflow');
  return new Uint8Array(count * stride);
}

export function gridCopy(output: BurikoBpPointer | null, bytes: Uint8Array): void {
...
export class BurikoLogicalGridPath {
  ...
  private index(x: number, y: number): number {
    if (this.width === undefined || this.height === undefined) {
      throw new Error('Buriko logical-grid path reads uninitialized dimensions');
    }
  ...
  copyResults(output: BurikoBpPointer | null): number {
  ...
  copyMetric(output: BurikoBpPointer | null, x: number, y: number, direction: boolean): number {
    if (this.results === null) return 0x80000003;
  ...
  directionsToCoordinates(
    output: BurikoBpPointer | null,
    directions: BurikoBpPointer | null,
    count: number,
    x: number,
    y: number,
  ): number {
