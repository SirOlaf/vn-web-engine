...
import type {BurikoBacklog} from './backlog.js';
...
export function createGroup80Backlog(
  backlog: BurikoBacklog,
  errors: BurikoEngineErrors,
): BurikoNativeSlotDefinition[] {
  const pointer = (h: BurikoBpOpcodeContext): BurikoBpPointer | null =>
  ...
  const read = (h: BurikoBpOpcodeContext, extended: boolean): 0 | Promise<never> => {
