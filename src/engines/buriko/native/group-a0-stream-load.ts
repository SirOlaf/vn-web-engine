...
import {
  BurikoAudioMusicResources,
  burikoMissingWaveDiagnostic,
  burikoInvalidWaveDiagnostic,
...
export function createGroupA0StreamLoad(
  musicResources: BurikoAudioMusicResources,
  ...
  return [
    {
      ...
      execute: async (h): Promise<0> => {
        ...
        const name = () => {
          if (pointer === null) throw new Error('Buriko music name consumes a null native pointer');
        ...
        if (status === 12 || status === 14)
          return errors.threadFatal(
            ...
            h.diagnostics,
            formatBurikoAudioNames(
              status === 12 ? burikoMissingWaveDiagnostic : burikoInvalidWaveDiagnostic,
              [name()],
              256,
            ),
          );
