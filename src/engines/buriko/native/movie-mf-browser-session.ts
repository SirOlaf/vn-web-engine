...
import {
  ...
  type BurikoMfMovieDisplayControl,
} from './movie-fullscreen-state.js';
import {BurikoMfMovieDocuments} from './movie-mf-document.js';
import type {BurikoMfMovieVolumeController} from './movie-mf-volume-policy.js';
...
class BurikoBrowserMfDisplayControl implements BurikoMfMovieDisplayControl {
  ...
  constructor(
    ...
    private readonly window: BurikoBrowserMainWindow,
  ) {
    ...
    if (frameContext === null || targetContext === null)
      throw new Error('Buriko MF video display service cannot acquire its scoped canvas');
  ...
  repaint(): void {
    if (this.closed) throw new Error('Buriko MF display control was released');
  ...
  resize(width: number, height: number): void {
    ...
    if (!Number.isSafeInteger(width) || !Number.isSafeInteger(height) || width < 0 || height < 0)
      throw new RangeError('Buriko MF video destination rectangle is invalid');
...
export class BurikoBrowserMfController implements BurikoMfMovieVolumeController {
  nativeState84 = 0;
  nativeStateB8 = 0;
  clampedVolume = 0;
  private display: BurikoBrowserMfDisplayControl | null = null;
  ...
  constructor(
    ...
    private readonly window: BurikoBrowserMainWindow,
    private readonly fullscreen: BurikoFullscreenMovieState,
  ) {}
  ...
  async open(source: Blob, signal: AbortSignal): Promise<void> {
    if (this.closed || this.video !== null)
      throw new Error('Buriko MF controller cannot open a second live source');
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        const failed = (): void => {
          cleanup();
          reject(new Error('Buriko MF browser media source failed to resolve'));
        };
        const aborted = (): void => {
          cleanup();
      ...
      if (video.videoWidth > 0 && video.videoHeight > 0) {
        const display = new BurikoBrowserMfDisplayControl(this.document, video, this.window);
  ...
  private startFrameDelivery(
    ...
    display: BurikoBrowserMfDisplayControl,
  ): void {
    const receive = (): void => {
      this.callback = null;
      this.timer = null;
...
export class BurikoBrowserMfMovieSession {
  private current: BurikoBrowserMfController | null = null;
  ...
  constructor(
    ...
    readonly window: BurikoBrowserMainWindow,
    readonly fullscreen: BurikoFullscreenMovieState,
    readonly volume: BurikoMfMovieVolumePolicy,
  ) {
    if (volume.fullscreen !== fullscreen)
      throw new Error('Buriko MF session requires one fullscreen and volume owner');
  }
  ...
  async start(
    ...
    if (this.closed || this.resetting) throw new Error('Buriko MF source admission is closed');
    this.generation++;
  ...
  private async startSelected(
    ...
    this.releaseCurrent();
    const controller = new BurikoBrowserMfController(this.document, this.window, this.fullscreen);
  ...
  private releaseCurrent(): number {
    ...
    if (this.fullscreen.controller !== controller)
      throw new Error('Buriko MF controller owner mismatch');
  ...
  async beginReset(): Promise<() => void> {
    if (this.closed || this.resetting) throw new Error('Buriko MF reset admission is unavailable');
    this.resetting = true;
    this.generation++;
