...
import type {BurikoNativeRectangle} from './display-state.js';

/** The three verified CDspObj virtual calls used by native input-region nodes. */
export interface BurikoInputHitObject {
  inputActive(): number; // vtable +10
  inputRectangle(mode: 0): BurikoNativeRectangle | BurikoBitmapRectangle; // vtable +48
  inputHitTest(x: number, y: number, mode: 1): number; // vtable +C8
}
interface Capture {
  token: number;
  rectangle: BurikoNativeRectangle;
  object: BurikoInputHitObject | null;
}
export type BurikoInputCaptureView = Readonly<Pick<Capture, 'token' | 'object'>>;
...
const EMPTY_RECTANGLE: BurikoNativeRectangle = [0, 0, 0, 0];
const FULL_RECTANGLE: BurikoNativeRectangle = [-2147483648, -2147483648, 2147483647, 2147483647];

/** Buriko's input globals and capture lists; no state is shared with other engine families. */
export class BurikoNativeInput {
  ...
  usesClock(clock: BurikoNativeClock): boolean {
    return this.clock === clock;
  }

  private keyIndex(key: number): number {
    key >>>= 0;
    if (key >= 256) throw new RangeError('Buriko native virtual-key index outside 0..255');
    return key * 6;
  }
  ...
  consumeKey(key: number, recursion = new Set<number>()): number {
    ...
    if (key >= 0xc1 && key <= 0xd7) {
      if (recursion.has(key)) throw new Error('Buriko native recursive virtual-key mapping');
      recursion.add(key);
      const keys =
        key === 0xc1
  ...
  captureDiagnosticView(): {
    pointer: readonly BurikoInputCaptureView[];
    key: readonly BurikoInputCaptureView[];
  } {
  ...
  releaseObjectCapture(object: BurikoInputHitObject): boolean {
