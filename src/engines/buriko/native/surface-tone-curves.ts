...
export interface BurikoSurfaceToneCurve {
  readonly key: number;
...
export class BurikoSurfaceToneCurves {
  private readonly entries: BurikoSurfaceToneCurve[] = [];
  find(key: number): BurikoSurfaceToneCurve | null {
  ...
  set(key: number, pointer: BurikoBpPointer): 0 | 0x16 {
    ...
    for (let channel = 2; channel >= 0; channel--) {
      ...
      if (path === null)
        throw new Error('Buriko tone curve consumes unwritten interpolation scratch');
  ...
  configure(key: number, pointer: BurikoBpPointer | null): number {
