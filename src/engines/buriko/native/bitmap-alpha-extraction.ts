...
function extractBurikoBitmapAlphaPixels(
  destination: BurikoBitmap,
  x: number,
  y: number,
  first: BurikoBitmap,
  ...
  const outputRectangle = burikoBitmapRectangle(target);
  translateBurikoBitmapRectangle(outputRectangle, -x | 0, -y | 0);
  const overlap = burikoBitmapRectangle(firstCopy);
  intersectBurikoBitmapRectangle(overlap, burikoBitmapRectangle(secondCopy));
  if (!intersectBurikoBitmapRectangle(overlap, outputRectangle)) {
    clearBurikoBitmap(destination);
    return 0;
  }
  cropBurikoBitmap(firstCopy, overlap);
  cropBurikoBitmap(secondCopy, overlap);
  translateBurikoBitmapRectangle(overlap, x, y);
  cropBurikoBitmap(target, overlap);
  const status = extract(target, firstCopy, second === null ? null : secondCopy, level);
  if (status !== 0) return status;
  if (y > 0)
    clearBurikoBitmap(destination, {...burikoBitmapRectangle(destination), bottom: (y - 1) | 0});
  const bottom = (target.height + y) | 0;
  if (x > 0)
    clearBurikoBitmap(destination, {left: 0, top: y, right: (x - 1) | 0, bottom: (bottom - 1) | 0});
  ...
  if (bottom >>> 0 < destination.height >>> 0)
    clearBurikoBitmap(destination, {...burikoBitmapRectangle(destination), top: bottom});
  return 0;
}

export const extractBurikoBitmapAlpha = withBurikoBitmapText(extractBurikoBitmapAlphaPixels, {
