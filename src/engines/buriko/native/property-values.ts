...
export interface BurikoPropertyWordBinding {
  readPropertyWord(): number;
}
export type BurikoPropertySource = BurikoBpPointer | BurikoPropertyWordBinding;

export function propertyTextPointer(source: BurikoPropertySource | null): BurikoBpPointer {
  if (source === null || 'readPropertyWord' in source)
    throw new Error('Buriko property text requires native byte storage');
  return source;
}
...
export function formatPropertyScalar(kind: number, value: number): string {
  ...
  switch (kind | 0) {
    ...
    default:
      throw new RangeError('Buriko scalar property formatter requires native type 0..4');
  }
}
...
export function formatPropertySource(
  ...
  if (kind === 5) {
    if (source === null) throw new Error('Buriko property text decoder dereferences a null source');
