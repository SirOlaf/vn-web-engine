...
import {
  ...
  writePropertyWord,
  propertyTextPointer,
  type BurikoPropertySource,
} from './property-values.js';
import {BurikoWindowMessages, type BurikoWindowMessage} from './window-messages.js';

export interface BurikoPropertyRow {
  readonly name: string;
  readonly kind: number;
  value: number;
  rawCopy: Uint8Array;
  formatted: string;
  readonly source: BurikoPropertySource | null;
  readonly mutable: boolean;
  readonly editable: number;
  readonly refreshCallback: BurikoPropertyCallback | null;
  readonly refreshContext: unknown;
  readonly editCallback: BurikoPropertyCallback | null;
  readonly editContext: unknown;
}
export type BurikoPropertyCallback = (row: BurikoPropertyRow, context: unknown, mode: 0) => number;
interface PropertyTab {
  readonly name: string;
  readonly rows: BurikoPropertyRow[];
  selected: number;
}
...
export class BurikoPropertyEditors {
  private nextId = 0;
  ...
  create(
    ...
    subtitle: BurikoBpPointer | null,
    position: BurikoBpPointer | null,
    firstWidth: number,
    secondWidth: number,
  ): number {
    ...
    if (this.records.has(id))
      throw new Error(
        'Buriko property ID collision leaves the newly shown HWND without its native record',
      );
  ...
  getPosition(output: BurikoBpPointer | null, id: number): number {
    const record = this.records.get(id >>> 0);
    if (!record) return 0x80000007;
  ...
  setPosition(id: number, position: BurikoBpPointer | null): number {
    const record = this.records.get(id >>> 0);
    if (!record) return 0x80000007;
  ...
  addButton(output: BurikoBpPointer | null, id: number, label: BurikoBpPointer | null): number {
    const record = this.records.get(id >>> 0);
    if (!record) return 0x80000007;
    if (record.buttons.length >= 3) return 0x80000006;
    if (label === null) throw new Error('Buriko property button decoder dereferences a null label');
  ...
  addTab(output: BurikoBpPointer | null, id: number, label: BurikoBpPointer | null): number {
    const record = this.records.get(id >>> 0);
    ...
    if (label === null)
      throw new Error('Buriko property tab construction dereferences the null decoded label');
  ...
  addRow(
    ...
    kind: number,
    source: BurikoPropertySource | null,
    mutable: number,
    editable: number,
    refreshCallback: BurikoPropertyCallback | null = null,
    refreshContext: unknown = null,
    editCallback: BurikoPropertyCallback | null = null,
  ...
  private row(id: number, tab: number, index: number): {result: number; row?: BurikoPropertyRow} {
    const record = this.records.get(id >>> 0);
  ...
  getValue(
    output: BurikoBpPointer | null,
    typeOutput: BurikoBpPointer | null,
    id: number,
    tab: number,
    index: number,
  ): number {
    ...
    for (const record of [...this.records.values()].sort((a, b) => a.id - b.id)) {
      ...
      for (const [tabIndex, tab] of record.tabs.entries()) {
        ...
        for (const row of tab.rows) {
          ...
          if (row.kind === 5) {
            if (row.source === null)
              throw new Error('Buriko live property string dereferences a null source');
  ...
  poll(output: BurikoBpPointer | null, id: number): number {
    const record = this.records.get(id >>> 0);
    if (!record) return 0x80000007;
  ...
  async handleMessage(message: BurikoWindowMessage): Promise<boolean> {
    ...
    if (message.message === 0x10) {
      ...
      if (!notification)
        throw new Error('Buriko property notification dereferences an unknown native record');
      if (notification.kind === 'tab') {
        record.selectedTab = notification.tab >>> 0;
        if (!record.tabs[record.selectedTab])
          throw new RangeError('Buriko property tab callback indexes outside native records');
        this.renderRows(record);
        record.events.push({kind: 2, value: record.selectedTab});
      } else {
        const tab = record.tabs[record.selectedTab];
        if (!tab) throw new RangeError('Buriko property list callback indexes outside native tabs');
        if (notification.kind === 'selection') {
          ...
          if (tab.selected >= 0) {
            ...
            if (!row) throw new RangeError('Buriko property edit indexes outside native rows');
  ...
  private async editRow(record: PropertyRecord, row: BurikoPropertyRow): Promise<number> {
  ...
  private showRowDialog(record: PropertyRecord, row: BurikoPropertyRow): Promise<number> {
          ...
          if (accept) {
            if (row.kind === 4) {
              ...
              if (row.kind === 5) {
                ...
                const parsed = parseBurikoPropertyNumber(row.kind as 0 | 1 | 2 | 3, input);
                if (parsed.result === 0) {
