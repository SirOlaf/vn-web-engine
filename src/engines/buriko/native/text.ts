...
/** Mutable globals DAT_140274584 and DAT_1401ca4b4 belong to one Buriko runtime. */
export class BurikoNativeText {
  ...
  detectEncoding(bytes: Uint8Array, offset = 0, skipControl3 = false): BurikoTextMode {
    let sequences = 0;
    for (;;) {
      const first = textByte(bytes, offset);
  ...
  readCharacter(bytes: Uint8Array, offset = 0, mode: number = -1): BurikoTextCharacter {
  ...
  decodeMixed(pointer: BurikoBpPointer): string {
    ...
    const convert = (mode: 0 | 1, start: number, size: number, capacity: number): number => {
      ...
      if (start < 0 || size < 0 || start + size > input.length)
        throw new RangeError('Buriko mixed text conversion reads outside native input');
      ...
      if (outputOffset + written > output.length)
        throw new RangeError('Buriko mixed text conversion exceeds its native allocation');
    ...
    for (const unit of output) {
      if (unit === undefined)
        throw new Error('Buriko mixed text decoder reads unwritten native allocation');
      if (unit === 0) return result;
      result += String.fromCharCode(unit);
    }
    throw new RangeError(
      'Buriko mixed text decoder reads beyond its unterminated native allocation',
    );
  }
