...
test('80:F2 runs the mounted file and uninstaller transaction in its progress modal', async () => {
  ...
  const installerDialogHost = {
    async chooseDestination() {
      throw new Error('unexpected destination dialog');
    },
    async chooseComponent() {
      throw new Error('unexpected component dialog');
    },
  ...
  try {
    ...
    assert.equal(
      definitions.filter((slot) => slot.primary === 0x80 && slot.secondary === 0xf2).length,
      1,
    );
    ...
    assert.ok(
      await graph.registry.storage.getValue(
        {
          hive: 'HKLM',
          view: '64',
