...
import {reduceAokanaBitmapHalf} from '../dist/engines/buriko/games/aokana/native/bitmap-reduce.js';
...
function bitmap(width, height, values, format = 2) {
  ...
  return {storage: new AokanaBitmapStorage(bytes, true), offset: 0, stride: width * 4,
    width, height, format, bytesPerPixel: 4};
}
...
test('half reduction preserves vertical-then-horizontal byte rounding and odd edges', () => {
  ...
  reduceAokanaBitmapHalf(destination, source);
