...
import {AokanaNamedValueMap, aokanaNamedValueHash, aokanaCompareNamedBytes} from '../dist/engines/buriko/games/aokana/native/named-value-map.js';
...
const decode = (value) => new TextDecoder().decode(value.bytes.subarray(value.offset)).replace(/\0$/, '');

test('shared named map preserves copied keys, insertion order and fixed-width value updates', () => {
  const map = new AokanaNamedValueMap(4),
    first = pointer('First'),
  ...
  assert.equal(aokanaNamedValueHash({bytes: Uint8Array.of(0x80, 65, 0), offset: 0}), 0xffff8bc1);
  assert.equal(aokanaCompareNamedBytes({bytes: Uint8Array.of(255, 0), offset: 0}, pointer('a')), 1);
});

test('string-width named maps retain complete terminated strings through ordinary updates and removal', () => {
  const map = new AokanaNamedValueMap(0);
...
test('localized sections parse comments and language lists with newest matching section precedence', () => {
  const events = [],
    language = new AokanaNativeLanguage(() => {events.push('language'); return 0x409;}),
    text = new AokanaNativeText();
  const imported = new AokanaImportedTextMaps(text),
    clear = imported.clear.bind(imported);
  imported.clear = () => {events.push('clear-imported'); clear();};
  const messages = new AokanaLocalizedMessages(text, language, imported);
  ...
  assert.equal(messages.loadPeResources([{type: 'TEXT', id: 131, language: 0, codePage: 0, bytes: stored}]), 1);
...
test('an empty language section reuses its map and recognizes multibyte value boundaries', () => {
  const language = new AokanaNativeLanguage(() => 0x411),
    text = new AokanaNativeText(),
    messages = new AokanaLocalizedMessages(text, language, new AokanaImportedTextMaps(text));
  assert.equal(messages.load(new TextEncoder().encode('@languageid=411\n@languageid=0\nTEXT=青空\n')), 1);
...
test('eight-byte internal maps keep actual inner-map references in their shared value slots', () => {
  const outer = new AokanaNamedValueMap(8),
    first = new AokanaNamedValueMap(0),
    second = new AokanaNamedValueMap(0);
  ...
  assert.deepEqual([...outer.valuePointers()].map((pointer) => outer.referenceFromValuePointer(pointer)), [second]);
});
