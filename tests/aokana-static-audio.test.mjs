...
import {
  ...
  timeScaleBurikoStaticPcm,
} from '../dist/engines/buriko/native/audio/static-pcm.js';
import {
  BurikoWaveStatic,
  createBurikoWaveStatic,
  fadeInBurikoStaticPcm,
} from '../dist/engines/buriko/native/audio/wave-static.js';
import {parseBurikoWaveBoxHeader} from '../dist/engines/buriko/native/audio/wavebox-header.js';
function pcm(values, bits = 16) {
  const width = bits / 8,
    bytes = new Uint8Array(values.length * width);
  values.forEach((value, index) => {
    for (let byte = 0; byte < width; byte++) bytes[index * width + byte] = value >> (byte * 8);
  });
...
function samples(storage, bits = 16) {
  ...
  for (let at = 0; at < bytes.length; at += width) {
    let value = bytes[at];
    for (let byte = 1; byte < width; byte++) value |= bytes[at + byte] << (byte * 8);
...
function wave(
  values,
  {field08 = values.length * 2, frames = values.length, loop = 0, loopStart = 0} = {},
) {
  const bytes = new Uint8Array(64 + values.length * 2),
  ...
  bytes.set(pcm(values).bytes, 64);
  return bytes;
}
test('short 16/24-bit joins retain the nominal five-millisecond denominator', () => {
  for (const bits of [16, 24]) {
    const source = pcm([0, 100, 200, 1000, 1100, 1200], bits),
      result = pcm([99, 99, 99], bits);
    crossfadeBurikoStaticPcm(result, 0, source, 0, (3 * bits) / 8, 3, 1000, 1, bits);
    assert.deepEqual(samples(result, bits), [0, 350, 700]);
    crossfadeBurikoStaticPcm(result, 0, source, 0, (3 * bits) / 8, 1, 200, 1, bits);
    assert.equal(samples(result, bits)[0], -(2 ** (bits - 1)));
  }
});
test('8-bit join uses its final-tenth source window and integer tail boundary', () => {
  const source = pcm([...new Array(20).fill(100), ...new Array(20).fill(200)], 8),
    output = pcm(new Array(20).fill(0), 8);
  crossfadeBurikoStaticPcm(output, 0, source, 0, 20, 20, 48000, 1, 8);
...
test('static time scaling preserves native thirty-millisecond skips and unwritten allocation tails', () => {
  ...
  const fast = timeScaleBurikoStaticPcm(source, 10, 100, 1, 16, 2);
  assert.equal(fast.allocatedFrames, 6);
  assert.equal(fast.writtenLength, 12);
  assert.deepEqual(samples(fast), [0, 1, 2, 6, 7, 8]);
  const slow = timeScaleBurikoStaticPcm(source, 10, 100, 1, 16, 0.5);
  assert.equal(slow.allocatedFrames, 21);
  assert.equal(slow.writtenLength, 30);
  ...
  assert.throws(() => timeScaleBurikoStaticPcm(source, 10, 0, 1, 16, 2), /divides by zero/);
  source.initialized[2] = 0;
  assert.equal(timeScaleBurikoStaticPcm(source, 10, 100, 1, 16, 1).initialized[2], 0);
  assert.throws(() => timeScaleBurikoStaticPcm(source, 10, 100, 1, 16, 2), /unwritten/);
});
test('static fade-in uses signed DWORD products, SSE refinement, and preserves samples beyond the duration', () => {
  for (const bits of [16, 24]) {
    ...
    assert.equal(fadeInBurikoStaticPcm(source, 5, 1000, 1, bits, 4), 0);
    assert.deepEqual(samples(source, bits), [0, -250, 1000, -1500, 99]);
    assert.equal(fadeInBurikoStaticPcm(source, 5, 1000, 1, bits, 6), 0xffffffff);
    assert.deepEqual(samples(source, bits), [0, -250, 1000, -1500, 99]);
  }
});
test('static PCM source has separate decoded capacity, playback cursor, and native EOF sentinel', async () => {
  const model = await createBurikoWaveStatic(wave([100, 200], {field08: 2}), {
    gain: 1,
    prefer24Bit: false,
  });
  ...
  model.reset();
  assert.equal(model.readInto(output, 0, 1), 1);
  model.dispose();
  assert.throws(
    () => model.reset(),
...
test('static PCM loop seek retains the native source-frame-count divided-by-four offset', () => {
  ...
  const model = new BurikoWaveStatic(parseBurikoWaveBoxHeader(source), 16, source.subarray(64));
