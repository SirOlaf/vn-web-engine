...
test('80:F0/F1 use the selected modal form owner and preserve native outputs', async () => {
  ...
  const installerDialogHost = {
    ...
    async runProgress() {
      throw new Error('This test does not run installation');
    },
  };
  ...
  try {
    ...
    for (const secondary of [0xf0, 0xf1])
      assert.equal(
        definitions.filter((slot) => slot.primary === 0x80 && slot.secondary === secondary).length,
        1,
      );
    ...
    assert.equal(
      await invoke(
        0x80,
        0xf0,
        [0, 0x100, 0x104, addresses[0], 0, 1, addresses[1], addresses[2]],
        0,
      ),
      1,
    );
    ...
    assert.equal(
      await invoke(
        0x80,
        0xf0,
        [0x900, 0x100, 0x104, addresses[0], 0, 1, addresses[1], addresses[2]],
        0,
      ),
      1,
    );
    ...
    assert.equal(
      await invoke(0x80, 0xf1, [addresses[2], addresses[3], addresses[4], addresses[5], 1, 2], 0),
      1,
    );
