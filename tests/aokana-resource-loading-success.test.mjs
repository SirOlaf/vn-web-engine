...
import {AokanaBitmapPreloadCache} from '../dist/engines/buriko/games/aokana/native/bitmap-preload-cache.js';
import {AokanaResourceCache} from '../dist/engines/buriko/games/aokana/native/resource-cache.js';
...
test('bitmap preload keys retain byte encoding while copies, duplicate insertion and consumption share one cache', () => {
  const text = new AokanaNativeText(), cache = new AokanaBitmapPreloadCache(text);
  const archive = bytes('DATA.ARC'), name = bytes('Face'), payload = Uint8Array.of(4, 5, 6);
  assert.equal(cache.insert(archive, name, payload), 1);
  archive.fill(90); name.fill(90); payload.fill(99);
  ...
  const cp932 = text.encodeWide('あABC', 0), utf8 = text.encodeWide('あabc', 1);
...
test('preload archive null and empty-string matches preserve native newest-first asymmetry', () => {
  const cache = new AokanaBitmapPreloadCache(new AokanaNativeText());
...
test('optional resource cache decodes wide keys and promotes size queries before ordinary capacity eviction', () => {
  const text = new AokanaNativeText(), cache = new AokanaResourceCache(text);
  cache.configure(12);
  for (const [name, n] of [['first', 1], ['second', 2], ['third', 3]])
  ...
  read[0] = 99;
  assert.deepEqual(cache.read(text.encodeWide('あarc', 1), text.encodeWide('あbmp', 1)), Uint8Array.of(7, 8));
...
function setup() {
  ...
  const unavailable = () => assert.fail('Successful synthetic resource lookup does not open a dialog');
  const resources = new AokanaProgramResources(files, {
    ...
    secondaryMediaPath: 'C:\\disc\\', searchDirectoriesEnabled: 1, searchDirectories: [bytes('sub')],
    retryTitle: bytes('Media'), retryMessage: bytes('Insert'), quitConfirmation: bytes('Quit?'),
  }, {show: unavailable}, {fatal: unavailable}, processing);
  return {loading: new AokanaResourceLoadingState(resources),
    mount(path, data) { sources.attach(path, new BlobSource(new Blob([data]))); }};
}

function archive(payload, metadata = 0x123456789abcdefn) {
  const output = new Uint8Array(144 + payload.length), view = new DataView(output.buffer);
  output.set(bytes('BURIKO ARC20')); view.setUint32(12, 1, true);
  output.set(bytes('entry'), 16); view.setUint32(116, payload.length, true);
  view.setBigUint64(120, metadata, true); output.set(payload, 144);
  return output;
}
...
test('actual resource FIFO copies names, processes one head and preserves the native result ordering', async () => {
  ...
  const first = {bytes: null}, second = {bytes: null}, a = {value: 9}, b = {value: 9};
  ...
  loading.enterProcedure(); loading.enterProcedure(); loading.leaveProcedure();
...
test('queued raw reads use stored ranges and archive metadata through the existing archive cache', async () => {
  ...
  const output = {bytes: null}, status = {value: 7}, metadata = {value: 0n};
