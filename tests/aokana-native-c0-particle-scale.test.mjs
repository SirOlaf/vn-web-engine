...
import {scaleAokanaBitmap} from '../dist/engines/buriko/games/aokana/native/bitmap-scale.js';
...
test('native bilinear scaling uses its endpoint spacing and independent RGBA channels', () => {
  ...
  scaleAokanaBitmap(enlarged, source, 0x20000);
  assert.deepEqual(
    words(enlarged),
    Array.from(
      {length: 16},
...
test('native weighted shrinking retains coverage-table quantization and centering', () => {
  ...
  scaleAokanaBitmap(half, source, 0x8000);
  assert.deepEqual(
    words(half),
    [16, 17, 26, 27].map((value) => (0xff000000 | value) >>> 0),
  );
  scaleAokanaBitmap(threeQuarters, source, 0xc000);
...
test('native integer shrinking excludes transparent RGB but averages alpha over the whole box', () => {
  ...
  scaleAokanaBitmap(destination, source, 0x4000);
...
test('centered native scaling preserves destination pixels outside the scaled rectangle', () => {
  ...
  scaleAokanaBitmap(destination, source, 0x20000);
  assert.deepEqual(
    words(destination),
