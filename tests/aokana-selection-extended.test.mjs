...
import {createGroup90SelectionExtended} from '../dist/engines/buriko/native/group-90-selection-extended.js';
...
import {BurikoNativeCursorMotion} from '../dist/engines/buriko/native/cursor-motion.js';
...
test('extended selections retain real marker overlays and complete interactive and timed procedures', async () => {
  ...
  const moves = [];
  const cursor = new BurikoNativeCursorMotion(input, clock, {
  ...
  const procedures = new BurikoProcedureState(),
    ...
    settings = new BurikoSelectionState();
