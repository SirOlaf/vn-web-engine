...
import {
  AokanaRubyAnnotations,
  aokanaTextCodes,
} from '../dist/engines/buriko/games/aokana/native/text-annotations.js';
...
function setup() {
  ...
  const annotations = new AokanaRubyAnnotations(text);
...
test('persistent extraction follows source order, skips each matched word and feeds a separate invocation context', () => {
  ...
  const invocation = new AokanaRubyAnnotations(text);
...
test('ruby lookup compares supplied bytes while removal converts them, and ordinary BMP codes retain native counts', () => {
  ...
  assert.equal(aokanaTextCodes(text, value('Aあｶ', 0), codes), 3);
