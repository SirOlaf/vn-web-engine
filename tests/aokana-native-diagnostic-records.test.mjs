...
import {listThreadModules, attachModule} from '../dist/engines/buriko/bp/modules.js';
...
import {createGroupE0Records} from '../dist/engines/buriko/native/group-e0-records.js';
...
test('module dumps retain borrowed-region ordering, local-only child dumps and raw name pointers', () => {
  ...
  attach(owner, 'base');
  attach(owner, 'second');
  const a = new BurikoBpSharedThread({id: 5, operandCapacity: 2});
  const b = new BurikoBpSharedThread({id: 6, operandCapacity: 2});
  assert.equal(
  ...
  const diagnostic = new BurikoBpDiagnostics(() => {});
...
test('E0 state bindings match all four verified entry points', () => {
  const slots = createGroupE0Records(
    new BurikoDiagnosticCounts(),
    new BurikoPooledAllocationDiagnostics(),
  );
  assert.deepEqual(
...
test('module list formatting reverses the borrowed dump and retains the modal output alias', async () => {
  ...
  attach(owner, 'two');
  assert.equal(
    new TextDecoder().decode(formatBurikoModuleList(owner)).split('\0')[0],
    'one - $000000\ntwo - $000004\n',
  );
  const events = [],
...
test('grouped bitmap descriptions copy all units before formatting and preserve exact fixed fields', () => {
  ...
  assert.equal(
    ...
    BURIKO_NATIVE_SLOT_ADDRESSES[0xe0][0x3f],
  );
});
