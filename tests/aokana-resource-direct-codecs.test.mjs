...
test('direct resource codecs retain real caller pixels and share successful legacy/sliced output semantics', async () => {
  ...
  const result = await decodeBurikoResource(modernCbg(8, 8, 32, true), processing, 0, 0, {
    bytes,
    initialized,
  });
  ...
  assert.equal(
    (
      await decodeBurikoResource(legacy, processing, 0, 0, {
        bytes: legacyOutput,
  ...
  assert.equal((await decodeBurikoResource(legacy, processing, 20, 4, {bytes: slice})).status, 0);
  assert.deepEqual(slice, Uint8Array.of(2, 2, 2, 0));
  const cropped = new Uint8Array(24).fill(0x55);
  assert.equal(
    (await decodeBurikoResource(modernCbg(2, 1, 24), processing, 0, 0, {bytes: cropped})).status,
    0,
  );
