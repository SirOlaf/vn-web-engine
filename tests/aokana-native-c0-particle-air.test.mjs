...
import {
  BurikoParticleAir,
  burikoParticleRandom,
} from '../dist/engines/buriko/native/particle-air.js';
...
test('particle random consumes two 15-bit CRT draws for inclusive signed ranges', () => {
  ...
  assert.equal(burikoParticleRandom(random, 0), 0);
  assert.equal(burikoParticleRandom(random, 100), 32770 % 101);
  assert.equal(burikoParticleRandom(random, 0x7fffffff), 0x3fffffff);
  assert.equal(burikoParticleRandom(random, -99), -((3 * 32768 + 7) % 98));
  assert.equal(burikoParticleRandom(random, -0x80000000), -294923);
...
test('particle air holds and interpolates independently randomized Q8 endpoints', () => {
  ...
  const air = new BurikoParticleAir(random);
...
test('particle air rate changes preserve current transition and zero durations become one step', () => {
  const air = new BurikoParticleAir({next: () => 0});
  air.setInterval(10);
