...
import {mapBurikoMovieAudioSpan} from '../dist/engines/buriko/native/movie-audio-samples.js';
import {
...
test('actual decoded PCM planes map repeated unit-rate edits with exact frame-start boundaries', () => {
  ...
  const chunk = {
    ...
    sampleRate: 4,
    frameCount: 8,
    mediaStart: burikoIsoTime.fraction(1n, 8n),
  };
  const spans = mapBurikoMovieAudioSpan(chunk, track, timeline);
  assert.deepEqual(
  ...
  const sought = mapBurikoMovieAudioSpan(chunk, track, timeline, f(1n, 2n));
  assert.deepEqual(
