...
import {decodeAokanaResource as decodeWithProcessing, AokanaUndefinedResourceRead} from '../dist/engines/buriko/games/aokana/native/resource-decode.js';
...
const decodeAokanaResource = (bytes, offset, length) => decodeWithProcessing(bytes, mainProcessing, offset, length);
const decodeAokanaBfFrame = (bytes, width, height, depth, quantization, destination) => frameWithProcessing(bytes, width, height, depth, quantization, new AokanaDistributedProcessing(allocator, 2), destination);
const decodeAokanaCompressedBgV2 = (bytes) => cbgWithProcessing(bytes, new AokanaDistributedProcessing(allocator, 2));
...
function setup(present = async () => 1) {
  ...
  const text = new AokanaNativeText(), media = new AokanaProgramMedia();
  ...
  const dialogs = new AokanaEngineDialogs({show: present}, text, clock, input, new AokanaNativeCursor({style: {}}),
    {isPresent: () => false, refresh() {throw Error('Unexpected refresh');}}, display, null, bytes('Game'));
  const errors = new AokanaEngineErrors(files, dialogs, bytes('C:\\game\\'), bytes('C:\\game\\'));
  const resources = new AokanaProgramResources(files, {nativeFileRoot: 'C:\\game\\', primaryRoot: bytes('C:\\game\\'), secondaryRoot: bytes('C:\\disc\\'),
    secondaryMediaPath: 'C:\\disc\\', searchDirectoriesEnabled: 0, searchDirectories: [], retryTitle: bytes('Media'),
    retryMessage: bytes('Insert'), quitConfirmation: bytes('Quit?')}, dialogs, errors, mainProcessing);
  return {sources, files, resources, media, errors, mount(path, data) {sources.attach(path, new BlobSource(new Blob([data])));}};
}
function archive(entries, packed = false) {
  const record = packed ? 32 : 128, base = 16 + entries.length * record;
  const output = new Uint8Array(base + entries.reduce((sum, e) => sum + e.data.length, 0));
  output.set(bytes(packed ? 'PackFile    ' : 'BURIKO ARC20')); put(output, 12, entries.length);
  let offset = 0;
  entries.forEach((entry, index) => {
    ...
    if (!packed) new DataView(output.buffer).setBigUint64(16 + index * record + 104, entry.metadata ?? 0n, true);
    output.set(entry.data, base + offset); offset += entry.data.length;
  });
  return output;
}
test('native drive gate distinguishes fixed, nonfixed, UNC and relative paths', () => {
  ...
  s.media.volumePresent[2] = 0; s.media.operatingSystemType = 1;
  assert.equal(s.files.isAvailable(bytes('C:\\x')), true);
});
test('ARC20 and PackFile normalize raw names, preserve first duplicates, and retain metadata', async () => {
  for (const packed of [false, true]) {
    ...
    s.mount('/game/data.arc', archive([{name, data: Uint8Array.of(1, 2), metadata: 0xfeedn},
      {name: bytes('あabc'), data: Uint8Array.of(9)}], packed));
    const nameUtf8 = bytes('あabc');
    assert.equal(await s.resources.archives.size(bytes('C:\\GAME\\DATA.ARC'), nameUtf8), 2);
    assert.deepEqual((await s.resources.archives.read(bytes('C:\\game\\data.arc'), nameUtf8)).bytes, Uint8Array.of(1, 2));
    assert.equal(await s.resources.archives.metadata(bytes('C:\\game\\data.arc'), nameUtf8), packed ? 0n : 0xfeedn);
  }
});
test('archive index stays cached while payload reads reopen the mounted file', async () => {
  const s = setup(), path = bytes('C:\\game\\a.arc'), name = bytes('x');
...
test('resource priority is primary loose then archive; subdirectories only follow missing relative loose paths', async () => {
  ...
  assert.deepEqual(await s.resources.readModule(bytes('a.arc'), bytes('x'), false), Uint8Array.of(1));
...
test('size-query rejects primary missing archive without bd7d0 secondary-entry fallback', async () => {
  ...
  assert.deepEqual(await s.resources.readModule(bytes('a.arc'), bytes('x'), false), Uint8Array.of(9));
});
test('deferred archive pointers are consumed only after a primary loose miss, including BWEF loads', async () => {
  const s = setup(), content = new Uint8Array(0x124);
  content.set(bytes('bwef    ')); put(content, 0x14, 1); put(content, 0x18, 17); put(content, 0x120, 100);
  s.mount('/game/x', content);
  let reads = 0;
  const deferred = () => {reads++; return bytes('missing.arc');};
  ...
  const memory = new AokanaBpMemory(new Uint8Array(256)), thread = new AokanaBpThread({id: 1, operandCapacity: 16, moduleCapacity: 0, frameCapacity: 0});
  ...
  reads = 0;
  await assert.rejects(s.resources.load(() => {
    if (++reads === 2) throw new Error('native retry format reads archive');
    return bytes('missing.arc');
  }, bytes('missing'), true), /native retry format reads archive/);
  assert.equal(reads, 2);
});
test('resource retry cancellation preserves native quit code and follows both dialogs', async () => {
  const calls = [];
  const s = setup(async (dialog) => {calls.push(dialog); return calls.length === 1 ? 2 : 6;});
  await assert.rejects(s.resources.readModule(null, bytes('missing')), (error) => error.code === 0x7fffffff);
  assert.deepEqual(calls.map((c) => [c.text, c.buttons, c.defaultSecondButton]), [['Insert', 'ok-cancel', false], ['Quit?', 'yes-no', true]]);
});
...
function varint(n) {const out = []; do {const low = n & 127; n >>>= 7; out.push(low | (n ? 128 : 0));} while (n); return out;}
function frame(depth, count = depth === 8 ? 64 : depth === 16 ? 128 : 192, mask = 1, alphaMode = 1) {
  const head = new Uint8Array(200); head[0] = head[16] = 1; put(head, 192, 200);
  const row = Uint8Array.from([mask, ...varint(count), 0, 0, 0]); put(head, 196, 200 + row.length);
  let alpha = [];
  if (depth === 32) {
    if (alphaMode === 1) alpha = [1, 0, 0, 0, 2, 170, 63, 120];
    else { const weights = new Array(256).fill(0); weights[1] = 65; alpha = [2, 0, 0, 0, 65, 0, 0, 0, ...weights, ...new Array(9).fill(0)]; }
  }
...
test('BF alpha workers use the descriptor extent while retaining a larger backing allocation', () => {
  for (const [depth, mode] of [[24, 1], [32, 1], [32, 2]]) {
    const surface = {bytes: new Uint8Array(272).fill(0xa5), initialized: new Uint8Array(272).fill(1)};
    decodeAokanaBfFrame(frame(depth, undefined, 1, mode), 8, 8, depth, new Uint8Array(128).fill(1), surface);
...
function cbg(depth, payload = frame(depth)) {
  const input = new Uint8Array(176 + payload.length); input.set(bytes('CompressedBG___\0'));
  const data = new DataView(input.buffer); data.setUint16(16, 2, true); data.setUint16(18, 1, true);
  data.setUint16(20, depth, true); data.setUint16(46, 2, true); put(input, 36, 1); put(input, 40, 128);
  const next = randomByteGenerator(1); for (let i = 0; i < 128; i++) input[48 + i] = (1 + next()) & 255;
  input[44] = 128; input[45] = 0; input.set(payload, 176); return input;
}
test('native BF grayscale, RGB and both alpha modes use the verified reconstruction and retained masks', () => {
  ...
  for (const depth of [8, 24, 32]) for (const alpha of [1, 2]) {
    const result = decodeAokanaBfFrame(frame(depth, undefined, 1, alpha), 8, 8, depth, quantization);
    for (let offset = 0; offset < 256; offset += 4) assert.deepEqual(Array.from(result.bytes.subarray(offset, offset + 4)), [128, 128, 128, depth !== 32 ? 0 : alpha === 1 ? 170 : 1]);
    assert.equal(result.initialized.includes(0), false);
    const retained = decodeAokanaBfFrame(frame(depth, 0, 1, alpha), 8, 8, depth, quantization, result);
    assert.deepEqual(retained.bytes, result.bytes);
  }
  assert.throws(() => decodeAokanaBfFrame(frame(16), 8, 8, 16, quantization), AokanaUndefinedResourceRead);
});
test('CBG v2 preserves crop bytes, depth24 unwritten allocation tail and zero-count retained output', async () => {
  const gray = await decodeAokanaResource(cbg(8)); assert.equal(gray.status, 0);
  assert.deepEqual(Array.from(gray.bytes.subarray(16)), [128, 128]);
  const rgba = await decodeAokanaResource(cbg(32)); assert.equal(rgba.status, 0);
  assert.deepEqual(Array.from(rgba.bytes.subarray(16)), [128, 128, 128, 170, 128, 128, 128, 170]);
  const rgb = await decodeAokanaCompressedBgV2(cbg(24)); assert.equal(rgb.bytes.length, 24); assert.equal(rgb.initializedLength, 22);
...
test('8040 appends the module and 8044 links an independently allocated runnable thread', async () => {
  const s = setup(), module = new Uint8Array(20); put(module, 0, 16); put(module, 4, 4); module.set([1, 2, 3, 4], 16);
  s.mount('/game/program', module);
  const control = new AokanaVmControlState(), root = new AokanaBpThread({id: control.allocateThreadId(), operandCapacity: 32, moduleCapacity: 64, frameCapacity: 64});
  const scheduler = new AokanaBpScheduler(root, () => 1), memory = new AokanaBpMemory(new Uint8Array(64));
  memory.globalMemory.set(bytes('program\0'), 4);
  const slots = new Map(createGroup80Resources(scheduler, control, s.resources).map((slot) => [slot.secondary, slot]));
  const context = {thread: root, memory};
  push32(root, 0); push32(root, 4); assert.equal(await slots.get(0x40).execute(context), 0); assert.equal(pop32(root), 0);
  assert.deepEqual(root.moduleMemory.subarray(0, 4), Uint8Array.of(1, 2, 3, 4));
  for (const value of [0, 4, 32, 128, 256]) push32(root, value);
  assert.equal(await slots.get(0x44).execute(context), 0); assert.equal(pop32(root), 1);
...
test('native entropy performs full lookahead, zero-bit access, and wrapping varints', async () => {
  ...
  const tree = aokanaBfTree([1, 0]), bits = new AokanaBfBits(Uint8Array.of(0));
  ...
  const resource = cbg(8), truncated = resource.subarray(0, resource.length - 2);
