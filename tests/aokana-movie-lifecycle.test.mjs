...
function type(kind = 0, width = 2, height = -2) {
  const format = new Uint8Array(88),
    v = new DataView(format.buffer);
  v.setBigInt64(40, 400000n, true);
  v.setInt32(52, width, true);
  v.setInt32(56, height, true);
  return {
    majorType: '73646976-0000-0010-8000-00aa00389b71',
    subtype:
      ['e436eb7e', 'e436eb7d', 'e436eb7c', 'e436eb7b'][kind] + '-524f-11ce-9f53-0020af0ba770',
    formatType: '05589f80-c356-11ce-bf01-00aa0055595a',
    format,
  };
}
function image(kind, width, height) {
  const image = new BurikoMovieImage(new BurikoMovieImageConfiguration()),
    media = type(kind, width, height);
  assert.equal(image.checkMediaType(media), 0);
  image.setMediaType(media);
  return image;
}
const sample = (bytes) => ({
...
test('movie media negotiation preserves native HRESULT precedence and signed requested geometry', () => {
  const configuration = new BurikoMovieImageConfiguration(),
    decoder = new BurikoMovieImage(configuration),
    media = type();
  assert.equal(decoder.checkMediaType(null), 0x80004003);
  assert.equal(
    decoder.checkMediaType({...media, majorType: 'wrong', subtype: 'wrong'}),
    0x80040200,
  );
  assert.equal(
    decoder.checkMediaType({...media, subtype: 'wrong', formatType: 'wrong'}),
    0x80040201,
  );
  assert.equal(decoder.checkMediaType({...media, formatType: 'wrong'}), 0x80040206);
  assert.equal(decoder.checkMediaType(media), 0);
  decoder.setMediaType(media);
  assert.equal(decoder.height, 2);
  configuration.dimensionMode = 1;
  assert.equal(decoder.height, -2);
...
test('movie RGB32/24 conversion clears X, preserves row padding and uses native bottom-up destination', () => {
  const bgra = image(0, 2, 2),
    target = allocateBurikoBitmap(2, 2, 1);
  bgra.copySample(
    bgra.orientDestination(target),
    sample([1, 2, 3, 255, 4, 5, 6, 255, 7, 8, 9, 255, 10, 11, 12, 255]),
  );
  assert.deepEqual(words(target), [0x090807, 0x0c0b0a, 0x030201, 0x060504]);
  const bgr = image(1, 5, -1),
    converted = allocateBurikoBitmap(5, 1, 1);
...
test('movie RGB555/565 really collapse nonzero color to boolean pixels', () => {
  for (const [kind, expected] of [
    ...
    const decoder = image(kind, 3, -1),
      output = allocateBurikoBitmap(3, 1, 1);
...
function controller() {
  const video = new Video(),
    ...
    decoder = image(0, 2, -2);
  const renderer = new BurikoMovieRenderer(null, 3, decoder, new BurikoNativeNotifications());
  renderer.attachGraph(graph, 0, 7);
  return {renderer, video, graph};
}
test('movie controller keeps stop, script pause and engine suspension as separate native states', async () => {
  ...
  assert.equal(renderer.isPlaying(), 0);
  assert.equal((await renderer.start()).remainingMilliseconds, 2000);
  assert.equal(renderer.isPlaying(), 1);
  assert.equal(await renderer.pause(1), 0);
  assert.equal(renderer.isPlaying(), 1);
  assert.equal(renderer.suspend(), 0);
  assert.equal(await renderer.pause(0), 0);
  assert.equal(video.paused, true);
  await renderer.resume();
  assert.equal(video.paused, false);
  renderer.stop();
  assert.equal(renderer.isPlaying(), 0);
  assert.equal(renderer.started, 1);
  video.currentTime = 0.4;
  assert.deepEqual(renderer.framePosition(), {status: 0, value: 10});
  assert.equal(renderer.volume(128), 0);
  assert.equal(video.volume, 1);
  assert.equal(renderer.volume(129), 0x80000003);
  renderer.dispose();
  assert.equal(renderer.started, 0);
...
test('movie registry unlinks before synchronous graph teardown and retains monotonically advancing IDs', () => {
  const registry = new BurikoMovieRegistry(),
    first = controller(),
    second = controller();
  assert.equal(registry.append(first.renderer), 0);
  assert.equal(registry.append(second.renderer), 1);
  const pause = first.video.pause.bind(first.video);
  first.video.pause = () => {
    assert.equal(registry.find(0), null);
    pause();
  };
  assert.equal(registry.remove(0), 1);
  assert.equal(registry.remove(0), 0);
  assert.equal(registry.find(1), second.renderer);
  registry.clear();
  const next = controller();
  assert.equal(registry.append(next.renderer), 2);
  registry.clear();
});
test('native notifications retain all three DWORD values in FIFO order through a drain/refill', () => {
  const queue = new BurikoNativeNotifications();
  queue.push(0x10000, 3, 0);
  queue.push(-1, -2, -3);
  assert.deepEqual(queue.take(), {type: 0x10000, value1: 3, value2: 0});
  assert.deepEqual(queue.take(), {type: 0xffffffff, value1: 0xfffffffe, value2: 0xfffffffd});
  assert.equal(queue.take(), null);
  queue.push(1, 2, 3);
  queue.clear();
  assert.equal(queue.take(), null);
  queue.push(4, 5, 6);
  assert.deepEqual(queue.take(), {type: 4, value1: 5, value2: 6});
});

import {readBurikoIsoMovieTracks} from '../dist/engines/buriko/native/movie-media-timing.js';
const atom = (name, ...parts) => {
  const payload = Uint8Array.from(parts.flatMap((p) => Array.from(p))),
...
function track(id, enabled, timeRuns) {
  return atom(
    'trak',
    atom('tkhd', u32(enabled, 0, 0, id)),
    atom(
      'mdia',
      atom('mdhd', u32(0, 0, 0, 1000)),
      atom('hdlr', u32(0, 0), new TextEncoder().encode('vide')),
      atom('minf', atom('stbl', atom('stts', u32(0, timeRuns.length, ...timeRuns.flat())))),
    ),
  );
}
test('ISO video timing retains multiple tracks and exact variable-duration totals', () => {
  ...
  const result = readBurikoIsoMovieTracks(source);
  assert.deepEqual(result, [
    {
      ...
      timescale: 1000,
      sampleCount: 30n,
      sampleDuration: 1220n,
      averageFrameTime: 406666n,
    },
    {
      id: 8,
      enabled: false,
...
test('fragmented ISO timing handles trex defaults, tfhd overrides and per-sample trun durations', () => {
  ...
  const second = atom(
    ...
    atom(
      'traf',
      atom('tfhd', u32(8, 7, 50)),
      atom('trun', u32(0, 2)),
      atom('trun', u32(0x100, 2, 60, 70)),
    ),
  );
  const data = Uint8Array.from([...movie, ...first, ...second]);
  assert.deepEqual(readBurikoIsoMovieTracks(data), [
    {
      id: 7,
      enabled: true,
      timescale: 1000,
  ...
  assert.throws(() => readBurikoIsoMovieTracks(data.subarray(0, data.length - 1)), /invalid size/);
});
